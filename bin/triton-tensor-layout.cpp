...
cl::OptionCategory PrinterCategory("Available Print Options", "Options for the tensor layout printing.");

static cl::opt<std::string> InputFile("i", cl::desc("File that contains the tensor data layout attributes"),
                                      cl::init(""), cl::value_desc("filename"), cl::cat(PrinterCategory));

static cl::opt<std::string> OutputFile("o", cl::desc("Output file to write the layout into"), cl::init(""),
                                       cl::value_desc("filename"), cl::cat(PrinterCategory));

static cl::opt<std::string> DataLayoutStr("l", cl::desc("Tensor data layout attribute in string"),
                                          cl::value_desc("layout-string"), cl::init(""), cl::cat(PrinterCategory));

static cl::list<std::string> AliasName("alias-names",
                                       cl::desc("A list of alias names (separated by comma) of the "
                                                "layout attributes in the input file"),
                                       cl::value_desc("name1,name2,name3,..."), cl::CommaSeparated, cl::ZeroOrMore,
                                       cl::cat(PrinterCategory));

static cl::opt<bool>
    UseHWPointOfView("use-hw-view",
                     llvm::cl::desc("Print the layout in hardware point of view. This means the output is "
...
static cl::opt<std::string> TensorStr("t", cl::desc("Tensor shape and element type (e.g., tensor<2x2xf32>)"),
...
LogicalResult layoutPrint(RankedTensorType tensorType, raw_ostream &os)
{
    ...
    llvm::errs() << "Unsupported tensor layout attribute: " << tensorType.getEncoding() << "\n";
    return failure();
}

LogicalResult printLayoutFromFile(MLIRContext *context, StringRef filename, ArrayRef<std::string> names,
                                  TensorType tensorTy, raw_string_ostream &ss)
{
    ...
    auto printLambda = [&](StringRef name, mlir::Attribute attr) {
        ...
        auto rankedTensorTy = RankedTensorType::get(tensorTy.getShape(), tensorTy.getElementType(), attr);
...
LogicalResult printLayoutFromString(MLIRContext *context, StringRef layoutAttrStr, TensorType tensorTy,
                                    raw_string_ostream &ss)
{
    ...
    auto rankedTensorTy = RankedTensorType::get(tensorTy.getShape(), tensorTy.getElementType(), layout);
...
int main(int argc, char **argv)
{
    ...
    if (OutputFile.empty()) {
        ...
        if (ec) {
            llvm::errs() << "Error: " << ec.message() << " : unable to open " << OutputFile << " for output\n";
            return 1;
        }
        outFs << ss.str();
        outFs.close();
