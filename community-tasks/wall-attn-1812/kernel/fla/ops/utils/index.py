...
#
# Vendored from upstream fla/ops/utils/index.py, reduced to the
# prepare_chunk_indices chain required by fla.ops.wall_attn (varlen).
...
def _segmented_arange(counts: torch.LongTensor) -> tuple[torch.LongTensor, torch.LongTensor]:
    ...
    With device-side ``counts``, ``repeat_interleave`` reads ``counts.sum()`` on
    the host (one device sync). Pass host-side counts to avoid it.
    """
