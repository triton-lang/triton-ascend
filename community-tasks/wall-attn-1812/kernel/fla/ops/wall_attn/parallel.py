...
def parallel_wall_attn_bwd_kernel_dq(
    ...
    for i_s in range(i_t * BT, min((i_t + 1) * BT, T), BS):
        ...
        if USE_WINDOW:
            b_p = tl.where((o_q[:, None] >= o_k[None, :]) & (o_q[:, None] - o_k[None, :] < W) & m_k[None, :],
    ...
    # NPU: 当 off-diag 循环零次执行（i_t == 0，或滑窗使 i_start == i_t*BT）时，
    # A2（910B）后端未对 b_dq_til 的 tl.zeros 初值生效，循环后读到未初始化内存；
    # 显式按循环是否执行选择 off-diag 项（循环执行时数值与上游完全一致）。
...
def parallel_wall_attn_bwd_kernel_dkv(
    ...
    for i_s in range(i_t * BT, min((i_t + 1) * BT, T), BS):
        ...
        if USE_SCALAR_G:
            ...
            b_s += b_cq[None, :] - b_ck[:, None]
        if USE_WINDOW:
            b_p = tl.where((o_k[:, None] <= o_q[None, :]) & (o_q[None, :] - o_k[:, None] < W) & m_q[None, :],
                           exp2(b_s - b_lse[None, :]), 0)
        else:
...
def parallel_wall_attn_fwd(
    ...
    if g_cumsum.dim() != 4 or g_cumsum.shape != (*q.shape[:-1], q.shape[-1]):
        raise ValueError(f"`g_cumsum` must be [B, T, HQ, K] matching `q`; got {g_cumsum.shape} vs q {q.shape}")
...
class WallParallelAttentionFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, g, sink_bias, scale, window_size, cu_seqlens, g_scalar=None, chunk_indices=None):
        if g.shape[-1] != q.shape[-1] or g.shape[2] != q.shape[2]:
            raise ValueError(f"`g` must be [B, T, HQ, K] with same HQ and K as `q`; got {g.shape} vs q {q.shape}")
        ...
        if g_scalar is not None:
            ...
            if _DEBUG_ASSERTS:
                assert torch.isfinite(
                    c).all(), f"[wall-attn fwd] NaN/Inf in c=cumsum(g_scalar): c abs max={c.abs().max()}"
        else:
            c = None
        ...
        if _DEBUG_ASSERTS:
            assert torch.isfinite(o).all(), (f"[wall-attn fwd] NaN/Inf in output o: o abs max={o.abs().max()}, "
    ...
    def backward(ctx, do):
        ...
        if _DEBUG_ASSERTS:
            ...
            assert torch.isfinite(dP).all(), (f"[wall-attn bwd] NaN/Inf in dP: dP abs max={dP.abs().max()}, "
...
def parallel_wall_attn(
    ...
    *,
    ...
    if pad_v:
        v = F.pad(v, (0, pad_v))
    o = WallParallelAttentionFunction.apply(q, k, v, g, sink_bias, scale, window_size, cu_seqlens, g_scalar, None)
