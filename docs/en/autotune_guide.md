...
## Use Costmodel to Reduce the Number of Benchmarked Configurations

When the candidate set is large, Costmodel can predict and prune candidates
before real NPU benchmarking:
...
This option works with both handwritten `triton.Config` objects and candidates
generated by `configs=[]`. `top_k` can be either a retained count or a retained
fraction in `(0, 1]`. You can also use
`prune_configs_by={"costmodel": True}` to enable the defaults.

Costmodel is only a prefilter; real hardware benchmarking still selects the
final `best_config`. `costmodel` and the community `perf_model` are mutually
exclusive. If configured, `early_config_prune` runs before either one. For the
complete options, failure fallback behavior, and low-level API example, see the
[Costmodel example](./examples/09_costmodel_example.md).
