...
| **Debugging and logging**| TRITON_PRINT_AUTOTUNING | **0** or not set| Output start/end markers and elapsed wall time for each autotuning stage, followed by the optimal configuration and total benchmark time of each kernel.| **0**: Do not output.<br>**1**: Output.| |
