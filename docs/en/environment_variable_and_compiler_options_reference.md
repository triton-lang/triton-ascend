...
| **Debugging and logging**| TRITON_PRINT_AUTOTUNING | **0** or not set| Output start/end markers and elapsed wall time for each autotuning stage, followed by the optimal configuration and total benchmark time of each kernel. For multi-configuration NPU profiling, also output per-configuration prewarm, enqueue, synchronization, profiling wall/device timing, and a profiler phase summary.| **0**: Do not output.<br>**1**: Output.| |
