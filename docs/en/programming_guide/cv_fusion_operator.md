...
For simple CV fusion, start from the stable `tl.dot` main computation in the [Matrix Multiplication example](../examples/05_matrix_multiplication_example.md), then add Vector post-processing before the store. For more complex sliced updates, see the [Fused Attention example](../examples/04_fused_attention_example.md). The minimal path is as follows:
...
For attention-style CV fusion, start with non-causal, short-sequence, and small-head-dimension cases, then gradually add the following:

- Causal mask processing in stages.
- Long-sequence K/V block loops.
- Numerically stable softmax updates for `m_i`/`l_i`.
- Accumulator workspace and sub-block splitting for large `HEAD_DIM`.
- Load reorganization for discrete KV-cache indices.
