...
def add_kernel(x_ptr,  # Pointer to the first input vector
    ...
    # Different data is handled by different “programs”, so we need to assign:
    pid = tl.program_id(axis=0)  # Use a 1D launch grid, so axis is 0
    # This program will process inputs offset from the initial data
    # For example, with a vector of length 256 and block size 64, programs will access elements [0:64, 64:128, 128:192, 192:256] respectively
    ...
    # Load x and y from DRAM, masking out extra elements if the input is not a multiple of the block size
...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    # Launch grid represents the number of kernel instances running in parallel
    # Can be Tuple[int] or Callable(metaparameters) -> Tuple[int]
    # In this case, use a 1D grid where size is the number of blocks:
