...
def _layer_norm_fwd_fused(
    ...
    Mean,  # Mean pointer
    Rstd,  # 1/std 指针
    stride,  # How many elements to move the pointer to advance one row
    N,  # X 的列数
    eps,  # Epsilon to avoid division by zero
    BLOCK_SIZE: tl.constexpr,
):
    # Map program ID to the corresponding row of X and Y for computation
    row = tl.program_id(0)
    Y += row * stride
    X += row * stride
    # Compute mean
...
def layer_norm(x, weight, bias, eps=1e-5):
    ...
    # Reshape input x into 2D shape [-1, feature_dim] to process the last dimension
