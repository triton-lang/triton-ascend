...
def triton_dot_2_Bias(
    ...
    A: tl.constexpr,  # First dimension size (batch/rows)
    B: tl.constexpr,  # Shared dimension (columns of x, rows of y)
    C: tl.constexpr   # Second dimension size (columns)
):
    ...
    # Construct linear index for x: (A, B) -> flatten to A*B
    Xidx = bidx[:, None] * B + cidx[None, :]  # Broadcast to form (A, B) index grid

    # Construct linear index for y: (B, C) -> flatten to B*C
    ...
    ret = tl.dot(X, Y) + Z  # tl.dot performs (A, B) x (B, C) -> (A, C)
...
def torch_dot_Bias(x0, x1, bias):
    """PyTorch reference implementation: perform matrix multiplication and add bias."""
    res = torch.matmul(x0, x1) + bias
    return res

def get_torch_typename(dtype):
    """Map string data type to corresponding torch.dtype."""
    if dtype == 'float32':
...
def generate_tensor(shape, dtype):
     """Generate a random tensor with the specified shape and dtype, adapted to the value range of different numeric types."""
...
def validate_cmp(dtype, y_cal, y_ref):
    """Compare Triton computation result with PyTorch reference on NPU, set tolerance or strict equality by data type."""
    y_cal=y_cal.npu()
    y_ref=y_ref.npu()
    if dtype == 'float16': 
...
def test_dot_2_Bias(sigtype, A, B, C):
    ...
    # Generate bias uniformly in float32 (avoid precision issues with integer bias)
    if 'int' in sigtype:
