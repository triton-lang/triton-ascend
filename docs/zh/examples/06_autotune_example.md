...
# - Pass a list of runtime parameter names; a change in any parameter value triggers regeneration and evaluation of candidate configs
# Note: 1. If hints passes split axis (split_params), tiling axis (tiling_params), low-dim axis (low_dim_axes), and reduction axis (reduction_axes) parameter info, the key type must be Dict[str,str], as in Example 1:
#      2. If hints does not pass split axis (split_params), tiling axis (tiling_params), low-dim axis (low_dim_axes), and reduction axis (reduction_axes) parameter info, the key type must be list[str], and the axis info is assigned in parameter order, as in Example 2:
