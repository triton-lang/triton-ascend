...
def test_triton_autotune():
    ...
    @triton.autotune(
        ...
        key=["numel"],                      # Trigger autotune when numel size changes
    )
    @triton.jit
    ...
    def triton_calc_func(x0, x1):
        ...
        grid = lambda meta: (triton.cdiv(n, meta["XS"]), 1, 1)  # Compute grid size
...
```Python
# The following explains key parameter usage differences between advanced autotune and the community version
#
# configs:
# - Community autotune (default) requires explicit triton.Config list; the framework compiles and benchmarks each config to select the optimal one
# - Advanced autotune: the framework automatically generates candidate tiling configs based on the kernel, then compiles and benchmarks each config to select the optimal one
# * Note: 1. Advanced mode requires manual import of triton.backends.ascend.runtime;
#        2. When configs=[], the framework generates candidate tiling configs based on the kernel. Note that @triton.autotune must be applied directly above @triton.jit,
#           with no other decorators (e.g., libentry) in between;
#        3. When configs is not empty, the framework will not automatically generate candidate tiling configs by default;
#        4. When configs is not empty and hints.auto_gen_config=True, the framework auto-generates Configs and merges them with user-defined Configs for selection;
#        5. The advanced version supports setting the performance collection method via os.environ["TRITON_BENCH_METHOD"] = "npu".
# 
# hints(Dict[str, str]):
# Note: 1. hints is optional. When not provided, the framework automatically parses split_params, tiling_params, and other related parameters
...
#
# split_params (Dict[str, str]): Dictionary of axis name: argument name, where argument is the tunable parameter for the split axis, e.g., 'XBLOCK'
#     The axis name must be in the set of axis names of the parameter key. Do not add prefix r to the axis name
#     This parameter can be empty. When both split_params and tiling_params are empty, automatic optimization will not be performed
#     Split axes are typically determined by the `tl.program_id()` kernel dispatch statement
# tiling_params (Dict[str, str]): Dictionary of axis name: argument name, where argument is the tunable parameter for the tiling axis, e.g., 'XBLOCK_SUB'
#     The axis name must be in the set of axis names of the parameter key. Do not add prefix r to the axis name
#     This parameter can be empty. When both split_params and tiling_params are empty, automatic optimization will not be performed
#     Tiling axes are typically determined by the `tl.arange()` tiling expression
# low_dim_axes (List[str]): List of axis names of all low-dimensional axes. Axis names must be in the set of axis names of the parameter key
# reduction_axes (List[str]): List of axis names of all reduction axes. Axis names must be in the set of axis names of the parameter key. Add prefix r before the axis name
# auto_gen_config (bool): Default is False, involving the following scenarios:
#     1. When user does not define Config, the framework defaults to auto-generating Config regardless of auto_gen_config setting;
#     2. When user defines Config and auto_gen_config=False, the framework does not auto-generate Config and only uses user-defined Configs;
#     3. When user defines Config and auto_gen_config=True, the framework auto-generates Configs and merges them with user-defined Configs for selection;
#
# key (list[str]/Dict[str,str]):
# - Pass the list of runtime parameter names; any change in any parameter in the list triggers regeneration and re-evaluation of candidate configs
# Note: 1. When hints passes split_params, tiling_params, low_dim_axes, reduction_axes info, the key type must be Dict[str,str], as in Example 1:
#      2. When hints does not pass split_params, tiling_params, low_dim_axes, reduction_axes info, the key type must be list[str]; axis information will be assigned according to parameter order, as in Example 2:
...
def add_kernel(
    ...
    # Note: `constexpr` means it can be determined at compile time and can be used as a shape value
):
    ...
    # For example, if you have a vector of length 256 with a block size of 64, each program
    # will access elements [0:64, 64:128, 128:192, 192:256] respectively
    # Note, offsets is a list of pointers:
    block_start = pid * BLOCK_SIZE
    offsets = block_start + tl.arange(0, BLOCK_SIZE)
    # Create a mask to prevent memory operations from going out of bounds
    mask = offsets < n_elements
    # Load x and y, using the mask to filter out extra elements in case the input vector length is not a multiple of the block size
...
# XBLOCK and XBLOCK_SUB are not passed in, so they become candidates for split and tiling axis parameters
# BLOCK_SIZE is passed as a keyword argument and will not be considered as a candidate
...
# Although BLOCK_SIZE also appears in tl.arange and is compared with n_cols for mask computation, it is not a tiling axis parameter
```
...
```Python
@triton.autotune(
    key={"n_rows", "n_cols"} # Will be automatically assigned as {"x": "n_rows", "y": "n_cols"} in order
    ...
)
@triton.jit
def triton_func(...):
...
# Parsed result: low_dim_axes = {"y"}
# Although row_offsets is also computed via tl.arange and compared with n_rows for mask, the slicing expands in the low dimension, so x is not a low-dimensional axis
```
...
# Parsed result: pointer type parameters are: input_ptr, output_ptr
```
...
```Python
# Automatically generate profiling results for the current autotune optimal kernel configuration in the `auto_profile_dir` directory, using performance data collected by `torch_npu.profiler.profile`
