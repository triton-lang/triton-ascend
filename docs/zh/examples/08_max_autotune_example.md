...
def test_max_autotune():
    ...
    # Basic configuration: only the block size needs to be provided; other tuning parameters are auto-generated by the decorator
    base_configs = [
    ...
    @max_autotune(
        ...
        key=["numel"],
        kernel_type="vector",          # Kernel type: cube / mix / vector, defaults to mix
    )
    @triton.jit
    ...
    # Compare against the PyTorch reference result
...
def test_max_autotune():
    ...
    @max_autotune(
        configs=base_configs,          # Basic configuration list
        key=["numel"],                 # Re-trigger tuning when numel changes
        kernel_type="vector",          # Kernel type: cube / mix / vector
        # The following parameters are optional tuning lists; built-in defaults are used when not provided
        num_stages=[1, 2],
