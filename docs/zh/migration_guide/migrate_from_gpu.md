...
```diff
import torch
import torch_npu  # [Added] Import the Ascend NPU PyTorch adapter library to provide NPU device support
import triton
...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    # assert x.device == DEVICE and y.device == DEVICE and output.device == DEVICE  # [Removed] GPU device consistency check; NPU does not need an explicit assertion
...
torch.manual_seed(0)
size = 98432
# x = torch.rand(size, device='cuda')  # [Removed] Specify GPU device
x = torch.rand(size, device='npu')  # [Modified] Specify the Ascend NPU device
# y = torch.rand(size, device='cuda')  # [Removed] Specify GPU device
...
```diff
optimal_block_size = 32768  # Optimized value derived from calculation
...
def masked_fill_kernel(inp, expand_mask, value, out, N,
    ...
    # Process in tiles to avoid UB overflow
...
def masked_fill(inp, expand_mask, value):
    ...
    # Use optimized parameter config
    MAIN_BLOCK_SIZE = 32768  # Ensure coreDim is within limits
...
chunk_fwd_kernel_o[(NT, B * H)](
    ...
    block_ptr = tl.make_block_ptr(
        ...
        offsets=(i_t * 16,), # Start from the beginning offset
