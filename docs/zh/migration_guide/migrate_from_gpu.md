...
def zeros_like(x, *, dtype=None, layout=None, device=None, pin_memory=None, memory_format=None):
    ...
    zeros_kernel[grid_fn](out, N, BLOCK_SIZE=1024)  # Original value is too small
    return out
```
...
```diff
optimal_block_size = 32768  # Optimal value derived from calculation
...
def masked_fill_kernel(inp, expand_mask, value, out, N,
    ...
    for sub_block_idx in range(num_sub_blocks):
        ...
        # Then overwrite with the target value at positions that need filling
