...
- 示例1：减少同步，提升并行度
    ...
    def npu_vector_add_kernel(
        ...
        idx = tl.arange(0, N)
        mask = idx < M
        data = tl.load(input + idx, mask = mask) # Or specify other=-1 or similar values
    ```
    ...
    def npu_vector_add_kernel(
        ...
        mask = idx < M
    -   data = tl.load(input + idx, mask = mask) # Or specify other=-1 or similar values
    +   data = tl.load(input + idx, mask = mask, care_padding=False) # Or specify other=-1 or similar values值
    ```
