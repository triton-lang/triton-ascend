...
class AliasInfo {
  public:
    ...
    bool operator==(const AliasInfo &other) const { return allocs == other.allocs; }

    /// The pessimistic value state of a value without alias
    static AliasInfo getPessimisticValueState(MLIRContext *context = nullptr) { return AliasInfo(); }
...
//===----------------------------------------------------------------------===//
class SharedMemoryAliasAnalysis : public dataflow::SparseForwardDataFlowAnalysis<dataflow::Lattice<AliasInfo>> {
  public:
    using dataflow::SparseForwardDataFlowAnalysis<dataflow::Lattice<AliasInfo>>::SparseForwardDataFlowAnalysis;
    using dataflow::SparseForwardDataFlowAnalysis<dataflow::Lattice<AliasInfo>>::getLatticeElement;
    ...
    {
        propagateIfChanged(lattice, lattice->join(AliasInfo::getPessimisticValueState(lattice->getAnchor())));
    }

    /// Computes if the alloc set of the results are changed.
    LogicalResult visitOperation(Operation *op, ArrayRef<const dataflow::Lattice<AliasInfo> *> operands,
