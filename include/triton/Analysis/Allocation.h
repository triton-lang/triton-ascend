...
struct ScratchConfig {
    ...
    ScratchConfig(SmallVector<unsigned> repShape, SmallVector<unsigned> paddedRepShape, unsigned inVec = 1,
                  unsigned outVec = 1)
        : repShape(repShape), paddedRepShape(paddedRepShape), inVec(inVec), outVec(outVec)
    {
    }
...
template <typename T> class Interval {
  public:
    ...
    bool intersects(const Interval &R) const { return Start < R.End && R.Start < End; }
    bool operator==(const Interval &R) const { return Start == R.Start && End == R.End; }
    bool operator!=(const Interval &R) const { return !(*this == R); }
    bool operator<(const Interval &R) const { return std::make_pair(Start, End) < std::make_pair(R.Start, R.End); }
...
class Allocation {
  public:
    ...
    static constexpr BufferId InvalidBufferId = std::numeric_limits<BufferId>::max();
    ...
    size_t getOffset(BufferId bufferId) const { return bufferSet.at(bufferId).offset; }

    /// Returns the size of the given buffer in the shared memory.
    size_t getAllocatedSize(BufferId bufferId) const { return bufferSet.at(bufferId).size; }
  ...
  private:
    ...
    struct BufferT {
        ...
        BufferT(BufferKind kind, size_t size, size_t alignment = 4, size_t offset = 0, int sharingGroup = -1)
            : kind(kind), id(nextId++), size(size), alignment(alignment), offset(offset), sharingGroup(sharingGroup)
        {
        }

        size_t setOffsetAligned(size_t newOffset) { return offset = llvm::alignTo(newOffset, alignment); }
    };
  ...
  private:
    template <BufferT::BufferKind Kind, typename KeyType, typename... Args> void addBuffer(KeyType &key, Args &&...args)
    {
    ...
    void addAlias(Value value, Value alloc) { aliasBuffer[value].insert(valueBuffer[alloc]); }
...
class ModuleAllocation : public CallGraph<Allocation> {
  public:
    ...
    explicit ModuleAllocation(ModuleOp moduleOp) : CallGraph<Allocation>(moduleOp)
    {
    ...
    size_t getSharedMemorySize(FunctionOpInterface funcOp) { return getFuncData(funcOp)->getSharedMemorySize(); }

    void setFunctionSharedMemoryValue(FunctionOpInterface funcOp, Value value) { sharedMemoryValue[funcOp] = value; }

    Value getFunctionSharedMemoryBase(FunctionOpInterface funcOp) { return sharedMemoryValue[funcOp]; }
