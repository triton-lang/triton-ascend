...
inline bool isZeroConst(Value v)
{
    ...
    if (auto denseAttr = dyn_cast<DenseIntElementsAttr>(constantOp.getValueAttr()))
...
std::optional<mlir::triton::LinearLayout> minimalCvtLayout(RankedTensorType srcTy, RankedTensorType dstTy);
...
SetVector<Operation *> multiRootGetSlice(Operation *op, TransitiveFilter backwardFilter = nullptr,
...
template <typename T> class CallGraph {
  public:
    ...
    template <WalkOrder UpdateEdgeOrder = WalkOrder::PreOrder, WalkOrder UpdateNodeOrder = WalkOrder::PreOrder,
    ...
    void walk(UpdateEdgeFn updateEdgeFn, UpdateNodeFn updateNodeFn)
    {
        ...
        for (auto root : roots) {
            doWalk<UpdateEdgeOrder, UpdateNodeOrder>(root, visited, updateEdgeFn, updateNodeFn);
        }
    }
    ...
    bool isRoot(FunctionOpInterface funcOp) const { return llvm::is_contained(roots, funcOp); }

    /// Maps the data and the graph nodes associated with a funcOp to a
    /// targetFuncOp.
    template <typename FROM, typename TO> void mapFuncOp(FROM funcOp, TO targetFuncOp)
    {
    ...
    template <typename FROM, typename TO> void mapCallOp(FROM callOp, TO targetCallOp)
    {
        // Iterate over graph and replace
        for (auto &kv : graph) {
            for (auto &edge : kv.second) {
  ...
  private:
    void build()
    {
        ...
        moduleOp.walk([&](Operation *op) {
            ...
            if (auto callOp = dyn_cast<CallOpInterface>(op)) {
                ...
                if (funcOp) {
                    graph[caller].emplace_back(std::pair<CallOpInterface, FunctionOpInterface>(callOp, funcOp));
    ...
    template <WalkOrder UpdateEdgeOrder = WalkOrder::PreOrder, WalkOrder UpdateNodeOrder = WalkOrder::PreOrder,
              typename UpdateEdgeFn, typename UpdateNodeFn>
    void doWalk(FunctionOpInterface funcOp, DenseSet<FunctionOpInterface> &visited, UpdateEdgeFn updateEdgeFn,
  ...
  protected:
    ModuleOp moduleOp;
    DenseMap<FunctionOpInterface, SmallVector<std::pair<CallOpInterface, FunctionOpInterface>>> graph;
