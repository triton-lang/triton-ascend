...
template <typename T, typename VecU> SmallVector<T> convertType(const VecU &in)
{
...
template <typename Int> Int ceil(Int m, Int n)
{
    return (m + n - 1) / n;
}
...
template <typename T, typename U> SmallVector<T> applyPermutation(ArrayRef<T> vec, ArrayRef<U> permutation)
{
...
template <typename VecT, typename PermT> auto applyPermutation(const VecT &vec, const PermT &permutation)
{
    return applyPermutation(ArrayRef(vec), ArrayRef(permutation));
}

template <typename T> [[nodiscard]] SmallVector<T> inversePermutation(ArrayRef<T> permutation)
{
    // Check that `permutation` is actually a permutation.
#ifndef NDEBUG
...
template <typename VecT> [[nodiscard]] auto inversePermutation(const VecT &permutation)
{
    return inversePermutation(ArrayRef(permutation));
}

template <typename T, typename U> [[nodiscard]] SmallVector<T> gather(ArrayRef<T> elems, ArrayRef<U> indices)
{
    SmallVector<T> ret;
...
template <typename VecT, typename IdxT> [[nodiscard]] auto gather(const VecT &elems, const IdxT &indices)
{
...
{
    return std::min_element(llvm::adl_begin(Range), llvm::adl_end(Range), std::forward<Compare>(C));
}
template <typename R> auto max_element(R &&Range)
{
    return std::max_element(llvm::adl_begin(Range), llvm::adl_end(Range));
}
template <typename R, typename T, typename Compare> auto max_element(R &&Range, Compare &&C)
{
    return std::max_element(llvm::adl_begin(Range), llvm::adl_end(Range), std::forward<Compare>(C));
}
