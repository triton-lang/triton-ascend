...
public:
  ...
  /// loaded via Python import (push model) where the .so is already loaded
  /// by Python. The info must outlive the program (return a pointer to a
  /// static struct, as with tritonGetPluginInfo).
