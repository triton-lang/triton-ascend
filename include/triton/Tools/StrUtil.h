...
template <typename C> std::string join(C &&container, llvm::StringRef sep = ", ")
{
    std::string ret;
...
//
template <typename C, typename Fn> std::string join(C &&container, llvm::StringRef sep, Fn &&fn)
{
    std::string ret;
    llvm::raw_string_ostream s(ret);
    for (const auto &elem : container) {
