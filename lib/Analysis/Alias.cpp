...
LogicalResult SharedMemoryAliasAnalysis::visitOperation(Operation *op,
                                                        ...
                                                        ArrayRef<dataflow::Lattice<AliasInfo> *> results)
{
    ...
    if (auto memdescTy = dyn_cast<triton::MemDescType>(result.getType())) {
        if (!isa_and_nonnull<triton::gpu::SharedMemorySpaceAttr>(memdescTy.getMemorySpace()))
            return success();
    }
    ...
    if (isa<triton::gpu::LocalAllocOp>(op)) {
        ...
        assert(!isa<triton::MemDescType>(result.getType()) && "unknown operation creating memory descriptor");
    }
...
ModRefResult SharedMemoryAliasAnalysis::getModRef(Operation *op, Value location)
{
    // TODO: implement
