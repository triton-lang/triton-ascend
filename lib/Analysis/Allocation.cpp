...
static std::pair<SmallVector<unsigned>, SmallVector<unsigned>> getCvtOrder(Attribute srcLayout, Attribute dstLayout)
{
    ...
    assert(!(srcMmaLayout && dstMmaLayout && !srcMmaLayout.isAmpere() && !srcMmaLayout.isHopper()) &&
...
static SmallVector<unsigned> getRepShapeForCvt(RankedTensorType srcTy, RankedTensorType dstTy)
{
    ...
    for (unsigned d = 0; d < rank; ++d) {
        repShape[d] = std::max(std::min<unsigned>(srcShapePerCTA[d], srcShapePerCTATile[d]),
...
ScratchConfig getScratchConfigForCvt(RankedTensorType srcTy, RankedTensorType dstTy)
{
    ...
    scratchConfig.inVec = outOrd[0] != innerDim ? 1 : inOrd[0] != innerDim ? 1 : srcContigPerThread;
...
class AllocationAnalysis {
  public:
    AllocationAnalysis(Operation *operation, Allocation::FuncAllocMapT *funcAllocMap, Allocation *allocation)
        : operation(operation), funcAllocMap(funcAllocMap), allocation(allocation)
    {
        run();
    }
  ...
  private:
    ...
    void getExplicitValueSize(Operation *op)
    {
        for (Value result : op->getResults()) {
            ...
            if (alloc && alloc.isSharedMemoryAlloc()) {
                ...
                auto bytes = product<int64_t>(shapePerCTA) * allocType.getElementTypeBitWidth() / 8;
                ...
                if (alloc->hasAttr("allocation.shareGroup")) {
                    sharingGroup = mlir::cast<IntegerAttr>(alloc->getAttr("allocation.shareGroup")).getInt();
                    LDBG("with shareGroup of " << sharingGroup);
                }
                allocation->addBuffer<BufferT::BufferKind::Explicit>(result, bytes, alignment, 0, sharingGroup);
            }
        }
    }

    template <BufferT::BufferKind T> void maybeAddScratchBuffer(Operation *op, unsigned bytes, unsigned alignment)
    {
        if (bytes > 0)
            allocation->addBuffer<T>(op, bytes, alignment);
    }

    template <BufferT::BufferKind T> void maybeAddScratchBuffer(Operation *op, unsigned bytes)
    {
        if (bytes > 0)
    ...
    void getScratchValueSize(Operation *op)
    {
        ...
        if (auto reduceOp = dyn_cast<triton::ReduceOp>(op)) {
            ...
            int threadsPerWarp = triton::gpu::TritonGPUDialect::getThreadsPerWarp(op->getParentOfType<ModuleOp>());
            ...
            if (mlir::isa<SharedEncodingAttr>(srcEncoding) || mlir::isa<SharedEncodingAttr>(dstEncoding)) {
            ...
            auto bytes = isa<triton::PointerType>(srcTy.getElementType())
            ...
            if (dyn_cast<RankedTensorType>(value.getType())) {
                ...
                auto elemTy = cast<triton::PointerType>(value.getType()).getPointeeType();
            ...
            maybeAddScratchBuffer<BufferT::BufferKind::Virtual>(op, bytes, scratchAlignment);
        } else if (auto createTensormap = dyn_cast<ExperimentalTensormapCreateOp>(op)) {
            constexpr int32_t kTMASize = 128;
            constexpr int32_t kTMAAlign = 128;
            maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(op, kTMASize, kTMAAlign);
        }
    }

    void getValueAlias(Value value, SharedMemoryAliasAnalysis &analysis)
    {
        dataflow::Lattice<AliasInfo> *latticeElement = analysis.getLatticeElement(value);
        if (latticeElement) {
    ...
    void getValuesAndSizes()
    {
        ...
        for (auto valueBufferIter : allocation->valueBuffer) {
            ...
            LLVM_DEBUG(llvm::dbgs() << "-- buffer " << buffer->id << " " << buffer->size << " " << buffer->offset << " "
                                    << buffer->sharingGroup << "\n");
        }
        // Get the alias values
        std::unique_ptr<DataFlowSolver> solver = createDataFlowSolver();
        SharedMemoryAliasAnalysis *aliasAnalysis = solver->load<SharedMemoryAliasAnalysis>();
    ...
    void resolveExplicitBufferLiveness(function_ref<Interval<size_t>(Value value, BufferT *buffer)> getLiveness)
    {
        for (auto valueBufferIter : allocation->valueBuffer) {
    ...
    void resolveAliasBufferLiveness(function_ref<Interval<size_t>(Value value, BufferT *buffer)> getLiveness)
    {
    ...
    void resolveScratchBufferLiveness(const DenseMap<Operation *, size_t> &operationId)
    {
        ...
        auto processScratchMemory = [&](const auto &container) {
            for (auto opScratchIter : container) {
                ...
                if (getAsyncTaskIds(op).empty()) {
                    bufferRange.insert({buffer, Interval(operationId.lookup(op), operationId.lookup(op) + 1)});
                } else {
    ...
    void resolveLiveness()
    {
        ...
        operation->walk<WalkOrder::PostOrder>([&](Operation *op) { operationId[op] = operationId.size(); });
        ...
        auto getValueLivenessRange = [&](Value value, BufferT *buffer) {
            ...
            std::for_each(liveOperations.begin(), liveOperations.end(), [&](Operation *liveOp) {
    ...
    void dumpBuffers()
    {
        ...
        for (auto bufferIter : bufferRange) {
            LLVM_DEBUG({
                llvm::dbgs() << "-- " << bufferIter.first->id << " " << bufferIter.first->size << " "
                             << bufferIter.first->offset << " " << bufferIter.first->sharingGroup << " regions [";
    ...
    void computeOffsets()
    {
        ...
        for (auto bufferIter : bufferRange) {
            if (sharingIdToRep.find(bufferIter.first->sharingGroup) != sharingIdToRep.end()) {
                if (bufferIter.first != sharingIdToRep[bufferIter.first->sharingGroup]) {
                    LDBG("-- ignore shared buffer " << bufferIter.first->size << " " << bufferIter.first->offset << " "
        ...
        for (auto &kv : toGroup) {
            ...
            for (auto *buf : kv.second) {
                if (buf != rep) {
                    ...
                    LDBG("-- set sharing buffer's offset " << buf->size << " " << buf->offset << " "
                                                           << buf->sharingGroup);
                }
            }
        }
    ...
    void calculateStarts(const SmallVector<BufferT *> &buffers)
    {
        ...
        while (!xBuffers.empty()) {
            ...
            auto bufferIt = std::find_if(xBuffers.begin(), xBuffers.end(), [&](auto *buffer) {
            ...
            if (bufferIt != xBuffers.end()) {
                ...
                tripleMap.insert({alignOffset + xSize, Interval {std::max(range.start(), xRange.start()),
    ...
    void buildInterferenceGraph(const SmallVector<BufferT *> &buffers, GraphT &interference)
    {
    ...
    void allocate(const SmallVector<BufferT *> &buffers, const GraphT &interference)
    {
        ...
        for (auto x : buffers) {
            ...
            allocation->sharedMemorySize = std::max(allocation->sharedMemorySize, x->offset + x->size);
        }
    }
...
std::map<Operation *, SmallVector<Allocation::BufferId>> Allocation::getLiveBuffers()
{
