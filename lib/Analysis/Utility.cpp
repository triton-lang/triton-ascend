...
{
    return getParentAxis(getSrcLayout(), axis) == getParentOrder(getSrcLayout())[0];
}
...
bool shouldUseDistSmem(Attribute srcLayout, Attribute dstLayout)
{
    ...
    assert(numCTAs == getNumCTAs(dstLayout) && "Invalid layout conversion: the numbers of CTAs of src and dst "
...
unsigned ReduceOpHelper::getInterWarpSize()
{
    ...
    return std::min(srcReduceDimSize / sizeIntraWarps, getWarpsPerCTA(getSrcLayout())[axis]);
}
...
{
    auto srcReduceDimSize = static_cast<unsigned>(srcShape[axis]);
    unsigned elementPerThreads = getUniqueContigPerThread(getSrcLayout(), getSrcShape())[axis];
...
Region &ScanLoweringHelper::getCombineOp()
{
...
unsigned ScanLoweringHelper::getAxisNumBlocks()
{
    ...
    return ceil<unsigned>(getShape()[axis], (sizePerThreads[axis] * threadsPerWarp[axis] * warpsPerCTA[axis]));
}
...
unsigned ScanLoweringHelper::getNonAxisNumBlocks()
{
    ...
    for (unsigned i = 0; i < sizePerThreads.size(); i++) {
        if (i == axis)
            continue;
        numBlocks *= ceil<unsigned>(getShape()[i], (sizePerThreads[i] * threadsPerWarp[i] * warpsPerCTA[i]));
    }
    return numBlocks;
}
...
unsigned ScanLoweringHelper::getScratchSizeInElems()
{
    ...
    unsigned numNonAxisElementsPerWarp = getNonAxisNumThreadsPerWarp() * getNonAxisNumElementsPerThread();
    unsigned numElements = numWarps * numNonAxisElementsPerWarp * getAxisNumBlocks() * getNonAxisNumBlocks();
    return numElements;
}
...
SmallVector<std::pair<SmallVector<int64_t>, SmallVector<int64_t>>> getReshapeDecomposition(ArrayRef<int64_t> srcShape,
                                                                                           ArrayRef<int64_t> dstShape)
{
    ...
    while (srcIdx < srcShape.size() || dstIdx < dstShape.size()) {
        if (srcNElems < dstNElems || //
            (srcIdx < srcShape.size() && srcNElems == 1) || (srcIdx < srcShape.size() && srcShape[srcIdx] == 1))
        {
            assert(srcIdx < srcShape.size());
            srcNElems *= srcShape[srcIdx];
            ret.back().first.push_back(srcIdx);
            srcIdx++;
        } else if (dstNElems < srcNElems || (dstIdx < dstShape.size() && dstShape[dstIdx] == 1)) {
...
unsigned ScanLoweringHelper::getAxisBlockStride()
{
    ...
    for (unsigned dim : order) {
        if (dim == getAxis())
            return stride;
        stride *= ceil<unsigned int>(getShape()[dim], sizePerThreads[dim] * threadsPerWarp[dim] * warpsPerCTA[dim]);
    }
    llvm_unreachable("Axis not found in order");
}
...
bool supportMFMATypes(Type a, Type b)
{
    ...
    DenseSet<std::pair<TypeID, TypeID>> supportedTypes = {{F32, F32},
                                                          {F16, F16},
                                                          {BF16, BF16},
                                                          {F8E5M2, F8E5M2},
                                                          {F8E4M3FN, F8E4M3FN},
...
bool supportMMA(triton::DotOp op, int version)
{
    ...
    if (version == 3) {
        ...
        if (rank == 3)
            return false;
        if (!(numWarps % 4 == 0 && retShapePerCTA[rank - 2] % 64 == 0 && retShapePerCTA[rank - 1] % 8 == 0 &&
              (aElemTy.isFloat8E5M2() || aElemTy.isFloat8E4M3FN() || aElemTy.isInteger(8) || aElemTy.isF16() ||
               aElemTy.isBF16() || aElemTy.isF32())))
        {
            return false;
        }
        // We cannot use MMA_V3 if we need to accumulate in F32 within the MMA op.
        if (op.getMaxNumImpreciseAcc() < 32 && (aElemTy.isFloat8E5M2() || aElemTy.isFloat8E4M3FN()) &&
...
bool supportMMA(Value value, int version)
{
    ...
    assert((version == 1 || version == 2 || version == 3) && "Unexpected MMA layout version found");
    auto elemTy = cast<TensorOrMemDesc>(value.getType()).getElementType();
    // FP8 is not natively supported on all mma versions but it can always be
    // promoted to fp16 therefore we can always support it.
    bool isFP8 =
        elemTy.isFloat8E5M2() || elemTy.isFloat8E4M3FN() || elemTy.isFloat8E5M2FNUZ() || elemTy.isFloat8E4M3FNUZ();
    return isFP8 || elemTy.isF16() || elemTy.isBF16() || (elemTy.isF32() && version >= 2) ||
...
bool isBlockedToDotShortcut(RankedTensorType &srcTy, RankedTensorType &dstTy)
{
    ...
    if (blockedLayout == nullptr || dotOperandLayout == nullptr)
        return false;
    auto parentLayout = dyn_cast<BlockedEncodingAttr>(dotOperandLayout.getParent());
  ...
  //
    ...
    bool ctaLayoutCompatible =
        ctaLayout.getCTASplitNum()[kDim] == 1 && blockedLayout.getCTALayout() == parentLayout.getCTALayout();
    bool threadHoldsWholeKDim = blockedLayout.getSizePerThread()[kDim] == opShape[kDim];
    bool nonKDimCompatible = blockedLayout.getOrder() == parentLayout.getOrder() &&
                             blockedLayout.getSizePerThread()[nonKDim] == parentLayout.getSizePerThread()[nonKDim] &&
                             blockedLayout.getThreadsPerWarp()[nonKDim] == parentLayout.getThreadsPerWarp()[nonKDim] &&
                             blockedLayout.getWarpsPerCTA()[nonKDim] == parentLayout.getWarpsPerCTA()[nonKDim];
    bool matrixDimsCompatible = ctaLayoutCompatible && threadHoldsWholeKDim && nonKDimCompatible;
    if (rank == 2)
        return matrixDimsCompatible;

    // additional check for batch dimension if it is present
    assert(rank == 3);
    bool bDimCompatible = blockedLayout.getSizePerThread()[0] == parentLayout.getSizePerThread()[0] &&
                          blockedLayout.getThreadsPerWarp()[0] == parentLayout.getThreadsPerWarp()[0] &&
...
bool isMfmaToDotShortcut(RankedTensorType srcTy, RankedTensorType dstTy)
{
    ...
    // layout when opIdx == 1.
    return mfmaLayout.getWarpsPerCTA()[1] == 1 && dotOperandLayout.getOpIdx() == 0 && mfmaLayout.getIsTransposed() &&
           dotOperandLayout.getKWidth() == getContigPerThread(mfmaLayout)[1] &&
           dotOperandLayout.getParent() == mfmaLayout && (mfmaLayout.getMDim() == 32 || mfmaLayout.getMDim() == 16) &&
           (srcTy.getElementType().isF16() || srcTy.getElementType().isBF16());
}

// For MMAV3 dotOperand layout matches mma operand for f16 and bf16 cases.
bool matchMmaV3AndDotOperandLayout(RankedTensorType srcTy, RankedTensorType dstTy)
{
    ...
    auto parentTy = RankedTensorType::get(srcTy.getShape(), srcTy.getElementType(), dotOperandLayout.getParent());
    auto ans = mmaLayout.getVersionMajor() == 3 && dotOperandLayout.getOpIdx() == 0 &&
               mmaLayout.getWarpsPerCTA()[1] == 1 && !cvtNeedsSharedMemory(parentTy, srcTy) &&
...
std::optional<LinearLayout> minimalCvtLayout(RankedTensorType srcTy, RankedTensorType dstTy)
{
    MLIRContext *ctx = srcTy.getContext();
    std::optional<LinearLayout> srcLayout = toLinearLayout(srcTy.getShape(), srcTy.getEncoding());
    std::optional<LinearLayout> dstLayout = toLinearLayout(dstTy.getShape(), dstTy.getEncoding());
...
bool cvtNeedsWarpShuffle(RankedTensorType srcTy, RankedTensorType dstTy)
{
    ...
    return llvm::to_vector(layout->getOutDimNames()) == llvm::SmallVector<StringAttr, 2> {kRegister, kLane};
}
...
bool cvtNeedsSharedMemory(RankedTensorType srcTy, RankedTensorType dstTy)
{
    ...
    return !cvtReordersRegisters(srcTy, dstTy) && !isBlockedToDotShortcut(srcTy, dstTy) &&
           !isMmaToDotShortcut(srcTy, dstTy) && !isMfmaToDotShortcut(srcTy, dstTy);
}
...
bool isMmaToDotShortcut(RankedTensorType srcTy, RankedTensorType dstTy)
{
    ...
    auto mmaLayout = dyn_cast<NvidiaMmaEncodingAttr>(srcTy.getEncoding());
    auto dotOperandLayout = dyn_cast<DotOperandEncodingAttr>(dstTy.getEncoding());
    return mmaLayout && dotOperandLayout && mmaLayout.getVersionMajor() == 2 && mmaLayout.getWarpsPerCTA()[1] == 1 &&
           dotOperandLayout.getOpIdx() == 0 && dotOperandLayout.getParent() == mmaLayout &&
...
struct DFSState {
    ...
    void addToReadyQueue(Operation *op, DFSSubgraphState &subGraph, SmallVector<Operation *, 4> &readyQueue)
    {
        bool ready = true;
...
SetVector<Operation *> multiRootTopologicalSort(const SetVector<Operation *> &toSort)
{
    if (toSort.empty()) {
        return toSort;
    }
    ...
    for (auto it = state.topologicalCounts.rbegin(), eit = state.topologicalCounts.rend(); it != eit; ++it) {
        res.insert(*it);
    }
    return res;
}

SetVector<Operation *> multiRootGetSlice(Operation *op, TransitiveFilter backwardFilter, TransitiveFilter forwardFilter)
{
  ...
  public:
    ...
    LogicalResult visit(ProgramPoint *point) override
    {
        ...
        if (matchPattern(op, m_Constant(&value))) {
            ...
            propagateIfChanged(constant, constant->join(dataflow::ConstantValue(value, op->getDialect())));
            return success();
        }
...
MakeTensorPtrOp getMakeTensorPtrOp(Value v)
{
    ...
    auto moduleOp = v.getParentBlock()->getParentOp()->getParentOfType<ModuleOp>();
    ...
    if (auto forOp = dyn_cast<scf::ForOp>(argOwner))
        return getMakeTensorPtrOp(forOp.getOperand(argNum + forOp.getNumControlOperands() - 1));
