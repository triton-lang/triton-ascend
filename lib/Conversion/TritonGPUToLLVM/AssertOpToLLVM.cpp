...
struct AssertOpConversion : public ConvertOpToLLVMPattern<triton::AssertOp> {
    explicit AssertOpConversion(LLVMTypeConverter &typeConverter, const TargetInfoBase &targetInfo,
                                PatternBenefit benefit)
        : ConvertOpToLLVMPattern<triton::AssertOp>(typeConverter, benefit), targetInfo(targetInfo)
    {
    }
                                  ...
                                  ConversionPatternRewriter &rewriter) const override
    {
        ...
        for (auto elem : elems) {
            if (elemTy.isSignedInteger() || elemTy.isSignlessInteger()) {
                condition =
                    or_(condition,
                        icmp_eq(elem, rewriter.create<LLVM::ConstantOp>(loc, elemTy, rewriter.getZeroAttr(elemTy))));
            } else {
    ...
    void llAssert(Operation *op, Value condition, StringRef message, ConversionPatternRewriter &rewriter) const
    {
...
void mlir::triton::populateAssertOpToLLVMPattern(LLVMTypeConverter &typeConverter, RewritePatternSet &patterns,
