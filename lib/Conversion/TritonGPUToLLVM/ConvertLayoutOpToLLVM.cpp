...
struct ConvertLayoutOpConversion : public ConvertOpToLLVMPattern<ConvertLayoutOp> {
  public:
    ConvertLayoutOpConversion(LLVMTypeConverter &typeConverter, const TargetInfoBase &targetInfo,
                              PatternBenefit benefit = 1)
        : ConvertOpToLLVMPattern(typeConverter, benefit), targetInfo(targetInfo)
    {
    }

    LogicalResult matchAndRewrite(ConvertLayoutOp op, OpAdaptor adaptor,
  ...
  private:
    bool isSupported(Attribute srcLayout, Attribute dstLayout) const
    {
        return isa<BlockedEncodingAttr, MmaEncodingTrait, SliceEncodingAttr>(srcLayout) &&
               isa<BlockedEncodingAttr, MmaEncodingTrait, SliceEncodingAttr>(dstLayout) && !isLayoutMmaV1(srcLayout) &&
    ...
    LogicalResult lowerDistributedToDistributed(ConvertLayoutOp op, OpAdaptor adaptor,
...
struct ConvertLayoutOpBlockedToDotOpShortcutConversion : public ConvertOpToLLVMPattern<ConvertLayoutOp> {
    const TargetInfoBase &targetInfo;
    explicit ConvertLayoutOpBlockedToDotOpShortcutConversion(LLVMTypeConverter &typeConverter,
                                                             const TargetInfoBase &targetInfo,
                                                             PatternBenefit benefit = 1)
        : ConvertOpToLLVMPattern(typeConverter, benefit), targetInfo(targetInfo)
    {
                                  ...
                                  ConversionPatternRewriter &rewriter) const override
    {
        ...
        if (!dstDotEncoding)
            return failure();
        if (!isa<BlockedEncodingAttr>(srcTy.getEncoding()) || !isa<BlockedEncodingAttr>(dstDotEncoding.getParent()))
            return failure();
...
struct ConvertLayoutOpUsingLinearLayoutsConversion : public ConvertOpToLLVMPattern<ConvertLayoutOp> {
    const TargetInfoBase &targetInfo;
    ...
    explicit ConvertLayoutOpUsingLinearLayoutsConversion(LLVMTypeConverter &typeConverter,
                                                         const TargetInfoBase &targetInfo, PatternBenefit benefit = 2)
        : ConvertOpToLLVMPattern(typeConverter, benefit), targetInfo(targetInfo)
    {
    }
                                  ...
                                  ConversionPatternRewriter &rewriter) const override
    {
        ...
        if (!conversion.has_value()) {
            return rewriter.notifyMatchFailure(op, "NYI. srcTy and/or dstTy don't implement LLs yet");
        }

        assert(to_vector(conversion->getInDimNames()) == to_vector(conversion->getOutDimNames()));
        ...
        if (llvm::is_contained(dims, str_attr("block"))) {
            ...
            LinearLayout srcLayout = *toLinearLayout(srcTy.getShape(), srcTy.getEncoding());
            LinearLayout dstLayout = *toLinearLayout(dstTy.getShape(), dstTy.getEncoding());
    ...
    LogicalResult transferWithinThread(ConvertLayoutOp op, const LinearLayout &conversion, OpAdaptor adaptor,
                                       ConversionPatternRewriter &rewriter) const
    {
        ...
        Value result = packLLElements(loc, getTypeConverter(), outVals, rewriter, op.getType());
        rewriter.replaceOp(op, result);
        return success();
    }

    LogicalResult transferWithinBlock(ConvertLayoutOp op, const LinearLayout &srcLayout, const LinearLayout &dstLayout,
                                      OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const
    {
        ...
        std::function<bool(Attribute)> layoutIsOK = [&](Attribute layout) {
            ...
            if (auto dotOperand = dyn_cast<DotOperandEncodingAttr>(layout)) {
                if (auto nvidiaMma = dyn_cast<NvidiaMmaEncodingAttr>(dotOperand.getParent())) {
                    ...
                    bool largeKWidth = dotOperand.getKWidth() * dstTy.getElementTypeBitWidth() > 64;
        ...
        SmallVector<Value> outVals =
            transferWithinBlockImpl(inVals, op, srcLayoutWithinBlock, dstLayoutWithinBlock, adaptor, rewriter);
        ...
        if (isa<DotOperandEncodingAttr>(dstTy.getEncoding())) {
            auto concat = [&](Value a, Value b) {
                return or_(zext(i32_ty, bitcast(a, i16_ty)), shl(zext(i32_ty, bitcast(b, i16_ty)), i32_val(16)));
            };
    ...
    SmallVector<Value> transferWithinBlockImpl(ArrayRef<Value> inVals, ConvertLayoutOp op,
                                               ...
                                               OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const
    {
        ...
        auto tensorShapePerCTA = convertType<unsigned, int64_t>(
            getShapePerCTA(op.getSrc().getType().getEncoding(), op.getType().getShape()));
        // Input dims: [offset, iteration, block]
        // Output dims: dimN-1, dimN-2, ..., dim0, where N is obtained from repShape
        LinearLayout sharedLayout =
            chooseShemLayoutForRegToRegConversion(ctx, tensorShapePerCTA, scratchConfig.repShape, scratchConfig.order);
        ...
        std::optional<LinearLayout> shmemStoreLayout = chooseStMatrixLayout(
            ctx, op.getSrc().getType(), scratchConfig.repShape, scratchConfig.paddedRepShape, scratchConfig.order,
            /*swizzleByteSize=*/0);
        ...
        const int shmemAllocatedNumElems = getNumScratchElements(scratchConfig.paddedRepShape);
        ...
        assert(shmemStoreLayout->sublayoutIsZero({kLane, kWarp, kBlock}, {kIteration}));
        assert(shmemLoadLayout.sublayoutIsZero({kLane, kWarp, kBlock}, {kIteration}));

        // iteration -> registers
        SmallVector<SmallVector<int>> inRegsForIter = collectRegsForIter(ctx, *shmemStoreLayout);
        SmallVector<SmallVector<int>> outRegsForIter = collectRegsForIter(ctx, shmemLoadLayout);
        ...
        auto paddedSize = scratchConfig.paddedRepShape[scratchConfig.order[0]] - paddedStride;
        ...
        auto getVecAddr = [&](LinearLayout &layout, Value &regBase, int regSlice) -> Value {
            auto regIdx = layout.apply({{kRegister, regSlice}, {kLane, 0}, {kWarp, 0}, {kBlock, 0}})[0].second;
            ...
            if (paddedSize > 0) {
                ...
                offset = add(shl(lshr(offset, i32_val(rshiftVal)), i32_val(lshiftVal)), offset);
            }
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            // not contiguous
            auto inVec = isStMatrix ? shmemStoreLayout->getNumConsecutiveInOut() : scratchConfig.inVec;
            ...
            for (int j = 0; j < outSize / iterations; j += scratchConfig.outVec) {
                ...
                Value valsVec =
                    targetInfo.loadDShared(rewriter, loc, vecAddr, std::nullopt, vec_ty(elemTy, scratchConfig.outVec),
                                           /*pred=*/true_val());
    ...
    SmallVector<SmallVector<int> /*registers*/> collectRegsForIter(MLIRContext *ctx, const LinearLayout &layout) const
    {
        StringAttr kRegister = str_attr("register");
        StringAttr kLane = str_attr("lane");
        StringAttr kWarp = str_attr("warp");
...
void mlir::triton::populateConvertLayoutOpUsingLinearLayoutsToLLVMPattern(LLVMTypeConverter &typeConverter,
                                                                          const TargetInfoBase &targetInfo,
                                                                          RewritePatternSet &patterns,
                                                                          PatternBenefit benefit)
{
    patterns.add<ConvertLayoutOpUsingLinearLayoutsConversion>(typeConverter, targetInfo, benefit);
}

void mlir::triton::populateConvertLayoutOpToLLVMPatterns(LLVMTypeConverter &typeConverter,
                                                         ...
                                                         PatternBenefit benefit)
{
    ...
    // one left.
    mlir::triton::populateConvertLayoutOpUsingLinearLayoutsToLLVMPattern(typeConverter, targetInfo, patterns,
                                                                         benefit.getBenefit() + 1);
    patterns.add<ConvertLayoutOpBlockedToDotOpShortcutConversion>(typeConverter, targetInfo, benefit);
