...
static int log2Int(int64_t num)
{
...
// only popcount those.
static SmallVector<Value> computeWarpLevelHistogram(Location loc, RankedTensorType srcType,
                                                    SmallVector<Value> &srcValues, int numBins, int numThreadPerWarp,
                                                    Value threadId, ConversionPatternRewriter &rewriter,
                                                    const TargetInfoBase &targetInfo)
{
    assert(numBins % numThreadPerWarp == 0 && "numBins must be divisible by numThreadPerWarp");
    ...
    unsigned numThreadWithUniqueData =
        triton::gpu::getThreadsPerWarpWithUniqueData(srcType.getEncoding(), srcType.getShape())[0];
    ...
    for (int i = 0; i < numElementsPerThreads; ++i) {
        ...
        for (int j = 0; j < numBits; ++j) {
            ...
            Value bit = targetInfo.ballot(rewriter, loc, int_ty(numThreadPerWarp), cmp);
            ballotBits.push_back(bit);
        }
        uint64_t fullMaskValue = numThreadPerWarp == 32 ? 0xFFFFFFFF : 0xFFFFFFFFFFFFFFFF;
        ...
        for (int i = 0; i < numBitsLaneId; i++) {
            Value updateMask =
                select(icmp_ne(and_(threadId, i32_val(1 << i)), zero), int_val(numThreadPerWarp, 0), fullMask);
            mask = and_(mask, xor_(ballotBits[i + numBits - numBitsLaneId], updateMask));
        }
        ...
        for (int k = 0; k < warpLevelHistogram.size(); k++) {
            ...
            for (int j = 0; j < numBits - numBitsLaneId; j++) {
                Value updateMask = int_val(numThreadPerWarp, ((k & (1 << j)) ? 0 : fullMaskValue));
                binMask = and_(binMask, xor_(ballotBits[j], updateMask));
            }
            // at this point, 'bin_mask' tells you which elements are in the kth bin
            // owned by this thread.
            Value bitCount = rewriter.create<LLVM::CtPopOp>(loc, int_ty(numThreadPerWarp), binMask);
...
static void atomicAdd(Value ptr, Value val, Location loc, ConversionPatternRewriter &rewriter)
{
    rewriter.create<LLVM::AtomicRMWOp>(loc, LLVM::AtomicBinOp::add, ptr, val, LLVM::AtomicOrdering::monotonic);
}

static SmallVector<Value> computeCrossWarpHistogram(Location loc, ConversionPatternRewriter &rewriter,
                                                    RankedTensorType srcType, Value baseSharedMemPtr,
                                                    ...
                                                    Value threadId, int numWarps)
{
    ...
    unsigned numWarpsWithUniqueData =
        mlir::triton::gpu::getWarpsPerCTAWithUniqueData(srcType.getEncoding(), srcType.getShape())[0];
    ...
    for (int i = 0; i < numElementPerThread; ++i) {
        ...
        Value sharedMemPtr = gep(baseSharedMemPtr.getType(), i32_ty, baseSharedMemPtr, offset);
    ...
    if (numWarpsWithUniqueData < numWarps) {
        ...
        afterAtomics = rewriter.splitBlock(currentBlock, rewriter.getInsertionPoint());
        Block *atomicBlock = rewriter.createBlock(afterAtomics);
        rewriter.setInsertionPointToEnd(currentBlock);
        Value cond = icmp_ult(threadId, i32_val(numWarpsWithUniqueData * numThreadPerWarp));
    ...
    for (int i = 0; i < warpLevelHistogram.size(); ++i) {
        ...
        Value offset = add(mul(laneId, i32_val(warpLevelHistogram.size())), i32_val(i));
        Value sharedMemPtr = gep(baseSharedMemPtr.getType(), i32_ty, baseSharedMemPtr, offset);
    ...
    for (Value index : indices) {
        Value sharedMemPtr = gep(baseSharedMemPtr.getType(), i32_ty, baseSharedMemPtr, index);
...
namespace {
struct HistogramOpConversion : public ConvertOpToLLVMPattern<triton::HistogramOp> {
  public:
    using ConvertOpToLLVMPattern<triton::HistogramOp>::ConvertOpToLLVMPattern;

    explicit HistogramOpConversion(LLVMTypeConverter &typeConverter, const TargetInfoBase &targetInfo,
                                   PatternBenefit benefit = 1)
        : ConvertOpToLLVMPattern(typeConverter, benefit), targetInfo(targetInfo)
    {
    }

    LogicalResult matchAndRewrite(triton::HistogramOp op, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override
    {
        ...
        int numThreadsPerWarp = triton::gpu::TritonGPUDialect::getThreadsPerWarp(mod);
        assert(numThreadsPerWarp == 32 || numThreadsPerWarp == 64 && "Only supports 32 or 64 threads per warp");
        ...
        SmallVector<Value> warpLevelHistogram = computeWarpLevelHistogram(
            loc, srcType, srcValues, numBins, numThreadsPerWarp, threadId, rewriter, targetInfo);
        ...
        auto indices = emitIndices(op.getLoc(), rewriter, targetInfo, dstEncoding, dstType, true);
        SmallVector<Value> innerDimIndices;
        for (int i = 0; i < indices.size(); ++i)
            innerDimIndices.push_back(indices[i][0]);
        SmallVector<Value> histogramValue =
            computeCrossWarpHistogram(loc, rewriter, srcType, baseSharedMemPtr, warpLevelHistogram, numBins,
                                      numThreadsPerWarp, innerDimIndices, threadId, numWarps);

        Value results = packLLElements(loc, typeConverter, histogramValue, rewriter, op.getType());
...
void mlir::triton::populateHistogramOpToLLVMPatterns(LLVMTypeConverter &typeConverter, RewritePatternSet &patterns,
