...
struct PrintOpConversion : public ConvertOpToLLVMPattern<triton::PrintOp> {
    explicit PrintOpConversion(LLVMTypeConverter &typeConverter, const TargetInfoBase &targetInfo,
                               PatternBenefit benefit)
        : mlir::ConvertOpToLLVMPattern<triton::PrintOp>(typeConverter, benefit), targetInfo(targetInfo)
    {
    }
                                  ...
                                  ConversionPatternRewriter &rewriter) const override
    {
        ...
        auto getPid = [&](int axis) {
            return targetInfo.programId(rewriter, loc, op->getParentOfType<ModuleOp>(), axis);
        };
        ...
        if (op.getNumOperands() == 0) {
            ...
            os << "pid (" << getFormatSubstr(pid[0]) << ", " << getFormatSubstr(pid[1]) << ", "
        ...
        for (size_t i = 0; i < op.getNumOperands(); i++) {
            ...
            if (auto rankedTy = dyn_cast<RankedTensorType>(op.getOperand(i).getType())) {
                indices = emitIndices(loc, rewriter, targetInfo, rankedTy.getEncoding(), rankedTy, true);
            ...
            if (!elems.empty()) {
                printTensor(op.getPrefix(), /*operand=*/i,
                            /*numOperands=*/op.getNumOperands(), elems, pid, indices, dimWidths, op.getHex(), rewriter,
                            isSigned);
            }
        }
        rewriter.eraseOp(op);
    ...
    void printTensor(StringRef prefixStr, size_t operand, size_t numOperands, ArrayRef<Value> elems,
                     std::array<Value, 3> pid, ArrayRef<SmallVector<Value>> indices, ArrayRef<int> dimWidths, bool hex,
                     ConversionPatternRewriter &rewriter, bool isSigned) const
    {
        ...
        for (int i = 0; i < elems.size(); i++) {
            ...
            if (i == 0) {
                formatStrValue = llPrintf(formatStr, printfOperands, rewriter, &formatStrByteCount);
            } else {
                targetInfo.printf(rewriter, formatStrValue, formatStrByteCount, printfOperands);
            }
        }
    }

    std::string getFormatSubstr(Value value, bool hex = false, std::optional<int> width = std::nullopt,
    ...
    Value llPrintf(StringRef msg, ValueRange args, ConversionPatternRewriter &rewriter,
...
void mlir::triton::populatePrintOpToLLVMPattern(LLVMTypeConverter &typeConverter, RewritePatternSet &patterns,
