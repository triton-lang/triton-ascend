...
static SmallVector<Value> accumulate(ScanLoweringHelper &helper, ConversionPatternRewriter &rewriter, ValueRange acc,
                                         ...
                                         ConversionPatternRewriter &rewriter, ScanLoweringHelper &helper)
{
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        unsigned accIndex = (srcIndex % stride) + ((srcIndex / stride) / scanElementsPerThreads) * stride;
...
static void warpScan(SmallVector<SmallVector<Value>> &srcValues, ConversionPatternRewriter &rewriter,
                     const TargetInfoBase &targetInfo, ScanLoweringHelper &helper, Value laneIdAxis)
{
...
static void storeWarpAccumulator(SmallVector<SmallVector<Value>> &srcValues, ConversionPatternRewriter &rewriter,
                                 ScanLoweringHelper &helper, Value laneId, Value warpId, SmallVector<Value> smemBases,
                                 SmallVector<Type> smemTypes, Value parallelLaneId, const TargetInfoBase &targetInfo)
{
    Location loc = helper.getLoc();
    unsigned scanElementsPerThreads = helper.getAxisNumElementsPerThread();
    unsigned scanDim = helper.getAxisNumThreadsPerWarpWithUniqueData();
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        for (unsigned i = 0; i < lastElement.size(); ++i) {
            Value writePtr = gep(smemBases[i].getType(), smemTypes[i], smemBases[i], index);
...
static void AddPartialReduce(SmallVector<SmallVector<Value>> &srcValues, ConversionPatternRewriter &rewriter,
                             const TargetInfoBase &targetInfo, ScanLoweringHelper &helper, ArrayRef<Value> smemBases,
                             ArrayRef<Type> smemTypes, Value warpId, Value laneIdAxis, Value parallelLaneId)
{
    Location loc = helper.getLoc();
    unsigned numParallelLane = helper.getNonAxisNumThreadsPerCTA();
    unsigned scanElementsPerThreads = helper.getAxisNumElementsPerThread();
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        unsigned parallelBlockId = blockId % blockStride + ((blockId / blockStride) / numScanBlocks) * blockStride;
        unsigned accumulatorIndex = chunkId % parallelElementsPerThread + parallelBlockId * parallelElementsPerThread;
        ...
        for (unsigned i = 0; i < axisNumWarps; ++i) {
            Value index = add(parallelLaneId, i32_val(numParallelLane * (i + chunkId * axisNumWarps)));
            ...
            accumulator.acc = accumulate(helper, rewriter, accumulator.acc, partialReduce);
            for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                accumulator.maskedAcc[j] = select(mask, accumulator.acc[j], accumulator.maskedAcc[j]);
            }
        }

        Value pred = axisBlockId == 0 ? maskNotFirstWarp : Value {};
        auto temp = accumulate(helper, rewriter, accumulator.maskedAcc, srcValues[srcIndex], pred);
        if (axisBlockId == 0) {
        ...
        for (unsigned i = 1; i < scanElementsPerThreads; ++i) {
            ...
            if (axisBlockId == 0) {
                ...
                // accumulate.
                for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                    laneValue[j] = select(maskNotFirstThread, laneValue[j], srcValues[srcIndex - i * elementStride][j]);
                }
            }
...
static void AddPartialReduceOneWarp(SmallVector<SmallVector<Value>> &srcValues, ConversionPatternRewriter &rewriter,
                                    const TargetInfoBase &targetInfo, ScanLoweringHelper &helper, Value warpId,
                                    Value laneIdAxis, Value laneIdLast)
{
    ...
    unsigned numParallelBlocks = helper.getNonAxisNumBlocks();
    assert(numScanBlocks * numParallelBlocks * parallelElementsPerThread * scanElementsPerThreads == srcValues.size());
    SmallVector<SmallVector<Value>> accumulators(numParallelBlocks * parallelElementsPerThread);
    unsigned chunkId = 0;
    unsigned blockStride = helper.getAxisBlockStride();
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        unsigned elementIdx = (srcIndex / elementStride) % scanElementsPerThreads;
        ...
        if (axisBlockId == 0) // First chunk and first block
            ...
            srcValues[srcIndex] = accumulate(helper, rewriter, accumulator, srcValues[srcIndex]);
        // Update the rest of the contiguous elements.
        auto lastElement = srcValues[srcIndex];
        if (scanDim > 1) {
            for (unsigned i = 0; i < helper.getNumOperands(); ++i) {
                lastElement[i] = targetInfo.shuffleUp(rewriter, loc, srcValues[srcIndex][i], threadStride);
                lastElement[i] = select(maskFirstLane, accumulator[i], lastElement[i]);
                if (numScanBlocks > 1)
                    // Update accumulator with the value from the last lane.
                    accumulator[i] = targetInfo.shuffleIdx(rewriter, loc, srcValues[srcIndex][i], laneIdLast);
            }
        ...
        for (unsigned i = 1; i < scanElementsPerThreads; ++i) {
            ...
            if (axisBlockId == 0) {
                for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                    // For the first warp and first chunk we don't have anything to
                    // accumulate.
                    laneValue[j] = select(maskFirstThread, srcValues[srcIndex - i * elementStride][j], laneValue[j]);
                }
            }
            srcValues[srcIndex - i * elementStride] = std::move(laneValue);
        }
...
namespace {
struct ScanOpConversion : public ConvertTritonGPUReduceScanToLLVMPattern<triton::ScanOp> {
  public:
    using ConvertTritonGPUReduceScanToLLVMPattern<triton::ScanOp>::ConvertTritonGPUReduceScanToLLVMPattern;
    explicit ScanOpConversion(LLVMTypeConverter &typeConverter, const TargetInfoBase &targetInfo,
                              PatternBenefit benefit = 1)
        : ConvertTritonGPUReduceScanToLLVMPattern<triton::ScanOp>(typeConverter, benefit), targetInfo(targetInfo)
    {
    }
  ...
  private:
    ...
    SmallVector<Value> getMultiDimLaneId(ConversionPatternRewriter &rewriter, ScanLoweringHelper &helper,
                                         Value laneId) const;
    SmallVector<Value> getMultiDimWarpId(ConversionPatternRewriter &rewriter, ScanLoweringHelper &helper,
                                         Value warpId) const;
    std::tuple<Value, Value, Value> getDelinearizedIds(ConversionPatternRewriter &rewriter, ScanLoweringHelper &helper,
                                                       Value laneId, Value warpId) const;
    LogicalResult emitFastScan(triton::ScanOp op, triton::ScanOpAdaptor adaptor, ConversionPatternRewriter &rewriter,
                               const TargetInfoBase &targetInfo) const;
};

SmallVector<Value> ScanOpConversion::getMultiDimLaneId(ConversionPatternRewriter &rewriter, ScanLoweringHelper &helper,
                                                       Value laneId) const
{
    auto loc = helper.getLoc();
...
SmallVector<Value> ScanOpConversion::getMultiDimWarpId(ConversionPatternRewriter &rewriter, ScanLoweringHelper &helper,
                                                       Value warpId) const
{
    auto loc = helper.getLoc();
    unsigned axis = helper.getAxis();
...
std::tuple<Value, Value, Value> ScanOpConversion::getDelinearizedIds(ConversionPatternRewriter &rewriter,
                                                                     ...
                                                                     Value warpId) const
{
    ...
    auto order = triton::gpu::getOrder(srcEncoding);
    auto warpOrder = triton::gpu::getWarpOrder(srcEncoding);
    SmallVector<Value> multiDimLaneId = delinearize(rewriter, loc, laneId, threadsPerWarp, order);
    ...
    Value laneIdParallel = linearize(rewriter, loc, multiDimLaneId, threadsPerWarp, order);
    multiDimWarpId[axis] = i32_val(0);
    warpsPerCTA[axis] = 1;
    Value warpIdParallel = linearize(rewriter, loc, multiDimWarpId, warpsPerCTA, warpOrder);
    Value flatIdParallel = add(laneIdParallel, mul(warpIdParallel, i32_val(helper.getNonAxisNumThreadsPerWarp())));
    return std::make_tuple(laneIdAxis, warpIdAxis, flatIdParallel);
}

SmallVector<SmallVector<Value>> unpackInputs(Location loc, triton::ScanOp op, triton::ScanOpAdaptor adaptor,
                                             ConversionPatternRewriter &rewriter, const LLVMTypeConverter &converter)
{
...
SmallVector<SmallVector<Value>> flipSrcValues(Location loc, triton::ScanOp op, ConversionPatternRewriter &rewriter,
                                              const TargetInfoBase &targetInfo,
    ...
    for (int i = 0; i < srcValues.size(); ++i) {
        ...
        for (unsigned j = 0; j < op.getNumOperands(); ++j) {
            for (unsigned k = iWarpSize / 2; k >= 1; k = k / 2) {
                srcValues[revIndex][j] = targetInfo.shuffleXor(rewriter, loc, srcValues[revIndex][j], k);
            }
...
LogicalResult ScanOpConversion::emitFastScan(triton::ScanOp op, triton::ScanOpAdaptor adaptor,
                                             ...
                                             const TargetInfoBase &targetInfo) const
{
    ...
    auto [laneIdAxis, warpIdAxis, flatIdParallel] = getDelinearizedIds(rewriter, helper, laneId, warpId);
    ...
    if (axisNumWarps > 1) {
        ...
        SmallVector<Value> smemBases = getSmemBases(op, elems, rewriter, targetInfo);
        ...
        storeWarpAccumulator(srcValues, rewriter, helper, laneIdAxis, warpIdAxis, smemBases, smemTypes, flatIdParallel,
                             targetInfo);
        barrier();
        ...
        AddPartialReduce(srcValues, rewriter, targetInfo, helper, smemBases, smemTypes, warpIdAxis, laneIdAxis,
                         flatIdParallel);
        ...
        auto laneIdLast =
            linearize(rewriter, loc, multiDimLaneId, threadsPerWarp, triton::gpu::getOrder(helper.getEncoding()));
        AddPartialReduceOneWarp(srcValues, rewriter, targetInfo, helper, warpIdAxis, laneIdAxis, laneIdLast);
    ...
    for (unsigned i = 0; i < op.getNumOperands(); ++i) {
        ...
        results[i] = packLLElements(loc, getTypeConverter(), valuesTransposed[i], rewriter, resultTy);
    }
...
void mlir::triton::populateScanOpToLLVMPatterns(LLVMTypeConverter &typeConverter, RewritePatternSet &patterns,
