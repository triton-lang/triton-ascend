...
{
    effects.emplace_back(MemoryEffects::Read::get(), &getPtrMutable(), triton::GlobalMemory::get());
    if (getIsVolatile())
                   ...
                   bool isVolatile)
{
    LoadOp::build(builder, state, ptr, /*mask=*/ {}, /*other=*/ {},
                  /*boundaryCheck=*/ArrayRef<int32_t> {}, /*padding=*/std::nullopt, cache, evict, isVolatile);
}

void LoadOp::build(OpBuilder &builder, OperationState &state, Value ptr, ArrayRef<int32_t> boundaryCheck,
                   std::optional<PaddingOption> padding, CacheModifier cache, EvictionPolicy evict, bool isVolatile)
{
    LoadOp::build(builder, state, ptr, /*mask=*/ {}, /*other=*/ {}, boundaryCheck, padding, cache, evict, isVolatile);
}

void LoadOp::build(OpBuilder &builder, OperationState &state, Value ptr, Value mask, CacheModifier cache,
...
void LoadOp::build(OpBuilder &builder, OperationState &state, Value ptr, Value mask, Value other, CacheModifier cache,
                   EvictionPolicy evict, bool isVolatile)
{
    LoadOp::build(builder, state, ptr, mask, other,
                  /*boundaryCheck=*/ArrayRef<int32_t> {},
...
void LoadOp::build(OpBuilder &builder, OperationState &state, Value ptr, Value mask, Value other,
                   ArrayRef<int32_t> boundaryCheck, std::optional<PaddingOption> padding, CacheModifier cache,
                   EvictionPolicy evict, bool isVolatile)
{
    auto paddingAttr =
        padding.has_value() ? PaddingOptionAttr::get(builder.getContext(), padding.value()) : PaddingOptionAttr();
    LoadOp::build(builder, state, ptr, mask, other, builder.getDenseI32ArrayAttr(boundaryCheck), paddingAttr, cache,
                  evict, isVolatile);
}
...
struct CanonicalizeMaskedLoadPattern : public OpRewritePattern<LoadOp> {
    CanonicalizeMaskedLoadPattern(MLIRContext *context) : OpRewritePattern<LoadOp>(context, 1) {}
    ...
    LogicalResult matchAndRewrite(LoadOp loadOp, PatternRewriter &rewriter) const override
    {
        ...
        if (splatMask.getSplatValue<IntegerAttr>().getValue() == true) {
            // mask = splat(1)
            rewriter.replaceOpWithNewOp<LoadOp>(loadOp, loadOp.getType(), loadOp.getPtr(), Value(), Value(),
...
//-- StoreOp --
void StoreOp::build(OpBuilder &builder, OperationState &state, Value ptr, Value value, CacheModifier cache,
...
void StoreOp::build(OpBuilder &builder, OperationState &state, Value ptr, Value value, Value mask, CacheModifier cache,
                    EvictionPolicy evict)
{
    return StoreOp::build(builder, state, ptr, value, mask, /*boundaryCheck=*/ {}, cache, evict);
}

void StoreOp::build(OpBuilder &builder, OperationState &state, Value ptr, Value value, ArrayRef<int32_t> boundaryCheck,
                    CacheModifier cache, EvictionPolicy evict)
{
    return StoreOp::build(builder, state, ptr, value, /*mask=*/ {}, builder.getDenseI32ArrayAttr(boundaryCheck), cache,
                          evict);
}
...
struct CanonicalizeMaskedStorePattern : public OpRewritePattern<StoreOp> {
    CanonicalizeMaskedStorePattern(MLIRContext *context) : OpRewritePattern<StoreOp>(context, 1) {}
    ...
    LogicalResult matchAndRewrite(StoreOp storeOp, PatternRewriter &rewriter) const override
    {
        ...
        if (splatMask.getSplatValue<IntegerAttr>().getValue() == true) {
            // mask = splat(1)
            rewriter.replaceOpWithNewOp<StoreOp>(storeOp, storeOp.getPtr(), storeOp.getValue(), storeOp.getCache(),
                                                 storeOp.getEvict());
        } else {
            // mask = splat(0)
...
LogicalResult TransOp::inferReturnTypes(MLIRContext *context, std::optional<Location> location, ValueRange operands,
                                        ...
                                        SmallVectorImpl<Type> &inferredReturnTypes)
{
    ...
    if (argEncoding) {
        ...
        if (inferLayoutInterface->inferTransOpEncoding(argEncoding, order, retEncoding).failed()) {
            return failure();
        }
    }
    if (auto memDescTy = dyn_cast<MemDescType>(argTy)) {
        inferredReturnTypes.push_back(MemDescType::get(retShape, retEltTy, retEncoding, memDescTy.getMemorySpace(),
                                                       memDescTy.getMutableMemory()));
    } else {
        inferredReturnTypes.push_back(RankedTensorType::get(retShape, retEltTy, retEncoding));
    }
    return success();
}
...
//-- DotOp --
LogicalResult DotOp::inferReturnTypes(MLIRContext *context, std::optional<Location> location, ValueRange operands,
                                      DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
                                      SmallVectorImpl<Type> &inferredReturnTypes)
{
...
LogicalResult DotOp::verify()
{
    ...
    if (aTy.getElementType().getIntOrFloatBitWidth() != bTy.getElementType().getIntOrFloatBitWidth())
        return emitError("element types of operands A and B must have same bit width");
    ...
    return interface->verifyDotOpEncodingCompatibility(getOperation(), aEncoding, bEncoding);
}
...
LogicalResult MakeRangeOp::verify()
{
    ...
    if (end - start != ty.getShape()[0]) {
        return this->emitOpError() << "number of elements in returned tensor, " << ty.getShape()[0]
                                   << ", must match size of range [" << start << ", " << end << "), which has "
                                   << end - start << " elements";
    }
...
//-- ReduceOp --
static LogicalResult inferReduceReturnShape(RankedTensorType argTy, Type retEltTy, int axis,
                                            SmallVectorImpl<Type> &inferredReturnTypes)
{
    ...
    if (retShape.empty()) {
        ...
        if (argEncoding) {
            Dialect &dialect = argEncoding.getDialect();
            auto inferLayoutInterface = dyn_cast<DialectInferLayoutInterface>(&dialect);
            if (inferLayoutInterface->inferReduceOpEncoding(argEncoding, axis, retEncoding).failed()) {
...
void ReduceOp::build(OpBuilder &builder, OperationState &state, ValueRange operands, int axis)
{
...
LogicalResult ReduceOp::inferReturnTypes(MLIRContext *context, std::optional<Location> location, ValueRange operands,
                                         DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
                                         SmallVectorImpl<Type> &inferredReturnTypes)
{
    ...
    for (auto arg : operands) {
        ...
        auto retEltTy = argTy.getElementType();
        if (inferReduceReturnShape(argTy, retEltTy, axis, inferredReturnTypes).failed()) {
            return failure();
        }
    }
    return success();
...
template <class Op> LogicalResult verifyReduceScan(Op &op)
{
    ...
    for (auto [opElemTy, resTy] : llvm::zip(op.getElementTypes(), op.getResultTypes())) {
...
template <class ReturnOp, class Op> static LogicalResult verifyRegionsImpl(Op &op)
{
    ...
    if (block.getNumArguments() != numArgs) {
        return op.emitOpError() << "nested block must take " << numArgs << " arguments, but given block with "
    ...
    for (unsigned i = 0; i < numArgs; ++i) {
        ...
        if (blockArgTy != argElemTy) {
            return op.emitOpError() << "type mismatch on combine operation. Expected argument " << i << " to have type "
                                    << argElemTy << " but got " << blockArgTy;
        }
    }

    auto terminator = dyn_cast<ReturnOp>(block.getTerminator());
    if (!terminator) {
        return op.emitOpError() << "combine operation must be terminated "
    ...
    if (combineResults.size() != operands.size()) {
        return op.emitOpError() << "expected combine operation to return " << operands.size() << " values but got "
                                << combineResults.size();
    }
    for (unsigned i = 0; i < combineResults.size(); ++i) {
        ...
        if (resultTy != argElemTy) {
            return op.emitOpError() << "type mismatch on combine operation. Expected argument " << i << " to have type "
                                    << argElemTy << " but got " << resultTy;
        }
    }
    return success();
}

static llvm::SmallVector<RankedTensorType> getInputTypesImpl(const Operation::operand_range &operands)
{
...
static llvm::SmallVector<Type> getElementTypesImpl(const Operation::operand_range &operands)
{
...
//-- ScanOp --
void ScanOp::build(OpBuilder &builder, OperationState &state, ValueRange operands, int axis, bool reverse)
{
    SmallVector<Type> inferredReturnTypes;
...
LogicalResult ScanOp::inferReturnTypes(MLIRContext *context, std::optional<Location> location, ValueRange operands,
                                       DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
                                       SmallVectorImpl<Type> &inferredReturnTypes)
{
    for (auto arg : operands)
...
unsigned ScanOp::getNumOperands()
{
    return this->getOperands().size();
}
...
//-- ExpandDimsOp --
LogicalResult ExpandDimsOp::inferReturnTypes(MLIRContext *context, std::optional<Location> loc, ValueRange operands,
                                             ...
                                             RegionRange regions, SmallVectorImpl<Type> &inferredReturnTypes)
{
    ...
    if (argEncoding) {
        Dialect &dialect = argEncoding.getDialect();
        auto inferLayoutInterface = dyn_cast<DialectInferLayoutInterface>(&dialect);
        if (inferLayoutInterface->inferExpandDimsOpEncoding(argEncoding, axis, retEncoding, loc).failed())
            return emitOptionalError(loc, "failed to infer layout for ExpandDimsOp");
    }
    // create type
    auto argEltTy = argTy.getElementType();
    inferredReturnTypes.push_back(RankedTensorType::get(retShape, argEltTy, retEncoding));
    return success();
}
    ...
    if (auto broadcast = dyn_cast<BroadcastOp>(definingOp)) {
        ...
        if (auto srcEnc = srcTy.getEncoding()) {
            if (dyn_cast<DialectInferLayoutInterface>(&srcEnc.getDialect())
                    ->inferExpandDimsOpEncoding(srcEnc, op.getAxis(), newExpandEnc, op.getLoc())
                    .failed())
            {
                return emitOptionalError(op.getLoc(), "failed to infer layout for ExpandDimsOp");
            }
        }

        auto newExpandTy = RankedTensorType::get(newExpandShape, srcTy.getElementType(), newExpandEnc);
        auto newExpand = rewriter.create<ExpandDimsOp>(op.getLoc(), newExpandTy, src, op.getAxis());
        auto newBroadcast = rewriter.create<BroadcastOp>(broadcast.getLoc(), op.getType(), newExpand.getResult());
...
template <typename ViewLikeOp> static OpFoldResult foldViewLikeOp(ViewLikeOp op, Attribute value)
{
    if (!value)
        return {};
...
//-- ReshapeOp --
template <typename OpType> LogicalResult canonicalizeViewOrBroadcast(OpType op, PatternRewriter &rewriter)
{
    auto definingOp = op.getSrc().getDefiningOp();
    if (!definingOp) {
        return failure();
    ...
    if (auto parentView = dyn_cast<OpType>(definingOp)) {
        rewriter.replaceOpWithNewOp<OpType>(op, TypeRange({op.getType()}), parentView->getOperands(),
                                            parentView->getAttrs());
        return success();
    }
...
LogicalResult ReshapeOp::verify()
{
    ...
    if (getType().getNumElements() != srcTy.getNumElements()) {
        return emitError("number of src and dst elements of reshape must be the same");
    }
    ...
    if (srcEnc && !getAllowReorder()) {
        ...
        if (cast<DialectInferLayoutInterface>(&srcEnc.getDialect())
                ->inferReshapeOpNoReorderEncoding(srcTy.getShape(), srcEnc, dstTy.getShape(), inferredDstEnc, getLoc())
                .failed())
        {
        ...
        if (inferredDstEnc != dstEnc) {
            return emitError("Expected result encoding ") << inferredDstEnc << " but was " << dstEnc;
        }
    }
...
LogicalResult FpToFpOp::verify()
{
    ...
    if ((dstType.getIntOrFloatBitWidth() < srcType.getIntOrFloatBitWidth()) && (!getRounding().has_value())) {
...
LogicalResult BroadcastOp::verify()
{
    ...
    for (int i = 0; i < srcShape.size(); i++) {
        if (srcShape[i] != 1 && srcShape[i] != resultShape[i]) {
            return emitError("Different dimensions at index ") << i << " between source and result.  "
...
//-- MakeTensorPtrOp --
void MakeTensorPtrOp::build(OpBuilder &builder, OperationState &state, Value base, ValueRange shape, ValueRange strides,
                            ValueRange offsets, ArrayRef<int32_t> tensorShape, ArrayRef<int32_t> order)
{
    ...
    auto tensorType = RankedTensorType::get(SmallVector<int64_t>(tensorShape.begin(), tensorShape.end()),
                                            pointerType.getPointeeType());
    auto result = PointerType::get(tensorType, 1);

    return build(builder, state, result, base, shape, strides, offsets, builder.getDenseI32ArrayAttr(order));
}

// FIXME TODO: When upgrading to Triton 3.4.0, remove the commented line below
//-- MakeTensorDescOp --
void MakeTensorDescOp::build(OpBuilder &builder, OperationState &state, Value base, ValueRange shape,
                             ValueRange strides, ArrayRef<int32_t> blockShape, bool isSignedInteger)
{
    ...
    auto descTy = TensorDescType::get(builder.getContext(), blockTy, isSignedInteger);
    return build(builder, state, descTy, base, shape, strides);
}

// -- DescriptorLoadOp --
static LogicalResult verifyDescriptorLoadStoreType(Operation *op, TensorDescType desc, RankedTensorType tensor)
{
    ...
    if (blockShape.size() > tensorShape.size()) {
        ...
        for (int i = 0; i < blockShape.size() - tensorShape.size(); ++i) {
            if (blockShape[i] != 1)
                return op->emitOpError("ranked reduce load only allowed for unit dimension leading dim.");
        }
        blockShape = blockShape.take_back(tensorShape.size());
    }

    if (blockShape == tensorShape && block.getElementType() == tensor.getElementType()) {
        return success();
    }
...
{
    return verifyDescriptorLoadStoreType(*this, getDesc().getType(), getSrc().getType());
}
...
// -- JoinOp --
LogicalResult JoinOp::inferReturnTypes(MLIRContext *context, std::optional<Location> location, ValueRange operands,
                                       DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
                                       SmallVectorImpl<Type> &inferredReturnTypes)
{
    ...
    inferredReturnTypes.push_back(RankedTensorType::get(retShape, srcTy.getElementType(), retEnc));
    return success();
}

// -- SplitOp --
LogicalResult SplitOp::inferReturnTypes(MLIRContext *context, std::optional<Location> location, ValueRange operands,
                                        DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
                                        SmallVectorImpl<Type> &inferredReturnTypes)
{
    // These should have been checked by tablegen-generated code.
    ...
    if (srcShape.empty() || srcShape.back() != 2) {
        return emitOptionalError(location, "last dimension of input tensor must be 2");
    }
...
void ElementwiseInlineAsmOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>> &effects)
{
    if (getPure())
        return;
    effects.emplace_back(MemoryEffects::Write::get(), SideEffects::DefaultResource::get());
    ...
    if (getNumOperands() >= 1) {
        ...
        if (numInputElems % this->getPackedElement() != 0) {
            return emitError("number of input elements ")
                   << numInputElems << " must be a multiple of the op's packed_element attribute, "
                   << getPackedElement();
        }
    }
    return success();
...
// -- ExternElementwiseOp --
void ExternElementwiseOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>> &effects)
{
    if (getPure())
        return;
    effects.emplace_back(MemoryEffects::Write::get(), SideEffects::DefaultResource::get());
...
LogicalResult ExperimentalTensormapCreateOp::verify()
{
    ...
    if (getGlobalDim().size() != rank) {
        return emitError("Rank mismatch for global dim. Got") << getGlobalDim().size() << " but expected " << rank;
    }
...
LogicalResult GatherOp::verify()
{
    ...
    for (int dim = 0; dim < indicesTy.getRank(); ++dim) {
        ...
        if (indicesTy.getShape()[dim] != srcTy.getShape()[dim]) {
            return emitOpError("indices dimension ") << dim << " must match the corresponding input dimension";
        }
    }
...
LogicalResult GatherOp::inferReturnTypes(MLIRContext *context, std::optional<Location> location, ValueRange operands,
                                         DictionaryAttr attributes, OpaqueProperties properties, RegionRange regions,
                                         SmallVectorImpl<Type> &inferredReturnTypes)
{
    ...
    inferredReturnTypes.push_back(
        RankedTensorType::get(indicesType.getShape(), srcType.getElementType(), indicesType.getEncoding()));
    return success();
}
