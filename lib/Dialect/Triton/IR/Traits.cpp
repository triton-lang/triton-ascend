...
static LogicalResult verifySameEncoding(Type typeA, Type typeB, bool allowTensorPointerType)
{
...
LogicalResult OpTrait::impl::verifySameOperandsEncoding(Operation *op, bool allowTensorPointerType)
{
...
LogicalResult OpTrait::impl::verifySameOperandsAndResultEncoding(Operation *op, bool allowTensorPointerType)
{
    if (op->getNumOperands() == 0)
        return success();

    if (failed(verifyAtLeastNOperands(op, 1)) || failed(verifyAtLeastNResults(op, 1)))
        return failure();

    auto type = op->getOperand(0).getType();
    for (auto resultType : op->getResultTypes())
        if (failed(verifySameEncoding(resultType, type, allowTensorPointerType)))
            return op->emitOpError() << "requires the same encoding for all operands and results";
        ...
        if (auto tensorType = dyn_cast<RankedTensorType>(opType)) {
            ...
            if (numElements > maxTensorNumElements)
                return op->emitError("Maximum allowed number of elements is ")
                       << maxTensorNumElements << ", but " << *op << " has more than that";
...
LogicalResult OpTrait::impl::verifyTensorLayouts(Operation *op)
{
        ...
        if (auto blocked = dyn_cast<ttg::BlockedEncodingAttr>(layout)) {
            ...
            if (layoutRank != rankedTy.getRank()) {
                return makeErr() << layout << ".\nLayout has rank " << layoutRank
                                 << ", but the tensor it's attached to has rank " << rankedTy.getRank() << ".";
            }
            ...
            if (layoutThreadsPerWarp != moduleThreadsPerWarp) {
                return makeErr() << layout << ".\nLayout has a total of " << layoutThreadsPerWarp
                                 << " threads per warp, but the module specifies " << moduleThreadsPerWarp
                                 << " threads per warp.";
            }
            ...
            if (layoutWarpsPerCTA != moduleWarpsPerCTA) {
                return makeErr() << layout << ".\nLayout has a total of " << layoutWarpsPerCTA
                                 << " warps per CTA, but the module specifies " << moduleWarpsPerCTA
                                 << " warps per CTA.";
            }

            if (blocked.getCTALayout().getCTAsPerCGA().size() > 0) {
                int moduleCTAsPerCGA = ttg::TritonGPUDialect::getNumCTAs(module);
                int64_t layoutCTAsPerCGA = product(blocked.getCTALayout().getCTAsPerCGA());
                if (layoutCTAsPerCGA != moduleCTAsPerCGA) {
                    return makeErr() << layout << ".\nLayout has a total of " << layoutCTAsPerCGA
                                     << " CTAs per CGA, but the module specifies " << moduleCTAsPerCGA
                                     << " CTAs per CGA.";
                }
            }
        }
    ...
    for (size_t i = 0; i < op->getNumOperands(); i++) {
        ...
        auto err = checkLayout(operand, [&]() {
            ...
            return op->emitError("Operand ") << i << " (" << operand << ") has an invalid layout: ";
        });
        if (!err.succeeded())
            return err;
    }
...
LogicalResult OpTrait::impl::verifySameLoadStoreOperandsAndResultShape(Operation *op)
{
    if (failed(verifyAtLeastNOperands(op, 1)) || failed(verifyAtLeastNResults(op, 1)))
        return failure();
    ...
    for (auto type : op->getResultTypes())
        if (failed(verifyCompatibleShape(getTypeShape(type), firstOperandShape)))
            return op->emitOpError() << "requires the same shape for all operands and results";
