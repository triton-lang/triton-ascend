...
Operation *cloneWithNewArgsAndResultTypes(PatternRewriter &rewriter, Operation *op, ValueRange newOperands,
                                          TypeRange newTypes)
{
...
struct MoveSplatAfterElementwisePattern : public OpTraitRewritePattern<OpTrait::Elementwise> {

    MoveSplatAfterElementwisePattern(MLIRContext *context) : OpTraitRewritePattern(context) {}
    ...
    void rewrite(Operation *op, PatternRewriter &rewriter) const override
    {
        ...
        for (unsigned iOp = 0; iOp < operands.size(); ++iOp) {
            ...
            } else if (matchPattern(definingOp, m_Constant(&constAttr)) && constAttr.isSplat()) {
                auto value = constAttr.getSplatValue<Attribute>();
                scalarOperands[iOp] = arith::ConstantOp::materialize(rewriter, value, constAttr.getElementType(), loc);
            } else {
        ...
        auto newOp = cloneWithNewArgsAndResultTypes(rewriter, op, scalarOperands, scalarResultTys);

        for (unsigned iRes = 0; iRes < resultTypes.size(); ++iRes) {
            auto newResult = rewriter.create<SplatOp>(loc, resultTypes[iRes], newOp->getResult(iRes));
...
struct MoveBroadcastAfterElementwisePattern : public OpTraitRewritePattern<OpTrait::Elementwise> {

    MoveBroadcastAfterElementwisePattern(MLIRContext *context) : OpTraitRewritePattern(context) {}
    ...
    LogicalResult match(Operation *op) const override
    {
        ...
        for (auto operand : operands) {
            ...
            auto getSrcShape = [](BroadcastOp b) { return b.getSrc().getType().getShape(); };
    ...
    void rewrite(Operation *op, PatternRewriter &rewriter) const override
    {
        ...
        for (auto operand : operands) {
            ...
            auto elemTy = dyn_cast<RankedTensorType>(operand.getType()).getElementType();
            ...
            DenseElementsAttr constAttr;
            if (matchPattern(definingOp, m_Constant(&constAttr)) && constAttr.isSplat()) {
                auto scalarValue = constAttr.getSplatValue<Attribute>();
                auto splatValue = SplatElementsAttr::get(newTy, scalarValue);
                auto newConstant = rewriter.create<arith::ConstantOp>(loc, newTy, splatValue);
        ...
        for (auto resultTy : resultTypes) {
            ...
            newResultTypes.push_back(RankedTensorType::get(srcShape, elemTy, srcEncoding));
        }

        // Create new op and broadcast results
        auto newOp = cloneWithNewArgsAndResultTypes(rewriter, op, newOperands, newResultTypes);
        for (unsigned iRes = 0; iRes < newResultTypes.size(); ++iRes) {
            auto newResult = rewriter.create<BroadcastOp>(loc, resultTypes[iRes], newOp->getResult(iRes));
            rewriter.replaceAllUsesWith(op->getResult(iRes), newResult);
        }
    }
};

template <typename OpType> class CanonicalizePattern : public OpRewritePattern<OpType> {
  public:
    explicit CanonicalizePattern(MLIRContext *context) : OpRewritePattern<OpType>(context) {}
...
class ReorderBroadcastPass : public ::impl::TritonReorderBroadcastBase<ReorderBroadcastPass> {
  public:
