...
void assertIsRegisterLayout(const LinearLayout &layout)
{
    ...
    assert(llvm::SmallDenseSet<StringAttr>(outs.begin(), outs.end()) ==
           llvm::SmallDenseSet<StringAttr>(expectedOuts.begin(), expectedOuts.end()));
}
...
LinearLayout identityND(StringAttr inDimName, ArrayRef<unsigned> shape, ArrayRef<unsigned> order,
                                       ...
                                       const llvm::SmallDenseMap<StringAttr, int64_t> &shape)
{
    ...
    for (auto outDim : llvm::enumerate(layout.getOutDimNames())) {
        ...
        llvm::sort(sortedBases, [](auto a, auto b) { return std::get<2>(a) > std::get<2>(b); });
    ...
    return LinearLayout(std::move(bases), llvm::to_vector(layout.getOutDimNames()));
}
...
LinearLayout combineCtaCgaWithShape(LinearLayout ctaLayout, CTALayoutAttr cgaLayoutAttr, ArrayRef<int64_t> shape)
{
    int rank = shape.size();
    ...
    LinearLayout cgaLayout = ensureLayoutNotLargerThan(makeCgaLayout(cgaLayoutAttr), labeledShape)
    ...
    assert(llvm::to_vector(ctaLayout.getOutDimNames()) == llvm::to_vector(cgaLayout.getOutDimNames()));
    for (auto dim : ctaLayout.getOutDimNames()) {
        ctaShape[dim] = std::max(int64_t {1}, labeledShape[dim] / cgaLayout.getOutDimSize(dim));
    }
...
LinearLayout ampereMmaToLinearLayout(ArrayRef<int64_t> shape, NvidiaMmaEncodingAttr mma)
{
    int rank = shape.size();
    ...
    LinearLayout ctaLayout({{S("register"), {{1, 0}, {0, 8}}}, {S("lane"), {{2, 0}, {4, 0}, {0, 1}, {0, 2}, {0, 4}}}},
                           llvm::to_vector(llvm::reverse(ArrayRef(dimNames).take_back(2))));

    ctaLayout *= identityND(S("warp"), mma.getWarpsPerCTA(), llvm::to_vector(llvm::reverse(llvm::seq<unsigned>(rank))),
                            dimNames);

    return combineCtaCgaWithShape(ctaLayout, mma.getCTALayout(), shape);
}

LinearLayout hopperMmaToLinearLayout(ArrayRef<int64_t> shape, NvidiaMmaEncodingAttr mma)
{
    int rank = shape.size();
    assert(mma.isHopper());
    assert(rank == 2);
    ...
    ctaLayout *= LinearLayout::identity1D(n / ctaLayout.getOutDimSize(S("dim1")), S("register"), S("dim1"));

    // Expand the `warp` dimension according to warpsPerCTA.
    //
    // It's weird that this is order [0,1] when MMAv2's warpsPerCTA is [1,0], but
    // this really does seem to be correct.
    ctaLayout *= identityND(S("warp"), mma.getWarpsPerCTA(), /*order=*/ {0, 1}, {S("dim0"), S("dim1")})
                     .transposeOuts(llvm::to_vector(ctaLayout.getOutDimNames()));

    return combineCtaCgaWithShape(ctaLayout, mma.getCTALayout(), shape);
}

LinearLayout sharedToLinearLayoutNoLeadingOffset(ArrayRef<int64_t> shape, SharedEncodingAttr shared)
{
    ...
    int rank = shape.size();
    if (rank == 1) {
        return combineCtaCgaWithShape(LinearLayout::identity1D(shape[0], S("offset"), S("dim0")), shared.getCTALayout(),
                                      shape);
    }
    ...
    LinearLayout ctaLayout = LinearLayout({{S("offset"), bases2D}}, {rowDimName, colDimName});

    // Add the remaining dimensions.
    for (int i = 2; i < rank; i++) {
        int dim = shared.getOrder()[i];
        ctaLayout *= LinearLayout::identity1D(shape[dim], S("offset"), outDimNames[dim]);
    }

    return combineCtaCgaWithShape(ctaLayout, shared.getCTALayout(), shape);
}

LinearLayout sharedToLinearLayoutLeadingOffset(ArrayRef<int64_t> shape, SharedEncodingAttr shared, int32_t elemBitWidth)
{
    assert(shared.getHasLeadingOffset());
    ...
    if (shared.getPerPhase() == 4 && shared.getMaxPhase() == 2) {
        ...
        llvm::errs() << "Illegal shared encoding.  If hasLeadingOffset is true, "
    ...
    if (shape[colDim] < tileCols || shape[rowDim] < tileRows) {
        llvm::errs() << "Illegal shared layout; expected shape to be at least [" << tileRows << ", " << tileCols
                     << "], shape: [" << shape[rowDim] << ", " << shape[colDim] << "]\n";
        llvm::report_fatal_error("Illegal shared layout");
    }

    int vec = 8 * 16 / elemBitWidth;
    if (vec != shared.getVec()) {
        llvm::errs() << "Illegal shared layout; expected `vec` to be " << vec << ": " << shared << "\n";
        llvm::report_fatal_error("Illegal shared layout");
    }
    ...
    LinearLayout tileLayout = LinearLayout({{S("offset"), bases2D}}, {rowDimName, colDimName});

    // Add the remaining dimensions.
    for (int i = 2; i < rank; i++) {
        int dim = shared.getOrder()[i];
        tileLayout *= LinearLayout::identity1D(shape[dim], S("offset"), outDimNames[dim]);
    }
...
std::optional<LinearLayout> AMDMfmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const
{
    int rank = shape.size();
    ...
    assert(((getMDim() == 32 && getNDim() == 32) || (getMDim() == 16 && getNDim() == 16)) && "Unsupported mfma type");
    ...
    if (getMDim() == 32) {
        ...
        tileLayout = LinearLayout({{kRegister, {{0, 1}, {0, 2}, {0, 8}, /*gap*/ {0, 16}}},
        ...
        if (getIsTransposed())
            tileLayout = LinearLayout({{kRegister, {{1, 0}, {2, 0}, {8, 0}, /*gap*/ {16, 0}}},
        ...
        tileLayout = LinearLayout(
            {{kRegister, {{0, 1}, {0, 2}}}, {kLane, {{1, 0}, {2, 0}, {4, 0}, {8, 0}, /*gap*/ {0, 4}, {0, 8}}}},
            {outDimNames[order[0]], outDimNames[order[1]]});
        // For mfma.transposed layout, the element ownership among threads are
        // "transposed" within each warp.
        if (getIsTransposed())
            tileLayout = LinearLayout(
                {{kRegister, {{1, 0}, {2, 0}}}, {kLane, {{0, 1}, {0, 2}, {0, 4}, {0, 8}, /*gap*/ {4, 0}, {8, 0}}}},
                {outDimNames[order[0]], outDimNames[order[1]]});
    }
    if (hasBatchDim) {
        assert(order[2] == 0);
    ...
    LinearLayout warpLayout = identityND(S("warp"), getWarpsPerCTA(), order, outDimNames);
    LinearLayout ctaLayout = tileLayout * warpLayout;

    return combineCtaCgaWithShape(ctaLayout, getCTALayout(), shape);
}

std::optional<LinearLayout> dotOperandMfmaToLinearLayout(DotOperandEncodingAttr dotMfmaLayout, ArrayRef<int64_t> shape)
{
    ...
    LinearLayout warpLayout = identityND(kWarp, warpsPerCTA, warpOrder, outDimNames);

    LinearLayout ctaLayout = tileLayout.transposeOuts(outDimNames) * warpLayout.transposeOuts(outDimNames);

    return combineCtaCgaWithShape(ctaLayout, mfmaLayout.getCTALayout(), shape);
}

std::optional<LinearLayout> AMDWmmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const
{
    int rank = shape.size();
    assert(rank == getWarpsPerCTA().size());
    ...
    assert(((shape[mIndex] == 1 || shape[mIndex] >= mDim) && (shape[nIndex] == 1 || shape[nIndex] >= nDim)) &&
    ...
    assert(ver == 1 || ver == 2);
    LinearLayout tileLayout = ver == 1 ? LinearLayout({{kRegister, {/*gap*/ {0, 2}, {0, 4}, {0, 8}}},
...
std::optional<LinearLayout> BlockedEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const
{
    ...
    LinearLayout ctaLayout = identityND(S("register"), getSizePerThread(), order, outDimNames) *
...
std::optional<LinearLayout> NvidiaMmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const
{
    if (isAmpere()) {
...
std::optional<LinearLayout> SliceEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const
{
    MLIRContext *ctx = getContext();
    ...
    std::optional<LinearLayout> parentLL = triton::gpu::toLinearLayout(parentShape, getParent());
    ...
    for (auto [idx, outDim] : llvm::enumerate(parentLL->getOutDimNames())) {
        if (idx == getDim()) {
            ...
            transform *= LinearLayout::zeros1D(parentLL->getOutDimSize(outDim), outDim, outDimNames[0]);
        } else {
            transform *= LinearLayout::identity1D(parentLL->getOutDimSize(outDim), outDim,
    ...
    LinearLayout ret = LinearLayout(std::move(bases), llvm::to_vector(sliceLL.getOutDimNames()));
    ...
    int expectedNumRegisters = triton::gpu::getTotalElemsPerThread(
        RankedTensorType::get(shape, IntegerType::get(ctx, 32) /*dummy type*/, *this));
...
LinearLayout ampereDotToLinearLayout(ArrayRef<int64_t> shape, DotOperandEncodingAttr dot)
{
...
std::optional<LinearLayout> DotOperandEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const
{
                                                   ...
                                                   ArrayRef<unsigned> repShape, ArrayRef<unsigned> order)
{
    ...
    for (int i = 0; i < tensorShape.size(); i++) {
        ...
        layout *= LinearLayout::identity1D(repShape[dim], kOffset, outDimNames[dim]);
    ...
    // [offset, rep, block]
    return ret.reshapeIns({{kOffset, totalOffsets}, {kIteration, totalIters}, {kBlock, 1}});
}
...
bool canUseStMatrix(RankedTensorType tensorTy, ArrayRef<unsigned> repShape, ArrayRef<unsigned> paddedRepShape,
                    ArrayRef<unsigned> order, int swizzleByteSize)
{
    auto mmaLayout = mlir::dyn_cast<NvidiaMmaEncodingAttr>(tensorTy.getEncoding());
    ...
    if (tensorShapePerCTA.size() != 2)
        return false;
    auto numIterations =
        ceil<unsigned>(tensorShapePerCTA[1], repShape[1]) * ceil<unsigned>(tensorShapePerCTA[0], repShape[0]);
    if (numIterations > 1)
        return false;
    if (paddedRepShape[1] % 8 != 0)
        return false;
    if (swizzleByteSize != 0 && swizzleByteSize != 32 && swizzleByteSize != 64 && swizzleByteSize != 128)
        return false;
    return true;
}

std::optional<LinearLayout> chooseStMatrixLayoutLeadingOffset(MLIRContext *ctx, RankedTensorType tensorTy,
                                                              ...
                                                              ArrayRef<unsigned> order, int swizzleByteSize)
{
    ...
    LinearLayout layout = LinearLayout({{kReg, basesReg}, {kLane, basesLane}}, {kCol, kRow});

    // Expand the `warp` dimension according to warpsPerCTA.
    auto mma = cast<NvidiaMmaEncodingAttr>(tensorTy.getEncoding());
    layout *= identityND(kWarp, mma.getWarpsPerCTA(), /*order=*/ {0, 1}, {kRow, kCol})
    ...
    auto ret = combineCtaCgaWithShape(layout, mma.getCTALayout(), tensorTy.getShape());
    return ret.transposeOuts(llvm::to_vector(layout.getOutDimNames()))
        .reshapeOuts({{kOffset, ret.getTotalOutDimSize()}, {S("iteration"), 1}});
}

std::optional<LinearLayout> chooseStMatrixLayoutNoLeadingOffset(MLIRContext *ctx, RankedTensorType tensorTy,
                                                                ArrayRef<unsigned> repShape,
                                                                ArrayRef<unsigned> paddedRepShape,
                                                                ArrayRef<unsigned> order)
{
    ...
    std::vector<std::vector<int>> basesReg = {{1, 0}, {2, 0}, {4, 0}};
    std::vector<std::vector<int>> basesLane = {{0, 1}, {0, 2}, {0, 4}, {0, 8}, {8, 0}};
    LinearLayout layout = LinearLayout({{kReg, basesReg}, {kLane, basesLane}}, {kCol, kRow});

    // Expand the `register` dimension so the size of columns matches `n`.
    auto mma = cast<NvidiaMmaEncodingAttr>(tensorTy.getEncoding());
    int n = mma.getInstrShape()[1];
    layout *= LinearLayout::identity1D(n / layout.getOutDimSize(kCol), kReg, kCol);
    ...
    return ret.transposeOuts(llvm::to_vector(layout.getOutDimNames()))
               .reshapeOuts({{S("offset"), ret.getTotalOutDimSize()}, {S("iteration"), 1}}) *
                                                 ...
                                                 ArrayRef<unsigned> order, int swizzleByteSize)
{
    ...
    if (swizzleByteSize == 0)
        return chooseStMatrixLayoutNoLeadingOffset(ctx, tensorTy, repShape, paddedRepShape, order);
    else
        return chooseStMatrixLayoutLeadingOffset(ctx, tensorTy, repShape, paddedRepShape, order, swizzleByteSize);
}
