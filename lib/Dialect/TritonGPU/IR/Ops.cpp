...
LogicalResult UpcastMXFPOp::verify()
{
    ...
    if (xShape.size() != scaleShape.size() || xShape.size() < 2) {
        return emitOpError("operands must have the same number of dimensions, at least 2");
    }

    if (!(fpType == F8F6F4Type::E2M1 || fpType == F8F6F4Type::E4M3 || fpType == F8F6F4Type::E5M2)) {
    ...
    if (!std::equal(xShape.begin(), xShape.end() - 1, scaleShape.begin())) {
        return emitOpError("all dimensions except the last must match between operands");
    }
...
LogicalResult UpcastMXFPOp::inferReturnTypes(MLIRContext *ctx, std::optional<Location> loc, ValueRange operands,
                                             ...
                                             RegionRange regions, SmallVectorImpl<Type> &inferredReturnTypes)
{
    ...
    if (typeEncoded == F8F6F4Type::E2M1) {
        ...
        auto newVEncoding = DotOperandEncodingAttr::get(ctx, oldEncoding.getOpIdx(), oldEncoding.getParent(),
                                                        oldEncoding.getKWidth() * 2);
        auto newShape = SmallVector<int64_t>(xShape);
        newShape.back() *= 2;
        inferredReturnTypes.push_back(RankedTensorType::get(newShape, FloatType::getBF16(ctx), newVEncoding));
    } else {
