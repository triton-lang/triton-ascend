...
SmallVector<unsigned> warpsPerTileV2(DotOp dotOp, const ArrayRef<int64_t> shape, int numWarps)
{
    ...
    auto filter = [&dotOp](Operation *op) {
        return op->getParentRegion() == dotOp->getParentRegion() && !isa<TransOp>(op);
    };
    ...
    for (Operation *op : slices) {
        if (isa<DotOp>(op) && (op != dotOp)) {
            ...
            if (auto mmaEncoding = dyn_cast<NvidiaMmaEncodingAttr>(resTy.getEncoding())) {
...
SmallVector<unsigned, 2> warpsPerTileV3(DotOp dotOp, const ArrayRef<int64_t> shape, int numWarps,
                                        const SmallVector<unsigned, 3> &instrShape)
{
    ...
    // the same warp.
    if (llvm::find_if(slices, [](Operation *op) { return op->hasTrait<OpTrait::DotLike>(); }) != slices.end())
        return {(unsigned)numWarps, 1};
...
// given value.
static Value getSharedMemoryMMAOperand(Value v, mlir::PatternRewriter &rewriter, int opIdx, bool allowTranspose)
{
    ...
    Attribute SharedMemorySpace = SharedMemorySpaceAttr::get(argType.getContext());
    auto CTALayout = getCTALayout(argType.getEncoding());
    auto newLayout = SharedEncodingAttr::get(argType.getContext(), argType.getShape(), newOrder, CTALayout,
                                             argType.getElementType());
    auto newType = MemDescType::get(argType.getShape(), argType.getElementType(), newLayout, SharedMemorySpace);
...
class BlockedToMMA : public mlir::OpRewritePattern<DotOp> {
    ...
    static bool bwdFilter(Operation *op)
    {
        return op->getNumOperands() == 1 &&
               (isa<FpToFpOp, BitcastOp, ConvertLayoutOp>(op) || isPureUnaryInlineAsm(op) ||
                op->getDialect()->getTypeID() == mlir::TypeID::get<arith::ArithDialect>());
    }
  ...
  public:
    ...
    static SmallVector<unsigned, 3> getWarpsPerTile(DotOp dotOp, const ArrayRef<int64_t> shape, int version,
    ...
    mlir::LogicalResult matchAndRewrite(triton::DotOp dotOp, mlir::PatternRewriter &rewriter) const override
    {
        if (computeCapability < 70)
            return failure();
        // TODO: Check data-types and SM compatibility
        RankedTensorType oldRetType = dotOp.getType();
        if (!oldRetType.getEncoding() || mlir::isa<NvidiaMmaEncodingAttr>(oldRetType.getEncoding()))
            return failure();
        ...
        auto instrShape =
            mmaVersionToInstrShape(versionMajor, retShapePerCTA, dotOp.getA().getType().getElementType(), numWarps);
        // operands
        Value a = dotOp.getA();
        Value b = dotOp.getB();
        ...
        if (versionMajor == 1) {
            ...
            auto getCvtArgOrder = [](Operation *op) {
                return mlir::cast<BlockedEncodingAttr>(cast<ConvertLayoutOp>(op).getSrc().getType().getEncoding())
                    .getOrder();
            };
            bool isARow = true;
            bool isBRow = true;
            ...
            mmaEnc = NvidiaMmaEncodingAttr::get(oldRetType.getContext(), versionMajor, numWarps, CTALayout, instrShape,
            ...
            auto warpsPerTile = getWarpsPerTile(dotOp, retShapePerCTA, versionMajor, numWarps, instrShape);
            mmaEnc = NvidiaMmaEncodingAttr::get(oldRetType.getContext(), versionMajor, versionMinor, warpsPerTile,
                                                CTALayout, instrShape);
        }
        PatternRewriterWithAsyncTaskIds taskIdRewriter(rewriter, dotOp);
        auto newRetType = RankedTensorType::get(oldRetType.getShape(), oldRetType.getElementType(), mmaEnc);
        // convert accumulator
        ...
        if (versionMajor == 3) {
            ...
            newDot = taskIdRewriter.create<triton::nvidia_gpu::WarpGroupDotOp>(dotOp.getLoc(), newRetType, a, b, newAcc,
                                                                               nullptr, dotOp.getInputPrecision(),
            ...
            // convert A operand
            auto newAEncoding = DotOperandEncodingAttr::get(oldAType.getContext(), 0, newRetType.getEncoding(),
                                                            minBitwidth > 0 ? minType : oldAType.getElementType());
            auto newAType = RankedTensorType::get(oldAType.getShape(), oldAType.getElementType(), newAEncoding);
            a = rewriter.create<ConvertLayoutOp>(a.getLoc(), newAType, a);
            // convert B operand
            auto newBEncoding = DotOperandEncodingAttr::get(oldBType.getContext(), 1, newRetType.getEncoding(),
                                                            minBitwidth > 0 ? minType : oldBType.getElementType());
            auto newBType = RankedTensorType::get(oldBType.getShape(), oldBType.getElementType(), newBEncoding);
            b = rewriter.create<ConvertLayoutOp>(b.getLoc(), newBType, b);
            newDot = taskIdRewriter.create<DotOp>(dotOp.getLoc(), newRetType, a, b, newAcc, dotOp.getInputPrecision(),
                                                  dotOp.getMaxNumImpreciseAcc());
        }
        // convert dot instruction
        rewriter.replaceOpWithNewOp<ConvertLayoutOp>(dotOp, oldRetType, newDot->getResult(0));
        return success();
    }
};
} // namespace
...
static void decomposeMixedModeDotOp(ModuleOp mod, int computeCapability)
{
    mod.walk([=](DotOp dotOp) -> void {
        ...
        Type promoteType;
        NvidiaMmaEncodingAttr mmaLayout = dyn_cast<NvidiaMmaEncodingAttr>(D.getType().getEncoding());
        if (mmaLayout) {
            bool isNativeFP8 = AElType.isFloat8E5M2() || AElType.isFloat8E4M3FN();
            // promote operands for sm < 89 since fp8 mma is not natively supported
            // promote operands for sm >= 90 when mma is not v3
            if (!isNativeFP8 || (isNativeFP8 && (computeCapability == 89 || mmaLayout.isHopper())))
                return;
...
class ScaledBlockedToMMAv2 : public mlir::OpRewritePattern<triton::DotScaledOp> {
    int computeCapability;

  public:
    ScaledBlockedToMMAv2(mlir::MLIRContext *context, int computeCapability)
        : mlir::OpRewritePattern<triton::DotScaledOp>(context), computeCapability(computeCapability)
    {
    }

    mlir::LogicalResult matchAndRewrite(triton::DotScaledOp dotOp, mlir::PatternRewriter &rewriter) const override
    {
        ...
        assert((aType == F8F6F4Type::E4M3 || aType == F8F6F4Type::E5M2 || aType == F8F6F4Type::E2M1) &&
               "NYI: lhs supports fp4 or fp8");
        assert(bType == F8F6F4Type::E4M3 || bType == F8F6F4Type::E5M2 && "NYI: rhs supports fp8");
        ...
        auto instrShape = mmaVersionToInstrShape(versionMajor, retShapePerCTA, rewriter.getBF16Type(), numWarps);
        auto CTALayout = getCTALayout(oldRetType.getEncoding());
        ...
        auto mmaEnc = NvidiaMmaEncodingAttr::get(ctx, /*versionMajor=*/versionMajor,
                                                 /*versionMinor=*/0, warpsPerCTA, CTALayout, instrShape);
        auto newRetType = RankedTensorType::get(oldRetType.getShape(), oldRetType.getElementType(), mmaEnc);
        ...
        auto toMMABf16 = [&newRetType, &rewriter, &ctx, &enumToType](TypedValue<RankedTensorType> v, int idx,
            ...
            if (type == F8F6F4Type::E2M1) {
                ...
                auto newVEncoding = DotOperandEncodingAttr::get(ctx, idx, newRetType.getEncoding(), /*kWidth=*/4);
                auto newVType = RankedTensorType::get(vType.getShape(), vType.getElementType(), newVEncoding);
                return rewriter.create<ConvertLayoutOp>(v.getLoc(), newVType, v);
            } else {
                assert(type == F8F6F4Type::E5M2 || type == F8F6F4Type::E4M3);
                auto newVEncoding = DotOperandEncodingAttr::get(ctx, idx, newRetType.getEncoding(), /*kWidth=*/8);
                auto newVType = RankedTensorType::get(vType.getShape(), vType.getElementType(), newVEncoding);
                v = rewriter.create<ConvertLayoutOp>(v.getLoc(), newVType, v);

                // Bitcast
                auto vTypeFp8 = RankedTensorType::get(vType.getShape(), enumToType(type), newVEncoding);
                v = cast<TypedValue<RankedTensorType>>(rewriter.create<BitcastOp>(v.getLoc(), vTypeFp8, v).getResult());

                // Convert to bf16
                auto vTypeBf16 = RankedTensorType::get(vType.getShape(), rewriter.getBF16Type(), newVEncoding);
        ...
        assert(instrShape == ArrayRef<unsigned>({16, 8}) || instrShape == ArrayRef<unsigned>({1, 16, 8}));
        ...
        auto threadsPerWarp = SmallVector<unsigned> {shapeTileA[0], 32 / shapeTileA[0]};

        auto newScaleEncoding =
            triton::gpu::BlockedEncodingAttr::get(ctx, {1, 1}, threadsPerWarp, warpsPerCTA, {1, 0}, CTALayout);

        auto newScaleType =
            RankedTensorType::get(scale.getType().getShape(), scale.getType().getElementType(), newScaleEncoding);
        scale = rewriter.create<ConvertLayoutOp>(scale.getLoc(), newScaleType, scale);

        auto scaledA = rewriter.create<triton::gpu::UpcastMXFPOp>(dotOp.getLoc(), a, scale, dotOp.getLhsType());

        // convert dot instruction
        auto newDot = rewriter.create<DotOp>(dotOp.getLoc(), newRetType, scaledA, b, newAcc);
...
class TritonGPUAccelerateMatmulPass : public impl::TritonGPUAccelerateMatmulBase<TritonGPUAccelerateMatmulPass> {
  public:
    using impl::TritonGPUAccelerateMatmulBase<TritonGPUAccelerateMatmulPass>::TritonGPUAccelerateMatmulBase;
    ...
    void runOnOperation() override
    {
        ...
        patterns.add<BlockedToMMA, ScaledBlockedToMMAv2>(context, computeCapability);
