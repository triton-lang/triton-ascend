...
// rules.
static bool canMergeIntoIf(arith::SelectOp selectOp, scf::IfOp ifOp, DominanceInfo &dom)
{
...
class CombineTensorSelectAndIfPass : public impl::TritonGPUCombineTensorSelectAndIfBase<CombineTensorSelectAndIfPass> {
  public:
    void runOnOperation() override
    {
        ...
        m.walk([&](arith::SelectOp selectOp) {
            ...
            SetVector<Operation *> conditionUsers(condition.getUsers().begin(), condition.getUsers().end());
        ...
        for (auto [ifOp, selectOps] : selectToIf) {
            ...
            SmallVector<Type> newResultTypes = {ifOp.getResultTypes().begin(), ifOp.getResultTypes().end()};
            for (arith::SelectOp selectOp : selectOps) {
                newResultTypes.push_back(selectOp.getResult().getType());
            }
            auto newIfOp = builder.create<scf::IfOp>(loc, newResultTypes, ifOp.getCondition(), /*hasElse*/ true);
