...
std::optional<std::pair<Operation *, int>> findZeroInitOp(Value accUse, Operation *accDef, scf::ForOp forOp,
                                                          bool &loopArgIsZero)
{
    Value v = accUse;
    ...
    if (auto selOp = dyn_cast<arith::SelectOp>(defOp)) {
        if (isConstantZeroTensor(selOp.getTrueValue()) || isConstantZeroTensor(selOp.getFalseValue())) {
    ...
    if (auto ifOp = dyn_cast<scf::IfOp>(defOp)) {
        ...
        if (isConstantZeroTensor(thenVal) || isConstantZeroTensor(elseVal)) {
            ...
            // passed from outside
            if (thenVal.getParentBlock()->getParentOp() == ifOp || elseVal.getParentBlock()->getParentOp() == ifOp) {
                return std::nullopt;
            }
...
class OptimizeAccumulatorInitPass : public impl::TritonGPUOptimizeAccumulatorInitBase<OptimizeAccumulatorInitPass> {
  public:
    void runOnOperation() override
    {
        ...
        for (Operation *mmaOp : mmaOps) {
            ...
            Value vTrue = rewriter.create<arith::ConstantOp>(loc, rewriter.getBoolAttr(true));
            Value vFalse = rewriter.create<arith::ConstantOp>(loc, rewriter.getBoolAttr(false));
            ...
            scf::ForOp newForOp = replaceForOpWithNewSignature(rewriter, forOp, {loopArgFlagValue});
            forOp.erase();
            forOp = newForOp;
            loopArgFlagValue = forOp.getRegionIterArg(forOp.getNumRegionIterArgs() - 1);
            ...
            if (auto selOp = dyn_cast<arith::SelectOp>(zeroInitOp->first)) {
                ...
                oldValue = isConstantZeroTensor(selOp.getTrueValue()) ? selOp.getFalseValue() : selOp.getTrueValue();
                zeroValue = isConstantZeroTensor(selOp.getTrueValue()) ? selOp.getTrueValue() : selOp.getFalseValue();
            ...
            auto selectFlagOp = rewriter.create<arith::SelectOp>(
                loc, condition, thenInitsToZero ? vFalse : prevFlagValue, thenInitsToZero ? prevFlagValue : vFalse);
            setUseAccFlag(mmaOp, zeroingBeforeMMA ? selectFlagOp : loopArgFlagValue);
            auto forYield = cast<scf::YieldOp>(forOp.getBody()->getTerminator());
            forYield->insertOperands(forYield->getNumOperands(), {zeroingBeforeMMA ? vTrue : selectFlagOp});
            ...
            if (auto selOp = dyn_cast<arith::SelectOp>(zeroInitOp->first)) {
                ...
                auto zeroingYield = thenInitsToZero ? ifOp.thenYield() : ifOp.elseYield();
