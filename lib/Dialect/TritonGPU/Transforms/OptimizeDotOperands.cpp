...
class SwizzleShmemConvert : public OpRewritePattern<ConvertLayoutOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ConvertLayoutOp cvtOp, PatternRewriter &rewriter) const override
    {
        ...
        auto cvtEncoding = dyn_cast<DotOperandEncodingAttr>(sharedLoadTy.getEncoding());
        if (!cvtEncoding)
            return failure();
        ...
        // code.
        auto newInnerCvtEnc = SharedEncodingAttr::get(getContext(), cvtEncoding, srcTy.getShape(),
        ...
        auto alloc = rewriter.create<LocalAllocOp>(
            trans.getLoc(),
            MemDescType::get(srcTy.getShape(), srcTy.getElementType(), newInnerCvtEnc, sharedMemorySpace),
            trans.getSrc());
        auto newTrans = rewriter.create<TransOp>(trans.getLoc(), alloc, ArrayRef<int32_t>({1, 0}));
...
class HoistLayoutConversion : public OpRewritePattern<ConvertLayoutOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ConvertLayoutOp cvt, PatternRewriter &rewriter) const override
    {
        ...
        if (!all_of(src->getOperandTypes(), [](Type ty) { return isa<RankedTensorType>(ty); }))
            return failure();
        ...
        opt.filter = [&](Operation *op) { return op->getParentRegion() == cvt->getParentRegion(); };
        ...
        for (Operation *currOp : slice) {
            if (isa<LoadOp>(currOp)) {
                ...
                // Bitcast, or Arith.
                if (!isa<FpToFpOp, BitcastOp>(currOp) && !isPureUnaryInlineAsm(currOp) &&
                    currOp->getDialect()->getTypeID() != TypeID::get<arith::ArithDialect>())
                    return failure();
            }
        }
        if (!foundLoad)
        ...
        for (auto operand : src->getOperands()) {
            ...
            Type newCvtTy = RankedTensorType::get(srcTy.getShape(), operandTy.getElementType(), cvtTy.getEncoding());
            newOperands.push_back(rewriter.create<ConvertLayoutOp>(cvt.getLoc(), newCvtTy, operand));
        }
        auto newRet = rewriter.clone(*src);
        for (int i = 0; i < newOperands.size(); i++)
            newRet->setOperand(i, newOperands[i]);
        newRet->getResult(0).setType(
            RankedTensorType::get(srcTy.getShape(), srcTy.getElementType(), cvtTy.getEncoding()));
...
class FuseTransHopper : public OpRewritePattern<LocalAllocOp> {
  public:
    ...
    LogicalResult matchAndRewrite(LocalAllocOp allocOp, PatternRewriter &rewriter) const override
    {
        if (!allocOp->hasOneUse() || !allocOp->getUsers().begin()->hasTrait<OpTrait::DotLike>())
            return failure();
        ...
        // be used here. For tests where numCTAs = 1, this is not a problem since
        // all CTALayouts are the same.
        auto newInnerEnc = SharedEncodingAttr::get(getContext(), srcTy.getShape(), newInnerCvtOrder,
                                                   allocEncoding.getCTALayout(), srcTy.getElementType());

        MemDescType innerTy =
            MemDescType::get(srcTy.getShape(), srcTy.getElementType(), newInnerEnc, allocType.getMemorySpace());
        auto newAlloc = rewriter.create<LocalAllocOp>(allocOp.getLoc(), innerTy, trans.getSrc());
        rewriter.replaceOpWithNewOp<TransOp>(allocOp, newAlloc, ArrayRef<int32_t>({1, 0}));
        return success();
    }
};
...
struct MMAV3UseRegOperand : public OpRewritePattern<triton::nvidia_gpu::WarpGroupDotOp> {
    using OpRewritePattern::OpRewritePattern;

    LogicalResult matchAndRewrite(triton::nvidia_gpu::WarpGroupDotOp dotOp, PatternRewriter &rewriter) const override
    {
        auto alloc = dotOp.getOperand(0).getDefiningOp<LocalAllocOp>();
        if (!alloc || !alloc.getSrc())
            return failure();

        auto getEncoding = [](Value v) { return cast<TensorOrMemDesc>(v.getType()).getEncoding(); };
        ...
        if (!srcEnc || srcEnc.getVersionMajor() != 3 || !dstEnc || dstEnc.getVersionMajor() != 3)
            return failure();
        auto srcTy = cast<RankedTensorType>(alloc.getSrc().getType());
        auto dotOperandEnc = DotOperandEncodingAttr::get(dotOp.getContext(), /*opIdx=*/0, srcEnc, /*kWidth=*/0);
        auto newTy = RankedTensorType::get(srcTy.getShape(), srcTy.getElementType(), dotOperandEnc);
        if (!isMmaToDotShortcut(srcTy, newTy))
            return failure();

        Value newOperand = rewriter.create<ConvertLayoutOp>(dotOp.getLoc(), newTy, alloc.getSrc());
...
class TritonGPUOptimizeDotOperandsPass
    : public impl::TritonGPUOptimizeDotOperandsBase<TritonGPUOptimizeDotOperandsPass> {
  public:
    using impl::TritonGPUOptimizeDotOperandsBase<TritonGPUOptimizeDotOperandsPass>::TritonGPUOptimizeDotOperandsBase;
