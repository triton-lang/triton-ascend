...
// the reduction.
struct OptimizeReshapeLayoutPattern : public mlir::OpRewritePattern<triton::ReshapeOp> {
    OptimizeReshapeLayoutPattern(mlir::MLIRContext *context) : OpRewritePattern<triton::ReshapeOp>(context, 1) {}

    mlir::LogicalResult matchAndRewrite(triton::ReshapeOp viewOp, mlir::PatternRewriter &rewriter) const override
    {
        ...
        if (auto blocked = mlir::dyn_cast<triton::gpu::BlockedEncodingAttr>(tensorType.getEncoding())) {
            // If the layout already has all the elements along the reduction
            // dimension in the same thread we can skip.
            if (blocked.getThreadsPerWarp()[*reductionAxis] == 1 && blocked.getWarpsPerCTA()[*reductionAxis] == 1 &&
        ...
        triton::gpu::BlockedEncodingAttr encoding = triton::gpu::BlockedEncodingAttr::get(
            viewOp.getContext(), shape, sizePerThread, order, numWarps, threadsPerWarp, numCTAs);
        if (encoding == tensorType.getEncoding())
            return failure();
        RankedTensorType newType = RankedTensorType::get(shape, tensorType.getElementType(), encoding);
        ...
        auto cvt = rewriter.create<mlir::triton::gpu::ConvertLayoutOp>(viewOp.getLoc(), tensorType, viewOp.getResult());
...
class TritonGPUOptimizeThreadLocalityPass
    : public impl::TritonGPUOptimizeThreadLocalityBase<TritonGPUOptimizeThreadLocalityPass> {
    void runOnOperation() override
    {
        ...
        if (mlir::applyPatternsAndFoldGreedily(mod, std::move(viewLayoutPatterns)).failed()) {
            signalPassFailure();
        }
        ...
        mod.walk([&](triton::ReduceOp reduce) -> void {
            ...
            if (!reductionOp || !isa<arith::AddFOp, arith::MulFOp, arith::MaximumFOp, arith::MaxNumFOp,
                                     arith::MinimumFOp, arith::MinNumFOp>(reductionOp.value()))
                return;
            ...
            auto elemsPerThread = triton::gpu::getElemsPerThread(srcType)[reduce.getAxis()];
        ...
        for (auto reduce : reduceOps) {
            ...
            auto viewOpTensorType = RankedTensorType::get(viewOpTensorShape, srcType.getElementType(), blocked3d);
            auto slice2d = triton::gpu::SliceEncodingAttr::get(mod.getContext(), rank, blocked3d);
            // Get forOp
            ...
            // get oldAccum
            auto oldAccum = forOp.getInitArgs()[blockArgNum - forOp.getNumInductionVars()];
            // get old loop user
            Value loopResult = forOp.getResult(blockArgNum - forOp.getNumInductionVars());
            ...
            auto newAccum = createAccum(builder, reduce, oldAccum, viewOpTensorShape, slice2d);
            // create new loop by copying the old for op signature and appending
            // newAccum to the block arguments
            auto newLoop = replaceForOpWithNewSignature(builder, forOp, ValueRange {newAccum->getResult(0)});
            ...
            // create new yield
            auto newYield = createYield(builder, newLoop, oldYield, newUpdate->getResult(0), blockArgNum);
            ...
            auto finalOp = incorporateOriginalAccumulatorValue(builder, oldUpdate, cvtLayout, oldAccum);
  ...
  private:
    std::optional<Operation *> getReductionOp(triton::ReduceOp reduce) const
    {
        ...
        auto blockSizeWithoutTerminator = std::distance(blockWithoutTerminator.begin(), blockWithoutTerminator.end());
        if (blockSizeWithoutTerminator != 1)
            return std::nullopt;
        Operation *op = &block.front();
        return std::optional<Operation *>(op);
    }
    Operation *incorporateOriginalAccumulatorValue(OpBuilder &builder, Operation *oldUpdate, Operation *cvtLayout,
    ...
    Operation *createConvertLayout(OpBuilder &builder, Type destType, Operation *newReduce) const
    {
        builder.setInsertionPointAfter(newReduce);
        auto newCvt =
            builder.create<triton::gpu::ConvertLayoutOp>(newReduce->getLoc(), destType, newReduce->getResult(0));
        return newCvt;
    }

    Operation *createPostLoopReduce(OpBuilder &builder, scf::ForOp &loop, triton::ReduceOp &reduce) const
    {
        auto resultIndex = loop.getBody()->getNumArguments() - 1 - loop.getNumInductionVars();
    ...
    Operation *createYield(OpBuilder &builder, scf::ForOp &loop, scf::YieldOp &oldYield, Value newUpdate,
                           int oldAccumBlockArgNum) const
    {
        ...
        yieldValues[oldAccumBlockArgNum - 1] = loop.getBody()->getArgument(oldAccumBlockArgNum);
        yieldValues.push_back(newUpdate);
        auto newYield = builder.create<scf::YieldOp>(oldYield.getLoc(), yieldValues);
        return newYield;
    }

    Operation *createUpdate(OpBuilder &builder, scf::ForOp &loop, Operation *newReduce, Operation *oldUpdate) const
    {
    ...
    Operation *createReduce(OpBuilder &builder, triton::ReduceOp reduce, Type viewOpTensorType) const
    {
        auto srcType = cast<RankedTensorType>(reduce.getOperands()[0].getType());
        auto rank = srcType.getShape().size();
        ...
        IRMapping mapping;
        for (auto operand : reduce.getOperands()) {
            auto viewOp = builder.create<triton::ReshapeOp>(reduce.getLoc(), viewOpTensorType, operand,
        ...
        if (typeInfer) {
            ...
            auto success = typeInfer.inferReturnTypes(
                newReduce->getContext(), newReduce->getLoc(), newReduce->getOperands(), newReduce->getAttrDictionary(),
    ...
    std::optional<TypedAttr> getNeutralElement(Operation *op) const
    {
        if (isa<arith::MaxNumFOp, arith::MinNumFOp>(op)) {
            ...
            const llvm::fltSemantics &semantic = llvm::cast<FloatType>(resultType).getFloatSemantics();
            if (isa<arith::MaxNumFOp>(op)) {
                return builder.getFloatAttr(resultType, APFloat::getInf(semantic, /*Negative=*/true));
            }
            if (isa<arith::MinNumFOp>(op)) {
                return builder.getFloatAttr(resultType, APFloat::getInf(semantic, /*Negative=*/false));
            }
        } else {
    ...
    Operation *createAccum(OpBuilder &builder, triton::ReduceOp reduce, Value &oldAccum, SmallVector<int64_t> &shape,
                           Attribute &slice2d) const
    {
        ...
        auto newAccum = builder.create<arith::ConstantOp>(oldAccum.getLoc(), accumType, denseAttr);
        return newAccum;
    }

    SmallVector<int64_t> getThreadLocalityOptimizedShape(triton::ReduceOp reduce) const
    {
        auto srcType = cast<RankedTensorType>(reduce.getOperands()[0].getType());
        auto srcShape = srcType.getShape();
        auto rank = srcShape.size();
    ...
    Attribute getThreadLocalityOptimizedEncoding(triton::ReduceOp reduce) const
    {
        ...
        auto sizePerThread3d =
            insertValue(blocked.getSizePerThread(), rank, blocked.getSizePerThread()[reduce.getAxis()]);
        ...
        auto ctasPerCGA3d = insertValue(blocked.getCTALayout().getCTAsPerCGA(), rank, 1);
        auto ctasSplitNum3d = insertValue(blocked.getCTALayout().getCTASplitNum(), rank, 1);
        auto ctaOrder3d = insertValue(blocked.getCTALayout().getCTAOrder(), rank, rank);
        auto ctaLayout3d =
            triton::gpu::CTALayoutAttr::get(reduce.getContext(), ctasPerCGA3d, ctasSplitNum3d, ctaOrder3d);
        auto blocked3d = triton::gpu::BlockedEncodingAttr::get(reduce.getContext(), sizePerThread3d, threadsPerWarp3d,
                                                               warsPerCTA3d, order3d, ctaLayout3d);
        return blocked3d;
    }

    template <typename T> SmallVector<T> insertValue(ArrayRef<T> vec, unsigned index, int value) const
    {
        SmallVector<T> res(vec.begin(), vec.end());
        res.insert(res.begin() + index, static_cast<T>(value));
        return res;
    }
    template <typename T> SmallVector<T> insertValue(const SmallVector<T> &vec, unsigned index, int value) const
    {
        SmallVector<T> res(vec.begin(), vec.end());
        res.insert(res.begin() + index, static_cast<T>(value));
        return res;
