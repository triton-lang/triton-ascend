...
static void createAsyncCopy(scf::ForOp &forOp, tt::LoadOp loadOp, Value alloc, Value insertIdx, Value extractIdx,
                            tt::CoarseSchedule &schedule, tt::CoarseSchedule::Cluster prefetchCluster,
                            llvm::MapVector<Operation *, LoadInfo> &loadToInfo, int numStages)
{
    ...
    if (!isExpensiveLoadOrStore(loadOp) && loadToInfo[loadOp].blockedEncoding) {
        ...
        auto convertBlockLayout = [&](Value src) {
            ...
            auto newTy = RankedTensorType::get(ty.getShape(), ty.getElementType(), encoding);
            auto cvt = builder.create<ttg::ConvertLayoutOp>(loadOp->getLoc(), newTy, src);
    ...
    Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(forOp.getContext());
    tt::MemDescType subviewTy = tt::MemDescType::get(allocTy.getShape().drop_front(), allocTy.getElementType(),
                                                     allocTy.getEncoding(), sharedMemorySpace, /*mutableMemory=*/true);
    auto view = builder.create<ttg::MemDescSubviewOp>(loc, subviewTy, alloc, copyOffsets);
    Operation *copy = builder.create<ttg::AsyncCopyGlobalToLocalOp>(loc, src, view, mask, other, loadOp.getCache(),
                                                                    loadOp.getEvict(), loadOp.getIsVolatile());
    Operation *commmit = builder.create<ttg::AsyncCommitGroupOp>(loc, copy->getResult(0));
    Operation *wait = builder.create<ttg::AsyncWaitOp>(loc, commmit->getResult(0), 0);
    ...
    if (isMMV3Load) {
        ...
        auto sharedLoad = builder.create<ttg::LocalLoadOp>(loc, loadOp.getType(), viewLoad, wait->getResult(0));
        ...
        if (other && !isZeroConst(other)) {
            auto select = builder.create<arith::SelectOp>(loc, loadOp.getType(), mask, sharedLoad.getResult(), other);
...
static void createTMAAsyncCopy(scf::ForOp &forOp, tt::ExperimentalDescriptorLoadOp loadOp, Value alloc, Value insertIdx,
                               Value extractIdx, Value barrier, Operation *waitOp, Value phase,
                               tt::CoarseSchedule &schedule, llvm::MapVector<Operation *, LoadInfo> &loadToInfo,
                               int numStages)
{
    ...
    Operation *copy = builder.create<ttng::AsyncTMACopyGlobalToLocalOp>(loc, loadOp.getDescPtr(), loadOp.getIndices(),
                                                                        barrier, view, pred);
...
static std::optional<ttg::SharedEncodingAttr> getSharedEncIfAllUsersAreDotEnc(Value val, bool &incompatible)
{
    ...
    for (Operation *user : val.getUsers()) {
        ...
        if (user->getNumResults() != 1)
            return std::nullopt;
        if (auto memDesc = dyn_cast<triton::MemDescType>(user->getResult(0).getType())) {
            // First time we find a shared encoding in the chain, save it and try to
            // use it if it is compatible with the other users.
            tempAttr = cast<ttg::SharedEncodingAttr>(memDesc.getEncoding());
            if (!getSharedEncIfAllUsersAreDotEnc(user->getResult(0), incompatible).has_value())
                return std::nullopt;
        } else {
            ...
            tempAttr = ttg::SharedEncodingAttr::get(val.getContext(), dotOpEnc, srcTy.getShape(), order, CTALayout,
...
static ttg::BlockedEncodingAttr getBlockedEncoding(tt::LoadOp loadOp, tt::ModuleAxisInfoAnalysis &axisInfo)
{
    Value src = loadOp.getPtr();
    auto ty = cast<RankedTensorType>(src.getType());
    ...
    tt::AxisInfo::DimVectorT contiguity = axisInfo.getAxisInfo(src)->getContiguity();
    ...
    return ttg::BlockedEncodingAttr::get(loadOp->getContext(), ty.getShape(), sizePerThread, order, numWarps,
                                         threadsPerWarp, ctaLayout);
}

static std::optional<ttg::SharedEncodingAttr> getSharedEncoding(Operation *loadOp, bool isMMAV3)
{
    ...
    if (isMMAV3) {
        return ttg::SharedEncodingAttr::get(ty.getContext(), ty.getShape(), order, ctaLayout, ty.getElementType());
    }
    ...
    if (llvm::any_of(loadOp->getUsers(), [&](Operation *user) { return isa<ttg::LocalAllocOp>(user); })) {
        ttg::SharedEncodingAttr localAllocEnc;
        for (auto user : loadOp->getUsers()) {
            auto localAlloc = dyn_cast<ttg::LocalAllocOp>(user);
            if (!localAlloc)
                continue;
            auto enc = mlir::cast<ttg::SharedEncodingAttr>(localAlloc.getType().getEncoding());
            if (!localAllocEnc) {
                localAllocEnc = enc;
            }
    ...
    return ttg::SharedEncodingAttr::get(ty.getContext(), 1, 1, 1, order, ctaLayout);
}
...
static llvm::SmallVector<std::tuple<Operation *, int, Operation *>> loadOpsToIndirectionLevelAndUse(scf::ForOp forOp)
{
    llvm::SmallVector<std::tuple<Operation *, int, Operation *>> loadOpToIndLevelAndUse;
    DenseSet<Operation *> seen;

    std::function<void(Operation * op, int, Operation *)> dfs = [&](Operation *op, int distance, Operation *use) {
...
assignMemoryLayouts(llvm::SmallVector<std::tuple<Operation *, int, Operation *>> &loadOpToIndLevelAndUse,
                    tt::ModuleAxisInfoAnalysis &axisInfoAnalysis)
{
    ...
    for (auto &[op, dist, use] : loadOpToIndLevelAndUse) {
        ...
        if (auto loadOp = dyn_cast<tt::LoadOp>(op)) {
            assert(!isLoadFromTensorPtr(loadOp) && "Block ptr should have been lowered before this pass.");
        ...
        if (use->hasTrait<OpTrait::DotLike>()) {
            ...
            if (loadIsMMAv3(op)) {
                ...
                loadInfo.sharedEncoding = getSharedEncoding(op, /*loadIsMMAv3=*/true).value_or(nullptr);
            } else if (isa<tt::ExperimentalDescriptorLoadOp>(op)) {
                loadInfo.sharedEncoding = getSharedEncoding(op, /*loadIsMMAv3=*/true).value_or(nullptr);
            } else if (auto dot = dyn_cast<tt::DotOp>(use)) {
                bool incompatible = false;
                loadInfo.sharedEncoding =
                    getSharedEncIfAllUsersAreDotEnc(op->getResult(0), incompatible).value_or(nullptr);
                ...
                if (!loadInfo.sharedEncoding) {
                    if (auto dotEnc = dyn_cast<ttg::NvidiaMmaEncodingAttr>(dot.getResult().getType().getEncoding())) {
                        ...
                        for (int i = 0; i < mmaInstrShape.size(); i++) {
                            if (loadTy.getShape()[loadTy.getRank() - mmaInstrShape.size() + i] < mmaInstrShape[i]) {
                                ok = false;
                                break;
                            }
                        }
        ...
        if (!loadInfo.sharedEncoding && !isa<ttng::WarpGroupDotOp>(use)) {
            loadInfo.sharedEncoding = getSharedEncoding(op, /*isMMAV3=*/loadInfo.loadIsMMAV3).value_or(nullptr);
            if (auto loadOp = dyn_cast<tt::LoadOp>(op)) {
...
static llvm::MapVector<Operation *, LoadInfo> scheduleLoads(scf::ForOp forOp, tt::CoarseSchedule &schedule,
                                                            DenseSet<Operation *> &rootUsers, int numStages)
{
    ...
    // to the dot op.
    llvm::SmallVector<std::tuple<Operation *, int, Operation *>> loadOpToIndLevelAndUse =
        loadOpsToIndirectionLevelAndUse(forOp);
    LLVM_DEBUG({
    ...
    auto it = llvm::remove_if(loadOpToIndLevelAndUse, [=](auto op) { return std::get<1>(op) >= numStages - 1; });
    loadOpToIndLevelAndUse.erase(it, loadOpToIndLevelAndUse.end());

    // Check which loads are good for pipelining, and assign them
    // memory layouts.
    llvm::MapVector<Operation *, LoadInfo> loadToInfo = assignMemoryLayouts(loadOpToIndLevelAndUse, axisInfoAnalysis);
    ...
    unsigned stagesBetweenLoads = ceil<unsigned>(numStages - 2, maxIndirectionLevel + 1);
...
static tt::CoarseSchedule::Cluster schedulePrologueAndEpilogue(scf::ForOp forOp, tt::CoarseSchedule &schedule,
                                                               DenseSet<Operation *> &rootUsers, int numStages)
{
    ...
    for (auto rootUser : rootUsers) {
        ...
        for (auto op : forwardSlice) {
            ...
            if (ifOp == nullptr) {
                ...
                if (parentOp != nullptr && parentOp->getParentOp() == forOp.getOperation()) {
...
static void scheduleDependencies(scf::ForOp forOp, tt::CoarseSchedule &schedule, int numStages)
{
    SmallVector<std::tuple<Operation *, int, tt::CoarseSchedule::Cluster>> opsInOrder = schedule.getOpsInOrder(forOp);
...
static void scheduleDistanceOneDependencies(scf::ForOp forOp, tt::CoarseSchedule &schedule, int numStages)
{
...
static void scheduleRemainingToLastStage(scf::ForOp forOp, tt::CoarseSchedule &schedule,
...
static Value createAlloc(scf::ForOp &forOp, Operation *loadOp, ttg::SharedEncodingAttr sharedEnc, unsigned distance)
{
    OpBuilder builder(forOp);
    Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(forOp.getContext());
    auto ty = cast<RankedTensorType>(loadOp->getResultTypes()[0]);
    ...
    Type memdescType = mlir::triton::MemDescType::get(bufferShape, ty.getElementType(), sharedEnc, sharedMemorySpace,
                                                      /*mutableMemory*/ true);
    Value alloc = builder.create<mlir::triton::gpu::LocalAllocOp>(loadOp->getLoc(), memdescType, Value());
    return alloc;
}
...
static void createTMABarrierAndWait(scf::ForOp &forOp, SmallVector<AsyncLoad> &asyncLoads, Value insertIdx,
                                    ...
                                    const llvm::MapVector<Operation *, LoadInfo> &loadToInfo)
{
            ...
            for (Operation *user : loadInfo->loadOp->getUsers()) {
                ...
                if (it != loadToInfo.end()) {
                    ...
                    if (it->second.loadIsMMAV3) {
                        auto alloc = cast<ttg::LocalAllocOp>((*loadInfo->loadOp->getUsers().begin()));
    ...
    for (SmallVector<AsyncLoad *> &group : loadGroups) {
        ...
        for (AsyncLoad *asyncLoad : group) {
            auto tensorTy = cast<RankedTensorType>(asyncLoad->loadOp->getResult(0).getType());
            int loadSize = product(tensorTy.getShape());
            sizeInBytes += loadSize * tensorTy.getElementType().getIntOrFloatBitWidth() / 8;
        }
        ...
        Location loc = forOp.getLoc();
        OpBuilder builder(forOp);
        Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(builder.getContext());
        ...
        Value barrier =
            builder.create<ttg::MemDescSubviewOp>(loc, barrierTy, barrierAlloc, ArrayRef<Value>({insertIdx}));
        Value pred = builder.create<arith::ConstantIntOp>(loc, 1, 1);
        Operation *expect = builder.create<ttng::BarrierExpectOp>(forOp.getLoc(), barrier, sizeInBytes, pred);
        ...
        Value barrierViewWait =
            builder.create<ttg::MemDescSubviewOp>(loc, barrierTy, barrierAlloc, ArrayRef<Value>({extractIdx}));
        Operation *wait = builder.create<ttng::WaitBarrierOp>(loc, barrierViewWait, phase);
...
static SmallVector<Value> createAsyncOps(scf::ForOp &forOp, tt::CoarseSchedule &schedule,
                                         llvm::MapVector<Operation *, LoadInfo> &loadToInfo,
                                         SmallVector<Value> &barriers, int numStages)
{
    ...
    int numBuffers = llvm::max_element(llvm::make_second_range(loadToInfo), [](auto &lhs, auto &rhs) {
                         return lhs.distToUse < rhs.distToUse;
                     })->distToUse;
    bool hasMMAV3 = llvm::any_of(loadToInfo, [](auto &kv) { return kv.second.loadIsMMAV3; });
    if (hasMMAV3) {
    ...
    scf::ForOp newForOp = replaceForOpWithNewSignature(builder, forOp, newOperands);
    forOp.erase();
    forOp = newForOp;
    ...
    Value cndIns = builder.create<arith::CmpIOp>(loc, arith::CmpIPredicate::slt, insertIdx, numBuffersVal);
    insertIdx = builder.create<arith::SelectOp>(loc, cndIns, insertIdx, zero);

    extractIdx = builder.create<arith::AddIOp>(loc, extractIdx, one);
    Value cndExt = builder.create<arith::CmpIOp>(loc, arith::CmpIPredicate::slt, extractIdx, numBuffersVal);
    ...
    createTMABarrierAndWait(forOp, asyncLoads, insertIdx, extractIdx, phase, numBuffers, schedule, barriers,
                            loadToInfo);
    ...
    for (AsyncLoad &asyncLoad : asyncLoads) {
        if (auto loadOp = dyn_cast<tt::LoadOp>(asyncLoad.loadOp)) {
            createAsyncCopy(forOp, loadOp, asyncLoad.alloc, insertIdx, extractIdx, schedule, prefetchCluster,
                            loadToInfo, numStages);
        } else {
            auto descLoad = cast<tt::ExperimentalDescriptorLoadOp>(asyncLoad.loadOp);
            createTMAAsyncCopy(forOp, descLoad, asyncLoad.alloc, insertIdx, extractIdx, asyncLoad.barrier,
                               asyncLoad.waitOp, phase, schedule, loadToInfo, numStages);
        }
    }
...
static void invalidateBarriers(OpBuilder &builder, SmallVector<Value> &barriers)
{
    Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(builder.getContext());
    for (Value barrier : barriers) {
        ...
        for (int i = 0; i < numBarriers; i++) {
            ...
            tt::MemDescType barrierTy = tt::MemDescType::get(
                {1}, builder.getI64Type(), cast<tt::MemDescType>(barrier.getType()).getEncoding(), sharedMemorySpace,
                /*mutableMemory=*/true);
            Value barrierView = builder.create<ttg::MemDescSubviewOp>(barrier.getLoc(), barrierTy, barrier, idx);
            builder.create<ttng::InvalBarrierOp>(barrier.getLoc(), barrierView);
        }
    }
}

bool mlir::triton::preProcessLoopAndGetSchedule(scf::ForOp &forOp, int numStages,
                                                mlir::triton::PipeliningOption &options)
{
    ...
    llvm::MapVector<Operation *, LoadInfo> loadToInfo = scheduleLoads(forOp, coarseSchedule, rootUsers, numStages);
    if (loadToInfo.empty())
        return false;
    ...
    SmallVector<Value> allocs = createAsyncOps(forOp, coarseSchedule, loadToInfo, barriers, numStages);
    ...
    std::vector<std::pair<Operation *, unsigned>> schedule = coarseSchedule.createFinalSchedule(forOp);

    // Fill out the pipeline options.
    options.getScheduleFn = [schedule](scf::ForOp forOp, std::vector<std::pair<Operation *, unsigned>> &s) {
        s = std::move(schedule);
    };
    options.peelEpilogue = false;
    options.predicateFn = tt::predicateOp;
    options.supportDynamicLoops = true;
    options.annotateFn = [](Operation *op, mlir::triton::PipeliningOption::PipelinerPart part, unsigned iteration) {};
...
static int minNumInterleavedCommitOps(Operation *waitOp)
{
    ...
    std::function<int(Value, Operation *, int)> minOverHistories = [&](Value val, Operation *sinkOp,
    ...
    for (auto waitOp : waitOps) {
        ...
        while (next && isa<ttg::MemDescSubviewOp, ttg::AsyncWaitOp>(next)) {
            if (auto nextWait = dyn_cast<ttg::AsyncWaitOp>(next)) {
                ...
                depTokens.append(nextWait.getOperands().begin(), nextWait.getOperands().end());
            }
        ...
        auto newWaitOp = builder.create<ttg::AsyncWaitOp>(waitOp.getLoc(), depTokens, minWaitNumber);
...
// wait.
static void threadValuesThroughWait(ttng::WarpGroupDotWaitOp wait, MutableArrayRef<Value> values)
{
    ...
    SetVector<Value> newOperands(wait.getOperands().begin(), wait.getOperands().end());
    assert(newOperands.size() == origNumOperands && "Wait op has duplicate operands.");
    ...
    auto newWait =
        builder.create<ttng::WarpGroupDotWaitOp>(wait.getLoc(), llvm::to_vector(newOperands), wait.getPendings());

    auto dominatedByNewWait = [&](OpOperand &operand) {
        auto opInThisBlock = newWait->getBlock()->findAncestorOpInBlock(*operand.getOwner());
...
//
static std::optional<int> dotCanBeProperlyAsync(ttng::WarpGroupDotOp dotOp, scf::ForOp forOp)
{
    ...
    auto checkOperand = [&](Value operand) {
        if (!isa<ttg::SharedEncodingAttr>(cast<TensorOrMemDesc>(operand.getType()).getEncoding())) {
            return true;
        }
        ...
        while (isa_and_nonnull<ttg::ConvertLayoutOp, tt::TransOp>(transitiveOperand.getDefiningOp()) ||
               isa<BlockArgument>(transitiveOperand))
        {
            auto blockArg = dyn_cast<BlockArgument>(transitiveOperand);
            if (blockArg && blockArg.getOwner() == forOp.getBody()) {
                transitiveOperand =
                    cast<scf::YieldOp>(blockArg.getOwner()->getTerminator()).getOperand(blockArg.getArgNumber() - 1);
    ...
    if (llvm::all_of(iterArg.getUses(), [&](OpOperand &use) {
            return isa<ttng::WarpGroupDotOp>(use.getOwner()) && use.getOperandNumber() == 2;
        }))
    {
        return iterArgIdx;
    }
    ...
    auto firstWaitOpIter = llvm::find_if(waitOps, [&](auto waitOp) { return waitOp.getPendings() == 0; });
    if (firstWaitOpIter != waitOps.end() && llvm::all_of(iterArg.getUsers(), [&](Operation *user) {
...
// `wait 0` appears.
static void insertAsyncWarpGroupDotWaitInLoop(scf::ForOp forOp,
                                              const llvm::MapVector<Operation *, int /*iterArgIdx*/> &properlyAsyncDots)
{
    ...
    for (auto [asyncDot, iterArgIdx] : properlyAsyncDots) {
        ...
        for (auto [block, users] : blockToUsers) {
            ...
            auto newWait = builder.create<ttng::WarpGroupDotWaitOp>(asyncDot->getLoc(), ArrayRef<Value> {}, 0);
    ...
    auto wait = builder.create<ttng::WarpGroupDotWaitOp>(lastAsyncDot->getLoc(),
...
void triton::asyncLaunchDots(scf::ForOp forOp)
{
    ...
    for (auto WarpGroupDotOp : forOp.getBody()->getOps<ttng::WarpGroupDotOp>()) {
        ...
        if (auto iterArgIdx = dotCanBeProperlyAsync(WarpGroupDotOp, forOp)) {
            ...
            auto wait = builder.create<ttng::WarpGroupDotWaitOp>(WarpGroupDotOp.getLoc(), ArrayRef<Value> {},
                                                                 /*pendings=*/0);
    ...
    builder.setInsertionPointAfter(forOp);
    auto WarpGroupDotWaitAfterLoop = builder.create<ttng::WarpGroupDotWaitOp>(forOp.getLoc(), ArrayRef<Value> {}, 0);
