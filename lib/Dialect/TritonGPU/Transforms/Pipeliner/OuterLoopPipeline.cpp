...
static std::vector<std::pair<Operation *, unsigned>> createSchedule(scf::ForOp forOp, int numStages)
{
    ...
    tt::addOps(forOp, 1, schedule,
               [&](Operation *op) { return insertAndDeps.count(op) == 0 && epilogue.count(op) == 0; });

    // Then Schedule stage 0.
    tt::addOps(forOp, 0, schedule, [&](Operation *op) { return insertAndDeps.count(op); });

    // Then schedule the epilogue in stage 1
    tt::addOps(forOp, 1, schedule, [&](Operation *op) { return epilogue.count(op); });
    return schedule;
}
...
bool mlir::triton::getOuterLoopSchedule(scf::ForOp &forOp, int numStages, mlir::triton::PipeliningOption &options)
{
    ...
    std::vector<std::pair<Operation *, unsigned>> schedule = createSchedule(forOp, numStages);
