...
struct LoopPipelinerInternal {
  ...
  public:
    ...
    scf::ForOp createKernelLoop(const llvm::MapVector<Value, LiverangeInfo> &crossStageValues, RewriterBase &rewriter,
    ...
    LogicalResult createKernel(scf::ForOp newForOp, const llvm::MapVector<Value, LiverangeInfo> &crossStageValues,
    ...
    LogicalResult emitEpilogue(RewriterBase &rewriter, llvm::SmallVector<Value> &returnValues);
};

bool LoopPipelinerInternal::initializeLoopInfo(ForOp op, const triton::PipeliningOption &options)
{
    ...
    if (llvm::any_of(forOp.getBody()->getTerminator()->getOperands(), [this](Value operand) {
            Operation *def = operand.getDefiningOp();
            return !def || (!stages.contains(def) && forOp->isAncestor(def));
        }))
    {
...
static Operation *cloneAndUpdateOperands(RewriterBase &rewriter, Operation *op,
...
LogicalResult LoopPipelinerInternal::emitPrologue(RewriterBase &rewriter)
{
    ...
    for (auto [arg, operand] : llvm::zip(forOp.getRegionIterArgs(), forOp.getInitsMutable())) {
    ...
    for (int64_t i = 0; i < maxStage; i++) {
        ...
        Value iv = rewriter.create<arith::AddIOp>(
            loc, lb,
            rewriter.create<arith::MulIOp>(loc, step,
                                           rewriter.create<arith::ConstantOp>(loc, rewriter.getIntegerAttr(t, i))));
        setValueMapping(forOp.getInductionVar(), iv, i);

        if (dynamicLoop) {
            // pred = ub > lb + (i * step)
            predicates[i] = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::slt, iv, ub);
        }

        for (Operation *op : opOrder) {
            if (stages[op] > i)
                continue;
            Operation *newOp = cloneAndUpdateOperands(rewriter, op, [&](OpOperand *newOperand) {
            ...
            for (unsigned destId : llvm::seq(unsigned(0), op->getNumResults())) {
                ...
                for (OpOperand &operand : yield->getOpOperands()) {
                    if (operand.get() != op->getResult(destId))
                        continue;
                    if (predicates[predicateIdx] && !forOp.getResult(operand.getOperandNumber()).use_empty()) {
                        // If the value is used outside the loop, we need to make sure we
                        // return the correct version of it.
                        Value prevValue =
                            valueMapping[forOp.getRegionIterArgs()[operand.getOperandNumber()]][i - stages[op]];
                        source = rewriter.create<arith::SelectOp>(loc, predicates[predicateIdx], source, prevValue);
                    }
                    setValueMapping(forOp.getRegionIterArgs()[operand.getOperandNumber()], source, i - stages[op] + 1);
                }
                setValueMapping(op->getResult(destId), newOp->getResult(destId), i - stages[op]);
            }
        }
    }
    return success();
...
llvm::MapVector<Value, LoopPipelinerInternal::LiverangeInfo> LoopPipelinerInternal::analyzeCrossStageValues()
{
    ...
    for (Operation *op : opOrder) {
        ...
        auto analyzeOperand = [&](OpOperand &operand) {
            ...
            if (defStage == stages.end() || defStage->second == stage || defStage->second == stage + distance)
                return;
        ...
        visitUsedValuesDefinedAbove(op->getRegions(), [&](OpOperand *operand) { analyzeOperand(*operand); });
    }
...
scf::ForOp LoopPipelinerInternal::createKernelLoop(
    const llvm::MapVector<Value, LoopPipelinerInternal::LiverangeInfo> &crossStageValues, RewriterBase &rewriter,
    llvm::DenseMap<std::pair<Value, unsigned>, unsigned> &loopArgMap)
{
    ...
    // prologue.
    for (const auto &retVal : llvm::enumerate(forOp.getBody()->getTerminator()->getOperands())) {
        ...
        if (defStage != stages.end()) {
            Value valueVersion = valueMapping[forOp.getRegionIterArgs()[retVal.index()]][maxStage - defStage->second];
            assert(valueVersion);
    ...
    for (auto escape : crossStageValues) {
        ...
        for (unsigned stageIdx = 0; stageIdx < info.lastUseStage - info.defStage; stageIdx++) {
            Value valueVersion = valueMapping[value][maxStage - info.lastUseStage + stageIdx];
            assert(valueVersion);
            newLoopArg.push_back(valueVersion);
            loopArgMap[std::make_pair(value, info.lastUseStage - info.defStage - stageIdx)] = newLoopArg.size() - 1;
        }
    }
    ...
    if (peelEpilogue) {
        ...
        Value maxStageValue = rewriter.create<arith::ConstantOp>(loc, rewriter.getIntegerAttr(t, maxStage));
        Value maxStageByStep = rewriter.create<arith::MulIOp>(loc, step, maxStageValue);
        newUb = rewriter.create<arith::SubIOp>(loc, ub, maxStageByStep);
    }
    auto newForOp =
        rewriter.create<scf::ForOp>(forOp.getLoc(), forOp.getLowerBound(), newUb, forOp.getStep(), newLoopArg);
...
LogicalResult LoopPipelinerInternal::createKernel(
    scf::ForOp newForOp, const llvm::MapVector<Value, LoopPipelinerInternal::LiverangeInfo> &crossStageValues,
    const llvm::DenseMap<std::pair<Value, unsigned>, unsigned> &loopArgMap, RewriterBase &rewriter)
{
    valueMapping.clear();
        ...
        for (unsigned i = 0; i < maxStage; i++) {
            ...
            Value c = rewriter.create<arith::SubIOp>(
                ...
                rewriter.create<arith::MulIOp>(
                    loc, step,
                    rewriter.create<arith::ConstantOp>(loc, rewriter.getIntegerAttr(t, int64_t(maxStage - i)))));

            Value pred = rewriter.create<arith::CmpIOp>(newForOp.getLoc(), arith::CmpIPredicate::slt,
        ...
        for (OpOperand *operand : operands) {
            ...
            if (operand->get() == forOp.getInductionVar()) {
                ...
                Value offset = rewriter.create<arith::MulIOp>(
                    forOp.getLoc(), step,
                    rewriter.create<arith::ConstantOp>(forOp.getLoc(),
                                                       rewriter.getIntegerAttr(t, maxStage - stages[op])));
                Value iv = rewriter.create<arith::AddIOp>(forOp.getLoc(), newForOp.getInductionVar(), offset);
            ...
            if (arg && arg.getOwner() == forOp.getBody()) {
                Value ret = forOp.getBody()->getTerminator()->getOperand(arg.getArgNumber() - 1);
                ...
                if (stageDep->second == useStage + 1) {
                    nestedNewOp->setOperand(operand->getOperandNumber(), mapping.lookupOrDefault(ret));
                    continue;
                }
                source = ret;
            }
            ...
            if (stageDef == stages.end() || stageDef->second == useStage)
                continue;
            auto remap = loopArgMap.find(std::make_pair(operand->get(), useStage - stageDef->second));
            assert(remap != loopArgMap.end());
            nestedNewOp->setOperand(operand->getOperandNumber(), newForOp.getRegionIterArgs()[remap->second]);
        }
    ...
    for (OpOperand &yieldOperand : forOp.getBody()->getTerminator()->getOpOperands()) {
        ...
        // defStage.
        if (!peelEpilogue && !forOp.getResult(yieldOperand.getOperandNumber()).use_empty()) {
            ...
            if (def) {
                ...
                if (defStage != stages.end() && defStage->second < maxStage) {
                    ...
                    source = rewriter.create<arith::SelectOp>(
                        ...
                        newForOp.getBody()->getArguments()[yieldOperand.getOperandNumber() + 1]);
                }
            }
        }
    ...
    for (auto &it : crossStageValues) {
        ...
        for (unsigned i = 1; i < numVersionReturned; i++) {
            setValueMapping(it.first, newForOp->getResult(yieldOperands.size()), version++);
            yieldOperands.push_back(
                newForOp.getBody()->getArguments()[yieldOperands.size() + 1 + newForOp.getNumInductionVars()]);
        }
        setValueMapping(it.first, newForOp->getResult(yieldOperands.size()), version++);
    ...
    for (const auto &retVal : llvm::enumerate(forOp.getBody()->getTerminator()->getOperands())) {
        ...
        if (defStage == stages.end()) {
            for (unsigned int stage = 1; stage <= maxStage; stage++)
                setValueMapping(forOp.getRegionIterArgs()[retVal.index()], retVal.value(), stage);
        } else if (defStage->second > 0) {
            setValueMapping(forOp.getRegionIterArgs()[retVal.index()], newForOp->getResult(retVal.index()),
...
LogicalResult LoopPipelinerInternal::emitEpilogue(RewriterBase &rewriter, llvm::SmallVector<Value> &returnValues)
{
    Location loc = forOp.getLoc();
    Type t = lb.getType();
    ...
    auto createConst = [&](int v) { return rewriter.create<arith::ConstantOp>(loc, rewriter.getIntegerAttr(t, v)); };
    ...
    Value stepLessZero = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::slt, step, zero);
    Value stepDecr = rewriter.create<arith::SelectOp>(loc, stepLessZero, one, createConst(-1));
    ...
    Value rangeDecr = rewriter.create<arith::AddIOp>(loc, rangeIncrStep, stepDecr);
    ...
    Value iterI = rewriter.create<arith::SubIOp>(loc, totalIterations, createConst(maxStage));
    ...
    for (int64_t i = 1; i <= maxStage; i++) {
        ...
        Value newlastIter = rewriter.create<arith::AddIOp>(loc, lb, rewriter.create<arith::MulIOp>(loc, step, iterI));
        ...
        if (dynamicLoop) {
            ...
            // pred = total_iters >= i
            predicates[i] =
                rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::sge, totalIterations, createConst(i));
        }
    }
    ...
    for (int64_t i = 1; i <= maxStage; i++) {
        ...
        for (Operation *op : opOrder) {
            ...
            if (annotateFn)
                annotateFn(newOp, triton::PipeliningOption::PipelinerPart::Epilogue, i - 1);
            for (auto [opRes, newRes] : llvm::zip(op->getResults(), newOp->getResults())) {
        ...
        if (dynamicLoop) {
            ...
            for (auto pair : llvm::enumerate(returnValues)) {
                ...
                if (mapVal) {
                    ...
                    auto selOp = rewriter.create<arith::SelectOp>(loc, pred, pair.value(), prevValue);
...
void LoopPipelinerInternal::setValueMapping(Value key, Value el, int64_t idx)
{
    ...
    // versions.
    if (it == valueMapping.end())
        it = valueMapping.insert(std::make_pair(key, llvm::SmallVector<Value>(maxStage + 1))).first;
    it->second[idx] = el;
}

} // namespace

FailureOr<ForOp> mlir::triton::pipelineForLoop(RewriterBase &rewriter, ForOp forOp,
                                               const triton::PipeliningOption &options, bool *modifiedIR)
{
    ...
    // they are defined.
    llvm::MapVector<Value, LoopPipelinerInternal::LiverangeInfo> crossStageValues = pipeliner.analyzeCrossStageValues();
    ...
    ForOp newForOp = pipeliner.createKernelLoop(crossStageValues, rewriter, loopArgMap);
    // Create the kernel block, order ops based on user choice and remap
    // operands.
    if (failed(pipeliner.createKernel(newForOp, crossStageValues, loopArgMap, rewriter)))
        return failure();

    llvm::SmallVector<Value> returnValues = newForOp.getResults().take_front(forOp->getNumResults());
