...
static bool preCondition(scf::ForOp forOp)
{
    ...
    if (llvm::any_of(forOp.getBody()->getTerminator()->getOperands(), [](Value operand) {
...
static void tryAndPipelineOuterLoop(scf::ForOp forOp)
{
    ...
    FailureOr<scf::ForOp> newForOp = mlir::triton::pipelineForLoop(rewriter, forOp, options);
}
...
struct PipelinePass : public impl::TritonGPUPipelineBase<PipelinePass> {
    ...
    void runOnOperation() override
    {
        ...
        // simplify the IR.
        auto arithDialect = getOperation().getContext()->getLoadedDialect<arith::ArithDialect>();
