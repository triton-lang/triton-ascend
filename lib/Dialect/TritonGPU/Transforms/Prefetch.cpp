...
class Prefetcher {
    ...
    Value generatePrefetch(Value v, unsigned opIdx, bool isPrologue, Attribute dotEncoding, OpBuilder &builder,
                           std::optional<int64_t> offsetK = std::nullopt, std::optional<int64_t> shapeK = std::nullopt);

    void cloneElementwiseOps(Value &bRem, const SmallVector<Value> &vals, OpBuilder &builder);

  public:
    Prefetcher() = delete;

    Prefetcher(scf::ForOp forOp) : forOp(forOp) { yieldOp = cast<scf::YieldOp>(forOp.getBody()->getTerminator()); }
...
void Prefetcher::cloneElementwiseOps(Value &ret, const SmallVector<Value> &vals, OpBuilder &builder)
{
    IRMapping mapping;
    ...
    for (int i = 2; i < vals.size(); i++) {
        ...
        if (isa<RankedTensorType>(curr.getType())) {
            auto retType = RankedTensorType::get(
                ...
                cast<RankedTensorType>(curr.getDefiningOp()->getOperand(0).getType()).getEncoding());
...
Value Prefetcher::generatePrefetch(Value v, unsigned opIdx, bool isPrologue, Attribute dotEncoding, OpBuilder &builder,
                                   std::optional<int64_t> offsetK, std::optional<int64_t> shapeK)
{
    ...
    Value newSmem = builder.create<triton::gpu::MemDescSubviewOp>(
        v.getLoc(), triton::MemDescType::get(shape, elementType, type.getEncoding(), type.getMemorySpace()), v,
        offsetsVal);

    auto dotOperandEnc =
        triton::gpu::DotOperandEncodingAttr::get(builder.getContext(), opIdx, dotEncoding, prefetchWidth / 8);
    Value prefetchSlice = builder.create<triton::gpu::LocalLoadOp>(
        v.getLoc(), RankedTensorType::get(shape, elementType, dotOperandEnc), newSmem);
...
LogicalResult Prefetcher::initialize()
{
    ...
    for (Operation &op : *loop)
        if (auto dotOp = dyn_cast<triton::DotOp>(op)) {
            ...
            auto dstMfmaEnc = dyn_cast<AMDMfmaEncodingAttr>(getEncoding(dotOp.getResult()));
    ...
    for (triton::DotOp dot : dotsInFor) {
        ...
        auto aEnc = mlir::cast<triton::gpu::DotOperandEncodingAttr>(aType.getEncoding());
        auto bEnc = mlir::cast<triton::gpu::DotOperandEncodingAttr>(bType.getEncoding());
...
void Prefetcher::emitPrologue()
{
    ...
    for (triton::DotOp dot : dots) {
        ...
        Value aPrefetched = generatePrefetch(dot2aHeaderDef[dot], 0, true, dotEncoding, builder);
        cloneElementwiseOps(aPrefetched, dot2aVals[dot], builder);
        Value bPrefetched = generatePrefetch(dot2bHeaderDef[dot], 1, true, dotEncoding, builder);
...
scf::ForOp Prefetcher::createNewForOp()
{
    ...
    auto setInsertionPointBeforeYield = [](OpBuilder &builder, scf::ForOp newForOp) {
    ...
    for (Operation &op : forOp.getBody()->without_terminator()) {
        ...
        if (dot && dots.contains(dot)) {
            ...
            if (Value a = operand2headPrefetch.lookup(dot.getA()))
                firstDot->setOperand(0, newForOp.getTiedLoopRegionIterArg(&*a.use_begin()));
            if (Value b = operand2headPrefetch.lookup(dot.getB()))
                firstDot->setOperand(1, newForOp.getTiedLoopRegionIterArg(&*b.use_begin()));
            ...
            while (kRem != 0) {
                ...
                builder.setInsertionPoint(prevDot);
                Value aRem =
                    generatePrefetch(mapping.lookup(dot2aLoopArg[dot]), 0, false, dotEncoding, builder, kOff, kShape);
                cloneElementwiseOps(aRem, dot2aVals[dot], builder);
                Value bRem =
                    generatePrefetch(mapping.lookup(dot2bLoopArg[dot]), 1, false, dotEncoding, builder, kOff, kShape);
    ...
    for (triton::DotOp dot : dots) {
        Attribute dotEncoding = dot.getType().getEncoding();
        Value aToYield = generatePrefetch(mapping.lookup(dot2aYield[dot]), 0, true, dotEncoding, builder);
        cloneElementwiseOps(aToYield, dot2aVals[dot], builder);
        yieldValues.push_back(aToYield);
        // bToYield
        Value bToYield = generatePrefetch(mapping.lookup(dot2bYield[dot]), 1, true, dotEncoding, builder);
...
struct PrefetchPass : public impl::TritonGPUPrefetchBase<PrefetchPass> {
    void runOnOperation() override
    {
        ...
        if (mlir::applyPatternsAndFoldGreedily(getOperation(), std::move(cleanUpPatterns)).failed()) {
            signalPassFailure();
        }
