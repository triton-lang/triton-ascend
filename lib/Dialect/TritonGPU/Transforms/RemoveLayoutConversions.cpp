...
class LayoutPropagation {
  public:
    ...
    // in `changed`.
    void setEncoding(ValueRange values, LayoutInfo &info, SmallVector<Value> &changed, Operation *op);
    ...
    Operation *cloneElementwise(OpBuilder &rewriter, Operation *op, Attribute encoding);
...
class LayoutRematerialization {
  public:
    ...
    bool hasRematValue(Value value, Attribute encoding) { return rematMapping.contains({value, encoding}); }
    ...
    void rewriteSlice(SetVector<Value> &slice, DenseMap<Value, Attribute> &layout, ConvertLayoutOp convertOp,
                      IRMapping &mapping);
    void rewriteSlice(SetVector<Value> &slice, DenseMap<Value, Attribute> &layout, ConvertLayoutOp convertOp);
...
void LayoutRematerialization::addRematValue(Value old, Attribute encoding, Value newV)
{
...
void LayoutPropagation::setEncoding(ValueRange values, LayoutInfo &info, SmallVector<Value> &changed, Operation *op)
{
...
SmallVector<Value> LayoutPropagation::propagateToUsers(Value value, LayoutInfo &info)
{
    ...
    for (OpOperand &use : value.getUses()) {
        ...
        if (auto yieldOp = dyn_cast<scf::YieldOp>(user)) {
            ...
            if (auto forOp = dyn_cast<scf::ForOp>(parent))
                valuesToPropagate.push_back(forOp.getRegionIterArg(use.getOperandNumber()));
            if (auto whileOp = dyn_cast<scf::WhileOp>(parent))
                valuesToPropagate.push_back(whileOp.getBeforeArguments()[use.getOperandNumber()]);
            if (isa<scf::ForOp, scf::IfOp, scf::WhileOp>(parent))
        ...
        if (user->hasTrait<OpTrait::SameOperandsAndResultEncoding>() || user->hasTrait<OpTrait::Elementwise>() ||
            isa<ReduceOp, ExpandDimsOp, ReshapeOp, TransOp, JoinOp, SplitOp, ConvertLayoutOp>(user))
        {
...
void LayoutPropagation::propagateLayout()
{
    ...
    while (!queue.empty()) {
        ...
        LLVM_DEBUG({
            DBGS() << "propagateLayout considering " << currentValue << ", which has " << info.encodings.size()
...
void LayoutPropagation::resolveConflicts()
{
    for (auto &it : layouts) {
        ...
        for (Attribute e : info.encodings) {
            if ((isLoadOrStore && isa<BlockedEncodingAttr>(e)) || (!isLoadOrStore && isa<MmaEncodingTrait>(e))) {
                encoding = e;
                break;
            }
        }
        ...
        for (Operation &op : currentRegion->getOps()) {
            ...
            if (needRewrite) {
                ...
                for (OpOperand &operand : op.getOpOperands()) {
                    ...
                    if (it == layouts.end())
                        continue;
                    Attribute encoding = cast<RankedTensorType>(operand.get().getType()).getEncoding();
...
{
    rewriteMapping[{old, cast<RankedTensorType>(newV.getType()).getEncoding()}] = newV;
}
...
Value LayoutPropagation::getValueAs(Value value, Attribute encoding)
{
    if (auto tensorType = dyn_cast<RankedTensorType>(value.getType())) {
        ...
        if (cast<RankedTensorType>(rewrittenValue.getType()).getEncoding() == encoding)
            return rewrittenValue;
        OpBuilder rewriter(value.getContext());
        rewriter.setInsertionPointAfterValue(rewrittenValue);
        auto tmpType = RankedTensorType::get(tensorType.getShape(), tensorType.getElementType(), encoding);
        Value converted = rewriter.create<ConvertLayoutOp>(value.getLoc(), tmpType, rewrittenValue);
...
Operation *LayoutPropagation::cloneElementwise(OpBuilder &rewriter, Operation *op, Attribute encoding)
{
    ...
    for (OpOperand &operand : op->getOpOperands()) {
        newOp->setOperand(operand.getOperandNumber(), getValueAs(operand.get(), *operandEnc));
    }
    ...
    for (unsigned i = 0, e = op->getNumResults(); i < e; ++i) {
        ...
        if (!origType)
            continue;
        auto newType = RankedTensorType::get(origType.getShape(), origType.getElementType(), encoding);
...
Operation *LayoutPropagation::rewriteForOp(scf::ForOp forOp)
{
    ...
    for (auto [operand, result] : llvm::zip(forOp.getInitArgs(), forOp.getResults())) {
        Value convertedOperand = operand;
        if (layouts.count(result))
            convertedOperand = getValueAs(operand, *layouts[result].encodings.begin());
        operands.push_back(convertedOperand);
    }
    auto newForOp = rewriter.create<scf::ForOp>(forOp.getLoc(), forOp.getLowerBound(), forOp.getUpperBound(),
                                                forOp.getStep(), operands);
    newForOp->setAttrs(forOp->getAttrs());
    newForOp.getBody()->getOperations().splice(newForOp.getBody()->getOperations().begin(),
    ...
    for (auto [oldArg, newArg] : llvm::zip(forOp.getBody()->getArguments(), newForOp.getBody()->getArguments())) {
...
Operation *LayoutPropagation::rewriteWhileOp(scf::WhileOp whileOp)
{
    ...
    for (auto [operand, arg] : llvm::zip(whileOp->getOperands(), whileOp.getBeforeArguments())) {
        Value convertedOperand = operand;
    ...
    for (Value ret : whileOp.getResults()) {
        ...
        auto newType = RankedTensorType::get(origType.getShape(), origType.getElementType(), it->second.encodings[0]);
        returnTypes.push_back(newType);
    }

    auto newWhileOp = rewriter.create<scf::WhileOp>(whileOp.getLoc(), returnTypes, operands);
    SmallVector<Type> argsTypesBefore;
    for (Value operand : operands)
        argsTypesBefore.push_back(operand.getType());
    SmallVector<Location> bbArgLocsBefore(argsTypesBefore.size(), whileOp.getLoc());
    SmallVector<Location> bbArgLocsAfter(returnTypes.size(), whileOp.getLoc());
    rewriter.createBlock(&newWhileOp.getBefore(), {}, argsTypesBefore, bbArgLocsBefore);
    ...
    for (int i = 0; i < whileOp.getNumRegions(); ++i) {
        newWhileOp->getRegion(i).front().getOperations().splice(
            newWhileOp->getRegion(i).front().getOperations().begin(), whileOp->getRegion(i).front().getOperations());
    }
    ...
    for (auto [oldResult, newResult] : llvm::zip(whileOp.getResults(), newWhileOp.getResults()))
        remapArg(oldResult, newResult);
    for (auto [oldArg, newArg] : llvm::zip(whileOp.getBeforeArguments(), newWhileOp.getBeforeArguments()))
        remapArg(oldArg, newArg);
    for (auto [oldArg, newArg] : llvm::zip(whileOp.getAfterArguments(), newWhileOp.getAfterArguments()))
        remapArg(oldArg, newArg);
...
Operation *LayoutPropagation::rewriteIfOp(scf::IfOp ifOp)
{
    ...
    for (unsigned i = 0, e = ifOp->getNumResults(); i < e; ++i) {
        ...
        newResultTypes[i] = RankedTensorType::get(origType.getShape(), origType.getElementType(), encoding);
    }
    auto newIfOp = rewriter.create<scf::IfOp>(ifOp.getLoc(), newResultTypes, ifOp.getCondition(), true, true);
    newIfOp.getThenRegion().takeBody(ifOp.getThenRegion());
    newIfOp.getElseRegion().takeBody(ifOp.getElseRegion());
    for (auto [oldResult, newResult] : llvm::zip(ifOp.getResults(), newIfOp.getResults())) {
        if (oldResult.getType() == newResult.getType()) {
            oldResult.replaceAllUsesWith(newResult);
            continue;
        }
...
void LayoutPropagation::rewriteYieldOp(scf::YieldOp yieldOp)
{
    ...
    for (OpOperand &operand : yieldOp->getOpOperands()) {
        ...
        if (auto whileOp = dyn_cast<scf::WhileOp>(parentOp))
            yieldType = whileOp.getBeforeArguments()[operand.getOperandNumber()].getType();
...
Operation *LayoutPropagation::rewriteOp(Operation *op)
{
    ...
    if (auto convertOp = dyn_cast<ConvertLayoutOp>(op)) {
        ...
        auto newType = RankedTensorType::get(tensorType.getShape(), tensorType.getElementType(), encoding);
    ...
    if (canFoldIntoConversion(op, encoding)) {
        ...
        auto tensorType = cast<RankedTensorType>(op->getResult(0).getType());
        auto newType = RankedTensorType::get(tensorType.getShape(), tensorType.getElementType(), encoding);
        auto cvt = rewriter.create<ConvertLayoutOp>(op->getLoc(), newType, newOp->getResult(0));
        map(op->getResult(0), cvt.getResult());
        return cvt.getOperation();
    }
    if (op->hasTrait<OpTrait::SameOperandsAndResultEncoding>() || op->hasTrait<OpTrait::Elementwise>() ||
        isa<ReduceOp, ExpandDimsOp, ReshapeOp, TransOp, JoinOp, SplitOp, ConvertLayoutOp,
...
void LayoutRematerialization::updateRematMapping(SmallVector<std::tuple<Value, Value>> &values)
{
...
void LayoutRematerialization::rewriteSlice(SetVector<Value> &slice, DenseMap<Value, Attribute> &layout,
                                           ConvertLayoutOp convertOp, IRMapping &mapping)
{
    ...
    for (Operation *op : opsToRewrite) {
        if (auto forOp = dyn_cast<scf::ForOp>(op)) {
            ...
            for (auto arg : forOp.getRegionIterArgs()) {
                if (slice.count(arg)) {
                    ...
                    argMapping.push_back(std::make_pair(forOp.getTiedLoopResult(&initVal).getResultNumber(),
            ...
            scf::ForOp newForOp = replaceForOpWithNewSignature(builder, forOp, newOperands, replacements);
            ...
            for (auto m : argMapping) {
                ...
                mapping.map(loopBody.getArgument(m.first + numIndVars), loopBody.getArgument(m.second + numIndVars));
                LLVM_DEBUG({
                    DBGS() << "mapping forOp " << loopBody.getArgument(m.first + numIndVars) << " to "
                ...
                addRematValue(newForOp.getResult(m.first), layout[oldArg], newForOp.getResult(m.second));
                addRematValue(oldArg, layout[oldArg], loopBody.getArgument(m.second + numIndVars));
            }
            continue;
        }
        if (auto ifOp = dyn_cast<scf::IfOp>(op)) {
            ...
            for (auto res : ifOp.getResults()) {
                if (slice.count(res)) {
                    ...
                    auto newType = RankedTensorType::get(oldType.getShape(), oldType.getElementType(), it->second);
                    newTypes.push_back(newType);
                }
            }
            scf::IfOp newIfOp = replaceIfOpWithNewSignature(builder, ifOp, newTypes, replacements);
            unsigned oldIdx = 0;
            ...
            for (auto res : ifOp.getResults()) {
                if (slice.count(res)) {
                    ...
                    addRematValue(ifOp.getResult(oldIdx), layout[res], newIfOp.getResult(newIdx));
                    ++newIdx;
                }
                ++oldIdx;
            }
        ...
        if (isa<arith::ConstantOp>(op)) {
            ...
            auto tensorType = cast<RankedTensorType>(op->getResult(0).getType());
            auto newType =
                RankedTensorType::get(tensorType.getShape(), tensorType.getElementType(), layout[op->getResult(0)]);
            auto cvt = builder.create<ConvertLayoutOp>(op->getLoc(), newType, newOp->getResult(0));
            mapping.map(op->getResult(0), cvt.getResult());
            addRematValue(op->getResult(0), layout[op->getResult(0)], cvt.getResult());
            continue;
        }
        ...
        for (auto [old, newV] : llvm::zip(op->getResults(), newOp->getResults())) {
            ...
            if (it == layout.end())
                continue;
            auto newType = RankedTensorType::get(cast<RankedTensorType>(old.getType()).getShape(),
...
LogicalResult getRematerializableSlice(Value root, Attribute rootEncoding, SetVector<Value> &slice,
                                       DenseMap<Value, Attribute> &layout,
                                       std::function<bool(Operation *)> stopPropagation = nullptr)
{
    LogicalResult result = getConvertBackwardSlice(root, slice, rootEncoding, layout, stopPropagation);
...
void LayoutRematerialization::backwardRematerialization()
{
    ...
    funcOp.walk([&](ConvertLayoutOp convertOp) { convertOps.push_back(convertOp); });
...
void LayoutRematerialization::backwardRematerialization(ConvertLayoutOp convertOp)
{
    ...
    LDBG("check backward remat with source " << oldV << " encoding " << targetType.getEncoding());
    ...
    DenseMap<Value, Attribute> layout;
    LogicalResult result = getRematerializableSlice(convertOp.getSrc(), targetType.getEncoding(), slice, layout);
    if (result.failed()) {
...
// of the convert.
void LayoutRematerialization::hoistConvertOnTopOfExtOrBroadcast(ConvertLayoutOp convertOp)
{
    // we don't handle conversions to DotOperandEncodingAttr
    ...
    auto isExtOrBroadcastOp = [](Operation *op) {
        if (isa<arith::ExtSIOp, arith::ExtUIOp, arith::ExtFOp, BroadcastOp, ExpandDimsOp>(op)) {
            return true;
        }
        if (auto fpToFpOp = dyn_cast<FpToFpOp>(op)) {
            auto srcType = cast<RankedTensorType>(fpToFpOp.getOperand().getType());
            return getElementBitWidth(srcType) < getElementBitWidth(fpToFpOp.getType());
        }
        return false;
    };
    ...
    LogicalResult result =
        getRematerializableSlice(convertOp.getSrc(), targetType.getEncoding(), slice, layout, isExtOrBroadcastOp);
    if (result.failed())
        return;
    ...
    for (unsigned i = 0; i < sliceSize; i++) {
        ...
        if (isExtOrBroadcastOp(op)) {
            ...
            if (!srcEncoding)
                return;
            LogicalResult result = getRematerializableSlice(op->getOperand(0), *srcEncoding, tempSlice, tempLayout);
    ...
    std::optional<Attribute> srcEncoding = inferSrcEncoding(extOrBroadcatOp, dstEncoding);
    if (!srcEncoding)
        return;
    // Move the convert before the ext op and rewrite the slice.
    OpBuilder builder(extOrBroadcatOp);
    auto tensorType = cast<RankedTensorType>(extOrBroadcatOp->getOperand(0).getType());
    auto newType = RankedTensorType::get(tensorType.getShape(), tensorType.getElementType(), *srcEncoding);
    auto newConvertOp = builder.create<ConvertLayoutOp>(convertOp.getLoc(), newType, extOrBroadcatOp->getOperand(0));
    ...
    auto oldExtOrBroadcastType = cast<RankedTensorType>(extOrBroadcatOp->getResult(0).getType());
    Type newExtOrBroadcasrType =
...
class TritonGPURemoveLayoutConversionsPass
    : public impl::TritonGPURemoveLayoutConversionsBase<TritonGPURemoveLayoutConversionsPass> {
  public:
