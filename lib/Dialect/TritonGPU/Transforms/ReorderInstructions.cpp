...
static bool willIncreaseRegisterPressure(Operation *op)
{
    ...
    if (!cvt)
        return false;
    if (mlir::isa<triton::gpu::DotOperandEncodingAttr>(cvt.getType().getEncoding()))
        return true;
    return false;
}

class TritonGPUReorderInstructionsPass
    : public impl::TritonGPUReorderInstructionsBase<TritonGPUReorderInstructionsPass> {
  public:
    ...
    void runOnOperation() override
    {
        ...
        auto moveAfter = [](Operation *lhs, Operation *rhs) { lhs->moveAfter(rhs); };
        m.walk([&](Operation *op) {
            ...
            if (std::distance(user_begin, user_end) != 1)
                return;
            if (user_begin->getParentOfType<scf::ForOp>() == op->getParentOfType<scf::ForOp>())
                return;
        ...
        m.walk([&](triton::gpu::LocalLoadOp op) {
            auto dstEncoding = mlir::dyn_cast<triton::gpu::DotOperandEncodingAttr>(op.getType().getEncoding());
            if (!dstEncoding)
                return;
            ...
            if (!dotUser)
                return;
            auto AOp = dotUser.getOperand(0).getDefiningOp<triton::gpu::LocalLoadOp>();
            if (!AOp)
                return;
