...
void getAsyncTaskOps(triton::FuncOp funcOp, DenseSet<Operation *> &asyncTaskOps, int asyncTaskId)
{
    funcOp.walk([&](Operation *op) -> void {
        if (auto attr = op->getAttrOfType<mlir::DenseIntElementsAttr>("async_task_id")) {
...
void getAllParentOps(triton::FuncOp funcOp, DenseSet<Operation *> &parentOps, int asyncTaskId)
{
...
void populateUnlabledOpsAtLast(triton::FuncOp funcOp, ArrayRef<int> allAsyncTasks)
{
    ...
    funcOp.walk([&](Operation *op) -> void {
        if (isa<ModuleOp>(op) || isa<triton::FuncOp>(op) || isa<triton::ReturnOp>(op)) {
            return;
        }
        if (!op->hasAttr("async_task_id")) {
...
struct AsyncTaskIdsCompare {
    ...
    static bool isEqual(const SmallVector<AsyncTaskId> &LHS, const SmallVector<AsyncTaskId> &RHS) { return LHS == RHS; }
};

// Make sure the def chain contains the right taskId.
bool verifyTaskId(triton::FuncOp &funcOp, const llvm::DenseSet<Operation *> &anchorOps)
{
    bool retCode = true;
    ...
    funcOp.walk([&](Operation *op) {
        ...
        for (Value operand : op->getOperands()) {
            ...
            assert(oneVecCoversTheOther(defTaskIds, asyncTaskIds) && "defTaskIds should cover asyncTaskIds");
        }
    });
    return retCode;
}
...
void backwardPropagateTaskIds(Operation *op, const llvm::DenseSet<Operation *> &anchors)
{
    ...
    while (!queue.empty()) {
        ...
        if (auto blockArg = dyn_cast<BlockArgument>(value)) {
            ...
            if (auto forOp = dyn_cast<scf::ForOp>(parent)) {
                ...
                auto control = forOp.getOperands().take_front(forOp.getNumControlOperands());
...
void backwardPropagateTaskIds(llvm::DenseSet<Operation *> &rootOps, llvm::DenseSet<Operation *> &anchorOps)
{
...
void forwardPropagateTaskIds(Operation *root, const llvm::DenseSet<Operation *> &anchors)
{
...
void populateTaskIdsForControlDependencies(llvm::DenseSet<Operation *> &anchorOps)
{
...
class TritonGPUTaskIdPropagatePass : public impl::TritonGPUTaskIdPropagateBase<TritonGPUTaskIdPropagatePass> {
  public:
    using impl::TritonGPUTaskIdPropagateBase<TritonGPUTaskIdPropagatePass>::TritonGPUTaskIdPropagateBase;
    ...
    void runOnFuncOp(triton::FuncOp funcOp)
    {
        ...
        funcOp.walk([&](mlir::Operation *op) {
            auto asyncTasks = getAsyncTaskIds(op);
            if (!asyncTasks.empty() && !isa<arith::ConstantOp, arith::ConstantIntOp>(op))
                rootOps.insert(op);
        });
        ...
        SmallVector<int> allAsyncTasksVec(allAsyncTasks.begin(), allAsyncTasks.end());
