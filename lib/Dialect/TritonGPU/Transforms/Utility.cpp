...
SmallVector<unsigned, 3> mmaVersionToInstrShape(int version, const ArrayRef<int64_t> &shape, Type eltType, int numWarps)
{
    if (version == 1)
        return {16, 16};
    else if (version == 2) {
        ...
        if (eltType.isFloat8E5M2() || eltType.isFloat8E4M3FN() || eltType.isFloat8E4M3FNUZ() || eltType.isF16() ||
            eltType.isBF16() || eltType.isF32())
        {
            validN.assign({256, 248, 240, 232, 224, 216, 208, 200, 192, 184, 176, 168, 160, 152, 144, 136,
                           128, 120, 112, 104, 96,  88,  80,  72,  64,  56,  48,  40,  32,  24,  16,  8});
        }

        if (eltType.isInteger(8)) {
            validN.assign({224, 208, 192, 176, 160, 144, 128, 112, 96, 80, 64, 48, 32, 24, 16, 8});
        }
...
SmallVector<unsigned, 4> argSort(const SmallVector<int64_t> &arr)
{
    ...
    std::stable_sort(ret.begin(), ret.end(), [&](unsigned x, unsigned y) { return arr[x] > arr[y]; });
    return ret;
}
...
{
    auto typeForMem = isa<PointerType>(type.getElementType())
                          ? cast<PointerType>(type.getElementType()).getPointeeType()
                          : type.getElementType();
    return typeForMem.getIntOrFloatBitWidth();
}

unsigned getNumElementsPerThread(Operation *op, SmallVector<unsigned> order, ModuleAxisInfoAnalysis &axisInfoAnalysis)
{
    ...
    unsigned maxContig = std::min(valInfo.getContiguity(order[0]), shapePerCTA[order[0]]);
    unsigned alignment = std::min(maxMultiple, maxContig);
    unsigned currPerThread = std::min(alignment, 128 / elemNumBits);
    LDBG("elemNumBytes: " << elemNumBytes << ", divisibility: " << maxMultipleBytes
                          << ", contig: " << valInfo.getContiguity(order[0]) << ", alignment: " << alignment);
...
void GraphDumper::dumpToFile(triton::FuncOp func, const std::string &filename) const
{
...
std::string GraphDumper::emitNode(const std::string &id, const GraphDumper::NodeInfo info) const
{
    std::ostringstream oss;
    oss << "\"" << id << "\" [";
...
std::string GraphDumper::emitEdge(const std::string &srcId, const std::string &destId) const
{
    std::ostringstream oss;
    oss << "\"" << srcId << "\" -> \"" << destId << "\";";
    return oss.str();
...
std::string GraphDumper::emitValueNode(Value value) const
{
    ...
    if (info.find("label") == info.end()) {
        ...
        if (auto arg = mlir::dyn_cast<BlockArgument>(value))
            info["label"] = "BlockArg" + std::to_string(arg.getArgNumber()) + " " + shapeStr;
        else
            info["label"] = shapeStr;
    }
...
static std::optional<Attribute> inferDstEncoding(triton::ReduceOp op, Attribute encoding)
{
    return triton::gpu::SliceEncodingAttr::get(op->getContext(), op.getAxis(), encoding);
}

static std::optional<Attribute> inferDstEncoding(triton::ExpandDimsOp op, Attribute encoding)
{
...
static std::optional<Attribute> inferSrcEncoding(triton::ReduceOp op, Attribute encoding)
{
    auto sliceEncoding = mlir::dyn_cast<triton::gpu::SliceEncodingAttr>(encoding);
    if (!sliceEncoding)
        return std::nullopt;
...
static std::optional<Attribute> inferSrcEncoding(triton::ExpandDimsOp op, Attribute encoding)
{
    return triton::gpu::SliceEncodingAttr::get(op->getContext(), op.getAxis(), encoding);
}
...
static std::optional<Attribute> inferTransOpDstEncoding(Attribute srcEnc, ArrayRef<int32_t> order)
{
    // Simply forward to the existing inferTransOpEncoding function.
    Attribute retEncoding;
    if (succeeded(
            srcEnc.getDialect().getRegisteredInterface<triton::DialectInferLayoutInterface>()->inferTransOpEncoding(
                srcEnc, order, retEncoding)))
    {
        return retEncoding;
    }
    return std::nullopt;
...
static std::optional<Attribute> inferDstEncoding(triton::TransOp op, Attribute encoding)
{
    return inferTransOpDstEncoding(encoding, op.getOrder());
}

static std::optional<Attribute> inferSrcEncoding(triton::TransOp op, Attribute encoding)
{
    ...
    return inferTransOpDstEncoding(encoding, triton::inversePermutation(op.getOrder()));
}

static std::optional<Attribute> inferReshapeOpDstEncoding(ArrayRef<int64_t> srcShape, Attribute srcEnc,
                                                          ArrayRef<int64_t> dstShape, bool allowReorder)
{
    ...
    Attribute dstEnc;
    if (succeeded(srcEnc.getDialect()
                      .getRegisteredInterface<triton::DialectInferLayoutInterface>()
                      ->inferReshapeOpNoReorderEncoding(srcShape, srcEnc, dstShape, dstEnc, /*loc=*/std::nullopt)))
    {
        return dstEnc;
    }
    return std::nullopt;
...
static std::optional<Attribute> inferDstEncoding(triton::ReshapeOp op, Attribute encoding)
{
    return inferReshapeOpDstEncoding(op.getSrc().getType().getShape(), encoding, op.getType().getShape(),
                                     op.getAllowReorder());
}

static std::optional<Attribute> inferSrcEncoding(triton::ReshapeOp op, Attribute encoding)
{
    ...
    // way.
    return inferReshapeOpDstEncoding(op.getType().getShape(), encoding, op.getSrc().getType().getShape(),
                                     op.getAllowReorder());
}
...
std::optional<Attribute> inferSrcEncoding(Operation *op, Attribute encoding)
{
    ...
    if (op->hasTrait<mlir::OpTrait::SameOperandsAndResultEncoding>() ||
        ...
        isa<scf::WhileOp, scf::YieldOp, scf::ConditionOp, nvidia_gpu::WarpGroupDotWaitOp>(op))
    {
        return encoding;
    }
...
std::optional<Attribute> inferDstEncoding(Operation *op, Attribute encoding)
{
    ...
    if (op->hasTrait<mlir::OpTrait::SameOperandsAndResultEncoding>() ||
        op->hasTrait<mlir::OpTrait::SameLoadStoreOperandsAndResultEncoding>() ||
        op->hasTrait<mlir::OpTrait::Elementwise>() ||
        isa<scf::WhileOp, scf::ForOp, scf::YieldOp, scf::ConditionOp, nvidia_gpu::WarpGroupDotWaitOp>(op))
        return encoding;
    if (auto reduceOp = dyn_cast<triton::ReduceOp>(op))
...
bool isExpensiveToRemat(Operation *op, Attribute &targetEncoding)
{
    ...
    if (isa<triton::gpu::AsyncCopyGlobalToLocalOp, triton::AtomicRMWOp, triton::AtomicCASOp, triton::DotOp>(op))
        return true;
    if (isa<scf::YieldOp, scf::ForOp, scf::IfOp, scf::WhileOp, scf::ConditionOp>(op))
        return true;
    return false;
}
...
bool canFoldIntoConversion(Operation *op, Attribute targetEncoding)
{
    ...
    if (auto reshape = dyn_cast<triton::ReshapeOp>(op)) {
        ...
        RankedTensorType newDstType =
            RankedTensorType::get(reshapeDstType.getShape(), reshapeDstType.getElementType(), targetEncoding);
        return reshape.getAllowReorder() && !reshape.getEfficientLayout() &&
               !triton::gpu::isExpensiveView(reshape.getSrc().getType(), newDstType);
    }
    return isa<triton::gpu::ConvertLayoutOp, arith::ConstantOp, triton::MakeRangeOp, triton::SplatOp,
               triton::HistogramOp, triton::gpu::LocalAllocOp, triton::gpu::LocalStoreOp>(op);
}
                                        ...
                                        SmallVectorImpl<std::tuple<Value, Value>> &replacements)
{
    ...
    scf::ForOp newLoop = rewriter.create<scf::ForOp>(loop.getLoc(), loop.getLowerBound(), loop.getUpperBound(),
                                                     loop.getStep(), operands);
    newLoop->setAttrs(loop->getAttrs());
    newLoop.getBody()->erase();
    newLoop.getRegion().getBlocks().splice(newLoop.getRegion().getBlocks().begin(), loop.getRegion().getBlocks());
    for (Value operand : newIterOperands)
        newLoop.getBody()->addArgument(operand.getType(), operand.getLoc());

    for (auto it : llvm::zip(loop.getResults(), newLoop.getResults().take_front(loop.getNumResults())))
...
scf::ForOp replaceForOpWithNewSignature(RewriterBase &rewriter, scf::ForOp loop, ValueRange newIterOperands)
{
    ...
    auto newForOp = replaceForOpWithNewSignature(rewriter, loop, newIterOperands, replacements);
                                            ...
                                            SmallVectorImpl<std::tuple<Value, Value>> &replacements)
{
    ...
    scf::WhileOp newLoop = rewriter.create<scf::WhileOp>(loop.getLoc(), resultTypes, operands);
    newLoop->setAttrs(loop->getAttrs());
    ...
    rewriter.createBlock(&newLoop.getBefore(), {}, argsTypesBefore, bbArgLocsBefore);
    rewriter.createBlock(&newLoop.getAfter(), {}, resultTypes, bbArgLocsAfter);

    // Copy regions
    for (int i = 0; i < loop.getNumRegions(); ++i)
        newLoop->getRegion(i).front().getOperations().splice(newLoop->getRegion(i).front().getOperations().begin(),
                                                             loop->getRegion(i).front().getOperations());

    // Remap arguments
    for (auto [oldArg, newArg] : llvm::zip(loop.getBeforeArguments(),
                                           newLoop.getBeforeArguments().take_front(loop.getBeforeArguments().size())))
        rewriter.replaceAllUsesWith(oldArg, newArg);
    for (auto [oldArg, newArg] :
         llvm::zip(loop.getAfterArguments(), newLoop.getAfterArguments().take_front(loop.getAfterArguments().size())))
        rewriter.replaceAllUsesWith(oldArg, newArg);
...
{
    SmallVector<std::tuple<Value, Value>> replacements;
    auto newWhileOp = replaceWhileOpWithNewSignature(rewriter, loop, newIterOperands, newResultTypes, replacements);
    for (auto &kv : replacements) {
        rewriter.replaceAllUsesWith(std::get<0>(kv), std::get<1>(kv));
    }
    return newWhileOp;
                                      ...
                                      SmallVectorImpl<std::tuple<Value, Value>> &replacements)
{
    ...
    scf::IfOp newIf = rewriter.create<scf::IfOp>(ifOp.getLoc(), resultTypes, ifOp.getCondition(), /*withElse=*/true);
    newIf->setAttrs(ifOp->getAttrs());

    rewriter.inlineBlockBefore(ifOp.thenBlock(), newIf.thenBlock(), newIf.thenBlock()->begin());
    rewriter.inlineBlockBefore(ifOp.elseBlock(), newIf.elseBlock(), newIf.elseBlock()->begin());

    for (auto it : llvm::zip(ifOp.getResults(), newIf.getResults().take_front(ifOp.getNumResults())))
        replacements.push_back(it);
    return newIf;
}
...
{
    SmallVector<std::tuple<Value, Value>> replacements;
    auto newIfOp = replaceIfOpWithNewSignature(rewriter, ifOp, newResultTypes, replacements);
    for (auto &kv : replacements)
        rewriter.replaceAllUsesWith(std::get<0>(kv), std::get<1>(kv));
    return newIfOp;
}
...
Operation *cloneWithInferType(mlir::OpBuilder &rewriter, Operation *op, IRMapping &mapping)
{
    ...
    if (typeInfer) {
        ...
        auto success = typeInfer.inferReturnTypes(newOp->getContext(), newOp->getLoc(), newOp->getOperands(),
...
static bool isFreeConvert(Operation *op)
{
    ...
    if (!convertOp)
        return false;
    return cvtReordersRegisters(convertOp.getSrc().getType(), convertOp.getType());
}
                                      ...
                                      std::function<bool(Operation *)> stopPropagation)
{
    ...
    while (!queue.empty()) {
        ...
        if (auto forOp = dyn_cast<scf::ForOp>(parentOp)) {
            ...
            Value yieldOperand =
                forOp.getBody()->getTerminator()->getOperand(blockArg.getArgNumber() - forOp.getNumInductionVars());
...
Value linearize(OpBuilder &b, Location loc, ArrayRef<Value> multiDim, ArrayRef<unsigned> shape,
                ArrayRef<unsigned> order)
{
    return linearize(b, loc, triton::applyPermutation(multiDim, order), triton::applyPermutation(shape, order));
}
...
Value linearize(OpBuilder &b, Location loc, ArrayRef<Value> multiDim, ArrayRef<unsigned> shape)
{
    ...
    if (rank > 0) {
        ...
        for (auto [dim, dimShape] : llvm::reverse(llvm::zip(multiDim.drop_back(), shape.drop_back()))) {
            ...
            linear = b.create<arith::AddIOp>(loc, b.create<arith::MulIOp>(loc, linear, dimSize), dim);
        }
    }
    return linear;
}
...
bool isPureUnaryInlineAsm(Operation *op)
{
    ...
    if (!inlineAsmOp)
        return false;
    return op->getNumOperands() == 1 && op->getNumResults() == 1 && inlineAsmOp.getPure();
}

int getNVIDIAComputeCapability(Operation *module)
{
    assert(module->hasAttr(triton::AttrTargetName) && "Expected a target attribute on the module operation");

    StringAttr targetAttr = cast<StringAttr>(module->getAttr(triton::AttrTargetName));

    StringRef ref = targetAttr.strref();
    assert(ref.starts_with("cuda:") && "expected target attribute to be prefixed with \"cuda:\"");
    ...
    assert(!parseError && "invalid compute capability string in target attribute");
    ...
    LogicalResult matchAndRewrite(scf::ForOp forOp, PatternRewriter &rewriter) const final
    {
        ...
        while (!queue.empty()) {
            ...
            if (auto nestedFor = value.getDefiningOp<scf::ForOp>()) {
                ...
                auto nestedYieldOp = cast<scf::YieldOp>(nestedFor.getBody()->getTerminator());
                Value nestedYieldOperand = nestedYieldOp.getOperand(result.getResultNumber());
            ...
            if (auto nestedIf = value.getDefiningOp<scf::IfOp>()) {
                ...
                for (scf::YieldOp nestedYieldOp : {nestedIf.thenYield(), nestedIf.elseYield()}) {
                    Value nestedYieldOperand = nestedYieldOp.getOperand(result.getResultNumber());
                    markLive(nestedYieldOperand);
                }
                continue;
            ...
            if (auto forOwner = dyn_cast<scf::ForOp>(arg.getOwner()->getParentOp())) {
                ...
                Value yieldOperand = forOwner.getBody()->getTerminator()->getOperand(iterIdx);
        ...
        for (auto yieldOperand : llvm::enumerate(yieldOp->getOperands())) {
            ...
            if (!forOp->isAncestor(yieldOperand.value().getParentRegion()->getParentOp()) &&
