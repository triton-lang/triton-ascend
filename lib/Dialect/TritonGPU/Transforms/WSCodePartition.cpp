...
std::pair<int, bool> scanRegUsage(Block *block, AsyncTaskId asyncTaskId, int regDecProducer, int regIncConsumer)
{
...
unsigned getNumBuffersOrDefault(scf::ForOp forOp, unsigned numBuffers)
{
    ...
    if (!forOp->hasAttr(mlir::triton::kNumStagesAttrName))
        return numBuffers;
    return mlir::cast<IntegerAttr>(forOp->getAttr(mlir::triton::kNumStagesAttrName)).getInt();
}

// Collect argument indices that are used by the specific taskId.
static SmallVector<unsigned> collectBlockArgsForTask(scf::ForOp forOp, int asyncTaskId)
{
    ...
    for (unsigned i = forOp.getNumInductionVars(); i < block.getNumArguments(); ++i) {
...
Operation *SpecializeOp(Operation *op, IRMapping &mapping, OpBuilderWithAsyncTaskIds &builder, AsyncTaskId asyncTaskId);
...
static bool needAccumulatedLoopCnt(scf::IfOp ifOp, SmallVector<Operation *> &loopWithBufferReuse)
{
...
Value updateAccumLoopCount(SmallVector<Operation *> &opList, unsigned numBuffers, SmallVector<Operation *> &taskTopOps,
                           Operation *commonOuterLoop, SmallVector<Operation *> &loopWithBufferReuse, Value prevAccum);

scf::ForOp createNewLoopWrapper(scf::ForOp origForOp, unsigned numBuffers, SmallVector<Operation *> &taskTopOps,
                                Operation *commonOuterLoop, SmallVector<Operation *> &loopWithBufferReuse,
                                Value prevAccum);
...
scf::IfOp rewriteIfOp(scf::IfOp ifOp, unsigned numBuffers, SmallVector<Operation *> &taskTopOps,
                      Operation *commonOuterLoop, SmallVector<Operation *> &loopWithBufferReuse, Value prevAccum)
{
    LLVM_DEBUG({
    ...
    auto newIfOp =
        ifBuilder.createWithAsyncTaskIds<scf::IfOp>(ifOp.getLoc(), newResultTypes, ifOp.getCondition(), true, true);
    ...
    Value endAccum =
        updateAccumLoopCount(opList, numBuffers, taskTopOps, commonOuterLoop, loopWithBufferReuse, prevAccum);
    ...
    if (ifOp.elseBlock()) {
        ...
        auto yieldOp = newIfOp.getElseBodyBuilder().create<scf::YieldOp>(ifOp.getLoc());
        endAccum = prevAccum;
    }
...
Operation *SpecializeIfOp(scf::IfOp ifOp, IRMapping &mapping, OpBuilderWithAsyncTaskIds &builder,
                          AsyncTaskId asyncTaskId)
{
    ...
    auto newIfOp = builder.createWithAsyncTaskIds<scf::IfOp>(
        ifOp.getLoc(), newResultTypes, mapping.lookup(ifOp.getCondition()), true, ifOp.elseBlock());
...
Operation *SpecializeForOp(scf::ForOp forOp, IRMapping &mapping, OpBuilderWithAsyncTaskIds &builder,
                           AsyncTaskId asyncTaskId)
{
    ...
    // Create newForOp.
    auto newForOp =
        builder.createWithAsyncTaskIds<scf::ForOp>(forOp.getLoc(), newLowerBound, newUpperBound, newStep, newLoopArgs);
    ...
    if (newForOp.getBody()->mightHaveTerminator()) {
        auto initialYield = llvm::cast<scf::YieldOp>(newForOp.getBody()->getTerminator());
    ...
    if (createNewYield) {
        auto newYieldOp = forBuilder.create<scf::YieldOp>(yieldOp.getLoc(), newYieldOperands);
...
Operation *SpecializeOp(Operation *op, IRMapping &mapping, OpBuilderWithAsyncTaskIds &builder, AsyncTaskId asyncTaskId)
{
    ...
    if (op->getNumRegions() == 0) {
        ...
        if (auto ifOp = dyn_cast<scf::IfOp>(op)) {
            ...
            newOp->walk([&](Operation *childOp) { setAsyncTaskIds(childOp, asyncTaskId); });
            for (unsigned i = 0; i < op->getNumResults(); ++i)
                mapping.map(op->getResult(i), newOp->getResult(i));
            return newOp;
        } else {
...
DenseMap<AsyncTaskId, scf::IfOp> SpecializeRegion(triton::FuncOp funcOp, int regDecProducer, int regIncConsumer)
{
    ...
    for (AsyncTaskId asyncTaskId : getNestedAsyncTaskIds(funcOp)) {
        ...
        Value cond = builder.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, curAsyncTaskId,
    ...
    for (auto ifOps : tasksToIfOp) {
        ...
        taskBuilder.setAsynTaskIdsFromArray({asyncTaskId});
        auto regAlloc = scanRegUsage(ifOp.thenBlock(), asyncTaskId, regDecProducer, regIncConsumer);
        taskBuilder.setInsertionPointToStart(&(ifOp.getThenRegion().front()));
        if (regAlloc.second)
            taskBuilder.create<ttng::RegAllocOp>(loc, taskBuilder.getI32IntegerAttr(regAlloc.first));
        else
            taskBuilder.create<ttng::RegDeallocOp>(loc, taskBuilder.getI32IntegerAttr(regAlloc.first));
    }
...
struct Channel {
  public:
    ...
    Channel(int producer, SmallVector<int> &consumers, Operation *op, unsigned operandIdx, unsigned numBuffers)
        : relation(producer, consumers), op(op), operandIdx(operandIdx), numBuffers(numBuffers)
    {
    }

    bool operator==(const Channel &c) { return relation == c.relation && operandIdx == c.operandIdx && op == c.op; }
...
void getTransitiveUsers(Value root, SetVector<std::pair<Operation *, unsigned>> &users)
{
    for (Operation *userOp : root.getUsers()) {
        if (auto yieldOp = dyn_cast<scf::YieldOp>(userOp)) {
            for (OpOperand &operand : yieldOp->getOpOperands()) {
                if (operand.get() == root) {
                    auto result = yieldOp->getParentOp()->getResult(operand.getOperandNumber());
            ...
            assert(operandIndex < userOp->getNumOperands() && "root is not an operand of userOp");
...
void collectAsyncChannels(SmallVector<std::unique_ptr<Channel>> &channels, triton::FuncOp &funcOp, unsigned numBuffers)
{
    funcOp.walk([&](Operation *op) {
        if (isa<tt::LoadOp, tt::ExperimentalDescriptorLoadOp>(op) || op->hasTrait<OpTrait::DotLike>()) {
            ...
            for (auto result : op->getResults()) {
                ...
                for (auto user : users) {
                    ...
                    auto iter = std::remove(consumerTaskIds.begin(), consumerTaskIds.end(), producerTaskId);
                    ...
                    if (consumerTaskIds.size() > 0) {
                        channels.push_back(std::make_unique<Channel>(producerTaskId, consumerTaskIds, userOp,
                   ...
                   SmallVector<Channel *> &orderedChannels)
{
    ...
    auto channelCanBeMerged = [](Channel *c1, Channel *c2) -> bool {
        ...
        if (dst1 == dst2)
            return true;
        if (dst1->getBlock() != dst2->getBlock() || !dst1->hasOneUse() || !dst2->hasOneUse())
            return false;
    ...
    for (auto &kv : consumerChannels) {
        if (kv.second.size() > 1) {
            ...
            std::sort(kv.second.begin(), kv.second.end(), [&](Channel *a, Channel *b) {
                auto itrA = std::find_if(allOps.begin(), allOps.end(), [&](Operation &op) {
                    Operation *opPointer = &op;
                    return opPointer == a->getSrcOp();
                });
                auto itrB = std::find_if(allOps.begin(), allOps.end(), [&](Operation &op) {
                    Operation *opPointer = &op;
...
void reorderProducerOps(SmallVector<Channel *> &channels)
{
    ...
    for (auto &group : groupedProducerOps) {
        std::sort(group.second.begin(), group.second.end(),
                  [&](Channel *a, Channel *b) { return a->relation.second.size() < b->relation.second.size(); });
    }
...
Value getNumSteps(scf::ForOp forOp, OpBuilderWithAsyncTaskIds &builder)
{
    ...
    // numSteps = ((upperBound - lowerBound) + forOpStep - 1) / forOpStep
    Value numSteps = builder.createWithAsyncTaskIds<arith::SubIOp>(loc, forOp.getUpperBound(), forOp.getLowerBound());
    numSteps = builder.createWithAsyncTaskIds<arith::AddIOp>(loc, numSteps, forOp.getStep());
    if (forOp.getStep().getType() != builder.getI64Type())
        numSteps = builder.createWithAsyncTaskIds<arith::ExtSIOp>(loc, builder.getI64Type(), numSteps);
    ...
    if (forOp.getStep().getType() != builder.getI64Type())
        innerForStep = builder.createWithAsyncTaskIds<arith::ExtSIOp>(loc, builder.getI64Type(), forOp.getStep());
    numSteps = builder.createWithAsyncTaskIds<arith::DivUIOp>(loc, numSteps, innerForStep);
    return numSteps;
}
...
scf::ForOp createNewLoop(scf::ForOp forOp, int numBuffers, scf::ForOp &parentForOp, Value accumulatedLoopCount,
                         SmallVector<Operation *> &loopWithBufferReuse, bool hasParallelReuse, bool isOuterOfReuse)
{
    auto loc = forOp.getLoc();
    ...
    Value numBuffersVal = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(loc, numBuffers, 32);

    // Step 1: Append bufferIdx and phase as forOp arguments.
    Value tmpAccumLoopCount;
    if (isOuterOfReuse) {
        tmpAccumLoopCount = body->insertArgument(body->getNumArguments(), builder.getI64Type(), loc);
    }
    Value phase = body->insertArgument(body->getNumArguments(), builder.getI1Type(), loc);
    Value bufferIdx = body->insertArgument(body->getNumArguments(), builder.getI32Type(), loc);
    ...
    //   nextBufferIdx = bufferIdx + 1
    Value nextBufferIdx = builder.createWithAsyncTaskIds<arith::AddIOp>(loc, bufferIdx, one);
    Value bufferGECond =
        builder.createWithAsyncTaskIds<arith::CmpIOp>(loc, arith::CmpIPredicate::uge, nextBufferIdx, numBuffersVal);
    Value bufferLTCond =
        builder.createWithAsyncTaskIds<arith::CmpIOp>(loc, arith::CmpIPredicate::ult, nextBufferIdx, numBuffersVal);
    // nextBufferIdx >= numBuffers ? nextBufferIdx - numBuffers : nextBufferIdx
    Value moduloBufferIdx = builder.createWithAsyncTaskIds<arith::SubIOp>(loc, nextBufferIdx, numBuffersVal);
    nextBufferIdx =
        builder.createWithAsyncTaskIds<mlir::arith::SelectOp>(loc, bufferGECond, moduloBufferIdx, nextBufferIdx);

    // nextPhase = ((nextBufferIdx < numBuffers && curPhase) ||
    //              (nextBufferIdx >= numBuffers && curPhase^1))
    Value flipPhase = builder.createWithAsyncTaskIds<mlir::arith::XOrIOp>(loc, phase, _1_1b);
    Value cond0 = builder.createWithAsyncTaskIds<mlir::arith::AndIOp>(loc, bufferGECond, flipPhase);
    Value cond1 = builder.createWithAsyncTaskIds<mlir::arith::AndIOp>(loc, bufferLTCond, phase);
    Value nextPhase = builder.createWithAsyncTaskIds<mlir::arith::OrIOp>(loc, cond0, cond1);
    ...
    if (isOuterOfReuse) {
        ...
        yieldOp->insertOperands(yieldOp.getNumOperands(), {nextTmpIdx, nextPhase, nextBufferIdx});
    } else
    ...
    if (parentForOp) {
        if (hasParallelReuse) {
            ...
            // numSteps
            Value parentIterIdx = builder.createWithAsyncTaskIds<arith::SubIOp>(loc, parentForOp.getInductionVar(),
                                                                                parentForOp.getLowerBound());
            parentIterIdx = builder.createWithAsyncTaskIds<arith::DivUIOp>(loc, parentIterIdx, parentForOp.getStep());
            if (parentForOp.getStep().getType() != builder.getI64Type())
                parentIterIdx =
                    builder.createWithAsyncTaskIds<arith::ExtSIOp>(loc, builder.getI64Type(), parentIterIdx);
            initBufferIdx = builder.createWithAsyncTaskIds<arith::MulIOp>(loc, parentIterIdx, numSteps);
        }

        numBuffersVal = builder.createWithAsyncTaskIds<arith::ExtSIOp>(loc, builder.getI64Type(), numBuffersVal);
        // Calculate tmpIdx / numBuffers
        // initBufferIdx = tmpIdx - tmpIdx / numBuffers * numBuffers
        // initPhase = (tmpIdx / numBuffers) & 1
        Value bufferIdx = builder.createWithAsyncTaskIds<arith::DivUIOp>(loc, initBufferIdx, numBuffersVal);
        initBufferIdx = builder.createWithAsyncTaskIds<arith::SubIOp>(
            loc, initBufferIdx, builder.createWithAsyncTaskIds<arith::MulIOp>(loc, bufferIdx, numBuffersVal));
        initBufferIdx = builder.createWithAsyncTaskIds<arith::TruncIOp>(loc, builder.getI32Type(), initBufferIdx);

        Value one = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(loc, 1, 64);
        bufferIdx = builder.createWithAsyncTaskIds<arith::AndIOp>(loc, bufferIdx, one);
        initPhase = builder.createWithAsyncTaskIds<arith::TruncIOp>(loc, builder.getI1Type(), bufferIdx);
    } else {
        if (hasParallelReuse) {
    ...
    if (isOuterOfReuse) {
        ...
        Value initTmpIdx = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(loc, 0, 64);
    ...
    auto newForOp = builder.createWithAsyncTaskIds<scf::ForOp>(loc, forOp.getLowerBound(), forOp.getUpperBound(),
...
SmallVector<Operation *> getTaskTopRegion(triton::FuncOp funcOp, const SmallVector<Channel *> &channels)
{
...
static unsigned getNumChannelsInLoop(scf::ForOp forOp, const SmallVector<Channel *> &channels,
...
bool reuseBuffers(SmallVector<Operation *> &taskTopOps, const SmallVector<Channel *> &channels,
                  DenseMap<Channel *, Channel *> &mapToRepresenting, SmallVector<Operation *> &loopWithBufferReuse)
{
                           ...
                           Operation *commonOuterLoop, SmallVector<Operation *> &loopWithBufferReuse, Value prevAccum)
{
    for (Operation *op : opList) {
        if (auto forOp = dyn_cast<scf::ForOp>(op)) {
            auto newForOp =
                createNewLoopWrapper(forOp, numBuffers, taskTopOps, commonOuterLoop, loopWithBufferReuse, prevAccum);
            ...
            if (hasReuse) {
                ...
                prevAccum = builder.createWithAsyncTaskIds<arith::AddIOp>(newForOp.getLoc(), prevAccum, numSteps);
            }
            ...
            if (needAccumulatedLoopCnt(ifOp, loopWithBufferReuse)) {
                auto newIfOp =
                    rewriteIfOp(ifOp, numBuffers, taskTopOps, commonOuterLoop, loopWithBufferReuse, prevAccum);
                ...
                ifOp->walk<WalkOrder::PreOrder>([&](Operation *subOp) {
                    if (auto forOp = dyn_cast<scf::ForOp>(subOp)) {
                        // Handle forOp.
                        createNewLoopWrapper(forOp, numBuffers, taskTopOps, commonOuterLoop, loopWithBufferReuse,
                                             prevAccum);
                    }
                });
            }
                                ...
                                Value prevAccum)
{
    ...
    bool isOuterOfReuse = commonOuterLoop && commonOuterLoop == origForOp.getOperation();
    bool hasReuse = false;
    for (auto tLoop : loopWithBufferReuse)
    ...
    newForOp = createNewLoop(origForOp, loopNumBuffers, parentForOp, accumulatedLoopCount, loopWithBufferReuse,
                             hasReuse, isOuterOfReuse);
    LLVM_DEBUG({
    ...
    auto asyncTaskLoopForItr = std::find(taskTopOps.begin(), taskTopOps.end(), origForOp.getOperation());
    ...
    auto tmpIter = std::find(loopWithBufferReuse.begin(), loopWithBufferReuse.end(), origForOp.getOperation());
    ...
    Value endAccum = updateAccumLoopCount(opList, numBuffers, taskTopOps, commonOuterLoop, loopWithBufferReuse,
...
Value appendBufferIdxArgs(SmallVector<Operation *> &taskTopOps, unsigned numBuffers,
                          ...
                          SmallVector<Operation *> &loopWithBufferReuse)
{
    ...
    if (loopWithBufferReuse.size() > 1 && !commonOuterLoop) {
        ...
        tmpAccumLoopCount = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(oneFor->getLoc(), 0, 64);
    }
    ...
    updateAccumLoopCount(opList, numBuffers, taskTopOps, commonOuterLoop, loopWithBufferReuse, tmpAccumLoopCount);
...
static Value createBarrierAlloc(triton::FuncOp funcOp, unsigned distance)
{
    ...
    Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(funcOp.getContext());
    Location loc = funcOp.getLoc();
    auto context = funcOp.getContext();
    auto barrierCTALayout = ttg::CTALayoutAttr::get(context, /*CTAsPerCGA=*/ {1},
                                                    /*CTASplitNum=*/ {1}, /*CTAOrder=*/ {0});
    auto barrierEncoding = ttg::SharedEncodingAttr::get(context, 1, 1, 1, {0}, barrierCTALayout);
    Type barrierMemDescType = tt::MemDescType::get({distance}, builder.getI64Type(), barrierEncoding, sharedMemorySpace,
                                                   /*mutableMemory=*/true);
    Type singleBarrierMemDescType =
        tt::MemDescType::get({1}, builder.getI64Type(), barrierEncoding, sharedMemorySpace, /*mutableMemory=*/true);
    Value barrierAlloc = builder.create<mlir::triton::gpu::LocalAllocOp>(loc, barrierMemDescType, Value());
    for (unsigned i = 0; i < distance; i++) {
        Value idx = builder.create<arith::ConstantIntOp>(loc, i, 32);
        Value barrierView = builder.create<ttg::MemDescSubviewOp>(loc, singleBarrierMemDescType, barrierAlloc, idx);
...
createToken(const DenseMap<Channel *, SmallVector<Channel *>> &channelsGroupedByConsumers,
            const SmallVector<Channel *> &orderedChannels, triton::FuncOp funcOp, int numConsumerGroups,
    ...
    for (auto *key : orderedChannels) {
        ...
        for (auto consumerAsyncTaskId : channel->relation.second) {
            ...
            if (it->second.front()->getSrcOp()->getParentOfType<scf::ForOp>()) {
                v = builder.create<ttng::CreateTokenOp>(funcOp.getLoc(), channel->numBuffers);
            } else {
...
DenseMap<Channel *, Value> createBuffer(DenseMap<Channel *, SmallVector<Channel *>> &channelsGroupedByProducers,
                                        ...
                                        DenseMap<Channel *, SmallVector<Channel *>> &channelReuse)
{
    ...
    for (auto &item : channelsGroupedByProducers) {
        ...
        if (auto tensorType = dyn_cast<RankedTensorType>(srcValue.getType())) {
            ...
            auto sharedLayout = ttg::SharedEncodingAttr::get(context, sliceShape, order, CTALayout, elemType);
            ...
            Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(context);
            auto bufferType = RankedTensorType::get(bufferShape, elemType, sharedLayout);
            Type memdescType =
                tt::MemDescType::get(bufferShape, elemType, sharedLayout, sharedMemorySpace, /*mutableMemory*/ true);
            Value buffer = builder.create<ttg::LocalAllocOp>(funcOp.getLoc(), memdescType);
    ...
    for (auto &kv : channelReuse) {
        if (kv.second.size() <= 1)
            continue;
        bufferMap[kv.first].getDefiningOp()->setAttr("allocation.shareGroup",
                                                     IntegerAttr::get(IntegerType::get(context, 32), groupId));
        for (auto *c : kv.second)
            bufferMap[c].getDefiningOp()->setAttr("allocation.shareGroup",
                                                  IntegerAttr::get(IntegerType::get(context, 32), groupId));
        ++groupId;
    }
    return bufferMap;
...
static Operation *createAsyncCopy(const DenseMap<Channel *, Value> &bufferMap, Channel *c, Operation *op,
                                  SmallVector<AsyncTaskId> &asyncTasksPC, Value bufferIdx, Value bufferIdxExtract)
{
    ...
    Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(context);
    tt::MemDescType subviewTy = tt::MemDescType::get(sliceType.getShape(), sliceType.getElementType(),
                                                     sliceType.getEncoding(), sharedMemorySpace,
                                                     /*mutableMemory=*/true);
    Value zero = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(loadOp.getLoc(), 0, 32);
    SmallVector<Value> copyOffsets(sliceType.getRank() + 1, zero);
    copyOffsets[0] = bufferIdx;
    builder.setAsyncTaskIdsFromOp(loadOp);
    builder.setInsertionPointAfter(loadOp);
    auto view = builder.createWithAsyncTaskIds<ttg::MemDescSubviewOp>(loadOp.getLoc(), subviewTy, buffer, copyOffsets);
    // Create cp.async
    Operation *copy = builder.createWithAsyncTaskIds<ttg::AsyncCopyGlobalToLocalOp>(
        loadOp.getLoc(), loadOp.getPtr(), view, loadOp.getMask(), loadOp.getOther(), loadOp.getCache(),
    ...
    auto viewLoad =
        builder.createWithAsyncTaskIds<ttg::MemDescSubviewOp>(loadOp.getLoc(), subviewTy, buffer, loadOffsets);
    auto sharedLoad = builder.createWithAsyncTaskIds<ttg::LocalLoadOp>(loadOp.getLoc(), loadOp.getType(),
...
static void createLocalCopy(const DenseMap<Channel *, Value> &bufferMap, Channel *channel, Value srcBufferIdx,
                            Value dstBufferIdx)
{
    ...
    Value zero = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(dstOp->getLoc(), 0, 32);
    SmallVector<Value> loadOffsets(sliceType.getRank() + 1, zero);
    loadOffsets[0] = dstBufferIdx;
    auto dstView =
        builder.createWithAsyncTaskIds<ttg::MemDescSubviewOp>(dstOp->getLoc(), subviewTy, buffer, loadOffsets);
    auto sharedLoad = builder.createWithAsyncTaskIds<ttg::LocalLoadOp>(dstOp->getLoc(), srcValue.getType(), dstView);
    ...
    zero = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(srcOp->getLoc(), 0, 32);
    SmallVector<Value> storeOffsets(sliceType.getRank() + 1, zero);
    storeOffsets[0] = srcBufferIdx;
    builder.setInsertionPointAfter(srcOp);
    auto srcView =
        builder.createWithAsyncTaskIds<ttg::MemDescSubviewOp>(srcOp->getLoc(), subviewTy, buffer, storeOffsets);
    // Create local_alloc
    Operation *copy = builder.createWithAsyncTaskIds<ttg::LocalStoreOp>(srcOp->getLoc(), srcValue, srcView);
}
...
Value getBarrierForPipelineStage(OpBuilderWithAsyncTaskIds &builder, Value barrierAlloc, Value bufferIdx)
{
    auto context = barrierAlloc.getContext();
    Attribute sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(context);
    tt::MemDescType barrierTy = tt::MemDescType::get(
        {1}, builder.getI64Type(), cast<tt::MemDescType>(barrierAlloc.getType()).getEncoding(), sharedMemorySpace,
        /*mutableMemory=*/true);

    // Create barrierForTMA from barrierAlloc.
    return builder.createWithAsyncTaskIds<ttg::MemDescSubviewOp>(barrierAlloc.getLoc(), barrierTy, barrierAlloc,
                                                                 ArrayRef<Value>({bufferIdx}));
}

Value getBufferForPipelineStage(OpBuilderWithAsyncTaskIds &builder, Type loadType, Value buffer, Value bufferIdx,
                                bool mutableMem)
{
    ...
    Value zero = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(buffer.getLoc(), 0, 32);
    SmallVector<Value> copyOffsets(sliceType.getRank() + 1, zero);
    copyOffsets[0] = bufferIdx;

    return builder.createWithAsyncTaskIds<ttg::MemDescSubviewOp>(buffer.getLoc(), subviewTy, buffer, copyOffsets);
}

Operation *optimizeTMALoads(OpBuilderWithAsyncTaskIds &builder, SmallVector<tt::ExperimentalDescriptorLoadOp> &tmaLoads,
                            SmallVector<Value> &buffers, Value barrierAlloc, Value bufferIdx, Value bufferIdxExtract,
                            Value phase, Operation *headProducer, Operation *headConsumer)
{
    ...
    auto prodBarrier = getBarrierForPipelineStage(builder, barrierAlloc, bufferIdx);
    auto pred = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(loc, 1, 1);
    auto expect = builder.createWithAsyncTaskIds<ttng::BarrierExpectOp>(loc, prodBarrier, sizeInBytes, pred);
    ...
    for (auto [tmaLoad, buffer] : zip(tmaLoads, buffers)) {
        auto pipelineBuffer = getBufferForPipelineStage(builder, tmaLoad.getType(), buffer, bufferIdx, true);
        copy = builder.createWithAsyncTaskIds<ttng::AsyncTMACopyGlobalToLocalOp>(
            loc, tmaLoad.getDescPtr(), tmaLoad.getIndices(), prodBarrier, pipelineBuffer, pred);
    }
    ...
    auto consBarrier = getBarrierForPipelineStage(builder, barrierAlloc, bufferIdxExtract);
    phase = builder.createWithAsyncTaskIds<arith::ExtSIOp>(loc, builder.getI32Type(), phase);
    auto wait = builder.createWithAsyncTaskIds<ttng::WaitBarrierOp>(loc, consBarrier, phase);

    // Convert all the consumers to local_load
    for (auto [tmaLoad, buffer] : zip(tmaLoads, buffers)) {
        auto pipelineBuffer = getBufferForPipelineStage(builder, tmaLoad.getType(), buffer, bufferIdxExtract, false);
        auto sharedLoad = builder.createWithAsyncTaskIds<ttg::LocalLoadOp>(loc, tmaLoad.getType(), pipelineBuffer);
...
// channel.
void insertAsyncComm(triton::FuncOp funcOp,
                     const DenseMap<Channel *, SmallVector<Channel *>> &channelsGroupedByConsumers,
                     ...
                     const DenseMap<Channel *, Value> &bufferMap, int numConsumerGroups)
{
    ...
    auto consumerReleaseHeutistic = [&](Operation *p, Operation *c, int consumerAsyncTaskId) -> Operation * {
    ...
    for (auto kv : channelsGroupedByConsumers) {
        ...
        asyncTasksPC.insert(asyncTasksPC.end(), asyncTaskC.begin(), asyncTaskC.end());
        ...
        if (auto forOp = headProducer->getParentOfType<scf::ForOp>()) {
            ...
            bufferIdx = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(headProducer->getLoc(), 0, 32);
            phase = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(headProducer->getLoc(), 0, 1);
        }
        ...
        for (auto token : tokens) {
            ...
            builder.setInsertionPoint(headProducer);
            builder.createWithAsyncTaskIds<ttng::ProducerAcquireOp>(headProducer->getLoc(), token.second, bufferIdx,
                                                                    phase);
            ...
            if (!isa<tt::ExperimentalDescriptorLoadOp>(headProducer)) {
                ...
                builder.createWithAsyncTaskIds<ttng::ProducerCommitOp>(tailProducer->getLoc(), token.second, bufferIdx);
            }
        }
        ...
        for (auto token : tokens) {
            ...
            if (!isa<tt::ExperimentalDescriptorLoadOp>(headProducer)) {
                ...
                builder.createWithAsyncTaskIds<ttng::ConsumerWaitOp>(headConsumer->getLoc(), token.second, bufferIdx,
                                                                     phase);
            }

            // Insert ConsumerReleaseOp.
            auto consumerReleasePoint = consumerReleaseHeutistic(tailProducer, tailConsumer, token.first);
            builder.setInsertionPointAfter(consumerReleasePoint);
            builder.createWithAsyncTaskIds<ttng::ConsumerReleaseOp>(consumerReleasePoint->getLoc(), token.second,
                                                                    bufferIdx);
        }
        ...
        for (auto &c : kv.second) {
            if (auto tmaLoad = dyn_cast<tt::ExperimentalDescriptorLoadOp>(c->getSrcOp())) {
        ...
        if (tmaLoads.size() > 0) {
            ...
            optimizeTMALoads(builder, tmaLoads, buffers, barrierAlloc, bufferIdx, bufferIdx, phase, headProducer,
                             headConsumer);
        }
    }
}
...
void insertAsyncCopy(triton::FuncOp funcOp,
                     const DenseMap<Channel *, SmallVector<Channel *>> &channelsGroupedByProducers,
                     const DenseMap<Channel *, Value> &bufferMap)
{
    ...
    for (auto kv : channelsGroupedByProducers) {
        ...
        if (auto forOp = srcOp->getParentOfType<scf::ForOp>()) {
            ...
            builder.setAsynTaskIdsFromArray(asyncTasksPC);
            bufferIdx = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(srcOp->getLoc(), 0, 32);
        }
        ...
        for (auto channel : mutuallyNonDominatingChannels) {
            ...
            if (isa<triton::LoadOp>(srcOp)) {
                ...
                // valid.
                createAsyncCopy(bufferMap, channel, channel->getSrcOp(), asyncTasksPC, bufferIdx, bufferIdx);
            } else {
...
class TritonGPUWSCodePartitionPass : public impl::TritonGPUWSCodePartitionBase<TritonGPUWSCodePartitionPass> {
  public:
    using impl::TritonGPUWSCodePartitionBase<TritonGPUWSCodePartitionPass>::TritonGPUWSCodePartitionBase;
    ...
    void runOnFuncOp(triton::FuncOp funcOp)
    {
        ...
        DenseMap<Channel *, SmallVector<Channel *>> channelsGroupedByConsumers;
        SmallVector<Channel *> orderedChannels;
        groupChannels(channels, channelsGroupedByProducers, channelsGroupedByConsumers, orderedChannels);
        ...
        DenseMap<Channel *, Channel *> mapToRepresenting;
        SmallVector<Operation *> loopWithBufferReuse;
        reuseBuffers(asyncTaskTopOps, channels, mapToRepresenting, loopWithBufferReuse);
        // Use and update loopWithBufferReuse.
        appendBufferIdxArgs(asyncTaskTopOps, numBuffers, channels, mapToRepresenting, loopWithBufferReuse);
        LLVM_DEBUG({
        ...
        DenseMap<Channel *, SmallVector<Channel *>> channelReuse;
        DenseMap<Channel *, Value> bufferMap =
            createBuffer(channelsGroupedByProducers, funcOp, numConsumerGroups, mapToRepresenting, channelReuse);
        DenseMap<Channel *, DenseMap<int, Value>> barrierAllocMap;
        DenseMap<Channel *, DenseMap<int, Value>> tokenMap = createToken(
            channelsGroupedByConsumers, orderedChannels, funcOp, numConsumerGroups, channelReuse, barrierAllocMap);
        LLVM_DEBUG({
            LDBG("\n\nafter createBuffer");
            funcOp.dump();
        });
        ...
        // loads.
        insertAsyncComm(funcOp, channelsGroupedByConsumers, tokenMap, barrierAllocMap, bufferMap, numConsumerGroups);
        LLVM_DEBUG({
            LDBG("\n\nwith SyncOps");
            funcOp.dump();
        });
