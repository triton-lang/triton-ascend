...
static bool oneVecCoversTheOther(SmallVector<AsyncTaskId> &one, SmallVector<AsyncTaskId> &other)
{
...
void fixTaskId(triton::FuncOp &funcOp)
{
    funcOp.walk([&](Operation *op) {
        ...
        for (Value operand : op->getOperands()) {
            ...
            if (operand.hasOneUse() && !oneVecCoversTheOther(asyncTaskIds, defTaskIds)) {
...
void getBackwardSliceToPartition(Value root, unsigned dim, int sliceSize, SetVector<Operation *> &backwardSlice)
{
        ...
        if (auto op = v.getDefiningOp()) {
            if (backwardSlice.insert(op)) {
                if (op->hasTrait<OpTrait::Elementwise>() ||
                    isa<arith::ConstantOp, arith::ExtSIOp, arith::ExtUIOp, arith::ExtFOp, BroadcastOp, ExpandDimsOp,
                        MakeRangeOp, SplatOp, ConvertLayoutOp, triton::gpu::LocalAllocOp, LoadOp,
...
void getForwardSliceToPartition(Value root, unsigned dim, int sliceSize, SetVector<Operation *> &forwardSlice)
{
    SmallVector<Value> queue = {root};
...
// root op.
void getSliceToPartition(Value root, unsigned dim, int sliceSize, SetVector<Operation *> &slice)
{
    ...
    for (auto op : forwardSlice) {
        if (op->hasTrait<OpTrait::Elementwise>() || isa<tt::StoreOp, ExperimentalDescriptorStoreOp>(op)) {
            for (OpOperand &operand : op->getOpOperands()) {
                getBackwardSliceToPartition(operand.get(), dim, sliceSize, slice);
            }
        } else if (auto dotOp = dyn_cast<nvidia_gpu::WarpGroupDotOp>(op)) {
            getBackwardSliceToPartition(dim == 0 ? dotOp.getA() : dotOp.getB(), dim, sliceSize, slice);
...
bool computePartitionScheme(triton::FuncOp &funcOp, DataPartitionScheme &partitionScheme)
{
    ...
    int numWarps = TritonGPUDialect::getNumWarps(funcOp->getParentOfType<ModuleOp>());
    for (auto dotOp : dots) {
        ...
        if (partitionScheme.numPartitions == 0) {
            ...
            if (partitionScheme.partitionDim != partitionDim || partitionScheme.numPartitions != asyncTaskIds.size()) {
...
Operation *sliceOp(Value v, int offset, OpBuilderWithAsyncTaskIds &builder, IRMapping &mappings,
                   IRMapping &reverseMappings, DataPartitionScheme &partitionScheme);

Operation *sliceOp(Operation *op, int offset, OpBuilderWithAsyncTaskIds &builder, IRMapping &mappings,
                   IRMapping &reverseMappings, DataPartitionScheme &partitionScheme)
{
    ...
    auto cloneAndSetResultType = [&](Operation *op) {
        ...
        if (!op->getResults().empty()) {
            ...
            if (auto type = dyn_cast<MemDescType>(v.getType())) {
                ...
                shape[dim] = sliceSize;
                auto newType = MemDescType::get(shape, type.getElementType(), type.getEncoding(), type.getMemorySpace(),
                ...
                int sliceSize = shape[dim] / numOfPartitions;
                shape[dim] = sliceSize;
                auto newType = RankedTensorType::get(shape, type.getElementType(), type.getEncoding());
                newV.setType(newType);
            }
    ...
    Operation *newOp;
    if (op->hasTrait<OpTrait::Elementwise>() ||
        isa<ConvertLayoutOp, BroadcastOp, SplatOp, ExpandDimsOp, LocalAllocOp>(op))
    {
        for (Value operand : op->getOperands())
            sliceOp(operand, offset, builder, mappings, reverseMappings, partitionScheme);
        ...
        SmallVector<int64_t> shape {valType.getShape().begin(), valType.getShape().end()};
        int sliceSize = shape[dim] / numOfPartitions;
        shape[dim] = sliceSize;
        auto newValType = valType.clone(shape);
        auto newValAttr = valAttr.resizeSplat(newValType);
        newOp = builder.createWithAsyncTaskIds<arith::ConstantOp>(op->getLoc(), newValAttr);
        ...
        auto newType = RankedTensorType::get({sliceSize}, builder.getI32Type(), type.getEncoding());
        newOp = builder.createWithAsyncTaskIds<MakeRangeOp>(op->getLoc(), newType, newRangeStart, newRangeEnd);
        auto newV = newOp->getResult(0);
        mappings.map(v, newV);
        reverseMappings.map(newV, v);
        ...
        newOp = cloneAndSetResultType(op);
    } else if (isa<ExperimentalDescriptorLoadOp, ExperimentalDescriptorStoreOp>(op)) {
        ...
        if (offset) {
            ...
            newCoordVal = builder.createWithAsyncTaskIds<arith::AddIOp>(op->getLoc(), coordVal, offsetVal);
        ...
        sliceOp(dim == 0 ? dotOp.getA() : dotOp.getB(), offset, builder, mappings, reverseMappings, partitionScheme);
        sliceOp(dotOp.getC(), offset, builder, mappings, reverseMappings, partitionScheme);
        newOp = cloneAndSetResultType(op);
        ...
        for (unsigned i = 0; i < forOp.getInitArgs().size(); i++) {
            ...
            Value newInitArg;
            auto newInitArgOp = sliceOp(initArg, offset, builder, mappings, reverseMappings, partitionScheme);
            if (auto bbArg = dyn_cast<BlockArgument>(initArg)) {
                ...
                assert(argIndex < parentBlock->getNumArguments() && "new init argment not found");
                Region *parentRegion = parentBlock->getParent();
                Region &newParentRegion = newInitArgOp->getRegion(parentRegion->getRegionNumber());
            ...
            if (newInitArg != initArg) {
                ...
                forOp.getBody()->insertArgument(forOp.getBody()->getNumArguments(), newInitArg.getType(),
                                                forOp.getLoc());
        ...
        builder.setInsertionPoint(op);
        auto newForOp = builder.createWithAsyncTaskIds<scf::ForOp>(forOp.getLoc(), forOp.getLowerBound(),
                                                                   forOp.getUpperBound(), forOp.getStep(), newLoopArgs);
        assert(newForOp.getRegionIterArgs().size() == newForOp.getInitArgs().size());
        ...
        auto newThenYieldOp = sliceOp(thenYieldOp, offset, builder, mappings, reverseMappings, partitionScheme);
        sliceOp(elseYieldOp, offset, builder, mappings, reverseMappings, partitionScheme);
        assert(newThenYieldOp->getNumOperands() > ifOp->getNumResults() && "no need to slice if op");
        ...
        auto newIfOp = builder.create<scf::IfOp>(ifOp.getLoc(), newResultTypes, ifOp.getCondition());
        ...
        newOp->walk([&](Operation *childOp) { setAsyncTaskIds(childOp, sliceTaskIds); });
    } else {
...
{
    if (auto op = v.getDefiningOp()) {
        return sliceOp(op, offset, builder, mappings, reverseMappings, partitionScheme);
    } else {
        assert(isa<BlockArgument>(v) && "value is not an operation or block ");
        auto bbArg = cast<BlockArgument>(v);
        Operation *bbAargOwner = bbArg.getOwner()->getParentOp();
        return sliceOp(bbAargOwner, offset, builder, mappings, reverseMappings, partitionScheme);
    }
}
...
class TritonGPUWSDataPartitionPass : public impl::TritonGPUWSDataPartitionBase<TritonGPUWSDataPartitionPass> {
  public:
    using impl::TritonGPUWSDataPartitionBase<TritonGPUWSDataPartitionPass>::TritonGPUWSDataPartitionBase;
