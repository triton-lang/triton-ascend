...
{
    Value threadId = builder.create<::mlir::gpu::ThreadIdOp>(loc, builder.getIndexType(), ::mlir::gpu::Dimension::x);
    auto cast =
        builder.create<UnrealizedConversionCastOp>(loc, TypeRange {builder.getIntegerType(32)}, ValueRange {threadId});
...
LoadType scanLoadTypes(ttng::CreateTokenOp createTokenOp)
{
    ...
    createTokenOp->getBlock()->walk([&](Operation *op) {
        ...
        } else if (auto asyncCopy = dyn_cast<ttng::AsyncTMACopyGlobalToLocalOp>(op)) {
...
Value getMBarrierPhaseBit(OpBuilder &builder, Operation *op, bool emptyBarrier)
{
    auto loc = op->getLoc();
...
void processProducerAcquireOp(OpBuilder &builder, ttng::ProducerAcquireOp op, Value bufferEmpty)
{
    auto loc = op.getLoc();
...
void processProducerCommitOp(OpBuilder &builder, ttng::ProducerCommitOp op, Value bufferFull, LoadType loadType)
{
    auto loc = op.getLoc();
    int txCnt = 0;
    ...
    if (loadType == LoadType::LoadAsyncOp) {
        ...
        arriveOp = builder.create<ttng::MBarrierArriveOp>(loc, bufferFull, pred, /*remoteCTAId*/ nullptr,
                                                          /*trackAsyncOp*/ true, txCnt);
    } else {
        ...
        Value pred = builder.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, threadId, _0);
        arriveOp = builder.create<ttng::MBarrierArriveOp>(loc, bufferFull, pred, /*remoteCTAId*/ nullptr,
                                                          /*trackAsyncOp*/ false, txCnt);
    }
...
void processConsumerWaitOp(OpBuilder &builder, ttng::ConsumerWaitOp op, Value bufferFull)
{
    auto loc = op.getLoc();
...
void processConsumerReleaseOp(OpBuilder &builder, ttng::ConsumerReleaseOp op, Value bufferEmpty, int numCTAs)
{
    auto loc = op.getLoc();
    auto arriveOp = builder.create<ttng::MBarrierArriveOp>(loc, bufferEmpty, nullptr, nullptr, false, 0);
    assert(op.getOperation()->hasAttr("async_task_id"));
    setAsyncTaskIds(arriveOp, getAsyncTaskIds(op.getOperation()));
}

void lowerTokenOperations(Operation *parentOp, int numCTAs, int numConsumerGroups)
{
    ...
    parentOp->walk([&](ttng::CreateTokenOp createTokenOp) {
        ...
        Type barrierMemDescType =
            tt::MemDescType::get({createTokenOp.getNum()}, builder.getI64Type(), barrierEncoding, sharedMemorySpace,
                                 /*mutableMemory=*/true);
        ...
        Value bufferFullArray = builder.create<mlir::triton::gpu::LocalAllocOp>(loc, barrierMemDescType, Value());
        Value bufferEmptyArray = builder.create<mlir::triton::gpu::LocalAllocOp>(loc, barrierMemDescType, Value());

        for (unsigned i = 0; i < createTokenOp.getNum(); i++) {
            Value idx = builder.create<arith::ConstantIntOp>(loc, i, 32);
            Value barrierFullView =
                builder.create<ttg::MemDescSubviewOp>(loc, singleBarrierMemDescType, bufferFullArray, idx);
            unsigned bufferFullCount = loadType == LoadType::LoadTMAOp ? 1 : THREADS_PER_TASK;
            builder.create<ttng::InitBarrierOp>(loc, barrierFullView, bufferFullCount);

            Value barrierEmptyView =
                builder.create<ttg::MemDescSubviewOp>(loc, singleBarrierMemDescType, bufferEmptyArray, idx);
            builder.create<ttng::InitBarrierOp>(loc, barrierEmptyView, THREADS_PER_TASK);
        }
        ...
        auto extractBufferFull = [&](Location loc, Value idx) -> Value {
            return builder.create<ttg::MemDescSubviewOp>(loc, singleBarrierMemDescType, bufferFullArray, idx);
        };

        // Helper function for extracting one index from bufferEmptyArray.
        auto extractBufferEmpty = [&](Location loc, Value idx) -> Value {
            return builder.create<ttg::MemDescSubviewOp>(loc, singleBarrierMemDescType, bufferEmptyArray, idx);
        };
...
class TritonGPUWSLowering : public impl::TritonGPUWSLoweringBase<TritonGPUWSLowering> {
  public:
    using impl::TritonGPUWSLoweringBase<TritonGPUWSLowering>::TritonGPUWSLoweringBase;
    ...
    void runOnOperation() override
    {
        ...
        mod->setAttr("triton_gpu.num-warp-groups-per-cta", builder.getI32IntegerAttr(1 + numConsumerGroups));
    }
};
