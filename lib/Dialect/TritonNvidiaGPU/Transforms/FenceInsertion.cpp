...
struct FenceInsertionPass : public TritonGPUFenceInsertionBase<FenceInsertionPass> {

  public:
    FenceInsertionPass() = default;
    FenceInsertionPass(int computeCapability) { this->computeCapability = computeCapability; }
    ...
    void runOnOperation() override
    {
        ...
        mod.walk([&](Operation *op) {
            ...
            auto mmaEncoding =
                dyn_cast<ttg::NvidiaMmaEncodingAttr>(cast<RankedTensorType>(op->getResult(0).getType()).getEncoding());
            ...
            if (!aDependsOnShared && !bDependsOnShared)
                return WalkResult::advance();
            Operation *fence = builder.create<ttng::FenceAsyncSharedOp>(op->getLoc(), /*bCluster=*/false);
            ...
            while (auto loopOp = fence->getParentOfType<LoopLikeOpInterface>()) {
                if (aDependsOnShared && loopOp->isAncestor(a.getParentBlock()->getParentOp()))
                    break;
                if (bDependsOnShared && loopOp->isAncestor(b.getParentBlock()->getParentOp()))
                    break;
  ...
  private:
    bool dependOnSharedEncOperand(Value operand)
    {
        ...
        if (op && op->hasTrait<OpTrait::DotLike>())
            return false;
        // reach convertlayout
        if (op && isa<ttg::LocalAllocOp>(op) && cast<ttg::LocalAllocOp>(op).getSrc())
            return true;
        ...
        if (BlockArgument arg = dyn_cast<BlockArgument>(operand)) {
            ...
            if (auto forOp = dyn_cast<scf::ForOp>(argOwner)) {
                ...
                auto entry = std::make_pair<Operation *, unsigned>(std::move(yieldOp), std::move(argNum));
                // avoid cyclic
...
std::unique_ptr<Pass> mlir::createTritonNvidiaGPUFenceInsertionPass(int computeCapability)
{
