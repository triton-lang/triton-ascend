...
struct LLVMDIScopePass : public LLVMDIScopeBase<LLVMDIScopePass> {
    ...
    void setSubprogramAttr(LLVM::LLVMFuncOp funcOp)
    {
        ...
        if (ModuleOp module = funcOp->getParentOfType<ModuleOp>()) {
            auto fusedCompileUnitAttr = module->getLoc()->findInstanceOf<mlir::FusedLocWith<LLVM::DICompileUnitAttr>>();
        ...
        if (!fileLoc && compileUnitAttr) {
            ...
            fileAttr = LLVM::DIFileAttr::get(context, llvm::sys::path::filename(inputFilePath),
                                             llvm::sys::path::parent_path(inputFilePath));
        }
        auto subroutineTypeAttr = LLVM::DISubroutineTypeAttr::get(context, llvm::dwarf::DW_CC_normal, {});
        ...
        if (!funcOp.isExternal()) {
            ...
            if (!compileUnitAttr) {
                compileUnitAttr = LLVM::DICompileUnitAttr::get(
                    distinctId, llvm::dwarf::DW_LANG_C, fileAttr, StringAttr::get(context, "triton"),
        ...
        // the column offset
        auto subprogramAttr = LLVM::DISubprogramAttr::get(context, distinctId, compileUnitAttr, fileAttr, funcNameAttr,
    ...
    Location getNestedLoc(Operation *op, LLVM::DIScopeAttr scopeAttr, Location calleeLoc)
    {
        auto calleeFileName = extractFileLoc(calleeLoc).getFilename();
        auto context = op->getContext();
        LLVM::DIFileAttr calleeFileAttr = LLVM::DIFileAttr::get(context, llvm::sys::path::filename(calleeFileName),
                                                                llvm::sys::path::parent_path(calleeFileName));
        auto lexicalBlockFileAttr =
            LLVM::DILexicalBlockFileAttr::get(context, scopeAttr, calleeFileAttr, /*discriminator=*/0);
    ...
    void setLexicalBlockFileAttr(Operation *op)
    {
        ...
        if (auto callSiteLoc = dyn_cast<CallSiteLoc>(opLoc)) {
            ...
            auto loc = CallSiteLoc::get(getNestedLoc(op, scopeAttr, calleeLoc), callerLoc);
            op->setLoc(loc);
        }
    }
