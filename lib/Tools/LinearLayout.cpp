...
std::tuple<std::unique_ptr<uint64_t[]>, int /*numRows*/, int /*numCols*/> getInjectiveMat(const LinearLayout &layout)
{
    int numRows = layout.getTotalOutDimSizeLog2();
    int numCols = layout.getTotalInDimSizeLog2();
...
template <typename T, typename U> void assertDimsEqualIgnoringOrder(T &&a, U &&b)
{
...
template <typename T, typename U> void assertDimsSubsetIgnoringOrder(T &&small, U &&big)
{
    ...
    if (!llvm::set_is_subset(smallSet, bigSet)) {
        llvm::report_fatal_error("Dimensions must be a subset, ignoring order, but "
                                 ...
                                 Twine(triton::join(small, ", ")) + "] and [" + triton::join(big, ", ") + "]");
    }
}

// Check that elements common to both aDims and bDims
// appear in the same relative order.
template <typename T, typename U> void assertCommonDimsSameOrder(T &&aDims, U &&bDims)
{
...
void eraseEmptyInOutDims(BasesT &bases, llvm::MapVector<StringAttr, int32_t> &outDims)
{
    // Erase empty out-dims.
    SmallVector<int> emptyOutDims;
    for (auto [i, outDim] : llvm::enumerate(llvm::to_vector_of<StringAttr>(llvm::make_first_range(outDims)))) {
        if (outDims[outDim] == 1) {
    ...
    for (StringAttr inDim : llvm::to_vector_of<StringAttr>(llvm::make_first_range(bases))) {
...
LinearLayout::tryCreate(BasesT bases, ArrayRef<std::pair<StringAttr, int32_t>> outDims, bool requireSurjective)
{
...
LinearLayout::LinearLayout(BasesT bases, ArrayRef<std::pair<StringAttr, int32_t>> outDims, NoCheckInvariants)
...
LinearLayout::LinearLayout(BasesT bases, ArrayRef<StringAttr> outDimNames) : bases(std::move(bases))
{
    // Infer out-dim sizes.
    ...
    std::optional<std::string> error = checkInvariants(/*requireSurjective=*/true);
    if (error.has_value()) {
        llvm::report_fatal_error(StringRef(*error));
    }
}

LinearLayout::LinearLayout(BasesT bases, ArrayRef<std::pair<StringAttr, int32_t>> outDims, bool requireSurjective)
...
std::optional<std::string> LinearLayout::checkInvariants(bool requireSurjective)
{
        ...
        for (const auto &basis : inDimBases) {
            if (basis.size() != outDims.size()) {
                return "Invalid bases passed to LinearLayout.  Expect all bases to "
                       ...
                       std::to_string(outDims.size()) + ").  But this failed for in dimension '" + inDim.str() +
    ...
    for (const auto &[outDim, size] : outDims) {
        if (!llvm::isPowerOf2_32(size)) {
            return "Invalid out-dim size " + std::to_string(size) + " for out-dim '" + outDim.str() +
        ...
        for (const auto &basis : inDimBases) {
            for (int i = 0; i < basis.size(); i++) {
                if (basis[i] >= outDims[outDimNames[i]]) {
                    return "Invalid basis " + std::to_string(basis[i]) + " for in-dim '" + inDim.str() +
                           "' and out-dim '" + outDimNames[i].str() + "'.  Basis must be less than the out-dim size.\n";
                }
            }
        }
    }
    ...
    this->surjective = getMatrixRank(getMatrix(*this), /*numRows=*/getTotalOutDimSizeLog2(),
                                     /*numCols=*/getTotalInDimSizeLog2()) == getTotalOutDimSizeLog2();
...
LinearLayout::LinearLayout(ArrayRef<std::pair<StringAttr, std::vector<std::vector<int32_t>>>> bases,
...
/*static*/ LinearLayout LinearLayout::identity1D(int32_t size, StringAttr inDimName, StringAttr outDimName)
{
    if (size == 0)
...
/*static*/ LinearLayout LinearLayout::zeros1D(int32_t size, StringAttr inDimName, StringAttr outDimName)
{
    if (size == 0)
        return LinearLayout::empty();
...
int32_t LinearLayout::getOutDimIndex(StringAttr outDim) const
{
    ...
    llvm::report_fatal_error("outDim " + Twine(outDim) + " is not in layout" + toString());
}
...
int32_t LinearLayout::getTotalInDimSizeLog2() const
{
    return std::accumulate(getInDimNames().begin(), getInDimNames().end(), 0,
                           [&](int32_t acc, StringAttr inDim) { return acc + getInDimSizeLog2(inDim); });
}
...
int32_t LinearLayout::getTotalOutDimSizeLog2() const
{
    return std::accumulate(getOutDimNames().begin(), getOutDimNames().end(), 0,
                           [&](int32_t acc, StringAttr outDim) { return acc + getOutDimSizeLog2(outDim); });
}
...
int32_t LinearLayout::getNumConsecutiveInOut() const
{
    ...
    for (; consec < firstInDimBases.size(); consec++) {
        ...
        if (basis[0] != (1 << consec) || !std::all_of(basis.begin() + 1, basis.end(), [](int32_t x) { return x == 0; }))
        {
            break;
        }
    }
...
LinearLayout LinearLayout::transposeIns(ArrayRef<StringAttr> newInDims) const
{
    ...
    return LinearLayout(std::move(newBases), llvm::to_vector(outDims), surjective);
}

LinearLayout LinearLayout::transposeOuts(ArrayRef<StringAttr> newOutDims) const
{
...
LinearLayout LinearLayout::reshapeIns(ArrayRef<std::pair<StringAttr, int32_t>> newInDims) const
{
    assert(llvm::all_of(newInDims, [&](auto &inDim) { return llvm::isPowerOf2_32(inDim.second); }));
    assert(getTotalInDimSize() == std::accumulate(newInDims.begin(), newInDims.end(), 1,
...
LinearLayout LinearLayout::reshapeOuts(ArrayRef<std::pair<StringAttr, int32_t>> newOutDims) const
{
    assert(llvm::all_of(newOutDims, [&](auto &outDim) { return llvm::isPowerOf2_32(outDim.second); }));
    assert(getTotalOutDimSize() == std::accumulate(newOutDims.begin(), newOutDims.end(), 1,
...
LinearLayout operator*(LinearLayout inner, LinearLayout outer)
{
    ...
    for (auto [inDimName, inDimSizeLog2] : inDimSizesLog2) {
        ...
        // Fill with zeros.
        inDimBases = std::vector<std::vector<int32_t>>(inDimSizeLog2, std::vector<int32_t>(outDimSizesLog2.size(), 0));
    ...
    return LinearLayout(std::move(allBases), outDimSizes, inner.isSurjective() && outer.isSurjective());
}

bool LinearLayout::isTrivialOver(ArrayRef<StringAttr> dimNames) const
{
    for (StringAttr dim : dimNames) {
        if (!llvm::is_contained(getInDimNames(), dim) && !llvm::is_contained(getOutDimNames(), dim)) {
            return false;
        }
    }
    ...
    SmallVector<StringAttr> remainingInDimNames = getRemainingDimNames(getInDimNames());
    SmallVector<StringAttr> remainingOutDimNames = getRemainingDimNames(getOutDimNames());
    ...
    return squareSublayoutIsIdentity(dimNames) && sublayoutIsZero(remainingInDimNames, dimNames) &&
           sublayoutIsZero(dimNames, remainingOutDimNames);
}

std::optional<LinearLayout> LinearLayout::quotient(ArrayRef<StringAttr> dimNames) const
{
...
bool LinearLayout::sublayoutIsZero(ArrayRef<StringAttr> inDimNames, ArrayRef<StringAttr> outDimNames) const
{
...
bool LinearLayout::squareSublayoutIsIdentity(ArrayRef<StringAttr> dimNames) const
{
...
LinearLayout LinearLayout::compose(const LinearLayout &outer) const
{
    ...
    for (const auto &[inDim, inDimBases] : bases) {
        ...
        for (const auto &basis : inDimBases) {
            ...
            newInDimBases.push_back(std::vector<int32_t>(newBasesRange.begin(), newBasesRange.end()));
        }
    }

    bool compositionIsSurjective =
        isSurjective() && outer.isSurjective() && llvm::all_of(getOutDimNames(), [&](StringAttr outDim) {
            return getOutDimSize(outDim) == outer.getInDimSize(outDim);
        });
    return LinearLayout(std::move(newBases), llvm::to_vector(outer.outDims), compositionIsSurjective);
}
...
LinearLayout LinearLayout::invertAndCompose(const LinearLayout &outer) const
{
    ...
    auto [matOuter, numRowsOuter, numColsOuter] =
        getInjectiveMat(outer.transposeOuts(llvm::to_vector(this->getOutDimNames())));
    ...
    LinearLayout flatComposed(std::move(newBases), {{outDim1D, outer.getTotalInDimSize()}},
                              /*requireSurjective=*/false);
...
llvm::MapVector<StringAttr, int32_t> LinearLayout::getFreeVariableMasks() const
{
...
bool operator==(LinearLayout lhs, LinearLayout rhs)
{
    ...
    for (const auto &[lhsOutDimAndSize, rhsOutDimAndSize] : llvm::zip(lhs.outDims, rhs.outDims)) {
...
bool LinearLayout::equalIgnoringOutDimSizes(const LinearLayout &other) const
{
    ...
    if (llvm::to_vector(this->getOutDimNames()) != llvm::to_vector(other.getOutDimNames()))
        return false;
    if (this->bases.size() != other.bases.size())
        return false;
    for (auto it1 = this->bases.begin(), it2 = other.bases.begin(); it1 != this->bases.end(); ++it1, ++it2) {
        if (*it1 != *it2)
            return false;
    }
    return true;
        ...
        ret += " - " +
               join(llvm::seq(inDimBases.size()), "\n   ",
                    [&, &inDim = inDim, &inDimBases = inDimBases](int i) {
                        return inDim.str() + "=" + std::to_string(1 << i) + " -> (" + join(inDimBases[i], ", ") + ")";
                    }) +
               "\n";
    }
