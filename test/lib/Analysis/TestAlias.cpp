...
struct TestAliasPass : public PassWrapper<TestAliasPass, OperationPass<triton::FuncOp>> {
    ...
    static void print(StringRef name, SmallVector<std::string> &vals, raw_ostream &os)
    {
        if (vals.empty())
            return;
        os << name << " -> ";
    ...
    StringRef getDescription() const final { return "print the result of the alias analysis pass"; }
    ...
    void runOnOperation() override
    {
        ...
        SharedMemoryAliasAnalysis *analysis = solver->load<SharedMemoryAliasAnalysis>();
        ...
        auto getLocalAllocOpNames = [&](Value value) {
            ...
            if (latticeElement) {
                ...
                for (auto &alias : info.getAllocs()) {
                    auto opName = getValueOperandName(alias.getDefiningOp()->getResult(0), state);
...
{
    PassRegistration<TestAliasPass>();
}
} // namespace test
} // namespace mlir
