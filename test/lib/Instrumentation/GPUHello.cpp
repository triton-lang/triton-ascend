...
struct GpuHello : public PassInfoMixin<GpuHello> {
    PreservedAnalyses run(Module &module, ModuleAnalysisManager &)
    {
        ...
        return (modifiedCodeGen ? llvm::PreservedAnalyses::none() : llvm::PreservedAnalyses::all());
    }
...
bool GpuHello::runOnModule(Module &module)
{
    ...
    for (auto &function : module) {
        ...
        if (function.getCallingConv() == CallingConv::AMDGPU_KERNEL ||
            function.getCallingConv() == CallingConv::PTX_Kernel || functionName.contains("kernel"))
        {
            for (Function::iterator basicBlock = function.begin(); basicBlock != function.end(); basicBlock++) {
                for (BasicBlock::iterator inst = basicBlock->begin(); inst != basicBlock->end(); inst++) {
                    DILocation *debugLocation = dyn_cast<Instruction>(inst)->getDebugLoc();
                    std::string sourceInfo = (function.getName() + "\t" + debugLocation->getFilename() + ":" +
                                              Twine(debugLocation->getLine()) + ":" + Twine(debugLocation->getColumn()))
                                                 .str();
