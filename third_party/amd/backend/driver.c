...
#define HIP_SYMBOL_LIST(FOR_EACH_ERR_FN, FOR_EACH_STR_FN)                                                              \
    ...
    FOR_EACH_ERR_FN(hipGetDeviceProperties, hipDeviceProp_tR0000 *prop, int deviceId)                                  \
    FOR_EACH_ERR_FN(hipModuleLoadDataEx, hipModule_t *module, const void *image, unsigned int numOptions,              \
                    hipJitOption *options, void **optionValues)                                                        \
    FOR_EACH_ERR_FN(hipModuleGetFunction, hipFunction_t *function, hipModule_t module, const char *kname)              \
    FOR_EACH_ERR_FN(hipFuncGetAttribute, int *, hipFunction_attribute attr, hipFunction_t function)

// The HIP symbol table for holding resolved dynamic library symbols.
struct HIPSymbolTable {
#define DEFINE_EACH_ERR_FIELD(hipSymbolName, ...) hipError_t (*hipSymbolName)(__VA_ARGS__);
#define DEFINE_EACH_STR_FIELD(hipSymbolName, ...) const char *(*hipSymbolName)(__VA_ARGS__);
...
#define QUERY_EACH_FN(hipSymbolName, ...)                                                                              \
    *(void **)&hipSymbolTable.hipSymbolName = dlsym(lib, #hipSymbolName);                                              \
    error = dlerror();                                                                                                 \
    if (error) {                                                                                                       \
        PyErr_SetString(PyExc_RuntimeError, "cannot query " #hipSymbolName " from libamdhip64.so");                    \
        dlclose(lib);                                                                                                  \
        return false;                                                                                                  \
    }
...
static PyObject *getDeviceProperties(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue("{s:i, s:i, s:i, s:i, s:i, s:i, s:s, s:i, s:i}", "max_shared_mem", props.sharedMemPerBlock,
                         "max_num_regs", props.regsPerBlock, "multiprocessor_count", props.multiProcessorCount,
                         "sm_clock_rate", props.clockRate, "mem_clock_rate", props.memoryClockRate, "mem_bus_width",
                         props.memoryBusWidth, "arch", props.gcnArchName, "warpSize", props.warpSize,
...
static PyObject *loadBinary(PyObject *self, PyObject *args)
{
    ...
    hipJitOption opt[] = {hipJitOptionErrorLogBufferSizeBytes, hipJitOptionErrorLogBuffer,
                          hipJitOptionInfoLogBufferSizeBytes, hipJitOptionInfoLogBuffer, hipJitOptionLogVerbose};
    const unsigned int errbufsize = 8192;
    const unsigned int logbufsize = 8192;
    char _err[errbufsize];
    char _log[logbufsize];
    void *optval[] = {(void *)(uintptr_t)errbufsize, (void *)_err, (void *)(uintptr_t)logbufsize, (void *)_log,
                      (void *)1};
    ...
    hipSymbolTable.hipFuncGetAttribute(&n_spills, HIP_FUNC_ATTRIBUTE_LOCAL_SIZE_BYTES, fun);
    n_spills /= 4;
    if (PyErr_Occurred()) {
        return NULL;
    }
