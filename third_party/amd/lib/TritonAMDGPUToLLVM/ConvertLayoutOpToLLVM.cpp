...
struct LocalLoadOpConversion : public ConvertOpToLLVMPattern<triton::gpu::LocalLoadOp> {
  public:
                                  ...
                                  ConversionPatternRewriter &rewriter) const override
    {
        ...
        if (isa<DotOperandEncodingAttr>(dstLayout) &&
            isa<AMDMfmaEncodingAttr, AMDWmmaEncodingAttr>(cast<DotOperandEncodingAttr>(dstLayout).getParent()))
        {
  ...
  private:
                                     ...
                                     const DotOperandEncodingAttr &dotOperandLayout, bool isOuter) const
    {
        ...
        auto llvmElemTy = typeConverter->convertType(cast<MemDescType>(src.getType()).getElementType());
        ...
        if (!isOuter && isa<AMDMfmaEncodingAttr, AMDWmmaEncodingAttr>(dopOpParent)) {
            auto sharedToDotConvert = isa<AMDMfmaEncodingAttr>(dopOpParent) ? SharedToDotOperandMFMA::convertLayout
                                                                            : SharedToDotOperandWMMA::convertLayout;
            res = sharedToDotConvert(dotOperandLayout.getOpIdx(), rewriter, loc, src, dotOperandLayout, smemObj,
                                     typeConverter, tid_val());
        } else {
                                          ...
                                          ConversionPatternRewriter &rewriter) const
    {
        ...
        auto dotOperandLayout = cast<DotOperandEncodingAttr>(dstTensorTy.getEncoding());
        ...
        isOuter = K == 1;
        Value res = lowerSharedToDotOperandMMA(op, adaptor, typeConverter, rewriter, dotOperandLayout, isOuter);
        if (!res)
            return failure();
...
struct ConvertLayoutOpConversion : public ConvertOpToLLVMPattern<triton::gpu::ConvertLayoutOp> {
  public:
    using ConvertOpToLLVMPattern<triton::gpu::ConvertLayoutOp>::ConvertOpToLLVMPattern;
                                  ...
                                  ConversionPatternRewriter &rewriter) const override
    {
        ...
        if (isa<AMDMfmaEncodingAttr>(srcLayout) && isa<DotOperandEncodingAttr>(dstLayout)) {
  ...
  private:
    LogicalResult lowerMfmaToDotOperand(triton::gpu::ConvertLayoutOp op, OpAdaptor adaptor,
                                        ConversionPatternRewriter &rewriter) const
    {
        auto loc = op.getLoc();
