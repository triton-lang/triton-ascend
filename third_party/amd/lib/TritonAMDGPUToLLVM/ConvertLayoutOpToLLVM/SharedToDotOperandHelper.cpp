...
Value getWarpIdInBlock(ConversionPatternRewriter &rewriter, Location loc, Value warpId,
                       const ArrayRef<unsigned int> &wpt, int elemPerInstrNonK, int tensorSizeNonK, int nonKIdx,
                       const ArrayRef<unsigned int> &order)
{
    SmallVector<Value> multiDimWarpId = delinearize(rewriter, loc, warpId, wpt, order);

    return urem(multiDimWarpId[nonKIdx], i32_val(tensorSizeNonK / elemPerInstrNonK));
}
...
std::pair<mlir::Value, mlir::Value> swizzleIndexes(ConversionPatternRewriter &rewriter, Location loc, Value row,
...
Value computeOffset(ConversionPatternRewriter &rewriter, Location loc, Value row, Value col, SharedMemoryObject smemObj,
                    SharedEncodingAttr srcLayout)
{
    auto [swizzledRow, swizzledCol] = swizzleIndexes(rewriter, loc, row, col, smemObj, srcLayout);
...
Value computeBasePtr(ConversionPatternRewriter &rewriter, Location loc, const SharedMemoryObject &smemObj)
{
 ...
 * @return bool
 */
bool isSwizzlePatternFitsIntoBlock(const SharedEncodingAttr sharedLayout, int opIdx, const ArrayRef<int64_t> reps,
                                   const ArrayRef<int64_t> elemsPerInstr, unsigned warpsPerBlockNonK)
{
    ...
    const auto swizzleFastDimSize = sharedLayout.getMaxPhase() * sharedLayout.getVec();
    const auto swizzleSlowDimSize = sharedLayout.getMaxPhase() * sharedLayout.getPerPhase();
    const auto swizzlePatternSizeK = isKMajor(order, opIdx) ? swizzleFastDimSize : swizzleSlowDimSize;
    const auto swizzlePatternSizeNonK = !isKMajor(order, opIdx) ? swizzleFastDimSize : swizzleSlowDimSize;

    const auto blockSizeK = mfmaInstrK * reps[reps.size() - 1];
    const auto blockSizeNonK = mfmaInstrNonK * warpsPerBlockNonK;
    return blockSizeK % swizzlePatternSizeK == 0 && blockSizeNonK % swizzlePatternSizeNonK == 0;
}

llvm::SmallVector<Value> computeOffsetsAType(ConversionPatternRewriter &rewriter, Location loc,
                                             computeTensorElemMappingInBlockT fn,
                                             const ArrayRef<int64_t> &elemsPerInstr, Value warpId, Value laneId,
                                             int warpsPerBlock, int numOfElems, ArrayRef<int64_t> reps,
                                             SharedMemoryObject smemObj, SharedEncodingAttr srcLayout, unsigned nonKDim,
                                             unsigned kDim)
{
    ...
    auto mapping =
        fn(rewriter, loc, elemsPerInstr, warpId, laneId, numOfElems, reps, offsets, vectorSize, nonKDim, kDim);
    ...
    if (!isSwizzlePatternFitsIntoBlock(srcLayout, 0, reps, elemsPerInstr, warpsPerBlock)) {
        for (int block = 0; block < numBlocks; ++block) {
            ...
            for (int i = 0; i < blockSize; ++i) {
                ...
                aOffsets[block * blockSize + i] = computeOffset(rewriter, loc, row, col, smemObj, srcLayout);
            }
        }
    } else {
        ...
        for (int i = 0; i < mapping.size(); ++i) {
            ...
            Value col = mapping[i][1];
            inblockOffset[i] = computeOffset(rewriter, loc, row, col, smemObj, srcLayout);
        }
        for (int block = 0; block < numBlocks; ++block) {
            int blockNonKOffset = block * nonKDim * warpsPerBlock;
...
llvm::SmallVector<Value> computeOffsetsBType(ConversionPatternRewriter &rewriter, Location loc,
                                             computeTensorElemMappingInBlockT fn,
                                             const ArrayRef<int64_t> &elemsPerInstr, Value warpId, Value laneId,
                                             int warpsPerBlock, int numOfElems, ArrayRef<int64_t> reps,
                                             SharedMemoryObject smemObj, SharedEncodingAttr srcLayout, unsigned nonKDim,
                                             unsigned kDim)
{
    ...
    auto mapping =
        fn(rewriter, loc, tElemsPerInstr, warpId, laneId, numOfElems, tReps, tOffsets, vectorSize, nonKDim, kDim);
    ...
    if (!isSwizzlePatternFitsIntoBlock(srcLayout, 0, reps, elemsPerInstr, warpsPerBlock)) {
        for (int block = 0; block < numBlocks; ++block) {
            ...
            for (int i = 0; i < mapping.size(); ++i) {
                ...
                bOffsets[block * blockSize + i] = computeOffset(rewriter, loc, row, col, smemObj, srcLayout);
            }
        }
    } else {
        // compute inblock offsets once and reuse them for all blocks
