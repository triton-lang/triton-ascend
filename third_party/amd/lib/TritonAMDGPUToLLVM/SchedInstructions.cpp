...
void createSchedGroupBarrier(PatternRewriter &rewriter, Location loc, InstructionKindMask maskValue, int sizeValue,
                             int groupIdValue)
{
    MLIRContext *ctx = rewriter.getContext();
    const char *intrinsicName = "llvm.amdgcn.sched.group.barrier";

    Value mask = LLVM::createConstantI32(loc, rewriter, static_cast<int32_t>(maskValue));
    Value size = LLVM::createConstantI32(loc, rewriter, static_cast<int32_t>(sizeValue));
    Value groupId = LLVM::createConstantI32(loc, rewriter, static_cast<int32_t>(groupIdValue));

    LLVM::createLLVMIntrinsicCallOp(rewriter, loc, intrinsicName, TypeRange {}, ValueRange {mask, size, groupId});
}
...
Operation *createSchedBarrier(PatternRewriter &rewriter, Location loc, int64_t maskValue)
{
    MLIRContext *ctx = rewriter.getContext();
    ...
    Value mask = LLVM::createConstantI32(loc, rewriter, static_cast<int32_t>(maskValue));
    return LLVM::createLLVMIntrinsicCallOp(rewriter, loc, intrinsicName, TypeRange {}, ValueRange {mask});
}
...
Operation *createIglpOpt(PatternRewriter &rewriter, Location loc, int value)
{
    ...
    LLVM::FastmathFlagsAttr defaultFlags {};
    Value iglpValue = LLVM::createConstantI32(loc, rewriter, static_cast<int32_t>(value));
    return LLVM::createLLVMIntrinsicCallOp(rewriter, loc, intrinsicName, TypeRange {}, ValueRange {iglpValue});
}

struct InstructionSchedHintsRewriter : public OpRewritePattern<triton::amdgpu::InstructionSchedHint> {

    InstructionSchedHintsRewriter(mlir::MLIRContext *ctx, std::string variant) : OpRewritePattern(ctx)
    {
    ...
    LogicalResult matchAndRewrite(triton::amdgpu::InstructionSchedHint instructionSchedHint,
                                  PatternRewriter &rewriter) const override
    {
        ...
        const bool limitSchedulingRange =
            !(schedulingType == SchedulingType::IGLP0 || schedulingType == SchedulingType::IGLP1);
...
struct LowerInstructionSchedHints : public triton::impl::LowerInstructionSchedHintsBase<LowerInstructionSchedHints> {

    explicit LowerInstructionSchedHints(std::string variant) { this->variant = variant; }
...
struct InsertInstructionSchedHints : public triton::impl::InsertInstructionSchedHintsBase<InsertInstructionSchedHints> {
    void runOnOperation() override
    {
        MLIRContext *ctx = &getContext();
        ModuleOp mod = getOperation();
...
namespace mlir::triton {
std::unique_ptr<OperationPass<ModuleOp>> createLowerInstructionSchedHintsPass(std::string variant)
{
    return std::make_unique<LowerInstructionSchedHints>(variant);
}

std::unique_ptr<OperationPass<ModuleOp>> createInsertInstructionSchedHintsPass()
{
