...
SmallVector<unsigned, 3> warpsPerTile(Operation *dotOp, ArrayRef<int64_t> shape, int numWarps,
                                      std::pair<int64_t, int64_t> shapePerWarp)
{
    ...
    auto filter = [dotOp](Operation *op) { return op->getParentRegion() == dotOp->getParentRegion(); };
    ...
    do {
        if (ret[0] * ret[1] >= numWarps)
            break;
        if (tensorShape[0] / (shapePerWarp.first * 2) / ret[0] >= tensorShape[1] / shapePerWarp.second / ret[1]) {
...
SmallVector<unsigned, 3> warpsPerTileMFMA(Operation *dotOp, ArrayRef<int64_t> shape, int numWarps,
                                          std::pair<int64_t, int64_t> shapePerWarp)
{
    return warpsPerTile(dotOp, shape, numWarps, shapePerWarp);
}

SmallVector<unsigned, 3> warpsPerTileWMMA(Operation *dotOp, ArrayRef<int64_t> shape, int numWarps)
{
...
FailureOr<MfmaInsn> chooseMfmaInstruction(RankedTensorType cType, Type aElemType, Type bElemType, int inputKSize,
                                          int mfmaVersion, int enforcedNonKDim)
{
    ...
    if (enforcedNonKDim != 0) {
        ...
        if (minSize < 16) {
            if (M < 16 && N >= 64) {
                ...
                assert(inputKSize >= 64 && "k should be at least 64 to use this layout");
                mDim = 4;
                nDim = 4;
            }
        }
    ...
    auto maybeMfmaInsn = MfmaInsn::selectMfma(mDim, nDim, aElemType, bElemType, mfmaVersion);
...
FailureOr<MfmaInsn> chooseMfmaInstruction(tt::DotOp dot, int mfmaVersion, int nonKDim)
{
    RankedTensorType aType = dot.getA().getType();
    return chooseMfmaInstruction(dot.getC().getType(), aType.getElementType(), dot.getB().getType().getElementType(),
                                 aType.getShape().back(), mfmaVersion, nonKDim);
}

using OperandTypesVector = SmallVector<Type, 4>;
OperandTypesVector selectMatrixCoreOperandTypes(tt::DotOp dot, ArrayRef<OperandTypesVector> applicableTypes)
{
    SmallVector<Value> dotOperands = {dot.getA(), dot.getB(), dot.getC(), dot.getD()};
    OperandTypesVector initElemTypes;
    llvm::transform(dotOperands, std::back_inserter(initElemTypes),
                    [](Value v) { return cast<RankedTensorType>(v.getType()).getElementType(); });
    ...
    int maxConvertCost = std::numeric_limits<int32_t>::max() / applicableTypes.front().size();
    auto calcConvertCost = [&](Type fromTy, Type toTy) -> int32_t {
        ...
        if (fromTy.isIntOrIndex() && toTy.isIntOrIndex() && fromTy.isUnsignedInteger() != toTy.isUnsignedInteger())
            return fromTy.isUnsignedInteger() && fromTy.getIntOrFloatBitWidth() < toTy.getIntOrFloatBitWidth()
                       ? 1
                       : maxConvertCost;

        return fromTy.getIntOrFloatBitWidth() <= toTy.getIntOrFloatBitWidth() ? 1 : maxConvertCost;
    };
...
OperandTypesVector getOperandTypesForWmmaOp(PatternRewriter &rewriter, tt::DotOp dot, int version)
{
 ...
 */
Value convertAndCastTensor(PatternRewriter &rewriter, Value value, Attribute newEncoding, Type newElemType)
{
    ...
    auto convertedType = RankedTensorType::get(oldType.getShape(), oldElemType, newEncoding);

    Value convertedTensor = rewriter.create<ttg::ConvertLayoutOp>(loc, convertedType, value);
    ...
    if (newElemType.isIntOrIndex()) {
        ...
        if (oldWidth == newWidth)
            castedTensor = rewriter.create<arith::BitcastOp>(loc, convertedType, convertedTensor);
        else if (oldWidth > newWidth)
            castedTensor = rewriter.create<arith::TruncIOp>(loc, castedType, convertedTensor);
        else if (oldElemType.isSignedInteger())
            castedTensor = rewriter.create<arith::ExtSIOp>(loc, castedType, convertedTensor);
        else
            castedTensor = rewriter.create<arith::ExtUIOp>(loc, castedType, convertedTensor);
    } else {
        if (oldElemType.isF16() && newElemType.isF32())
            castedTensor = rewriter.create<arith::ExtFOp>(loc, castedType, convertedTensor);
        else if (oldElemType.isF32() && newElemType.isF16())
            castedTensor = rewriter.create<arith::TruncFOp>(loc, castedType, convertedTensor);
        else
            castedTensor = rewriter.create<tt::FpToFpOp>(loc, castedType, convertedTensor);
    }
    return castedTensor;
}
...
class BlockedToMFMA : public OpRewritePattern<tt::DotOp> {
  ...
  public:
    BlockedToMFMA(MLIRContext *context, int mfmaVersion, int nonKDim, int kPack, PatternBenefit benefit = 1)
        : OpRewritePattern(context, benefit), mfmaVersion(mfmaVersion), nonKDim(nonKDim), kPack(kPack)
    {
    }
    ...
    LogicalResult matchAndRewrite(tt::DotOp dotOp, PatternRewriter &rewriter) const override
    {
        ...
        mfmaEnc = ttg::AMDMfmaEncodingAttr::get(oldRetType.getContext(),
        ...
        auto newAEncoding = ttg::DotOperandEncodingAttr::get(ctx, 0, mfmaEnc, kWidth);
        auto newBEncoding = ttg::DotOperandEncodingAttr::get(ctx, 1, mfmaEnc, kWidth);
        a = convertAndCastTensor(rewriter, a, newAEncoding, mfmaInstr.value().getElementTypeA());
        b = convertAndCastTensor(rewriter, b, newBEncoding, mfmaInstr.value().getElementTypeB());
        auto newDot = rewriter.create<tt::DotOp>(dotOp.getLoc(), newAcc.getType(), a, b, newAcc,
                                                 dotOp.getInputPrecision(), dotOp.getMaxNumImpreciseAcc());

        Value dotOutput = convertAndCastTensor(rewriter, newDot, oldRetType.getEncoding(), oldRetType.getElementType());
...
static Value promoteOperand(OpBuilder &builder, Location loc, Value operand, Type promotedType)
{
    Type tensorPromotedType = cast<RankedTensorType>(operand.getType()).cloneWith(std::nullopt, promotedType);
...
static void decomposeMixedModeDotOp(ModuleOp mod)
{
    mod.walk([](triton::DotOp dotOp) -> void {
        ...
        if (isa<ttg::AMDMfmaEncodingAttr>(D.getType().getEncoding())) {
            ...
            promoteType = AElType.getIntOrFloatBitWidth() > BElType.getIntOrFloatBitWidth() ? AElType : BElType;
        } else {
            // FMA case.
            Type AElType = dotOp.getA().getType().getElementType();
            ...
            if (AElType.isIntOrIndex()) {
                assert(dotOp.getB().getType().getElementType().isIntOrIndex() &&
                       dotOp.getC().getType().getElementType().isIntOrIndex() && DElType.isIntOrIndex());
                auto convertTensorIToFP = [&](Value v) -> Value {
                    ...
                    return !srcElType.isUnsignedInteger()
                               ? builder.create<arith::SIToFPOp>(dotOp.getLoc(), dstType, v).getResult()
                               : builder.create<arith::UIToFPOp>(dotOp.getLoc(), dstType, v).getResult();
                };
                auto convertTensorFPToI = [&](Type dstElType, Value v) -> Value {
                    ...
                    return !dstElType.isUnsignedInteger()
                               ? builder.create<arith::FPToSIOp>(dotOp.getLoc(), dstType, v).getResult()
                               : builder.create<arith::FPToUIOp>(dotOp.getLoc(), dstType, v).getResult();
                };
                ...
                auto newDot =
                    builder.create<tt::DotOp>(dotOp.getLoc(), newCOperand.getType(), newAOperand, newBOperand,
                                              newCOperand, dotOp.getInputPrecision(), dotOp.getMaxNumImpreciseAcc());
...
class BlockedToWMMA : public OpRewritePattern<tt::DotOp> {
  ...
  public:
    BlockedToWMMA(MLIRContext *context, int wmmaVersion, PatternBenefit benefit = 1)
    ...
    LogicalResult matchAndRewrite(tt::DotOp dotOp, PatternRewriter &rewriter) const override
    {
        ...
        if (wmmaVersion == 2 && llvm::isa<FloatType>(oldAType) && oldAType.getIntOrFloatBitWidth() == 8) {
        ...
        wmmaEnc = ttg::AMDWmmaEncodingAttr::get(ctx, wmmaVersion, warpsPerTile, CTALayout);

        auto newRetType = RankedTensorType::get(retShape, operandTypes[3], wmmaEnc);

        // convert accumulator
        auto oldAcc = dotOp.getC();
        auto newAcc = convertAndCastTensor(rewriter, oldAcc, wmmaEnc, operandTypes[2]);

        auto newAType =
            RankedTensorType::get(aShape, operandTypes[0],
                                  ttg::DotOperandEncodingAttr::get(ctx, 0, wmmaEnc,
                                                                   wmmaEnc.getSizePerThreadForOperand(
                                                                       /*kWidth=*/0, /*opIdx=*/0)[rank - 1]));
        auto newBType =
            RankedTensorType::get(bShape, operandTypes[1],
                                  ttg::DotOperandEncodingAttr::get(ctx, 1, wmmaEnc,
                                                                   wmmaEnc.getSizePerThreadForOperand(
                                                                       /*kWidth=*/0, /*opIdx=*/1)[rank - 2]));

        Value castedA = convertAndCastTensor(rewriter, a, newAType.getEncoding(), operandTypes[0]);
        Value castedB = convertAndCastTensor(rewriter, b, newBType.getEncoding(), operandTypes[1]);
        auto newDot = rewriter.create<tt::DotOp>(dotOp.getLoc(), newRetType, castedA, castedB, newAcc,
                                                 dotOp.getInputPrecision(), dotOp.getMaxNumImpreciseAcc());

        Value dotOutput = convertAndCastTensor(rewriter, newDot, oldRetEncoding, oldRetType.getElementType());
        rewriter.replaceOp(dotOp, dotOutput);
        return success();
    }
};
...
class TritonAMDGPUAccelerateMatmulPass : public TritonAMDGPUAccelerateMatmulBase<TritonAMDGPUAccelerateMatmulPass> {
  public:
    TritonAMDGPUAccelerateMatmulPass() = default;
    TritonAMDGPUAccelerateMatmulPass(StringRef archGen, int matrixInstructionSize, int kPack)
    {
    ...
    void runOnOperation() override
    {
        ...
        switch (auto isaFamily = triton::AMD::deduceISAFamily(archGenerationName)) {
            ...
            case ISAFamily::CDNA2:
            case ISAFamily::CDNA3:
                patterns.add<::BlockedToMFMA>(context, getMfmaVersion(isaFamily), matrixInstructionSize, kPack);
                break;
...
std::unique_ptr<Pass> mlir::createTritonAMDGPUAccelerateMatmulPass(std::string archGen, int matrixInstructionSize,
                                                                   int kPack)
{
    return std::make_unique<TritonAMDGPUAccelerateMatmulPass>(archGen, matrixInstructionSize, kPack);
}
