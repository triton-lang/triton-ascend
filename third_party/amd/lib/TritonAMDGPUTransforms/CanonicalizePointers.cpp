...
class PointerCanonicalizer {
  public:
    explicit PointerCanonicalizer(ModuleOp moduleOp) : rewriter(moduleOp.getContext()), mod(moduleOp) {}
  ...
  private:
    ...
    struct FatPtr {
        ...
        FatPtr copy(Value newBasePtr, Value newOffset) { return FatPtr {newBasePtr, newOffset, canNarrow}; };
        FatPtr copyWithBase(Value newOffset) { return FatPtr {basePtr, newOffset, canNarrow}; }
        FatPtr copyWithOffset(Value newBase) { return FatPtr {newBase, offset, canNarrow}; }
        // Attribute functions
        void setAttr(NamedAttribute attr) { attributes.push_back(attr); }
        void setAttrs(ArrayRef<NamedAttribute> attrs) { llvm::append_range(attributes, attrs); }
    };
    ...
    LogicalResult rewriteSplatOp(triton::SplatOp splatOp, Location curLoc, Value &nextPtr);
    LogicalResult rewriteBroadcastOp(triton::BroadcastOp broadcastOp, Location curLoc, Value &nextPtr);
    LogicalResult rewriteAddPtrOp(triton::AddPtrOp addPtrOp, Location curLoc, Value &nextPtr);
    LogicalResult rewriteForOp(scf::ForOp forOp, Location curLoc, OpOperand *operand, Value &nextPtr);
    LogicalResult rewriteYieldOp(scf::YieldOp yieldOp, Location curLoc, OpOperand *operand, Value &nextPtr);
    LogicalResult rewriteWhileOp(scf::WhileOp whileOp, Location curLoc, OpOperand *operand, Value &nextPtr);
    LogicalResult rewriteConditionOp(scf::ConditionOp conditionOp, Location curLoc, OpOperand *operand, Value &nextPtr);
    LogicalResult rewriteCondBranchOp(cf::CondBranchOp condBrOp, Location curLoc, OpOperand *operand, Value &nextPtr);
    LogicalResult rewriteSelectOp(arith::SelectOp selectOp, Location curLoc, OpOperand *operand, Value &nextPtr);
    LogicalResult rewriteBranchOp(cf::BranchOp branchOp, Location curLoc, OpOperand *operand, Value &nextPtr);
    ...
    // std::pair{U, NU}
    std::pair<Value, Value> decomposeOffsetFromExpr(Location loc, Value expr, int64_t bitness);
    std::pair<Value, Value> decomposeOffsetFromAdd(Location loc, Value expr, int64_t bitness);
    std::pair<Value, Value> decomposeOffsetFromMul(Location loc, Value expr, int64_t bitness);

    // Return either the operation or its rewritten op
    template <typename OpTy> OpTy resolveOp(Operation *op, const DenseMap<Operation *, Operation *> &rewriteOpMap)
    {
...
static Value extend32bitOffsetTo64Bits(IRRewriter &rewriter, Location loc, Value offset)
{
    Type elementType = getElementTypeOrSelf(offset);

    if (auto tensorType = dyn_cast<RankedTensorType>(offset.getType())) {
        auto shape = tensorType.getShape();
        auto newTensorType = RankedTensorType::get(shape, rewriter.getI64Type(), tensorType.getEncoding());
...
static Value narrow64bitOffsetTo32bits(IRRewriter &rewriter, Location loc, Value offset)
{
    Type elementType = getElementTypeOrSelf(offset);
    ...
    if (auto tensorType = dyn_cast<RankedTensorType>(offset.getType())) {
        auto shape = tensorType.getShape();
        auto newTensorType = RankedTensorType::get(shape, rewriter.getI32Type(), tensorType.getEncoding());
...
Value getScalarConstant(IRRewriter &rewriter, Location loc, Value expr)
{
    ...
    if (auto constOp = dyn_cast_or_null<arith::ConstantOp>(op)) {
        ...
        if (matchPattern(val, m_Constant(&constVal)) && constVal.isSplat())
            return rewriter.create<arith::ConstantOp>(loc, constVal.getSplatValue<IntegerAttr>());
    }
...
void PointerCanonicalizer::collectFatPointerAttributes(Operation *op, Value val)
{
...
std::pair<Value, Value> PointerCanonicalizer::decomposeOffsetFromAdd(Location loc, Value expr, int64_t bitness)
{
    auto addOp = expr.getDefiningOp<arith::AddIOp>();
    auto [uniformOffsetL, nonUniformOffsetL] = decomposeOffsetFromExpr(loc, addOp.getLhs(), bitness);
    auto [uniformOffsetR, nonUniformOffsetR] = decomposeOffsetFromExpr(loc, addOp.getRhs(), bitness);
    Value uniformAdd = rewriter.create<arith::AddIOp>(loc, uniformOffsetL, uniformOffsetR);
    Value nonUniformAdd = rewriter.create<arith::AddIOp>(loc, nonUniformOffsetL, nonUniformOffsetR);
...
std::pair<Value, Value> PointerCanonicalizer::decomposeOffsetFromMul(Location loc, Value expr, int64_t bitness)
{
    auto mulOp = expr.getDefiningOp<arith::MulIOp>();
    auto [uniformOffsetL, nonUniformOffsetL] = decomposeOffsetFromExpr(loc, mulOp.getLhs(), bitness);
    auto [uniformOffsetR, nonUniformOffsetR] = decomposeOffsetFromExpr(loc, mulOp.getRhs(), bitness);
    Value uniformMul = rewriter.create<arith::MulIOp>(loc, uniformOffsetL, uniformOffsetR);

    Value uniformOffsetLSplat = rewriter.create<triton::SplatOp>(loc, nonUniformOffsetL.getType(), uniformOffsetL);
    Value uniformOffsetRSplat = rewriter.create<triton::SplatOp>(loc, nonUniformOffsetR.getType(), uniformOffsetR);

    Value nonUNonU = rewriter.create<arith::MulIOp>(loc, nonUniformOffsetL, nonUniformOffsetR);
    Value nonUU = rewriter.create<arith::MulIOp>(loc, uniformOffsetLSplat, nonUniformOffsetR);
    Value uNonU = rewriter.create<arith::MulIOp>(loc, nonUniformOffsetL, uniformOffsetRSplat);
...
std::pair<Value, Value> PointerCanonicalizer::decomposeOffsetFromExpr(Location loc, Value expr, int64_t bitness)
{
    ...
    if (Value scalarConst = getScalarConstant(rewriter, loc, expr)) {
        auto tensorZero = createTensorZero(rewriter, loc, cast<RankedTensorType>(expr.getType()));
    ...
    auto offsets =
        llvm::TypeSwitch<Operation *, std::pair<Value, Value>>(expr.getDefiningOp())
            .Case<triton::BroadcastOp>([&](auto broadcastOp) {
                auto [uniform, nonUniform] = decomposeOffsetFromExpr(loc, broadcastOp.getSrc(), bitness);
                auto broadcastNonUniform = rewriter.create<triton::BroadcastOp>(loc, broadcastOp.getType(), nonUniform);
                return std::make_pair(uniform, broadcastNonUniform);
            })
            .Case<triton::ExpandDimsOp>([&](auto expandOp) {
                auto [uniform, nonUniform] = decomposeOffsetFromExpr(loc, expandOp.getSrc(), bitness);
                auto expandNonUniform = rewriter.create<triton::ExpandDimsOp>(loc, nonUniform, expandOp.getAxis());
                return std::make_pair(uniform, expandNonUniform);
            })
            .Case<arith::AddIOp>([&](Operation *op) { return decomposeOffsetFromAdd(loc, expr, bitness); })
            .Case<arith::MulIOp>([&](Operation *op) { return decomposeOffsetFromMul(loc, expr, bitness); })
...
Value PointerCanonicalizer::createTensorPointer(FatPtr fatPtr, Location loc)
{
    ...
    if (!tensorType) {
        auto addPtrOp = rewriter.create<triton::AddPtrOp>(loc, basePtr.getType(), basePtr, offset);
    ...
    auto tensorPtrType = RankedTensorType::get(offsetShape, basePtr.getType(), tensorType.getEncoding());
    if (fatPtr.canNarrow)
        offset = narrow64bitOffsetTo32bits(rewriter, loc, offset);

    Value tensorPtr = rewriter.create<triton::SplatOp>(loc, tensorPtrType, basePtr);

    auto addPtrOp = rewriter.create<triton::AddPtrOp>(loc, tensorPtrType, tensorPtr, offset);
...
LogicalResult PointerCanonicalizer::materializeFatPointer(Operation *op, Location loc, Value ptr)
{
...
LogicalResult PointerCanonicalizer::rewriteSplatOp(triton::SplatOp splatOp, Location curLoc, Value &nextPtr)
{
    ...
    auto newOffsetType = RankedTensorType::get(ptrShape, fatPtr.offset.getType(), outType.getEncoding());
    Value offset = rewriter.create<triton::SplatOp>(curLoc, newOffsetType, fatPtr.offset);
...
LogicalResult PointerCanonicalizer::rewriteBroadcastOp(triton::BroadcastOp broadcastOp, Location curLoc, Value &nextPtr)
{
    ...
    auto newOffsetType = RankedTensorType::get(ptrShape, offsetType.getElementType(), outType.getEncoding());
    Value offset = rewriter.create<triton::BroadcastOp>(curLoc, newOffsetType, fatPtr.offset);
    pointers[nextPtr] = fatPtr.copyWithBase(offset);
    return success();
}

LogicalResult PointerCanonicalizer::rewriteAddPtrOp(triton::AddPtrOp addPtrOp, Location curLoc, Value &nextPtr)
{
    ...
    if (Value scalarConst = getScalarConstant(rewriter, curLoc, offset)) {
        newPtr = rewriter.create<triton::AddPtrOp>(curLoc, newPtr.getType(), newPtr, scalarConst);
    ...
    int64_t bitness = cast<RankedTensorType>(offset.getType()).getElementTypeBitWidth();
    auto [uniformOffset, nonUniformOffset] = decomposeOffsetFromExpr(curLoc, offset, bitness);

    // Scalar pointer update: bump the scalar pointer
    newPtr = rewriter.create<triton::AddPtrOp>(curLoc, newPtr.getType(), newPtr, uniformOffset);
    ...
    if (!isZeroConst(nonUniformOffset)) {
        ...
        if (addPtrOffsetType.isInteger(32) && fatPtrOffsetType.isInteger(64))
            nonUniformOffset = extend32bitOffsetTo64Bits(rewriter, curLoc, nonUniformOffset);
        else if (addPtrOffsetType.isInteger(64) && fatPtrOffsetType.isInteger(32))
            nonUniformOffset = narrow64bitOffsetTo32bits(rewriter, curLoc, nonUniformOffset);

        newOffset = rewriter.create<arith::AddIOp>(curLoc, nonUniformOffset, fatPtrOffset);
        propagateAtrs = false;
    }
    opToDelete.insert(addPtrOp);
...
LogicalResult PointerCanonicalizer::rewriteForOp(scf::ForOp forOp, Location curLoc, OpOperand *curOperand,
                                                 Value &nextPtr)
{
    ...
    auto newForOp = replaceForOpWithNewSignature(rewriter, forOp, {basePtr, offset});
    ...
    pointers[arg] = fatPtr.copy(newForOp.getRegionIterArg(numIterArgs - 2), newForOp.getRegionIterArg(numIterArgs - 1));
    ...
    pointers[nextPtr] = fatPtr.copy(newForOp->getResult(numResults - 2), newForOp.getResult(numResults - 1));
    opToDelete.insert(forOp);
    return success();
}

LogicalResult PointerCanonicalizer::rewriteYieldOp(scf::YieldOp yieldOp, Location curLoc, OpOperand *curOperand,
                                                   Value &nextPtr)
{
    ...
    if (auto forOp = dyn_cast<scf::ForOp>(yieldOp->getParentOp())) {
        ...
        if (yieldOp->getBlock() == &ifOp.getThenRegion().front()) {
            auto newIfOp =
                replaceIfOpWithNewSignature(rewriter, ifOp, {fatPtr.basePtr.getType(), fatPtr.offset.getType()});
            nextPtr = newIfOp.getResult(operandNum);
            size_t numResults = newIfOp->getNumResults();
            pointers[nextPtr] = fatPtr.copy(newIfOp->getResult(numResults - 2), newIfOp.getResult(numResults - 1));
            opToDelete.insert(ifOp);
        }

    } else if (auto whileOp = resolveOp<scf::WhileOp>(yieldOp->getParentOp(), rewriteOpMap)) {
        ...
        // set of return types)
        auto newWhileOp =
            replaceWhileOpWithNewSignature(rewriter, whileOp, {}, {fatPtr.basePtr.getType(), fatPtr.offset.getType()});
        nextPtr = newWhileOp.getResult(operandNum);
        size_t numResults = newWhileOp->getNumResults();
        pointers[nextPtr] = fatPtr.copy(newWhileOp->getResult(numResults - 2), newWhileOp->getResult(numResults - 1));
...
LogicalResult PointerCanonicalizer::rewriteWhileOp(scf::WhileOp whileOp, Location curLoc, OpOperand *curOperand,
                                                   Value &nextPtr)
{
    ...
    // set of return types)
    Value tensorPtr = createTensorPointer(fatPtr, curLoc);
    auto newWhileOp = replaceWhileOpWithNewSignature(rewriter, whileOp, {basePtr, offset}, {});
    ...
    pointers[arg] = fatPtr.copy(newWhileOp.getBeforeBody()->getArgument(numArguments - 2),
...
LogicalResult PointerCanonicalizer::rewriteConditionOp(scf::ConditionOp conditionOp, Location curLoc,
                                                       OpOperand *curOperand, Value &nextPtr)
{
    ...
    conditionOp.setOperand(operandNum, whileOp.getRegionIterArgs()[operandNum - 1]);
    pointers[nextPtr] =
        fatPtr.copy(afterBlock->getArgument(numArguments - 2), afterBlock->getArgument(numArguments - 1));
    return success();
}

LogicalResult PointerCanonicalizer::rewriteCondBranchOp(cf::CondBranchOp condBrOp, Location curLoc,
                                                        OpOperand *curOperand, Value &nextPtr)
{
    ...
    if (rewriteOpMap.contains(condBrOp)) {
        ...
        // update `operandNum`
        auto condBranchReplacement = dyn_cast<cf::CondBranchOp>(rewriteOpMap[condBrOp]);
        if (isFalseOperand) {
            // basePtr+offset need to be added if we are on the FalseOperands
            // side, but the true operands have been rewritten
            bool needOffset =
                (condBranchReplacement.getTrueDestOperands().size() != condBrOp.getTrueDestOperands().size());
        ...
        // operands
        falseOperands = llvm::to_vector(condBranchReplacement.getFalseDestOperands());
    ...
    if (isFalseOperand) {
        ...
        Value falseDestArg = falseDest->getArgument(operandNum - condBrOp.getNumTrueOperands() - 1);
    ...
    // cond branch
    Value tensorPtr = createTensorPointer(fatPtr, curLoc);
    auto newCondBranch = rewriter.create<cf::CondBranchOp>(curLoc, condBrOp.getCondition(), trueDest, trueOperands,
...
LogicalResult PointerCanonicalizer::rewriteSelectOp(arith::SelectOp selectOp, Location curLoc, OpOperand *curOperand,
                                                    Value &nextPtr)
{
    ...
    Value newBase = rewriter.create<arith::SelectOp>(curLoc, cond, fatPtrT.basePtr, fatPtrF.basePtr);
    Value newOffset = rewriter.create<arith::SelectOp>(curLoc, cond, fatPtrT.offset, fatPtrF.offset);
...
LogicalResult PointerCanonicalizer::rewriteBranchOp(cf::BranchOp branchOp, Location curLoc, OpOperand *curOperand,
                                                    Value &nextPtr)
{
    size_t operandNum = curOperand->getOperandNumber();
    FatPtr fatPtr = pointers[curOperand->get()];
...
LogicalResult PointerCanonicalizer::rewritePointer(Value argPtr)
{
    ...
    while (!queue.empty()) {
        ...
        llvm::TypeSwitch<Operation *>(curOp)
            .Case<triton::SplatOp>([&](auto splatOp) { res = rewriteSplatOp(splatOp, curLoc, nextPtr); })
            .Case<triton::BroadcastOp>(
                [&](auto broadcastOp) { res = rewriteBroadcastOp(broadcastOp, curLoc, nextPtr); })
            .Case<triton::AddPtrOp>([&](auto addPtrOp) { res = rewriteAddPtrOp(addPtrOp, curLoc, nextPtr); })
            .Case<scf::ForOp>([&](auto forOp) {
                res = rewriteForOp(resolveOp<scf::ForOp>(forOp, rewriteOpMap), curLoc, curOperand, nextPtr);
            })
            .Case<scf::YieldOp>([&](auto yieldOp) { res = rewriteYieldOp(yieldOp, curLoc, curOperand, nextPtr); })
            .Case<scf::WhileOp>([&](auto whileOp) {
                res = rewriteWhileOp(resolveOp<scf::WhileOp>(whileOp, rewriteOpMap), curLoc, curOperand, nextPtr);
            })
            .Case<scf::ConditionOp>(
                [&](auto conditionOp) { res = rewriteConditionOp(conditionOp, curLoc, curOperand, nextPtr); })
            .Case<cf::CondBranchOp>(
                [&](auto condBrOp) { res = rewriteCondBranchOp(condBrOp, curLoc, curOperand, nextPtr); })
            .Case<arith::SelectOp>([&](auto selectOp) { res = rewriteSelectOp(selectOp, curLoc, curOperand, nextPtr); })
            .Case<cf::BranchOp>([&](auto branchOp) { res = rewriteBranchOp(branchOp, curLoc, curOperand, nextPtr); })
            .Case<triton::LoadOp, triton::StoreOp, triton::AtomicCASOp, triton::AtomicRMWOp, triton::PtrToIntOp>(
                [&](Operation *op) { res = materializeFatPointer(curOp, curLoc, op->getOperand(0)); })
            .Default([&](Operation *op) {
...
LogicalResult PointerCanonicalizer::rewriteFunction(triton::FuncOp funcOp)
{
    ...
    for (auto [idx, arg] : llvm::enumerate(region.getArguments())) {
        ...
        if (!isa<triton::PointerType>(arg.getType()))
            continue;
        int64_t bitness = 64;
        if (IntegerAttr pointerRangeAttr = funcOp.getArgAttrOfType<IntegerAttr>(idx, "tt.pointer_range"))
            bitness = pointerRangeAttr.getInt();

        rewriter.setInsertionPointToStart(&region.front());
        Value zeroOffset = rewriter.create<arith::ConstantIntOp>(region.getLoc(), 0, bitness);
...
class TritonAMDGPUCanonicalizePointersPass
    : public TritonAMDGPUCanonicalizePointersBase<TritonAMDGPUCanonicalizePointersPass> {
  public:
