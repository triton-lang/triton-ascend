...
// op is encountered.
static llvm::ilist<Operation>::iterator findEarlyInsertionPoint(Block *block, Operation *move)
{
    Value src;
...
bool isCrossLoopBoundary(mlir::Operation *opInsideLoop, mlir::Operation *opOutsideLoop)
{
...
class TritonAMDGPUReorderInstructionsPass
    : public TritonAMDGPUReorderInstructionsBase<TritonAMDGPUReorderInstructionsPass> {
  public:
    ...
    void runOnOperation() override
    {
        ...
        m.walk([&](Operation *op) {
            ...
            if (user->getParentOfType<scf::ForOp>() == op->getParentOfType<scf::ForOp>())
                return;
    ...
    //
        ...
        for (auto op : llvm::reverse(moveOps)) {
            ...
            if (ipoint != block->end()) {
                ...
                llvm::erase_if(dfg, [&](Operation *op) { return !ipoint->isBeforeInBlock(op); });
