...
static Operation *streamPredication(RewriterBase &rewriter, Operation *op, Value pred)
{
    ...
    if (auto dotOp = dyn_cast<tt::DotOp>(op)) {
        ...
        auto ifOp = rewriter.create<scf::IfOp>(loc, dotOp.getResult().getType(), pred, /*withElseRegion=*/true);
...
class StreamPipeliner {
  public:
    ...
    Value createAlloc(Operation *loadOp, ttg::SharedEncodingAttr sharedEnc, unsigned numBuffers);
...
void StreamPipeliner::createStreamCopy(tt::LoadOp loadOp, Value alloc, Value extractIdx,
                                       tt::CoarseSchedule::Cluster prefetchCluster)
{
    ...
    auto sharedMemorySpace = triton::gpu::SharedMemorySpaceAttr::get(forOp.getContext());
    auto subviewTy = tt::MemDescType::get(allocTy.getShape().drop_front(), allocTy.getElementType(),
                                          allocTy.getEncoding(), sharedMemorySpace, /*mutableMemory=*/true);
    auto viewLoad = builder.create<ttg::MemDescSubviewOp>(loc, subviewTy, alloc, loadOffsets);
    auto storeOp = builder.create<ttg::LocalStoreOp>(loc, copy->getResult(0), viewLoad);
    ...
    auto sharedLoad = builder.create<ttg::LocalLoadOp>(loc, loadOp.getType(), viewLoad);
...
static std::optional<ttg::SharedEncodingAttr> getSharedEncIfAllUsersAreDotEnc(Value val)
{
    ...
    for (Operation *user : val.getUsers()) {
        ...
        if (auto memDesc = dyn_cast<triton::MemDescType>(user->getResult(0).getType())) {
            ...
            tempAttr = ttg::SharedEncodingAttr::get(val.getContext(), dotOpEnc, srcTy.getShape(), sharedOrder,
...
void StreamPipeliner::computeLoadOpsToIndirectionLevelAndUse()
{
    ...
    std::function<void(Operation *, int, Operation *)> dfs = [&](Operation *op, int distance, Operation *use) {
...
void StreamPipeliner::assignMemoryLayouts()
{
    for (auto &[op, dist, use] : loadOpToIndLevelAndUse) {
        ...
        auto pointeeTy = cast<tt::PointerType>(tensorTy.getElementType()).getPointeeType();
        ...
        if (use->hasTrait<OpTrait::DotLike>()) {
            ...
            loadInfo.sharedEncoding = getSharedEncIfAllUsersAreDotEnc(op->getResult(0)).value_or(nullptr);
...
void StreamPipeliner::scheduleLoads(DenseSet<Operation *> &rootUsers)
{
    ...
    // large.
    assert(numStages >= 2 && "requires num_stages=2 at least");
    unsigned stagesBetweenLoads = llvm::divideCeil(numStages - 2, maxIndirectionLevel + 1);
...
void StreamPipeliner::scheduleRemainingToLastStage(tt::CoarseSchedule::Cluster afterPrologue)
{
...
Value StreamPipeliner::createAlloc(Operation *loadOp, ttg::SharedEncodingAttr sharedEnc, unsigned numBuffers)
{
    OpBuilder builder(forOp);
    ...
    Type memdescType = tt::MemDescType::get(bufferShape, ty.getElementType(), sharedEnc, sharedMemorySpace,
                                            /*mutableMemory=*/true);
    return builder.create<ttg::LocalAllocOp>(loadOp->getLoc(), memdescType, Value());
}
...
void StreamPipeliner::createStreamOps()
{
    ...
    scf::ForOp newForOp = replaceForOpWithNewSignature(builder, forOp, {extractIdx});
    forOp.erase();
    forOp = newForOp;
...
bool StreamPipeliner::preprocessLoopAndBuildSchedule()
{
    ...
    std::vector<std::pair<Operation *, unsigned>> coarseSchedule = schedule.createFinalSchedule(forOp);

    // Fill out the pipeline options.
    options.getScheduleFn = [coarseSchedule](scf::ForOp, std::vector<std::pair<Operation *, unsigned>> &s) {
...
std::unique_ptr<Pass> mlir::createTritonAMDGPUStreamPipelineV2Pass(int numStages)
{
