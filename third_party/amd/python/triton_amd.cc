...
void init_triton_amd_passes_ttgpuir(py::module &&m)
{
    ...
    m.def("add_to_llvmir", [](mlir::PassManager &pm, const std::string &arch, bool ftz) {
        pm.addPass(createConvertTritonAMDGPUToLLVMPass(arch, ftz));
    });
    m.def("add_builtin_func_to_llvmir",
          [](mlir::PassManager &pm, bool ftz) { pm.addPass(createConvertBuiltinFuncToLLVMPass(ftz)); });
    m.def("insert_instruction_sched_hints",
          [](mlir::PassManager &pm) { pm.addPass(createInsertInstructionSchedHintsPass()); });
    m.def("lower_instruction_sched_hints", [](mlir::PassManager &pm, std::string variant) {
        pm.addPass(createLowerInstructionSchedHintsPass(variant));
    });
    m.def("add_decompose_unsupported_conversions", [](mlir::PassManager &pm, const std::string &arch) {
        pm.addPass(mlir::triton::AMD::createDecomposeUnsupportedConversionsPass(arch));
    });
    ADD_PASS_WRAPPER_2("add_optimize_lds_usage", mlir::triton::AMD::createOptimizeLDSUsagePass, const std::string &,
                       int32_t);
    ADD_PASS_WRAPPER_3("add_accelerate_matmul", mlir::createTritonAMDGPUAccelerateMatmulPass, const std::string, int,
                       int);
    ADD_PASS_WRAPPER_0("add_optimize_epilogue", mlir::createTritonAMDGPUOptimizeEpiloguePass);
    ADD_PASS_WRAPPER_0("add_canonicalize_pointers", mlir::createTritonAMDGPUCanonicalizePointersPass);
    ADD_PASS_WRAPPER_0("add_convert_to_buffer_ops", mlir::createTritonAMDGPUConvertToBufferOpsPass);
    ADD_PASS_WRAPPER_0("add_reorder_instructions", mlir::createTritonAMDGPUReorderInstructionsPass);
    ADD_PASS_WRAPPER_1("add_stream_pipelinev2", mlir::createTritonAMDGPUStreamPipelineV2Pass, int);
}

void addControlConstant(llvm::Module *module, const char *name, uint32_t bitwidth, uint32_t value)
{
    using llvm::GlobalVariable;

    llvm::IntegerType *type = llvm::IntegerType::getIntNTy(module->getContext(), bitwidth);
    auto *initializer = llvm::ConstantInt::get(type, value, /*isSigned=*/false);
    auto *constant = new llvm::GlobalVariable(*module, type, /*isConstant=*/true,
...
void init_triton_amd(py::module &&m)
{
    ...
    m.attr("CALLING_CONV_AMDGPU_KERNEL") = (unsigned)llvm::CallingConv::AMDGPU_KERNEL;
    ...
    m.def("attach_target_triple", [](llvm::Module *module) { module->setTargetTriple(amdTargetTriple); });
    ...
    m.def("set_bool_control_constant", [](llvm::Module *module, const std::string &name, bool enable) {
    ...
    m.def("set_abi_version", [](llvm::Module *module, int version) {
        ...
        llvm::GlobalVariable *abi = new llvm::GlobalVariable(
            *module, i32Ty, /*isConstant=*/true, llvm::GlobalValue::LinkageTypes::LinkOnceODRLinkage,
        ...
        module->addModuleFlag(llvm::Module::Error, "amdhsa_code_object_version", version);
    });
    ...
    m.def(
        "assemble_amdgcn",
        [](const std::string &assembly, const std::string &arch, const std::string &features) {
            std::string error;
            ...
            srcMgr.AddNewSourceBuffer(llvm::MemoryBuffer::getMemBuffer(assembly), llvm::SMLoc());

            const llvm::MCTargetOptions mcOptions;
            std::unique_ptr<llvm::MCRegisterInfo> mri(target->createMCRegInfo(amdTargetTriple));
            std::unique_ptr<llvm::MCAsmInfo> mai(target->createMCAsmInfo(*mri, amdTargetTriple, mcOptions));
            std::unique_ptr<llvm::MCSubtargetInfo> sti(target->createMCSubtargetInfo(amdTargetTriple, arch, features));

            llvm::MCContext ctx(triple, mai.get(), mri.get(), sti.get(), &srcMgr, &mcOptions);
            std::unique_ptr<llvm::MCObjectFileInfo> mofi(target->createMCObjectFileInfo(ctx, /*PIC=*/false,
            ...
            std::unique_ptr<llvm::MCCodeEmitter> ce(target->createMCCodeEmitter(*mcii, ctx));
            std::unique_ptr<llvm::MCAsmBackend> mab(target->createMCAsmBackend(*sti, *mri, mcOptions));
            std::unique_ptr<llvm::MCObjectWriter> ow(mab->createObjectWriter(svos));
            mcStreamer.reset(
                target->createMCObjectStreamer(triple, ctx, std::move(mab), std::move(ow), std::move(ce), *sti));

            std::unique_ptr<llvm::MCAsmParser> parser(createMCAsmParser(srcMgr, ctx, *mcStreamer, *mai));
        ...
        for (llvm::Function &f : module->functions()) {
            if (f.hasExternalLinkage() && f.hasName() && !f.hasExactDefinition()) {
                ...
                if (funcName.contains("__nv_")) {
                    ...
                    message << "Implicit conversion of CUDA " << funcName.str() << " device function has been dropped; "
