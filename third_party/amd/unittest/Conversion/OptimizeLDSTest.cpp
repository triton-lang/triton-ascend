...
{
    unsigned prod = std::reduce(a.begin(), a.end(), 1u, std::multiplies<unsigned>());
    return prod == P;
}
...
TEST(OptimizeLDSUtility, factorizePowerOf2)
{
    ...
    ASSERT_TRUE(std::all_of(output2.begin(), output2.end(), [](auto a) { return a.size() == 2; }));
    ...
    ASSERT_TRUE(std::all_of(output3.begin(), output3.end(), [](auto a) { return a.size() == 3; }));
}
