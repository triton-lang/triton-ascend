...
def _get_version(is_manylinux, get_git_commit_hash):
    version = os.environ.get("TRITON_VERSION", patches.get_default_version()) + \
...
def patch_module(mod):
    ...
    class CMakeBuild(_OrigCMakeBuild):
        ...
        def run(self):
            patches.apply_triton_ascend_patch()
...
def activate():
    ...
    def _ascend_setup(**kwargs):
        ...
        patches.print_patch_restore_warning()
        return dist
