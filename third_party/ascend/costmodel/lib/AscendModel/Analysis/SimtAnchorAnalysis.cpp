...
mlir::ascend::buildStageOwnedScopeDescriptor(llvm::ArrayRef<Operation *> roots,
  ...
  // outside user. BiShengIR's SIMT VF ABI can transfer only ranked tensors
  // back to the SIMD caller. Reject scalar and pointer-like escaping state
  // before the route is scored instead of letting the backend assert during
  ...
  for (Operation *root : roots)
    for (Value result : root->getResults()) {
      const bool escapes = llvm::any_of(result.getUses(), [&](OpOperand &use) {
        return !isInside(use.getOwner());
      });
      if (escapes && !isa<RankedTensorType>(result.getType()))
        return std::nullopt;
    }
