#!/usr/bin/env bash
set -eo pipefail
# Build the single-op scalar-store CAModel probes.
cd "$(dirname "$0")"
...
ccec -c -std=c++17 -O2 --cce-aicore-only --cce-aicore-arch=dav-c310 \
  -I"$INC" store_scalar_o1.cce -o store_scalar_o1.o

g++ -O2 store_scalar_o1_host.cpp -o store_scalar_o1_host \
  -I"$ASCEND_TOOLKIT_HOME/x86_64-linux/pkg_inc" \
  -I"$ASCEND_TOOLKIT_HOME/include" \
  -L"$ASCEND_TOOLKIT_HOME/lib64" -lruntime -lascendcl

echo "BUILD DONE: $(pwd)/store_scalar_o1.o $(pwd)/store_scalar_o1_host"
