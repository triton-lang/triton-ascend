...
_FA_TRIPWIRE_ITER = int(os.environ.get("FA_TRIPWIRE_ITER", "-1"))
...
def run_attention(q, k, v, sm_scale, use_cvsplit=False):
    ...
    _attn_fwd[grid](
        ...
        AICORE_NUM=CORE_NUM,
        TRIPWIRE_N=(_FA_TRIPWIRE_ITER * BLOCK_N if _FA_TRIPWIRE_ITER >= 0 else -1),
        **kwargs)
    return o
...
def check_accuracy(name, out, ref, atol=0.05, rtol=0.05):
    ...
    if out_flat.norm() < 1e-8 or ref_flat.norm() < 1e-8:
        ...
        cos_sim = torch.nn.functional.cosine_similarity(
            out_flat.unsqueeze(0), ref_flat.unsqueeze(0)).item()
