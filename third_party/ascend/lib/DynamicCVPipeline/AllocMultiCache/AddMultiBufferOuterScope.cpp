...
void AddMultiBufferOuterScopePass::runOnOperation()
{
    ...
    LDBG("[FlagBudget] used=" << flagCount << " (max=16)");
    if (flagCount > 15) {
        LDBG("[FlagBudget] FATAL: flag count " << flagCount << " > 15, halting pass");
        module->emitError() << "[FlagBudget] flag count " << flagCount << " > 15, halting pass";
        signalPassFailure();
        return;
    }
    if (flagCount > 7) {
        LDBG("[FlagBudget] flag count " << flagCount << " > 7, forcing single-buffer");
        isDoubleBuf = false;
    }
