...
const auto printFlags = OpPrintingFlags().enableDebugInfo(true, true).skipRegions();
...
OpAbility OpNode::canRunOn() const
{
    ...
    return llvm::TypeSwitch<Operation *, OpAbility>(op)
        ...
        .Case<arith::ConstantOp, triton::AdvanceOp, triton::TransOp>([](auto) { return OpAbility::CUBE_AND_VECTOR; })
        .Case<arith::SelectOp>([](arith::SelectOp op) {
            // when cond is vector, selectOp should be vector, otherwise scalar
            return (valueIsScalar(op.getCondition()) ? OpAbility::CUBE_AND_VECTOR : OpAbility::PREFER_VECTOR);
        })
...
OpNode::OpNode(Operation *op, Graph *graph) : Node(Node::NK_Op), op(op)
{
    ...
    if (auto branchOp = llvm::dyn_cast<RegionBranchOpInterface>(op)) {
            ...
            for (auto &succ : succRegions) {
                ...
                if (succ.isParent()) {
                    ...
                    if (!terminator && subgraph.terminator) {
                        ...
                        for (auto [forwardedVal, resultNode] : llvm::zip_equal(forwardedVal, outputs)) {
                    ...
                    // // [FIXME] what if a region can be a successor of multiple regions? Do we need to restrict to If
                    // For and While here? auto toString = [](auto& whatever) {
                    //   std::string result;
                    ...
                    for (auto [operand, succInput] : llvm::zip_equal(forwardedVal, succ.getSuccessorInputs())) {
        ...
        if (auto loopOp = llvm::dyn_cast<LoopLikeOpInterface>(op)) {
            // Step3: inits->iter_args (single directional) (should be handled in step 2: ) last terminator -> iter_args
            // (bidirectional)
            ...
            // for(auto [init, iterArgVal, yieldNode] : llvm::zip_equal(loopOp.getInits(), loopOp.getRegionIterArgs(),
            // terminator->outputs)) {
            //   auto& initNode = valueMap[init];
            //   auto& iterArgNode = valueMap[iterArgVal];
            //   initNode->outputs.push_back(iterArgNode.get());
...
CoreType Node::absorbCommon()
{
    ...
    for (auto output : outputs) {
        switch (output->isOn()) {
            ...
            case CUBE_ONLY:
                if (ability != OpAbility::PREFER_VECTOR || output->isUpstreamOfCubeMem || memPolicy == MemPolicy::READ)
                {
                    isUpstreamOfCubeMem =
                        (isUpstreamOfCubeMem || output->isUpstreamOfCubeMem || memPolicy == MemPolicy::READ);
...
std::unique_ptr<Graph> Graph::fromMultiBlockFunc(triton::FuncOp funcOp)
{
    ...
    for (auto &block : funcOp.getBody()) {
        auto &subgraph = dummyNode->subgraphs.emplace_back(&block, dummyGraph.get());
        opCount += subgraph.opCount;
    }
