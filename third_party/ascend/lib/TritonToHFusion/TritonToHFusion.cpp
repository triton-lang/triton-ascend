...
struct TritonModToHFusionConversion : OpRewritePattern<triton::ascend::ModOp> {
    ...
    LogicalResult matchAndRewrite(triton::ascend::ModOp op, PatternRewriter &rewriter) const final
    {
        ...
        auto emptyTensor = rewriter.create<tensor::EmptyOp>(op.getLoc(), lhsType.getShape(), lhsType.getElementType());
        auto newOp = hfusion::createBinaryOp<hfusion::ElemwiseBinaryOp, hfusion::BinaryFn, hfusion::BinaryFnAttr>(
            rewriter, op.getLoc(), hfusion::BinaryFn::mod, ValueRange({op.getLhs(), op.getRhs()}),
...
struct TritonHistogramToHFusionConversion : OpRewritePattern<triton::HistogramOp> {
    using OpRewritePattern<triton::HistogramOp>::OpRewritePattern;

    LogicalResult matchAndRewrite(triton::HistogramOp op, PatternRewriter &rewriter) const final
    {
        auto loc = op.getLoc();
        ...
        auto newOp = rewriter.create<hfusion::HistogramOp>(loc, resultType, input, numBinsAttr, Value());
...
struct TritonFpToFpToHFusionConversion : OpRewritePattern<triton::FpToFpOp> {
    ...
    LogicalResult matchAndRewrite(triton::FpToFpOp op, PatternRewriter &rewriter) const final
    {
        auto loc = op.getLoc();
        Value input = op.getSrc();
        auto resultType = op.getResult().getType();
        ...
        if (!srcType.getElementType().isIntOrFloat() || !dstType.getElementType().isIntOrFloat()) {
            return failure();
        }
        ...
        auto roundModeAttr = hfusion::RoundModeAttr::get(rewriter.getContext(), hfusionRoundMode);
        auto modeAttr = rewriter.getNamedAttr("mode", roundModeAttr);

        rewriter.replaceOpWithNewOp<hfusion::CastOp>(op, ValueRange {input}, ValueRange {dsts}, ArrayRef {modeAttr});
...
namespace {
struct TritonToHFusionPass : public mlir::triton::impl::TritonToHFusionBase<TritonToHFusionPass> {
