...
def _failed(error: str, *, prof_dir: Path | None = None, csv_path: Path | None = None, trace_path: Path | None = None,
            wait_info: dict[str, Any] | None = None) -> ProfileResult:
    timeline: dict[str, Any] = {}
    if wait_info is not None:
        timeline["profiler_file_wait"] = wait_info
    return ProfileResult(None, None, None, None, None, None, {}, timeline,
...
def _parse_kernel_timing_csv(csv_path: Path) -> dict[str, Any]:
            ...
            if not step_id:
                ...
                if len(blank_examples) < 3:
                    blank_examples.append({
                        "name": str(name), "type": str(op_type), "start_us": str(start_us), "duration_us": str(duration)
                    })
                continue
            step_kernel_times.setdefault(step_id, {}).setdefault(name, []).append(duration)
            window = step_windows.setdefault(
                step_id,
                {"start_us": start_us, "end_us": start_us + duration, "kernel_duration_sum_us": 0.0, "kernel_count": 0})
    ...
    if not step_kernel_times:
        if blank_step_rows:
            return {
                "error_msg":
                "kernel_details.csv contains only non-warmup kernels without Step Id; "
    ...
    median_step_kernel_sum_us = float(
        statistics.median([float(w["kernel_duration_sum_us"]) for w in step_windows.values()]))
    kernel_counts = [int(w["kernel_count"]) for w in step_windows.values()]
    return {
        "metrics": {
            "device_kernels": device_kernels, "kernel_sum_us": round(sum(device_kernels.values()),
                                                                     2), "active_window_us": round(active_window_us, 2),
            "median_step_kernel_sum_us": round(median_step_kernel_sum_us, 2), "window_gap_us":
            round(max(active_window_us - median_step_kernel_sum_us, 0.0), 2), "measured_step_count": len(step_windows),
            "kernel_count": int(round(statistics.median(kernel_counts))) if kernel_counts else 0, "step_windows":
            dict(sorted(step_windows.items(), key=lambda item: _step_sort_key(item[0]))), "blank_step_rows":
            blank_step_rows, "blank_step_examples": blank_examples
        }
    }


def _is_warmup_kernel(op_type: str, input_shapes: str) -> bool:
    return bool(op_type and input_shapes and ((op_type == "MatMulV3" and WARMUP_MATMUL_SHAPE in input_shapes) or
