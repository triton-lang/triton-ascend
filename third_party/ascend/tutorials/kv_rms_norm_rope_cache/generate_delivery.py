...
META_BY_DIR = {
    ...
    "dynamic_quant": {
        ...
        "design":
        "Triton row/row-stride kernel with chunked large-H path and libdevice round before quantized storage.",
...
def summary_items(meta: dict[str, Any], records: list[dict[str, Any]], jsonl_path: Path) -> list[tuple[str, Any]]:
    ...
    return [
        ...
        ("max candidate RMSE",
         fmt_num(max((fnum(acc(r, "triton").get("rmse")) or 0.0) for r in records) if records else 0.0)),
