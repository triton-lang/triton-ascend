...
// UpdateConditionInfo on scf.while: clone scf.condition(x) def-chain
// (extsi + cmpi) into each ssbuffer.if via whileBlockArgMap.

// CHECK-LABEL: func.func @while_update_condition_counter

// While-do remains; before-region still has extsi/cmpi i64 predicates.
// CHECK: scf.while
// CHECK: arith.extsi %{{.*}} : i32 to i64
// CHECK: arith.cmpi slt, %{{.*}} : i64
// CHECK: scf.condition

// Vector ssbuffer.if = 5: cross-core cond and-ed with cloned while counter.
// CHECK: %[[EXT5:.*]] = arith.extsi %{{.*}} : i32 to i64
// CHECK: %[[CMP5:.*]] = arith.cmpi slt, %[[EXT5]], %{{.*}} : i64
// CHECK: %[[AND5:.*]] = arith.andi %{{.*}}, %[[CMP5]]
// CHECK: scf.if %[[AND5]]
// CHECK: } {{.*}}ssbuffer.if = 5

// Vector ssbuffer.if = 6: same cloned counter shape.
// CHECK: %[[EXT6:.*]] = arith.extsi %{{.*}} : i32 to i64
// CHECK: %[[CMP6:.*]] = arith.cmpi slt, %[[EXT6]], %{{.*}} : i64
// CHECK: %[[AND6:.*]] = arith.andi %{{.*}}, %[[CMP6]]
// CHECK: scf.if %[[AND6]]
// CHECK: } {{.*}}ssbuffer.if = 6
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  func.func @while_update_condition_counter(%arg0: memref<?xi8>, %arg1: memref<?xi8>, %arg2: memref<?xf16> {tt.divisibility = 16 : i32, tt.tensor_kind = 0 : i32}, %arg3: memref<?xf16> {tt.divisibility = 16 : i32, tt.tensor_kind = 0 : i32}, %arg4: memref<?xf16> {tt.divisibility = 16 : i32, tt.tensor_kind = 1 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32, %arg9: i32, %arg10: i32, %arg11: i32, %arg12: i32, %arg13: i32) attributes {SyncBlockLockArgIdx = 0 : i64, WorkspaceArgIdx = 1 : i64, global_kernel = "local", mix_mode = "mix", parallel_mode = "simd"} {
