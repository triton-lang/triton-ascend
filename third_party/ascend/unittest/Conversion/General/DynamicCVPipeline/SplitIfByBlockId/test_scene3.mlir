...
module {
  // ==========================================================================
  // Scene 3 — one side multi-group, other side has exactly 1 group
  //
  // The multi-group side is split normally. The single-group side's ops are
  // absorbed into the LAST split-if's else block, so they execute when the
  // original condition evaluates to the other branch.
  // ==========================================================================

  // --------------------------------------------------------------------------
  // Case A: then has block_id={5, 3}, else has block_id={8} actual ops
  ...
  // --------------------------------------------------------------------------
  // Case B: then has block_id={10, 11}, else has block_id={5} actual ops
  // Yield if (N=2), then splits into 2 split-ifs, last if's else absorbs
  // the single-group else ops
  // --------------------------------------------------------------------------

  // CHECK-LABEL: func.func @test_scene3_case_b
  // for inside main_loop:
  // CHECK: scf.for
  // first split if (block_id=10, group A): yields only slot 0 (what it produces)
  // CHECK: [[SF0:%.*]] = scf.if
  ...
  // CHECK: else
  // CHECK: scf.yield
  // second split if (block_id=11, group B): last if carries original result types
  // CHECK: [[SF1:%.*]]:2 = scf.if
  // CHECK: arith.mulf [[SF0]], {{.*}} {ssbuffer.block_id = 11 : i32}
  // CHECK: scf.yield [[SF0]],
  // CHECK: else
