"""
Test Case: PCB05 - C2V2C chain (Cube1->Vec->Cube2) + independent Vector

[MLIR校验] 重构版本

Description: C2V2C chain (Cube1->Vec->Cube2) + independent Vector
...
Test Cases:
  ...
  - PCB05-TC02: float32, M=128, N=64, K=32
"""
...
def pcb05_tc01_c2v2c_chain(
    ...
    for k in range(K):
        ...
        c = tl.load(c_ptr + offs_k * stride_c, mask=offs_k < K, other=0.0)  # (K,)
        vec_result = cube1_result + c[:, None]  # (K, K)

        d = tl.load(d_ptr + k * stride_dm + offs_k * stride_dk, mask=offs_k < K, other=0.0)  # (K,)
        cube2_result = tl.dot(vec_result, d[:, None]).to(tl.float16)  # (K, 1)
        ...
        sum_result = tl.sum(e + f)  # scalar
        tl.store(out_vec_ptr, sum_result)  # scalar store


# ----------------------------------------------------------------------------
# PCB05-TC02: float32, M=128, N=64, K=32
# 测试目的: 验证float32下C2V2C链依赖 + 独立VECTOR(sum)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
def pcb05_tc02_c2v2c_chain(
    ...
    for k in range(K):
        ...
        d = tl.load(d_ptr + k * stride_dm + offs_k * stride_dk, mask=offs_k < K, other=0.0)  # (K,)
        cube2_result = tl.dot(vec_result, d[:, None])  # (K, 1)
        out_cube_ptrs = out_cube_ptr + offs_k[:, None] * stride_out_cube
        tl.store(out_cube_ptrs, cube2_result, mask=offs_k[:, None] < K)
...
def test_pcb05_tc01():
    """PCB05-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译pcb05_tc01_c2v2c_chain kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
    signature = _build_pcb05_signature("fp16")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb05_tc01_c2v2c_chain, signature, constants)
    _write_mlir_to_file(mlir, "pcb05_tc01_c2v2c_chain.mlir")

    assert mlir and len(mlir) > 0, "MLIR代码生成失败或为空"
    assert "func.func @pcb05_tc01_c2v2c_chain(" in mlir, \
...
def test_pcb05_tc02():
    ...
    测试步骤:
      1. 编译pcb05_tc02_c2v2c_chain kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
    signature = _build_pcb05_signature("fp32")
    constants = {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}

    mlir = compile_kernel(pcb05_tc02_c2v2c_chain, signature, constants)
    _write_mlir_to_file(mlir, "pcb05_tc02_c2v2c_chain.mlir")

    assert mlir and len(mlir) > 0, "MLIR代码生成失败或为空"
    assert "func.func @pcb05_tc02_c2v2c_chain(" in mlir, \
        "MLIR代码中未找到kernel函数定义"
    assert "scope" in mlir, "MLIR代码中未包含'scope'关键字"
...
if __name__ == "__main__":
    test_pcb05_tc01()
    test_pcb05_tc02()
    print("All PCB05 v3 MLIR validation tests passed!")
