...
# ----------------------------------------------------------------------------
# PCB08-TC01: float16, M=128, N=64, K=32
# 测试目的: 验证float16下Cube with transpose upstream + C2V dependency (tl.argmax)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
# ----------------------------------------------------------------------------
# PCB08-TC02: float32, M=128, N=64, K=32
# 测试目的: 验证float32下Cube with transpose upstream + C2V dependency (tl.argmax)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_pcb08_signature(dtype_str):
    """构建PCB08 kernel的参数类型签名。

    注意: out_ptr存储argmax结果,类型为i64。
    """
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
...
def test_pcb08_tc01():
    """PCB08-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译pcb08_tc01_kernel kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_pcb08_tc02():
    """PCB08-TC02: 验证float32 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译pcb08_tc02_kernel kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
