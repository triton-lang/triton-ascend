...
# ----------------------------------------------------------------------------
# PCB09-TC02: float32, M=128, N=64, K=32
# 测试目的: 验证float32下Cube with fill upstream + C2V dependency (tl.argmin)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def test_pcb09_tc01():
    """PCB09-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译pcb09_tc01_kernel kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_pcb09_tc02():
    ...
    测试步骤:
      1. 编译pcb09_tc02_kernel kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
