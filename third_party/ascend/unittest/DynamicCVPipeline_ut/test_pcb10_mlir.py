...
# ----------------------------------------------------------------------------
# PCB10-TC02: float32, M=128, N=64, K=32
# 测试目的: 验证float32下multiple V2C dependency (tl.xor_sum + add -> Cube)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def test_pcb10_tc01():
    """PCB10-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译pcb10_tc01_kernel kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_pcb10_tc02():
    ...
    测试步骤:
      1. 编译pcb10_tc02_kernel kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
