...
# ----------------------------------------------------------------------------
# PCB14-TC01: float16, M=128, N=64, K=32
# 测试目的: 验证float16下while循环A*B+C(C=广播scalar)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
def pcb14_tc01_while_matmul_scalar(
    a_ptr, b_ptr, c_scalar_ptr, out_ptr,
    M, N, K,
    stride_am, stride_ak,
    stride_bk, stride_bn,
...
# ----------------------------------------------------------------------------
# PCB14-TC02: float32, M=128, N=64, K=32
# 测试目的: 验证float32下while循环A*B+C(C= broadcast scalar)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def test_pcb14_tc01():
    """PCB14-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译pcb14_tc01_while_matmul_scalar kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_pcb14_tc02():
    """PCB14-TC02: 验证float32 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译pcb14_tc02_while_matmul_scalar kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
