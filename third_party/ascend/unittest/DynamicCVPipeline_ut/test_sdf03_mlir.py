...
# ----------------------------------------------------------------------------
# SDF03-TC01: float16, M=128, N=64, K=128
# 测试目的: 验证float16下V2C依赖 + RAW内存依赖的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
def sdf03_tc01_v2c_raw(
    ...
    for k in range(K):
        ...
        tl.store(out_ptr + offs_k[:, None] * stride_out_kk + offs_n[None, :] * stride_out_k, cube_result, mask=store_mask)


# ----------------------------------------------------------------------------
# SDF03-TC02: float32, M=128, N=64, K=128
# 测试目的: 验证float32下V2C依赖 + RAW内存依赖的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def test_sdf03_tc01():
    """SDF03-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译sdf03_tc01_v2c_raw kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_sdf03_tc02():
    """SDF03-TC02: 验证float32 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译sdf03_tc02_v2c_raw kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
