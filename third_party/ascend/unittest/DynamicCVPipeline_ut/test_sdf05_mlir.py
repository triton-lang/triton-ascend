...
# ----------------------------------------------------------------------------
# SDF05-TC01: float16, M=128, N=64, K=128, L=3
# 测试目的: 验证float16下2层嵌套,内层独立CV的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
def sdf05_tc01_nested(
    a_ptr, b_ptr, c_ptr, d_ptr, e_ptr, f_ptr, g_ptr, h_ptr, out1_ptr, out2_ptr,
    M, N, K, L,
    stride_am, stride_ak,
    stride_bk, stride_bn,
    stride_c, stride_d,
    ...
    for i in range(L):
        ...
        tl.store(out1_ptr + offs_k[:, None] * stride_out1_kk + offs_k[None, :] * stride_out1_k, outer_cube, mask=store_mask_k)
...
# ----------------------------------------------------------------------------
# SDF05-TC02: float32, M=128, N=64, K=128, L=3
# 测试目的: 验证float32下2层嵌套,内层独立CV的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def test_sdf05_tc01():
    """SDF05-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译sdf05_tc01_nested kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_sdf05_tc02():
    """SDF05-TC02: 验证float32 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译sdf05_tc02_nested kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
