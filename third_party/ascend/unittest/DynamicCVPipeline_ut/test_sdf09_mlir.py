...
# ----------------------------------------------------------------------------
# SDF09-TC02: float32, M=128, N=64, K=32, L=3
# 测试目的: 验证float32下2层嵌套C2C跨层依赖(外层CV -> 内层CV, iter_args)的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def test_sdf09_tc01():
    """SDF09-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译sdf09_tc01_c2c_iter_args kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_sdf09_tc02():
    ...
    测试步骤:
      1. 编译sdf09_tc02_c2c_iter_args kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
