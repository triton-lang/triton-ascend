...
# ----------------------------------------------------------------------------
# SDF15-TC02: float32, M=128, N=64, K=32, L=3, P=2
# 测试目的: 验证float32下3层嵌套内层C依赖外层C的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
...
def _build_sdf15_signature(dtype_str):
    ...
    注意: SDF15中K作为constexpr参数,不出现在signature中。
    """
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
...
def test_sdf15_tc01():
    """SDF15-TC01: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译sdf15_tc01_inner_c_dep_outer_c kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
...
def test_sdf15_tc02():
    ...
    测试步骤:
      1. 编译sdf15_tc02_inner_c_dep_outer_c kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
      4. 校验MLIR代码中包含"scope"关键字
    """
