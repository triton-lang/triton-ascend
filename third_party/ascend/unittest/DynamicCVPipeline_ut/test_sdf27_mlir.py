...
# 测试目的: 验证float16下5层嵌套内层C依赖外层C的MLIR生成
# ----------------------------------------------------------------------------
@triton.jit
def sdf27(
    ...
    stride_em, stride_ek, stride_fn, stride_out,
    K: tl.constexpr,
...
def _build_sdf27_signature(dtype_str):
    """构建SDF27 kernel的参数类型签名。

    注意: SDF27中K作为constexpr参数,不出现在signature中。
    """
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
...
def test_sdf27():
    """SDF27: 验证float16 kernel编译生成的MLIR代码正确性。

    测试步骤:
      1. 编译sdf27 kernel为MLIR
      2. 校验MLIR代码成功生成且非空
      3. 校验MLIR代码中包含函数定义
