...
def _assert_build_uses_expected_flags(
    utils, monkeypatch, tmp_path, *, obj_name, expected_flag_provider, forbidden_flag_provider
):
    ...
    src_path = tmp_path / f"{obj_name}.cpp"
    src_path.write_text("int main() { return 0; }\n")

    so_path = utils._build_npu_ext(obj_name, None, str(src_path), kernel_launcher="torch")

    assert so_path.endswith(".so")
    assert any(name == expected_flag_provider for name, _, _ in calls)
    assert not any(name == forbidden_flag_provider for name, _, _ in calls)


def test_npu_utils_build_uses_special_flags(monkeypatch, tmp_path):
    utils = _load_utils_module()
    _assert_build_uses_expected_flags(
        utils,
        monkeypatch,
        tmp_path,
        obj_name="npu_utils",
        expected_flag_provider="get_cc_cmd_npu_utils",
        forbidden_flag_provider="get_cc_cmd",
    )


def test_launcher_build_uses_general_flags(monkeypatch, tmp_path):
    utils = _load_utils_module()
    _assert_build_uses_expected_flags(
        utils,
        monkeypatch,
        tmp_path,
        obj_name="launcher",
        expected_flag_provider="get_cc_cmd",
        forbidden_flag_provider="get_cc_cmd_npu_utils",
    )
