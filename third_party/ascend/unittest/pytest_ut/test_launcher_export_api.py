...
def test_npu_launcher_exposes_launcher_so_path(
    ...
    assert not hasattr(launcher, "launcher_so_path")
    assert mock_launcher_stub.call_count == 1
    assert launcher.get_launcher_so_path() == "/tmp/fake_launcher.so"
    assert mock_header_src.call_count == 2
    assert mock_wrapper_src.call_count == 2
    assert mock_launcher_stub.call_count == 2
