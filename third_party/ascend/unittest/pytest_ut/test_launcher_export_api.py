...
@patch.object(driver, "get_ascend_arch_from_env", return_value="Ascend910B")
@patch.object(driver, "get_backend_func", side_effect=_mock_backend_func)
def test_generate_npu_wrapper_src_parses_signed_int8(
    _mock_backend_func_patch,
    _mock_arch,
    _mock_ffts,
    _mock_disable_ffts,
    ...
    src = driver.generate_npu_wrapper_src(
        constants={},
        signature={0: "i8", 1: "i16"},
        metadata=_make_metadata(),
    )

    assert 'args, "iiiKKOOOOO&h",' in src
    assert "_parse_int8, &_arg0, &_arg1" in src
    assert "int8_t _arg0;" in src
    assert "value < std::numeric_limits<int8_t>::min()" in src
    assert "value > std::numeric_limits<int8_t>::max()" in src
