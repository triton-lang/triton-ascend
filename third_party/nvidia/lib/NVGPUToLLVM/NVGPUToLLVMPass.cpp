...
const std::string Fence_Mbarrier_Init_Op = "fence.mbarrier_init.release.cluster;";
...
Value convertToType(Value val, std::string constraint, Location loc, PatternRewriter &rewriter)
{
    ...
    if (!isConstraintNumber) {
        ...
        if (isa<LLVM::LLVMPointerType>(val.getType())) {
            ...
            assert(val.getType().getIntOrFloatBitWidth() <= ty.getIntOrFloatBitWidth() &&
...
SmallVector<PTXBuilder::Operand *> getPtxOutputs(const nvgpu::Constraints &outputConstraints, PTXBuilder &ptxBuilder)
{
...
OperandsAndConstraints unpackOperands(const OperandsAndConstraints &operandsAndConstraints, PTXBuilder &ptxBuilder,
            ...
            for (unsigned i = 0; i < llvmStruct.getBody().size(); i++) {
                if (isConstraintNumber) {
                    ...
                    unpackedOperands.push_back(
                        {extract_val(llvmStruct.getBody()[i], operand, i), std::to_string(constraintInt)});
                } else {
                    unpackedOperands.push_back({extract_val(llvmStruct.getBody()[i], operand, i), constraint});
                }
            }
        } else {
...
SmallVector<PTXBuilder::Operand *> getPtxOperands(const OperandsAndConstraints &operandsAndConstraints,
                                                  PTXBuilder &ptxBuilder, Location loc, PatternRewriter &rewriter)
{
    SmallVector<PTXBuilder::Operand *> ptxOperands;
    auto unpackedOperandsAndConstraints = unpackOperands(operandsAndConstraints, ptxBuilder, loc, rewriter);
...
std::string patchPtxAsm(Operation *op, std::string ptxAsm)
{
    ...
    while (start != std::string::npos) {
        auto endIterator =
            std::find_if(ptxAsm.begin() + start + 1, ptxAsm.end(), [](unsigned char c) { return !std::isalnum(c); });
    ...
    assert(patchLocations.size() == patchValues.size() && "patchLocations and patchValues should have the same size");
...
template <typename SourceOp> class NVGPUOpGenericPattern : public OpRewritePattern<SourceOp> {
  public:
    explicit NVGPUOpGenericPattern(MLIRContext *context, std::string ptxAsm, Constraints outputConstraints,
                                   Constraints inputConstraints)
        : OpRewritePattern<SourceOp>(context), ptxAsm(std::move(ptxAsm)), outputConstraints(outputConstraints),
    ...
    LogicalResult matchAndRewrite(SourceOp op, PatternRewriter &rewriter) const override
    {
        ...
        for (unsigned i = 0; i < inputConstraints.size(); i++) {
            operandsAndConstraints.push_back({op->getOperand(i), inputConstraints[i]});
        }
        return rewriteAsPtxAsm(op, rewriter, ptxAsm, operandsAndConstraints, outputConstraints);
    }
...
class FenceAsyncSharedOpPattern : public OpRewritePattern<ttn::FenceAsyncSharedOp> {
  public:
    using OpRewritePattern<ttn::FenceAsyncSharedOp>::OpRewritePattern;

    LogicalResult matchAndRewrite(ttn::FenceAsyncSharedOp op, PatternRewriter &rewriter) const override
    {
        std::string ptxAsm = op.getBCluster() ? "fence.proxy.async.shared::cluster;" : "fence.proxy.async.shared::cta;";
...
class ClusterArriveOpPattern : public OpRewritePattern<ttn::ClusterArriveOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttn::ClusterArriveOp op, PatternRewriter &rewriter) const override
    {
        std::string ptxAsm =
            op.getRelaxed() ? "barrier.cluster.arrive.relaxed.aligned;" : "barrier.cluster.arrive.aligned;";
        return rewriteAsPtxAsm(op, rewriter, std::move(ptxAsm));
    }
};
...
class StoreMatrixOpPattern : public OpRewritePattern<ttn::StoreMatrixOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttn::StoreMatrixOp op, PatternRewriter &rewriter) const override
    {
        return rewriteAsPtxAsm(op, rewriter, getPtxAsm(op), getOperandsAndConstraints(op));
    }

    OperandsAndConstraints getOperandsAndConstraints(ttn::StoreMatrixOp op) const
    {
    ...
    std::string getPtxAsm(ttn::StoreMatrixOp op) const
    {
        ...
        switch (datas.size()) {
            ...
            case 2:
                ...
                break;
            case 4:
                ptxAsm = "stmatrix.sync.aligned.m8n8.x4.shared.b16 [$0], {$1, $2, $3, $4};";
                break;
            default:
...
class MBarrierArriveOpPattern : public OpRewritePattern<ttn::MBarrierArriveOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttn::MBarrierArriveOp op, PatternRewriter &rewriter) const override
    {
        return rewriteAsPtxAsm(op, rewriter, getPtxAsm(op), getOperandsAndConstraints(op));
    }

    OperandsAndConstraints getOperandsAndConstraints(ttn::MBarrierArriveOp op) const
    {
        OperandsAndConstraints operandsAndTypes;
    ...
    std::string getPtxAsm(ttn::MBarrierArriveOp op) const
    {
        ...
        switch (arriveType) {
            ...
            case ttn::MBarriveType::expect_tx:
                ...
                ptxAsm = "@$1 mbarrier.arrive.expect_tx.shared.b64 _, [$0], " + std::to_string(txCount) + ";";
                break;
            case ttn::MBarriveType::remote:
                assert(ctaId && "ctaId should have a valid value");
                ptxAsm = " { .reg .b32 remAddr32;                                       \n"
...
class WGMMAWaitGroupOpPattern : public OpRewritePattern<ttn::WGMMAWaitGroupOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttn::WGMMAWaitGroupOp op, PatternRewriter &rewriter) const override
    {
        return rewriteAsPtxAsm(op, rewriter, getPtxAsm(op), getOperandsAndConstraints(op), getOutputConstraints(op));
    }
    ...
    OperandsAndConstraints getOperandsAndConstraints(ttn::WGMMAWaitGroupOp op) const
    {
        OperandsAndConstraints operandsAndConstraints;
...
class WGMMAOpPattern : public OpRewritePattern<ttn::WGMMAOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttn::WGMMAOp op, PatternRewriter &rewriter) const override
    {
        return rewriteAsPtxAsm(op, rewriter, getPtxAsm(op), getOperandsAndConstraints(op), getOutputConstraints(op));
    }
    ...
    std::string getPtxAsm(ttn::WGMMAOp op) const
    {
        ...
        supported |= (eltTypeA == WGMMAEltType::f16) && (eltTypeB == WGMMAEltType::f16) &&
                     (eltTypeC == WGMMAEltType::f16 || eltTypeC == WGMMAEltType::f32) &&
                     (m == 64 && 8 <= n && n <= 256 && k == 16);
        supported |= (eltTypeA == WGMMAEltType::bf16) && (eltTypeB == WGMMAEltType::bf16) &&
                     (eltTypeC == WGMMAEltType::f32) && (m == 64 && 8 <= n && n <= 256 && k == 16);
        needTransArgs = supported;
        floatTypeWGMMA = supported;
        // Below instructions do not support transposing
        if (!supported && !transA && !transB) {
            supported |= (eltTypeA == WGMMAEltType::tf32) && (eltTypeB == WGMMAEltType::tf32) &&
                         (eltTypeC == WGMMAEltType::f32) && (m == 64 && 8 <= n && n <= 256 && k == 8);
            supported |= (eltTypeA == WGMMAEltType::e4m3 || eltTypeA == WGMMAEltType::e5m2) &&
                         (eltTypeB == WGMMAEltType::e4m3 || eltTypeB == WGMMAEltType::e5m2) &&
                         (eltTypeC == WGMMAEltType::f16 || eltTypeC == WGMMAEltType::f32) &&
                         (m == 64 && 8 <= n && n <= 256 && k == 32);
            floatTypeWGMMA = supported;
            // Below instructions are integer-based
            supported |= (eltTypeA == WGMMAEltType::s8) && (eltTypeB == WGMMAEltType::s8) &&
                         (eltTypeC == WGMMAEltType::s32) && (m == 64 && 8 <= n && n <= 224 && k == 32);
        }
        ...
        auto ptxAsm = "wgmma.mma_async.sync.aligned"
                      ...
                      stringifyEnum(eltTypeC).str() + "." + stringifyEnum(eltTypeA).str() + "." +
...
#define POPULATE_NVGPU_OP(SRC_OP, ASM)                                                                                 \
    patterns.add<NVGPUOpGenericPattern<SRC_OP>>(context, ASM, Constraints(), Constraints());
...
#undef POPULATE_NVGPU_OP
        patterns.add<NVGPUOpGenericPattern<ttn::NamedBarrierArriveOp>>(context, Named_Barrier_Arrive_Op, Constraints(),
                                                                       Constraints({"r", "r"}));
        patterns.add<NVGPUOpGenericPattern<ttn::NamedBarrierWaitOp>>(context, Named_Barrier_Wait_Op, Constraints(),
                                                                     Constraints({"r", "r"}));
        patterns.add<NVGPUOpGenericPattern<ttn::ClusterCTAIdOp>>(context, Cluster_Cta_Id_Op, Constraints({"=r"}),
                                                                 Constraints());
        patterns.add<NVGPUOpGenericPattern<ttn::CanonicalWarpIdOp>>(context, Canonical_Warp_Id_Op, Constraints({"=r"}),
                                                                    Constraints());

        patterns.add<FenceAsyncSharedOpPattern, StoreMatrixOpPattern, MBarrierArriveOpPattern, ClusterArriveOpPattern,
...
LogicalResult nvgpu::rewriteAsPtxAsm(Operation *op, PatternRewriter &rewriter, std::string ptxAsm,
                                     ...
                                     const Constraints &outputConstraints)
{
    ...
    auto ptxOperands = getPtxOperands(operandsAndConstraints, ptxBuilder, loc, rewriter);
    ...
    auto retTy = op->getNumResults() == 0 ? void_ty(ctx) : op->getResult(0).getType();
