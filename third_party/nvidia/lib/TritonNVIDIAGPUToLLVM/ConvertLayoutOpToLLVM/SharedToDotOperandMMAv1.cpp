...
static std::tuple<Value, Value, Value, Value> computeOffsets(Value threadId, bool isARow, bool isBRow,
                                                             ...
                                                             Type resultTy)
{
    ...
    auto wpt = cast<NvidiaMmaEncodingAttr>(
                   cast<DotOperandEncodingAttr>(cast<RankedTensorType>(resultTy).getEncoding()).getParent())
                   .getWarpsPerCTA();
...
static Value loadA(Value tensor, const SharedMemoryObject &smemObj, Value thread, Location loc,
                   const LLVMTypeConverter *typeConverter, ConversionPatternRewriter &rewriter, Type resultTy)
{
    ...
    auto resultEncoding = cast<DotOperandEncodingAttr>(cast<RankedTensorType>(resultTy).getEncoding());
    auto [offsetAM, offsetAK, _3, _4] =
        computeOffsets(thread, isARow, false, fpw, mmaEncoding.getMMAv1ShapePerWarp(resultEncoding.getOpIdx()),
                       mmaEncoding.getMMAv1Rep(resultEncoding.getOpIdx()), rewriter, loc, resultTy);
    ...
    auto ld = [&](decltype(has) &vals, int m, int k, Value val0, Value val1) { vals[{m, k}] = {val0, val1}; };
    auto loadA = [&](int m, int k) {
        ...
        Value offset = add(mul(i32_val(stepAM * strideRepM), strideAM), mul(i32_val(stepAK), strideAK));
    ...
    unsigned numM = mmaEncoding.getMMAv1NumOuter(shape, resultEncoding.getOpIdx());
...
static Value loadB(Value tensor, const SharedMemoryObject &smemObj, Value thread, Location loc,
                   const LLVMTypeConverter *typeConverter, ConversionPatternRewriter &rewriter, Type resultTy)
{
    static constexpr std::array<int, 3> fpw {{2, 2, 1}};
    auto mmaEncoding = cast<NvidiaMmaEncodingAttr>(
    ...
    auto [_3, _4, offsetBN, offsetBK] =
        computeOffsets(thread, false, isBRow, fpw, mmaEncoding.getMMAv1ShapePerWarp(resultEncoding.getOpIdx()),
                       mmaEncoding.getMMAv1Rep(resultEncoding.getOpIdx()), rewriter, loc, resultTy);
    ...
    auto ld = [&](decltype(hbs) &vals, int m, int k, Value val0, Value val1) { vals[{m, k}] = {val0, val1}; };
    ...
    auto loadB = [&](int n, int K) {
        ...
        Value offset = add(mul(i32_val(stepBN * strideRepN), strideBN), mul(i32_val(stepBK), strideBK));
    ...
    unsigned numN = mmaEncoding.getMMAv1NumOuter(shape, resultEncoding.getOpIdx());
    for (unsigned k = 0; k < NK; k += 4)
...
{
    if (opIdx == 0)
        return loadA(tensor, smemObj, thread, loc, typeConverter, rewriter, resultTy);
    else {
        assert(opIdx == 1);
        return loadB(tensor, smemObj, thread, loc, typeConverter, rewriter, resultTy);
    }
}
