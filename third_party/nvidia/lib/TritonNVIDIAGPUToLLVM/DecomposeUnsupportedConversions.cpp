...
class DecomposeLocalLoadToDotOperand : public OpRewritePattern<triton::gpu::LocalLoadOp> {
  public:
    using OpRewritePattern::OpRewritePattern;

    LogicalResult matchAndRewrite(triton::gpu::LocalLoadOp op, PatternRewriter &rewriter) const override
    {

        auto dstDotOp = dyn_cast<triton::gpu::DotOperandEncodingAttr>(op.getType().getEncoding());
        ...
        auto blockEncoding =
            getDefaultBlockedEncoding(op.getContext(), type.getShape(), numWarps, threadsPerWarp, numCTAs);
        auto tmpType = RankedTensorType::get(type.getShape(), type.getElementType(), blockEncoding);
        Value load = rewriter.create<LocalLoadOp>(op.getLoc(), tmpType, op.getSrc());
        auto newSharedDescTy =
            triton::MemDescType::get(type.getShape(), type.getElementType(),
                                     triton::gpu::SharedEncodingAttr::get(
                                         op.getContext(), dstDotOp, type.getShape(), triton::gpu::getOrder(parentEnc),
                                         triton::gpu::getCTALayout(parentEnc), type.getElementType()),
                                     srcType.getMemorySpace());
        auto tmp = rewriter.create<triton::gpu::LocalAllocOp>(op.getLoc(), newSharedDescTy, load);
        auto newConvert = rewriter.create<triton::gpu::LocalLoadOp>(op.getLoc(), type, tmp);
...
struct DecomposeUnsupportedConversions
    : public mlir::triton::impl::DecomposeUnsupportedNVIDIAConversionsBase<DecomposeUnsupportedConversions> {
    void runOnOperation() override
    {
        ModuleOp mod = getOperation();
        triton::gpu::decomposeSplatOpToSharedLayoutConversion(mod);
        triton::gpu::decomposeTensorCoreToDotLayoutConversion(mod, isMmaToDotShortcut);
