...
static ValueTable extractLoadedOperand(Value llStruct, int NK, ConversionPatternRewriter &rewriter,
                                       const LLVMTypeConverter *typeConverter, Type type)
{
    ValueTable rcds;
    SmallVector<Value> elems = unpackLLElements(llStruct.getLoc(), llStruct, rewriter);
                            ...
                            ConversionPatternRewriter &rewriter)
{
    ...
    auto mmaLayout = cast<NvidiaMmaEncodingAttr>(cast<RankedTensorType>(D.getType()).getEncoding());
    auto ALayout = cast<DotOperandEncodingAttr>(cast<RankedTensorType>(A.getType()).getEncoding());
    auto BLayout = cast<DotOperandEncodingAttr>(cast<RankedTensorType>(B.getType()).getEncoding());
    ...
    auto [isARow_, isBRow_, isAVec4_, isBVec4_, _] = mmaLayout.decodeVoltaLayoutStates();
    ...
    auto has = extractLoadedOperand(adaptor.getA(), NK, rewriter, typeConverter, ATensorTy);
    auto hbs = extractLoadedOperand(adaptor.getB(), NK, rewriter, typeConverter, BTensorTy);
