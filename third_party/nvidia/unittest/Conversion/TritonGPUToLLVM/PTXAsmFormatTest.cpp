  ...
  protected:
    ...
    void createValues()
    {
        ...
        for (int i = 0; i < numValues; i++) {
            v[i + 1] = builder.create<arith::ConstantIntOp>(builder.getUnknownLoc(), i, 32);
        }
    }
...
TEST_F(PTXAsmFormatTest, complexInstruction)
{
    ...
    EXPECT_EQ(builder.dump(), "@$1 ld.global.ca.L1::evict_first.L1::cache_hint.v2.b16 [ $0 + 128 ];");
    auto values = builder.getAllMLIRArgs();
...
{
    PTXBuilder builder;
    const char *ptxCode = ".param .b64 param0;\n" // prepare param0 (format string)
