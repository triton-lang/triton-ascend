...
class FlexibleMetric : public Metric {
  public:
    FlexibleMetric(const std::string &valueName, std::variant<MetricValueType> value, bool aggregable)
        : valueName(valueName), Metric(MetricKind::Flexible, 1), aggregable(aggregable)
    {
    ...
    const std::string getValueName(int valueId) const override { return valueName; }
...
class KernelMetric : public Metric {
  public:
    ...
    KernelMetric(uint64_t startTime, uint64_t endTime, uint64_t invocations, uint64_t deviceId, uint64_t deviceType)
        : KernelMetric()
    {
    ...
    virtual const std::string getValueName(int valueId) const { return VALUE_NAMES[valueId]; }

    virtual bool isAggregable(int valueId) const { return AGGREGABLE[valueId]; }

  private:
    const static inline bool AGGREGABLE[kernelMetricKind::Count] = {false, false, true, true, false, false};
    const static inline std::string VALUE_NAMES[kernelMetricKind::Count] = {
        "start_time (ns)", "end_time (ns)", "count", "time (ns)", "device_id", "device_type",
    };
};
...
class PCSamplingMetric : public Metric {
  public:
    ...
    PCSamplingMetric() : Metric(MetricKind::PCSampling, PCSamplingMetricKind::Count) {}

    PCSamplingMetric(PCSamplingMetricKind kind, uint64_t samples, uint64_t stalledSamples) : PCSamplingMetric()
    {
