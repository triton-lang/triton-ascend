...
#define DEFINE_DISPATCH_TEMPLATE(CheckSuccess, FuncName, ExternLib, FuncType, ...)                                     \
    template <> ExternLib::RetType FuncName<CheckSuccess>(DISPATCH_ARGS(__VA_ARGS__))                                  \
    {                                                                                                                  \
        typedef typename ExternLib::RetType (*FuncType##_t)(__VA_ARGS__);                                              \
        static FuncType##_t func = nullptr;                                                                            \
        return Dispatch<ExternLib>::exec<CheckSuccess, FuncType##_t>(func, #FuncType DISPATCH_VALS(__VA_ARGS__));      \
    }
  ...
  public:
    ...
    static void check(typename ExternLib::RetType ret, const char *functionName)
    {
        if (ret != ExternLib::success) {
            throw std::runtime_error("Failed to execute " + std::string(functionName) + " with error " +
                                     std::to_string(ret));
        }
    }

    template <bool CheckSuccess, typename FnT, typename... Args>
    static typename ExternLib::RetType exec(FnT &handler, const char *functionName, Args... args)
    {
        init(ExternLib::name, &ExternLib::lib);
        if (handler == nullptr) {
            handler = reinterpret_cast<FnT>(dlsym(ExternLib::lib, functionName));
            if (handler == nullptr) {
                throw std::runtime_error("Failed to load " + std::string(ExternLib::name));
            }
        }
