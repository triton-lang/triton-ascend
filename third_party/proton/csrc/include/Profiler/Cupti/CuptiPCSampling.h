...
struct CubinData {
    ...
    struct LineInfoKey {
        ...
        bool operator<(const LineInfoKey &other) const
        {
            return functionIndex < other.functionIndex ||
                   (functionIndex == other.functionIndex && pcOffset < other.pcOffset);
        }
    };
    ...
    struct LineInfoValue {
        ...
        LineInfoValue(uint32_t lineNumber, const std::string &functionName, const std::string &dirName,
                      const std::string &fileName)
            : lineNumber(lineNumber), functionName(functionName), dirName(dirName), fileName(fileName)
        {
        }
    };
...
class CuptiPCSampling : public Singleton<CuptiPCSampling> {
  ...
  private:
    ...
    void processPCSamplingData(ConfigureData *configureData, uint64_t externId, bool isAPI);
    ...
    // all of them
    ThreadSafeMap<size_t, std::pair<CubinData, /*count=*/size_t>> cubinCrcToCubinData;
