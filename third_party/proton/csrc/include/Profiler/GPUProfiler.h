...
class GPUProfiler : public Profiler, public ThreadLocalOpInterface, public Singleton<ConcreteProfilerT> {
  public:
    GPUProfiler() = default;
    virtual ~GPUProfiler() = default;

    using CorrIdToExternIdMap = ThreadSafeMap<uint64_t, std::pair<size_t, size_t>, /*<extern_id, num_kernels>*/
  ...
  protected:
    // OpInterface
    void startOp(const Scope &scope) override { this->correlation.pushExternId(scope.scopeId); }
    ...
    struct Correlation {
        ...
        void submit(const uint64_t correlationId) { atomicMax(maxSubmittedCorrelationId, correlationId); }

        void complete(const uint64_t correlationId) { atomicMax(maxCompletedCorrelationId, correlationId); }
        ...
        template <typename FlushFnT> void flush(uint64_t maxRetries, uint64_t sleepMs, FlushFnT &&flushFn)
        {
            flushFn();
    ...
    class GPUProfilerPimplInterface {
      public:
        GPUProfilerPimplInterface(ConcreteProfilerT &profiler) : profiler(profiler) {}
