...
class Session {
  ...
  private:
    Session(size_t id, const std::string &path, Profiler *profiler, std::unique_ptr<ContextSource> contextSource,
            std::unique_ptr<Data> data)
        : id(id), path(path), profiler(profiler), contextSource(std::move(contextSource)), data(std::move(data))
    {
    }
...
class SessionManager : public Singleton<SessionManager> {
  public:
    ...
    size_t addSession(const std::string &path, const std::string &profilerName, const std::string &contextSourceName,
    ...
    void addMetrics(size_t scopeId, const std::map<std::string, MetricValueType> &metrics, bool aggregable);

  private:
    std::unique_ptr<Session> makeSession(size_t id, const std::string &path, const std::string &profilerName,
    ...
    bool hasSession(const std::string &path) { return sessionPaths.find(path) != sessionPaths.end(); }

    bool hasSession(size_t sessionId) { return sessions.find(sessionId) != sessions.end(); }

    void removeSession(size_t sessionId);

    template <typename Interface, typename Counter> void registerInterface(size_t sessionId, Counter &interfaceCounts)
    {
    ...
    template <typename Interface, typename Counter> void unregisterInterface(size_t sessionId, Counter &interfaceCounts)
    {
        auto interfaces = sessions[sessionId]->getInterfaces<Interface>();
        for (auto *interface : interfaces) {
            interfaceCounts[interface] -= 1;
