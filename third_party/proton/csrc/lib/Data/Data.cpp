...
void Data::dump(OutputFormat outputFormat)
{
    ...
    if (path.empty() || path == "-") {
        ...
        out.reset(new std::ofstream(path + "." + outputFormatToString(outputFormat))); // Opening a file for output
    }
...
const std::string outputFormatToString(OutputFormat outputFormat)
{
    ...
    throw std::runtime_error("Unknown output format: " + std::to_string(static_cast<int>(outputFormat)));
}
