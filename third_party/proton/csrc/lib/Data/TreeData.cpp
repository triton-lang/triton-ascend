...
class TreeData::Tree {
  public:
    struct TreeNode : public Context {
        ...
        TreeNode() = default;
        explicit TreeNode(size_t id, const std::string &name) : id(id), Context(name) {}
        TreeNode(size_t id, size_t parentId, const std::string &name) : id(id), parentId(parentId), Context(name) {}
        virtual ~TreeNode() = default;

        void addChild(const Context &context, size_t id) { children[context] = id; }

        bool hasChild(const Context &context) const { return children.find(context) != children.end(); }

        size_t getChild(const Context &context) const { return children.at(context); }
    ...
    Tree() { treeNodeMap.try_emplace(TreeNode::RootId, TreeNode::RootId, "ROOT"); }
...
void TreeData::addMetrics(size_t scopeId, const std::map<std::string, MetricValueType> &metrics, bool aggregable)
{
    std::unique_lock<std::shared_mutex> lock(mutex);
    auto scopeIdIt = scopeIdToContextId.find(scopeId);
    ...
    for (auto [metricName, metricValue] : metrics) {
        if (node.flexibleMetrics.find(metricName) == node.flexibleMetrics.end())
            node.flexibleMetrics.emplace(metricName, FlexibleMetric(metricName, metricValue, aggregable));
        else {
...
void TreeData::dumpHatchet(std::ostream &os) const
{
    ...
    this->tree->template walk<Tree::WalkPolicy::PreOrder>([&](Tree::TreeNode &treeNode) {
        ...
        for (auto [metricKind, metric] : treeNode.metrics) {
            if (metricKind == MetricKind::Kernel) {
                std::shared_ptr<KernelMetric> kernelMetric = std::dynamic_pointer_cast<KernelMetric>(metric);
                uint64_t duration = std::get<uint64_t>(kernelMetric->getValue(KernelMetric::Duration));
                uint64_t invocations = std::get<uint64_t>(kernelMetric->getValue(KernelMetric::Invocations));
                uint64_t deviceId = std::get<uint64_t>(kernelMetric->getValue(KernelMetric::DeviceId));
                uint64_t deviceType = std::get<uint64_t>(kernelMetric->getValue(KernelMetric::DeviceType));
                std::string deviceTypeName = getDeviceTypeString(static_cast<DeviceType>(deviceType));
                (*jsonNode)["metrics"][kernelMetric->getValueName(KernelMetric::Duration)] = duration;
                (*jsonNode)["metrics"][kernelMetric->getValueName(KernelMetric::Invocations)] = invocations;
                (*jsonNode)["metrics"][kernelMetric->getValueName(KernelMetric::DeviceId)] = std::to_string(deviceId);
                (*jsonNode)["metrics"][kernelMetric->getValueName(KernelMetric::DeviceType)] = deviceTypeName;
                valueNames.insert(kernelMetric->getValueName(KernelMetric::Duration));
                valueNames.insert(kernelMetric->getValueName(KernelMetric::Invocations));
                deviceIds.insert({deviceType, {deviceId}});
            } else if (metricKind == MetricKind::PCSampling) {
                auto pcSamplingMetric = std::dynamic_pointer_cast<PCSamplingMetric>(metric);
                for (size_t i = 0; i < PCSamplingMetric::Count; i++) {
                    auto valueName = pcSamplingMetric->getValueName(i);
                    valueNames.insert(valueName);
                    std::visit([&](auto &&value) { (*jsonNode)["metrics"][valueName] = value; },
        ...
        for (auto [_, flexibleMetric] : treeNode.flexibleMetrics) {
            ...
            valueNames.insert(valueName);
            std::visit([&](auto &&value) { (*jsonNode)["metrics"][valueName] = value; }, flexibleMetric.getValues()[0]);
        }
    ...
    for (auto [deviceType, deviceIds] : deviceIds) {
        auto deviceTypeName = getDeviceTypeString(static_cast<DeviceType>(deviceType));
        ...
        for (auto deviceId : deviceIds) {
            ...
            deviceJson[deviceTypeName][std::to_string(deviceId)] = {{"clock_rate", device.clockRate},
...
TreeData::TreeData(const std::string &path, ContextSource *contextSource) : Data(path, contextSource)
{
    init();
}
