...
DEFINE_DISPATCH(ExternLibHip, deviceGetAttribute, hipDeviceGetAttribute, int *, hipDeviceAttribute_t, int);
...
{
    int clockRate;
    (void)hip::deviceGetAttribute<true>(&clockRate, hipDeviceAttributeClockRate, index);
    int memoryClockRate;
    (void)hip::deviceGetAttribute<true>(&memoryClockRate, hipDeviceAttributeMemoryClockRate, index);
    int busWidth;
    (void)hip::deviceGetAttribute<true>(&busWidth, hipDeviceAttributeMemoryBusWidth, index);
    int smCount;
    (void)hip::deviceGetAttribute<true>(&smCount, hipDeviceAttributeMultiprocessorCount, index);

    std::string arch = getHipArchName(index);

    return Device(DeviceType::HIP, index, clockRate, memoryClockRate, busWidth, smCount, arch);
}
...
const char *getKernelNameRef(const hipFunction_t f)
{
    ...
    if (func == nullptr)
        func = reinterpret_cast<hipKernelNameRef_t>(dlsym(ExternLibHip::lib, "hipKernelNameRef"));
...
const char *getKernelNameRefByPtr(const void *hostFunction, hipStream_t stream)
{
    ...
    Dispatch<ExternLibHip>::init(ExternLibHip::name, &ExternLibHip::lib);
    if (func == nullptr)
        func = reinterpret_cast<hipKernelNameRefByPtr_t>(dlsym(ExternLibHip::lib, "hipKernelNameRefByPtr"));
