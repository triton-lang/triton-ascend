...
size_t getNumStallReasons(CUcontext context)
{
    ...
    CUpti_PCSamplingGetNumStallReasonsParams numStallReasonsParams = {.size =
...
std::tuple<uint32_t, std::string, std::string> getSassToSourceCorrelation(const char *functionName, uint64_t pcOffset,
                                                                          const char *cubin, size_t cubinSize)
{
    ...
    auto fileNameStr = sassToSourceParams.fileName ? std::string(sassToSourceParams.fileName) : "";
    auto dirNameStr = sassToSourceParams.dirName ? std::string(sassToSourceParams.dirName) : "";
    ...
    return std::make_tuple(sassToSourceParams.lineNumber, fileNameStr, dirNameStr);
}

std::pair<char **, uint32_t *> getStallReasonNamesAndIndices(CUcontext context, size_t numStallReasons)
{
    char **stallReasonNames = static_cast<char **>(std::calloc(numStallReasons, sizeof(char *)));
    for (size_t i = 0; i < numStallReasons; i++) {
        stallReasonNames[i] = static_cast<char *>(std::calloc(CUPTI_STALL_REASON_STRING_SIZE, sizeof(char)));
    }
    uint32_t *stallReasonIndices = static_cast<uint32_t *>(std::calloc(numStallReasons, sizeof(uint32_t)));
...
size_t matchStallReasonsToIndices(size_t numStallReasons, char **stallReasonNames, uint32_t *stallReasonIndices,
                                  ...
                                  std::set<size_t> &notIssuedStallReasonIndices)
{
    ...
    for (size_t i = 0; i < numStallReasons; i++) {
        bool notIssued = std::string(stallReasonNames[i]).find("not_issued") != std::string::npos;
...
CUpti_PCSamplingData allocPCSamplingData(size_t collectNumPCs, size_t numValidStallReasons)
{
    ...
    if (libVersion < CUPTI_CUDA12_4_VERSION && CUPTI_API_VERSION >= CUPTI_CUDA12_4_VERSION)
        pcDataSize -= CUPTI_CUDA12_4_PC_DATA_PADDING_SIZE;
    CUpti_PCSamplingData pcSamplingData {
        .size = pcDataSize,
        .collectNumPcs = collectNumPCs,
        .pPcData = static_cast<CUpti_PCSamplingPCData *>(std::calloc(collectNumPCs, sizeof(CUpti_PCSamplingPCData)))};
    for (size_t i = 0; i < collectNumPCs; ++i) {
        pcSamplingData.pPcData[i].stallReason = static_cast<CUpti_PCSamplingStallReason *>(
            std::calloc(numValidStallReasons, sizeof(CUpti_PCSamplingStallReason)));
    }
...
void getPCSamplingData(CUcontext context, CUpti_PCSamplingData *pcSamplingData)
{
...
void setConfigurationAttribute(CUcontext context, std::vector<CUpti_PCSamplingConfigurationInfo> &configurationInfos)
{
...
CUpti_PCSamplingConfigurationInfo ConfigureData::configureStallReasons()
{
    ...
    numValidStallReasons = matchStallReasonsToIndices(numStallReasons, stallReasonNames, stallReasonIndices,
                                                      stallReasonIndexToMetricIndex, notIssuedStallReasonIndices);
    CUpti_PCSamplingConfigurationInfo stallReasonInfo {};
    stallReasonInfo.attributeType = CUPTI_PC_SAMPLING_CONFIGURATION_ATTR_TYPE_STALL_REASON;
    stallReasonInfo.attributeData.stallReasonData.stallReasonCount = numValidStallReasons;
    stallReasonInfo.attributeData.stallReasonData.pStallReasonIndex = stallReasonIndices;
...
CUpti_PCSamplingConfigurationInfo ConfigureData::configureSamplingPeriod()
{
    ...
    samplingPeriodInfo.attributeType = CUPTI_PC_SAMPLING_CONFIGURATION_ATTR_TYPE_SAMPLING_PERIOD;
    samplingPeriodInfo.attributeData.samplingPeriodData.samplingPeriod = DefaultFrequency;
...
CUpti_PCSamplingConfigurationInfo ConfigureData::configureSamplingBuffer()
{
    ...
    samplingBufferInfo.attributeType = CUPTI_PC_SAMPLING_CONFIGURATION_ATTR_TYPE_SAMPLING_DATA_BUFFER;
    this->pcSamplingData = allocPCSamplingData(DataBufferPCCount, numValidStallReasons);
    samplingBufferInfo.attributeData.samplingDataBufferData.samplingDataBuffer = &this->pcSamplingData;
...
CUpti_PCSamplingConfigurationInfo ConfigureData::configureScratchBuffer()
{
    ...
    scratchBufferInfo.attributeType = CUPTI_PC_SAMPLING_CONFIGURATION_ATTR_TYPE_SCRATCH_BUFFER_SIZE;
    scratchBufferInfo.attributeData.scratchBufferSizeData.scratchBufferSize = ScratchBufferSize;
...
CUpti_PCSamplingConfigurationInfo ConfigureData::configureHardwareBufferSize()
{
    ...
    hardwareBufferInfo.attributeType = CUPTI_PC_SAMPLING_CONFIGURATION_ATTR_TYPE_HARDWARE_BUFFER_SIZE;
    hardwareBufferInfo.attributeData.hardwareBufferSizeData.hardwareBufferSize = HardwareBufferSize;
...
CUpti_PCSamplingConfigurationInfo ConfigureData::configureStartStopControl()
{
    ...
    startStopControlInfo.attributeType = CUPTI_PC_SAMPLING_CONFIGURATION_ATTR_TYPE_ENABLE_START_STOP_CONTROL;
    startStopControlInfo.attributeData.enableStartStopControlData.enableStartStopControl = true;
...
CUpti_PCSamplingConfigurationInfo ConfigureData::configureCollectionMode()
{
    ...
    collectionModeInfo.attributeType = CUPTI_PC_SAMPLING_CONFIGURATION_ATTR_TYPE_COLLECTION_MODE;
    collectionModeInfo.attributeData.collectionModeData.collectionMode = CUPTI_PC_SAMPLING_COLLECTION_MODE_CONTINUOUS;
...
void CuptiPCSampling::initialize(CUcontext context)
{
    ...
    cupti::getContextId<true>(context, &contextId);
    doubleCheckedLock([&]() { return !contextInitialized.contain(contextId); }, contextMutex,
                      [&]() {
...
void CuptiPCSampling::start(CUcontext context)
{
    uint32_t contextId = 0;
    cupti::getContextId<true>(context, &contextId);
    doubleCheckedLock([&]() -> bool { return !pcSamplingStarted; }, pcSamplingMutex,
                      [&]() {
                          initialize(context);
...
void CuptiPCSampling::processPCSamplingData(ConfigureData *configureData, uint64_t externId, bool isAPI)
{
    ...
    while (pcSamplingData->totalNumPcs > 0 || pcSamplingData->remainingNumPcs > 0 || firstRound) {
        // Handle data
        for (size_t i = 0; i < pcSamplingData->totalNumPcs; ++i) {
            ...
            auto key = CubinData::LineInfoKey {pcData->functionIndex, pcData->pcOffset};
            if (cubinData->lineInfo.find(key) == cubinData->lineInfo.end()) {
                auto [lineNumber, fileName, dirName] = getSassToSourceCorrelation(
                    pcData->functionName, pcData->pcOffset, cubinData->cubin, cubinData->cubinSize);
            ...
            for (size_t j = 0; j < pcData->stallReasonCount; ++j) {
                ...
                if (!configureData->stallReasonIndexToMetricIndex.count(stallReason->pcSamplingStallReasonIndex))
                ...
                for (auto *data : dataSet) {
                    ...
                    if (lineInfo.fileName.size())
                        scopeId = data->addScope(scopeId, lineInfo.dirName + "/" + lineInfo.fileName + ":" +
                                                              lineInfo.functionName + "@" +
                                                              std::to_string(lineInfo.lineNumber));
                    auto metricKind = static_cast<PCSamplingMetric::PCSamplingMetricKind>(
                        configureData->stallReasonIndexToMetricIndex[stallReason->pcSamplingStallReasonIndex]);
                    auto samples = stallReason->samples;
                    auto stalledSamples =
                        configureData->notIssuedStallReasonIndices.count(stallReason->pcSamplingStallReasonIndex)
                            ? 0
                            : samples;
                    auto metric = std::make_shared<PCSamplingMetric>(metricKind, samples, stalledSamples);
