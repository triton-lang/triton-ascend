...
template <> thread_local std::deque<size_t> GPUProfiler<RoctracerProfiler>::Correlation::externIdQueue {};
    ...
    void initDeviceOffset()
    {
        ...
        hsa::iterateAgents(
            [](hsa_agent_t agent, void *data) {
                ...
                int nodeId;
                hsa::agentGetInfo<true>(agent, static_cast<hsa_agent_info_t>(HSA_AMD_AGENT_INFO_DRIVER_NODE_ID),
                                        &nodeId);
                int deviceType;
                hsa::agentGetInfo<true>(agent, static_cast<hsa_agent_info_t>(HSA_AGENT_INFO_DEVICE), &deviceType);
        ...
        case kHipVdiCommandKernel:
            {
                if (activity->begin_ns < activity->end_ns) {
                    metric = std::make_shared<KernelMetric>(
                        static_cast<uint64_t>(activity->begin_ns), static_cast<uint64_t>(activity->end_ns), 1,
                        static_cast<uint64_t>(DeviceInfo::instance().mapDeviceId(activity->device_id)),
...
void processActivityKernel(RoctracerProfiler::CorrIdToExternIdMap &corrIdToExternId, size_t externId,
                           std::set<Data *> &dataSet, const roctracer_record_t *activity, bool isAPI, bool isGraph)
{
...
void processActivity(RoctracerProfiler::CorrIdToExternIdMap &corrIdToExternId,
                     RoctracerProfiler::ApiExternIdSet &apiExternIds, size_t externId, std::set<Data *> &dataSet,
                     const roctracer_record_t *record, bool isAPI, bool isGraph)
{
    switch (record->kind) {
        case 0x11F1: // Task - kernel enqueued by graph launch
        case kHipVdiCommandKernel:
            {
                processActivityKernel(corrIdToExternId, externId, dataSet, record, isAPI, isGraph);
                break;
            }
        default:
            break;
...
struct RoctracerProfiler::RoctracerProfilerPimpl : public GPUProfiler<RoctracerProfiler>::GPUProfilerPimplInterface {
    ...
    static void apiCallback(uint32_t domain, uint32_t cid, const void *callbackData, void *arg);
    static void activityCallback(const char *begin, const char *end, void *arg);

    static constexpr size_t BufferSize = 64 * 1024 * 1024;

    ThreadSafeMap<uint64_t, bool, std::unordered_map<uint64_t, bool>> CorrIdToIsHipGraph;

    ThreadSafeMap<hipGraphExec_t, hipGraph_t, std::unordered_map<hipGraphExec_t, hipGraph_t>> GraphExecToGraph;

    ThreadSafeMap<hipGraph_t, uint32_t, std::unordered_map<hipGraph_t, uint32_t>> GraphToNumInstances;

    ThreadSafeMap<hipStream_t, uint32_t, std::unordered_map<hipStream_t, uint32_t>> StreamToCaptureCount;

    ThreadSafeMap<hipStream_t, bool, std::unordered_map<hipStream_t, bool>> StreamToCapture;
};

void RoctracerProfiler::RoctracerProfilerPimpl::apiCallback(uint32_t domain, uint32_t cid, const void *callbackData,
                                                            void *arg)
{
    ...
    auto &profiler = dynamic_cast<RoctracerProfiler &>(RoctracerProfiler::instance());
    ...
    if (domain == ACTIVITY_DOMAIN_HIP_API) {
        ...
        if (data->phase == ACTIVITY_API_PHASE_ENTER) {
            ...
            if (cid == HIP_API_ID_hipGraphLaunch) {
                ...
                if (!findGraph)
                    std::cerr << "[PROTON] Cannot find graph and it may cause a memory leak."
...
void RoctracerProfiler::RoctracerProfilerPimpl::activityCallback(const char *begin, const char *end, void *arg)
{
    auto &profiler = dynamic_cast<RoctracerProfiler &>(RoctracerProfiler::instance());
    auto *pImpl = dynamic_cast<RoctracerProfiler::RoctracerProfilerPimpl *>(profiler.pImpl.get());
    ...
    const roctracer_record_t *endRecord = reinterpret_cast<const roctracer_record_t *>(end);
    uint64_t maxCorrelationId = 0;

    while (record != endRecord) {
        // Log latest completed correlation id.  Used to ensure we have flushed all
        // data on stop
        maxCorrelationId = std::max<uint64_t>(maxCorrelationId, record->correlation_id);
        // TODO(Keren): Roctracer doesn't support cuda graph yet.
        auto externId = correlation.corrIdToExternId.contain(record->correlation_id)
        ...
        processActivity(correlation.corrIdToExternId, correlation.apiExternIds, externId, dataSet, record, isAPI,
                        isGraph);
...
{
    roctracer::enableDomainCallback<true>(ACTIVITY_DOMAIN_HIP_API, apiCallback, nullptr);
    // Activity Records
