...
std::unique_ptr<Data> makeData(const std::string &dataName, const std::string &path, ContextSource *contextSource)
{
...
std::unique_ptr<ContextSource> makeContextSource(const std::string &contextSourceName)
{
...
void throwIfSessionNotInitialized(const std::map<size_t, std::unique_ptr<Session>> &sessions, size_t sessionId)
{
    if (!sessions.count(sessionId)) {
        throw std::runtime_error("Session has not been initialized: " + std::to_string(sessionId));
    }
}
...
std::unique_ptr<Session> SessionManager::makeSession(size_t id, const std::string &path,
                                                     ...
                                                     const std::string &contextSourceName, const std::string &dataName)
{
    ...
    auto *session = new Session(id, path, profiler, std::move(contextSource), std::move(data));
...
size_t SessionManager::addSession(const std::string &path, const std::string &profilerName,
                                  const std::string &contextSourceName, const std::string &dataName)
{
    std::unique_lock<std::shared_mutex> lock(mutex);
    if (hasSession(path)) {
    ...
    sessions[sessionId] = makeSession(sessionId, path, profilerName, contextSourceName, dataName);
    return sessionId;
}

void SessionManager::finalizeSession(size_t sessionId, OutputFormat outputFormat)
{
    std::unique_lock<std::shared_mutex> lock(mutex);
    if (!hasSession(sessionId)) {
        return;
...
void SessionManager::addMetrics(size_t scopeId, const std::map<std::string, MetricValueType> &metrics, bool aggregable)
{
    std::shared_lock<std::shared_mutex> lock(mutex);
