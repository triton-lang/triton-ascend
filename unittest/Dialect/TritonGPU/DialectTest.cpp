...
// beginning.
bool advance(MutableArrayRef<unsigned> idx, ArrayRef<unsigned> shape, ArrayRef<unsigned> order)
{
    for (int dim : order) {
...
int64_t getFlatIdx(ArrayRef<unsigned> idx, ArrayRef<unsigned> shape, ArrayRef<unsigned> order)
{
    int64_t flatIdx = 0;
    int64_t stride = 1;
...
bool sameFlatIdxs(const MultiIdx &a, const MultiIdx &b)
{
    return a.flatIdx == b.flatIdx && //
           a.flatIdxInThread == b.flatIdxInThread && a.flatIdxInWarp == b.flatIdxInWarp &&
...
std::string multiIdxsToString(ArrayRef<std::unique_ptr<MultiIdx>> idxs)
{
    ...
    for (const auto &idxPtr : idxs) {
        ...
        ss //
            ...
            << "elem=[" << triton::join(idx.idxInThread, ",") << "] (" << idx.flatIdxInThread << ") "
            << "thread=[" << triton::join(idx.idxInWarp, ",") << "] (" << idx.flatIdxInWarp << ") "
            << "warp=[" << triton::join(idx.idxInCTA, ",") << "] (" << idx.flatIdxInCTA << ") "
            << "outer=[" << triton::join(idx.idxOuter, ",") << "] (" << idx.flatIdxOuter << ")\n";
    }
    return ss.str();
}

std::vector<std::unique_ptr<MultiIdx>> getMultiIdxs(ArrayRef<unsigned> shape, BlockedEncodingAttr enc)
{
    ...
    for (int i = 0; i < rank; i++) {
        numBlocks.push_back(ceil<unsigned>(shape[i], sizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i]));
    }
    ...
    int64_t nElems = product(sizePerThread) * product(threadsPerWarp) * product(warpsPerCTA) * product(numBlocks);
    ...
    for (int64_t i = 0; i < nElems; i++) {
        ...
        for (int i = 0; i < rank; i++) {
            e->idx.push_back(    //
                idxInThread[i] + //
                idxInWarp[i] * sizePerThread[i] + idxInCTA[i] * sizePerThread[i] * threadsPerWarp[i] +
        ...
        e->flatIdx = getFlatIdx(e->idx, shape, llvm::to_vector(llvm::reverse(llvm::seq(rank))));
    ...
    llvm::sort(elems, [](const std::unique_ptr<MultiIdx> &a, const std::unique_ptr<MultiIdx> &b) {
        return a->flatIdx < b->flatIdx;
    });
    return elems;
}
...
class InferLayoutTest : public ::testing::Test {
  public:
    InferLayoutTest()
        : inferLayout(ctx.getOrLoadDialect<TritonGPUDialect>()->getRegisteredInterface<DialectInferLayoutInterface>())
    {
    }
...
void testReshape(RankedTensorType srcTy, RankedTensorType dstTy, std::optional<BlockedEncodingAttr> expectedDstEnc,
                 std::optional<bool> expectSuccess, DialectInferLayoutInterface *inferLayout, bool longErrors = true,
                 bool *couldReshape = nullptr)
{
    ...
    *couldReshape = false;
    ...
    {
        ScopedDiagnosticHandler scopedHandler(ctx, [&](Diagnostic &diag) { diags.push_back("  - " + diag.str()); });
        result = inferLayout->inferReshapeOpNoReorderEncoding(srcTy.getShape(), srcTy.getEncoding(), dstTy.getShape(),
    ...
    if (expectSuccess.has_value() && !*expectSuccess) {
        EXPECT_FALSE(succeeded(result)) << "Expected reshape to be impossible, but got dst encoding: "
    ...
    // srcEnc.  (This is an invariant of the inference function.)
    {
        ...
        auto result = inferLayout->inferReshapeOpNoReorderEncoding(dstTy.getShape(), inferredEnc, srcTy.getShape(),
                                                                   inferredSrcEnc, UnknownLoc::get(ctx));
        EXPECT_TRUE(succeeded(result)) << "Inverse encoding inference (" << triton::join(dstTy.getShape(), "x") << " "
                                       << stringifyLLVMType(inferredEnc) << " -> "
        ...
        if (succeeded(result)) {
            EXPECT_EQ(inferredSrcEnc, srcTy.getEncoding())
                ...
                << stringifyLLVMType(inferredEnc) << " -> " << triton::join(srcTy.getShape(), "x")
                << " gave the wrong result.  Expected " << stringifyLLVMType(srcTy.getEncoding()) << " but got "
                << stringifyLLVMType(inferredSrcEnc) << ".\n";
        }
    }

    std::vector<std::unique_ptr<MultiIdx>> srcMultiIdxs =
        getMultiIdxs(SmallVector<unsigned>(srcTy.getShape()), mlir::cast<BlockedEncodingAttr>(srcTy.getEncoding()));

    std::vector<std::unique_ptr<MultiIdx>> dstMultiIdxs =
        getMultiIdxs(SmallVector<unsigned>(dstTy.getShape()), mlir::cast<BlockedEncodingAttr>(inferredEnc));

    if (srcMultiIdxs.size() != dstMultiIdxs.size() ||
        !llvm::all_of(llvm::zip_equal(srcMultiIdxs, dstMultiIdxs), [](const auto &pair) {
            const auto &[a, b] = pair;
        ...
        }))
    {
        ...
        ADD_FAILURE() << "Reified indices do not match for encodings:\n"
                      ...
                      << "  dst: [" << triton::join(dstTy.getShape(), "x") << "] " << stringifyLLVMType(inferredEnc);
    } else {
        *couldReshape = true;
    }
}
...
class InferReshapeOpNoReorderEncodingTest
    : public InferLayoutTest,
      public ::testing::WithParamInterface<
          std::tuple<std::string /*srcTy*/, std::string /*dstTy*/, bool /*expectSuccess*/>> {};
...
TEST_P(InferReshapeOpNoReorderEncodingTest, DoIt)
{
    ...
    testReshape(cast<RankedTensorType>(src), cast<RankedTensorType>(dst), expectedDstEnc, expectSuccess, inferLayout,
                /*longErrors=*/true);
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn(std::vector<std::tuple<std::string /*srcTy*/, std::string /*dstTy*/, bool /*expectSuccess*/>>({
        // Use raw strings in here so clang-format doesn't try to wrap them.
        {R"(T<128x64xf32, #B<{spt=[1,1], tpw=[1,32], wpc=[1,1], ord=[1,0]}>>)",
         R"(T<8192xf32,   #B<{spt=[1],   tpw=[32],   wpc=[1],   ord=[0]}>>)", true},

        {R"(T<128xf32,  #B<{spt=[4],   tpw=[32],   wpc=[1],   ord=[0]}>>)",
         R"(T<32x4xf32, #B<{spt=[1,4], tpw=[32,1], wpc=[1,1], ord=[1,0]}>>)", true},

        {R"(T<128xf32,  #B<{spt=[4],   tpw=[32],   wpc=[1],   ord=[0]}>>)",
         R"(T<16x8xf32, #B<{spt=[1,4], tpw=[16,2], wpc=[1,1], ord=[1,0]}>>)", true},

        {R"(T<32x32xf32, #B<{spt=[2,2], tpw=[32,1], wpc=[1,1], ord=[1,0]}>>)", "T<128xf32>", false},

        {R"(T<32x4xf32,     #B<{spt=[1,4],     tpw=[32,1],     wpc=[1,1],     ord=[1,0]}>>)",
         R"(T<2x16x2x2xf32, #B<{spt=[1,1,2,2], tpw=[2,16,1,1], wpc=[1,1,1,1], ord=[3,2,1,0]}>>)", true},

        {R"(T<4x32xf32,     #B<{spt=[4,1],     tpw=[1,32],     wpc=[1,1],     ord=[0,1]}>>)",
         R"(T<2x2x2x16xf32, #B<{spt=[2,2,1,1], tpw=[1,1,2,16], wpc=[1,1,1,1], ord=[1,0,3,2]}>>)", true},

        {R"(T<32x32xf32,     #B<{spt=[4,4],     tpw=[4,8],     wpc=[1,1],     ord=[1,0]}>>)",
         R"(T<2x16x2x16xf32, #B<{spt=[1,4,1,4], tpw=[1,4,2,4], wpc=[1,1,1,1], ord=[3,2,1,0]}>>)", true},

        {R"(T<32x32xf32,     #B<{spt=[4,4],     tpw=[4,8],     wpc=[1,1],     ord=[1,0]}>>)",
         R"(T<16x2x16x2xf32, #B<{spt=[2,2,2,2], tpw=[4,1,8,1], wpc=[1,1,1,1], ord=[3,2,1,0]}>>)", true},

        {R"(T<32x32xf32, #B<{spt=[4,4], tpw=[4,8], wpc=[1,1], ord=[0,1]}>>)", R"(T<16x2x16x2xf32>)", true},
        ...
        {R"(T<4x2x2x4xf32, #B<{spt=[2,1,1,2], tpw=[2,1,1,2], wpc=[2,2,1,1], ord=[0,3,1,2]}>>)",
         R"(T<4x2x2x4xf32, #B<{spt=[2,1,1,2], tpw=[2,1,1,2], wpc=[2,2,1,1], ord=[0,3,1,2]}>>)", true},

        {R"(T<2x4x2x4xf32, #B<{spt=[1,2,2,1], tpw=[1,2,1,2], wpc=[1,2,2,1], ord=[2,1,0,3]}>>)", R"(T<4x2x2x4xf32>)",
         false},

        {R"(T<1x2x2x4xf32, #B<{spt=[1,32,4,4], tpw=[4,4,16,16], wpc=[8,8,8,1], ord=[0,1,2,3]}>>)", R"(T<2x2x4x1xf32>)",
         false},

        {R"(T<2x2x2x2xf32, #B<{spt=[2,2,2,2], tpw=[1,1,1,1], wpc=[1,1,1,1], ord=[1,0,3,2]}>>)", R"(T<4x4xf32>)", true},

        {R"(T<16x8xf32, #B<{spt=[1,2], tpw=[2,4], wpc=[2,1], ord=[1,0]}>>)", R"(T<128xf32>)", true},

        {R"(T<16x1x8xf32, #B<{spt=[8,1,1], tpw=[2,1,1], wpc=[1,1,8], ord=[2,1,0]}>>)", R"(T<128x1xf32>)", false},

        {R"(T<16x1x8xf32, #B<{spt=[1,1,8], tpw=[2,1,1], wpc=[8,1,1], ord=[2,1,0]}>>)", R"(T<128x1xf32>)", true},

        {R"(T<32x32xf32, #B<{spt=[1,2], tpw=[1,8], wpc=[1,1], ord=[1,0]}>>)", R"(T<1024xf32>)", true},

        {R"(T<4x4xf32, #B<{spt=[1,1], tpw=[2,4], wpc=[2,1], ord=[0,1]}>>)", R"(T<16xf32>)", false},

        {R"(T<32xf32,   #B<{spt=[2],   tpw=[32],   wpc=[2],   ord=[0]}>>)",
         R"(T<16x2xf32, #B<{spt=[1,2], tpw=[32,1], wpc=[2,1], ord=[1,0]}>>)", true},

        {R"(T<2x1x2xf32, #B<{spt=[2,1,1], tpw=[2,1,2], wpc=[4,1,8], ord=[2,1,0]}>>)",
         R"(T<2x2xf32,   #B<{spt=[2,1],   tpw=[2,2],   wpc=[4,8],   ord=[1,0]}>>)", true},
    })));
...
TEST_F(InferLayoutTest, FuzzReshape)
{
    ...
    for (int i = 0; i < numTests; i++) {
        ...
        SmallVector<int64_t> srcShape(convertType<int64_t>(randPow2Vec(rank, /*maxPow2=*/4)));
        ...
        auto ctaLayout = CTALayoutAttr::get(&ctx, SmallVector<unsigned>(rank, 1), SmallVector<unsigned>(rank, 1),
                                            llvm::to_vector(llvm::reverse(llvm::seq<unsigned>(rank))));

        auto srcTy = RankedTensorType::get(
            srcShape, FloatType::getF32(&ctx),
            BlockedEncodingAttr::get(&ctx, sizePerThread, threadsPerWarp, warpsPerCTA, order, ctaLayout));
    ...
    printf("Fuzz success rate: %d/%d = %.2f%%\n", numSuccess, numTests, 100.0 * numSuccess / numTests);
}
...
class AMDMfmaLayoutTest : public ::testing::Test {
  public:
    AMDMfmaLayoutTest()
    {
        ...
        ctaLayout = triton::gpu::CTALayoutAttr::get(&ctx, ctaPerCGA, ctaSplit, ctaOrder);
        f16Ty = FloatType::getF16(&ctx);
    }

    triton::gpu::AMDMfmaEncodingAttr createMFMA(int mDim, int nDim, ArrayRef<unsigned> warpsPerCTA)
    {
        return triton::gpu::AMDMfmaEncodingAttr::get(&ctx, /*versionMajor=*/2, /*versionMinor=*/0, warpsPerCTA, mDim,
                                                     nDim,
                                                     /*isTransposed=*/false, ctaLayout);
    }

    triton::gpu::AMDMfmaEncodingAttr createTransposedMFMA(int mDim, int nDim, ArrayRef<unsigned> warpsPerCTA)
    {
        return triton::gpu::AMDMfmaEncodingAttr::get(&ctx, /*versionMajor=*/2, /*versionMinor=*/0, warpsPerCTA, mDim,
                                                     nDim,
    ...
    triton::gpu::DotOperandEncodingAttr createDotOperand(int idx, triton::gpu::AMDMfmaEncodingAttr parent, int kWidth)
    {
